...
(check-by-interp '(module (define L.not.1 (lambda (tmp.23) (not tmp.23))) (call L.not.1 #f)
                    ))
(check-by-interp '(module (define L.not.1 (lambda (tmp.23) (not tmp.23))) (call L.not.1 #t)
                    ))
(check-by-interp '(module (if #f empty 0)))
(check-by-interp '(module (define L.fixnum?.1 (lambda (tmp.17) (fixnum? tmp.17)))
                          (call L.fixnum?.1 #t)
                    ))
(check-by-interp '(module (define L.error?.1 (lambda (tmp.22) (error? tmp.22)))
                          (call L.error?.1 (void))
                    ))
(check-by-interp '(module (define L.fixnum?.1 (lambda (tmp.17) (fixnum? tmp.17)))
...
(check-by-interp '(module (define L.ascii-char?.1 (lambda (tmp.21) (ascii-char? tmp.21)))
                          (call L.ascii-char?.1 (void))
                    ))
(check-by-interp '(module (let ([foobar.8.1 #f]) foobar.8.1)))
(check-by-interp '(module (let ([foobar.2.2 #\d]
                                [bat.1.1 #\n])
                            foobar.2.2)))
(check-by-interp '(module (define L.void?.2 (lambda (tmp.20) (void? tmp.20)))
                          (define L.proc.0.1 (lambda () #t))
                    (call L.void?.2 1)))
(check-by-interp '(module (define L.<=.2
                            (lambda (tmp.10 tmp.11)
                              (if (fixnum? tmp.11)
                                  (if (fixnum? tmp.10)
                                      (unsafe-fx<= tmp.10 tmp.11)
                                      (error 5))
                                  (error 5))))
                          (define L.tmp.0.1 (lambda (foo.9.1) #t))
                    (call L.<=.2 0 0)))
...
(check-by-interp '(module (define L.eq?.2 (lambda (tmp.19 tmp.20) (eq? tmp.19 tmp.20)))
...
(check-by-interp '(module (define L.fn.0.1 (lambda (bat.3.2 bar.9.1) #f))
                    ...
                    #t))
(check-by-interp '(module (define L.eq?.2 (lambda (tmp.21 tmp.22) (eq? tmp.21 tmp.22)))
                          (define L.proc.0.1
                            (lambda (foobar.4.6 foo.8.5 bar.0.4 foobar.3.3 bar.9.2 bat.7.1) #t))
                    (call L.eq?.2 #\m 233919391)))
...
(check-by-interp '(module (define L.tmp.0.1
                    ...
                    (if #\e
                        (void)
                        empty)))
(check-by-interp '(module (define L.void?.3 (lambda (tmp.28) (void? tmp.28)))
...
(check-by-interp '(module (define L.func.0.1
                    ...
                    (void)))
(check-by-interp '(module (define L.boolean?.3 (lambda (tmp.25) (boolean? tmp.25)))
                          (define L.proc.0.1 (lambda (ball.8.2 ball.9.1) #\l))
                    (define L.fn.1.2 (lambda (bar.0.7 ball.3.6 ball.8.5 ball.9.4 foobar.5.3) #t))
                    (call L.boolean?.3 empty)))
(check-by-interp '(module (define L.ascii-char?.2 (lambda (tmp.25) (ascii-char? tmp.25)))
                          (define L.eq?.1 (lambda (tmp.19 tmp.20) (eq? tmp.19 tmp.20)))
                    (let ([foo.4.3 (call L.eq?.1 empty 0)]
                          [foobar.1.2 #f]
                          [ball.5.1 253953304])
                      (let ([ball.5.4 (if #t
                                          (void)
                                          foo.4.3)])
                        (call L.ascii-char?.2 ball.5.4)))))
...
(check-by-interp
 '(module (define L.proc.0.1
    ...
    #f))
(check-by-interp '(module (define L.ascii-char?.3 (lambda (tmp.30) (ascii-char? tmp.30)))
                          (define L.func.0.1 (lambda (foobar.4.5 bar.8.4 bat.7.3 bar.1.2 foo.9.1) #t))
                    (define L.func.1.2 (lambda (foobar.3.9 foo.6.8 foobar.4.7 bar.1.6) #f))
                    (call L.ascii-char?.3 #\P)))
(check-by-interp '(module (define L.x.0.1 (lambda (ball.3.1) 0))
                    ...
                    (let ([bat.7.8 empty]
                      ...
                      empty)))
(check-by-interp
 '(module (define L.boolean?.3 (lambda (tmp.30) (boolean? tmp.30)))
    ...
    (call L.boolean?.3 (void))))
(check-by-interp '(module (define L.ascii-char?.3 (lambda (tmp.32) (ascii-char? tmp.32)))
...
(check-by-interp '(module (define L.fn.0.1
                    ...
                    empty))
(check-by-interp
 '(module (define L.empty?.3 (lambda (tmp.33) (empty? tmp.33)))
          (define L.tmp.0.1
...
(check-by-interp '(module (define L.proc.0.1
                    ...
                    (if #f #t empty)))
(check-by-interp
 '(module (define L.ascii-char?.4 (lambda (tmp.32) (ascii-char? tmp.32)))
          (define L.fn.0.1
    ...
    (call L.ascii-char?.4 1)))
(check-by-interp '(module (define L.boolean?.4 (lambda (tmp.31) (boolean? tmp.31)))
                    ...
                    (call L.boolean?.4 #f)))
(check-by-interp '(module (define L.ascii-char?.4 (lambda (tmp.37) (ascii-char? tmp.37)))
                          (define L.tmp.0.1
                    ...
                    (call L.ascii-char?.4 empty)))
(check-by-interp '(module (define L.<.5
                            (lambda (tmp.20 tmp.21)
                              (if (fixnum? tmp.21)
                                  (if (fixnum? tmp.20)
                                      (unsafe-fx< tmp.20 tmp.21)
                                      (error 4))
                                  (error 4))))
                          (define L.eq?.4 (lambda (tmp.28 tmp.29) (eq? tmp.28 tmp.29)))
                    (define L.proc.0.1 (lambda (bat.2.4 ball.8.3 foobar.7.2 foo.1.1) #t))
                    (define L.proc.1.2 (lambda (bat.5.9 foo.6.8 foobar.3.7 foobar.7.6 bat.0.5) #f))
                    (define L.x.2.3 (lambda (bar.4.12 foobar.3.11 bar.9.10) 0))
                    (let ([bat.5.13 (call L.eq?.4 1 #f)]) (call L.<.5 -278642811 1))))
(check-by-interp
 '(module (define L.empty?.4 (lambda (tmp.35) (empty? tmp.35)))
          (define L.proc.0.1 (lambda (foo.9.6 ball.5.5 ball.4.4 bar.8.3 ball.1.2 bat.7.1) #f))
    ...
    (if (void)
        (let ([ball.1.16 (call L.empty?.4 #f)]) ball.1.16)
        1)))
(check-by-interp
 '(module (define L.fixnum?.4 (lambda (tmp.37) (fixnum? tmp.37)))
          (define L.x.0.1
