#lang racket
(require rackunit
         cpsc411/compiler-lib
         cpsc411/ptr-run-time
         cpsc411/langs/v8
         "../select-instructions.rkt")
(define (fail-if-invalid p)
  (when (not (asm-alloc-lang-v8? p))
    (error
     (~a
      (pretty-format p)
      "\n is not a semantically valid "
      "asm-alloc-lang-v8"
      " program")))
  p)
(define-syntax-rule
 (check-by-interp p)
 (check-equal?
  (interp-imp-cmf-lang-v8 p)
  (interp-asm-alloc-lang-v8 (fail-if-invalid (select-instructions p)))))

(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (begin (set! rax 840) (jump tmp-ra.50 rbp rax)))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (begin (set! rax 26942) (jump tmp-ra.50 rbp rax)))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (!= 14 6)
        (begin (set! rax 17214) (jump tmp-ra.50 rbp rax))
        (begin (set! rax 9790) (jump tmp-ra.50 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.51 r15)
      (begin
        (set! error0.1 28478)
        (begin (set! rax 1128) (jump tmp-ra.51 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (define L.fun/empty8385.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.50 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.50 rbp rax)))))
    (begin
      (set! tmp-ra.51 r15)
      (begin (set! r15 tmp-ra.51) (jump L.fun/empty8385.4 rbp r15)))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (!= 14 6)
        (begin (set! rax 25390) (jump tmp-ra.50 rbp rax))
        (begin (set! rax 25390) (jump tmp-ra.50 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (!= 14 6)
        (begin (set! rax 50238) (jump tmp-ra.50 rbp rax))
        (begin (set! rax 14398) (jump tmp-ra.50 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() ())))
    (define L.-.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.52 (bitwise-and tmp.20 7)) (= tmp.52 0))
                  (set! tmp.51 14)
                  (set! tmp.51 6))
                (!= tmp.51 6))
            (if (begin
                  (if (begin (set! tmp.54 (bitwise-and tmp.19 7)) (= tmp.54 0))
                    (set! tmp.53 14)
                    (set! tmp.53 6))
                  (!= tmp.53 6))
              (begin (set! rax (- tmp.19 tmp.20)) (jump tmp-ra.57 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.57 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.57 rbp rax))))))
    (define L.fun/fixnum8420.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin
          (set! oprand0.1 rdi)
          (begin (set! rax 400) (jump tmp-ra.58 rbp rax)))))
    (begin
      (set! tmp-ra.59 r15)
      (begin
        (begin
          (return-point L.rp.7
            (begin
              (set! rdi 25390)
              (set! r15 L.rp.7)
              (jump L.fun/fixnum8420.4 rbp r15 rdi)))
          (set! tmp.55 rax))
        (begin
          (begin
            (return-point L.rp.8
              (begin
                (set! rsi 2024)
                (set! rdi 168)
                (set! r15 L.rp.8)
                (jump L.-.6 rbp r15 rdi rsi)))
            (set! tmp.56 rax))
          (begin
            (set! rsi tmp.56)
            (set! rdi tmp.55)
            (set! r15 tmp-ra.59)
            (jump L.-.6 rbp r15 rdi rsi)))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (!= 14 6)
        (begin (set! rax 61502) (jump tmp-ra.50 rbp rax))
        (begin (set! rax 13118) (jump tmp-ra.50 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (())))
    (define L.fun/ascii-char8428.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.53 r15)
        (begin
          (set! oprand0.1 rdi)
          (begin (set! rax 25390) (jump tmp-ra.53 rbp rax)))))
    (define L.fun/empty8427.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.54 r15)
        (begin
          (set! oprand0.2 rdi)
          (begin (set! rax 22) (jump tmp-ra.54 rbp rax)))))
    (begin
      (set! tmp-ra.55 r15)
      (begin
        (begin
          (return-point L.rp.7
            (begin
              (set! rdi 22)
              (set! r15 L.rp.7)
              (jump L.fun/ascii-char8428.4 rbp r15 rdi)))
          (set! tmp.52 rax))
        (begin
          (set! rdi tmp.52)
          (set! r15 tmp-ra.55)
          (jump L.fun/empty8427.5 rbp r15 rdi))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (define L.fun/ascii-char8442.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin
          (set! oprand0.2 rdi)
          (set! oprand1.1 rsi)
          (begin (set! rax 25390) (jump tmp-ra.55 rbp rax)))))
    (begin
      (set! tmp-ra.56 r15)
      (if (if (!= 14 6) (!= 14 6) (!= 14 6))
        (begin
          (set! rsi 22)
          (set! rdi 25390)
          (set! r15 tmp-ra.56)
          (jump L.fun/ascii-char8442.4 rbp r15 rdi rsi))
        (begin
          (set! fixnum0.5 832)
          (set! fixnum1.4 608)
          (set! void2.3 30)
          (begin (set! rax 25390) (jump tmp-ra.56 rbp rax)))))))
(check-by-interp
 '(module
    ((new-frames (() () ())))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin
          (set! tmp.17 rdi)
          (set! tmp.18 rsi)
          (if (begin
                (if (begin (set! tmp.54 (bitwise-and tmp.18 7)) (= tmp.54 0))
                  (set! tmp.53 14)
                  (set! tmp.53 6))
                (!= tmp.53 6))
            (if (begin
                  (if (begin (set! tmp.56 (bitwise-and tmp.17 7)) (= tmp.56 0))
                    (set! tmp.55 14)
                    (set! tmp.55 6))
                  (!= tmp.55 6))
              (begin
                (set! tmp.57 (arithmetic-shift-right tmp.18 3))
                (begin (set! rax (* tmp.17 tmp.57)) (jump tmp-ra.58 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.58 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.58 rbp rax))))))
    (define L.fun/ascii-char8498.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/void8499.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/empty8500.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.61 rbp rax)))))
    (begin
      (set! tmp-ra.62 r15)
      (begin
        (begin
          (return-point L.rp.9
            (begin
              (set! rsi 272)
              (set! rdi 1856)
              (set! r15 L.rp.9)
              (jump L.*.8 rbp r15 rdi rsi)))
          (set! fixnum0.3 rax))
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/ascii-char8498.4 rbp r15)))
          (set! ascii-char1.2 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/void8499.5 rbp r15)))
          (set! void2.1 rax))
        (begin (set! r15 tmp-ra.62) (jump L.fun/empty8500.6 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () ())))
    (define L.-.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.54 (bitwise-and tmp.22 7)) (= tmp.54 0))
                  (set! tmp.53 14)
                  (set! tmp.53 6))
                (!= tmp.53 6))
            (if (begin
                  (if (begin (set! tmp.56 (bitwise-and tmp.21 7)) (= tmp.56 0))
                    (set! tmp.55 14)
                    (set! tmp.55 6))
                  (!= tmp.55 6))
              (begin (set! rax (- tmp.21 tmp.22)) (jump tmp-ra.57 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.57 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.57 rbp rax))))))
    (define L.fun/empty8553.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/error8554.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 43326) (jump tmp-ra.59 rbp rax)))))
    (begin
      (set! tmp-ra.60 r15)
      (begin
        (begin
          (return-point L.rp.8
            (begin (set! r15 L.rp.8) (jump L.fun/empty8553.4 rbp r15)))
          (set! empty0.3 rax))
        (begin
          (return-point L.rp.9
            (begin (set! r15 L.rp.9) (jump L.fun/error8554.5 rbp r15)))
          (set! error1.2 rax))
        (begin
          (return-point L.rp.10
            (begin
              (set! rsi 144)
              (set! rdi 1768)
              (set! r15 L.rp.10)
              (jump L.-.7 rbp r15 rdi rsi)))
          (set! fixnum2.1 rax))
        (begin (set! rax error1.2) (jump tmp-ra.60 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (())))
    (define L.fun/boolean8597.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin
          (set! oprand0.2 rdi)
          (set! oprand1.1 rsi)
          (begin (set! rax 6) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/ascii-char8598.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin
          (set! oprand0.4 rdi)
          (set! oprand1.3 rsi)
          (begin (set! rax 25390) (jump tmp-ra.56 rbp rax)))))
    (begin
      (set! tmp-ra.57 r15)
      (if (begin
            (begin
              (return-point L.rp.7
                (begin
                  (set! rsi 25390)
                  (set! rdi 22)
                  (set! r15 L.rp.7)
                  (jump L.fun/boolean8597.4 rbp r15 rdi rsi)))
              (set! tmp.54 rax))
            (!= tmp.54 6))
        (if (!= 6 6)
          (begin (set! rax 25390) (jump tmp-ra.57 rbp rax))
          (begin (set! rax 25390) (jump tmp-ra.57 rbp rax)))
        (begin
          (set! rsi 22)
          (set! rdi 624)
          (set! r15 tmp-ra.57)
          (jump L.fun/ascii-char8598.5 rbp r15 rdi rsi))))))
(check-by-interp
 '(module
    ((new-frames (() () () ())))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin
          (set! tmp.32 rdi)
          (set! tmp.33 rsi)
          (if (begin
                (if (begin (set! tmp.67 (bitwise-and tmp.33 7)) (= tmp.67 0))
                  (set! tmp.66 14)
                  (set! tmp.66 6))
                (!= tmp.66 6))
            (if (begin
                  (if (begin (set! tmp.69 (bitwise-and tmp.32 7)) (= tmp.69 0))
                    (set! tmp.68 14)
                    (set! tmp.68 6))
                  (!= tmp.68 6))
              (begin (set! rax (+ tmp.32 tmp.33)) (jump tmp-ra.73 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.73 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.73 rbp rax))))))
    (define L.fun/fixnum9203.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! oprand0.3 rdi)
          (set! oprand1.2 rsi)
          (set! oprand2.1 rdx)
          (begin (set! rax 1368) (jump tmp-ra.74 rbp rax)))))
    (define L.fun/fixnum9202.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! oprand0.6 rdi)
          (set! oprand1.5 rsi)
          (set! oprand2.4 rdx)
          (begin (set! rax 568) (jump tmp-ra.75 rbp rax)))))
    (define L.fun/fixnum9204.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! oprand0.9 rdi)
          (set! oprand1.8 rsi)
          (set! oprand2.7 rdx)
          (begin (set! rax oprand2.7) (jump tmp-ra.76 rbp rax)))))
    (define L.fun/boolean9201.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin
          (set! oprand0.12 rdi)
          (set! oprand1.11 rsi)
          (set! oprand2.10 rdx)
          (begin (set! rax 14) (jump tmp-ra.77 rbp rax)))))
    (begin
      (set! tmp-ra.78 r15)
      (begin
        (if (begin
              (begin
                (return-point L.rp.10
                  (begin
                    (set! rdx 22)
                    (set! rsi 25390)
                    (set! rdi 2878)
                    (set! r15 L.rp.10)
                    (jump L.fun/boolean9201.7 rbp r15 rdi rsi rdx)))
                (set! tmp.71 rax))
              (!= tmp.71 6))
          (begin
            (return-point L.rp.11
              (begin
                (set! rdx 30)
                (set! rsi 22)
                (set! rdi 30)
                (set! r15 L.rp.11)
                (jump L.fun/fixnum9202.5 rbp r15 rdi rsi rdx)))
            (set! tmp.70 rax))
          (begin
            (return-point L.rp.12
              (begin
                (set! rdx 6)
                (set! rsi 6)
                (set! rdi 22)
                (set! r15 L.rp.12)
                (jump L.fun/fixnum9203.4 rbp r15 rdi rsi rdx)))
            (set! tmp.70 rax)))
        (begin
          (if (if (!= 14 6) (!= 6 6) (!= 6 6))
            (begin
              (set! boolean0.16 6)
              (set! error1.15 59198)
              (set! fixnum2.14 216)
              (set! void3.13 30)
              (set! tmp.72 fixnum2.14))
            (begin
              (return-point L.rp.13
                (begin
                  (set! rdx 1776)
                  (set! rsi 25390)
                  (set! rdi 30)
                  (set! r15 L.rp.13)
                  (jump L.fun/fixnum9204.6 rbp r15 rdi rsi rdx)))
              (set! tmp.72 rax)))
          (begin
            (set! rsi tmp.72)
            (set! rdi tmp.70)
            (set! r15 tmp-ra.78)
            (jump L.+.9 rbp r15 rdi rsi)))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.-.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.55 (bitwise-and tmp.23 7)) (= tmp.55 0))
                  (set! tmp.54 14)
                  (set! tmp.54 6))
                (!= tmp.54 6))
            (if (begin
                  (if (begin (set! tmp.57 (bitwise-and tmp.22 7)) (= tmp.57 0))
                    (set! tmp.56 14)
                    (set! tmp.56 6))
                  (!= tmp.56 6))
              (begin (set! rax (- tmp.22 tmp.23)) (jump tmp-ra.69 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.69 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.69 rbp rax))))))
    (define L.+.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.70 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.59 (bitwise-and tmp.21 7)) (= tmp.59 0))
                  (set! tmp.58 14)
                  (set! tmp.58 6))
                (!= tmp.58 6))
            (if (begin
                  (if (begin (set! tmp.61 (bitwise-and tmp.20 7)) (= tmp.61 0))
                    (set! tmp.60 14)
                    (set! tmp.60 6))
                  (!= tmp.60 6))
              (begin (set! rax (+ tmp.20 tmp.21)) (jump tmp-ra.70 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.70 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.70 rbp rax))))))
    (define L.*.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.71 r15)
        (begin
          (set! tmp.18 rdi)
          (set! tmp.19 rsi)
          (if (begin
                (if (begin (set! tmp.63 (bitwise-and tmp.19 7)) (= tmp.63 0))
                  (set! tmp.62 14)
                  (set! tmp.62 6))
                (!= tmp.62 6))
            (if (begin
                  (if (begin (set! tmp.65 (bitwise-and tmp.18 7)) (= tmp.65 0))
                    (set! tmp.64 14)
                    (set! tmp.64 6))
                  (!= tmp.64 6))
              (begin
                (set! tmp.66 (arithmetic-shift-right tmp.19 3))
                (begin (set! rax (* tmp.18 tmp.66)) (jump tmp-ra.71 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.71 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.71 rbp rax))))))
    (define L.fun/ascii-char9328.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.72 rbp rax)))))
    (define L.fun/void9330.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.73 rbp rax)))))
    (define L.fun/error9331.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin (begin (set! rax 27198) (jump tmp-ra.74 rbp rax)))))
    (define L.fun/ascii-char9329.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.75 rbp rax)))))
    (begin
      (set! tmp-ra.76 r15)
      (begin
        (begin
          (begin
            (return-point L.rp.12
              (begin
                (set! rsi 1800)
                (set! rdi 440)
                (set! r15 L.rp.12)
                (jump L.*.9 rbp r15 rdi rsi)))
            (set! tmp.67 rax))
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 224)
                  (set! rdi 416)
                  (set! r15 L.rp.13)
                  (jump L.+.10 rbp r15 rdi rsi)))
              (set! tmp.68 rax))
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi tmp.68)
                  (set! rdi tmp.67)
                  (set! r15 L.rp.14)
                  (jump L.-.11 rbp r15 rdi rsi)))
              (set! fixnum0.4 rax))))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/ascii-char9328.4 rbp r15)))
          (set! ascii-char1.3 rax))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/ascii-char9329.7 rbp r15)))
          (set! ascii-char2.2 rax))
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/void9330.5 rbp r15)))
          (set! void3.1 rax))
        (begin (set! r15 tmp-ra.76) (jump L.fun/error9331.6 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () ())))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (set! tmp.18 rdi)
          (set! tmp.19 rsi)
          (if (begin
                (if (begin (set! tmp.55 (bitwise-and tmp.19 7)) (= tmp.55 0))
                  (set! tmp.54 14)
                  (set! tmp.54 6))
                (!= tmp.54 6))
            (if (begin
                  (if (begin (set! tmp.57 (bitwise-and tmp.18 7)) (= tmp.57 0))
                    (set! tmp.56 14)
                    (set! tmp.56 6))
                  (!= tmp.56 6))
              (begin
                (set! tmp.58 (arithmetic-shift-right tmp.19 3))
                (begin (set! rax (* tmp.18 tmp.58)) (jump tmp-ra.69 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.69 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.69 rbp rax))))))
    (define L.-.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.70 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.23 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.22 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (- tmp.22 tmp.23)) (jump tmp-ra.70 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.70 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.70 rbp rax))))))
    (define L.fun/ascii-char9350.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.71 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.71 rbp rax)))))
    (define L.fun/error9349.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin (begin (set! rax 34878) (jump tmp-ra.72 rbp rax)))))
    (begin
      (set! tmp-ra.73 r15)
      (begin
        (begin
          (begin
            (return-point L.rp.9
              (begin
                (set! rsi 1608)
                (set! rdi 1200)
                (set! r15 L.rp.9)
                (jump L.-.7 rbp r15 rdi rsi)))
            (set! tmp.63 rax))
          (begin
            (begin
              (return-point L.rp.10
                (begin
                  (set! rsi 936)
                  (set! rdi 760)
                  (set! r15 L.rp.10)
                  (jump L.-.7 rbp r15 rdi rsi)))
              (set! tmp.64 rax))
            (begin
              (return-point L.rp.11
                (begin
                  (set! rsi tmp.64)
                  (set! rdi tmp.63)
                  (set! r15 L.rp.11)
                  (jump L.-.7 rbp r15 rdi rsi)))
              (set! fixnum0.4 rax))))
        (begin
          (begin
            (return-point L.rp.12
              (begin
                (set! rsi 856)
                (set! rdi 1024)
                (set! r15 L.rp.12)
                (jump L.*.8 rbp r15 rdi rsi)))
            (set! tmp.65 rax))
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 776)
                  (set! rdi 712)
                  (set! r15 L.rp.13)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.66 rax))
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi tmp.66)
                  (set! rdi tmp.65)
                  (set! r15 L.rp.14)
                  (jump L.-.7 rbp r15 rdi rsi)))
              (set! fixnum1.3 rax))))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/error9349.5 rbp r15)))
          (set! error2.2 rax))
        (begin
          (begin
            (return-point L.rp.16
              (begin
                (set! rsi 32)
                (set! rdi 1064)
                (set! r15 L.rp.16)
                (jump L.-.7 rbp r15 rdi rsi)))
            (set! tmp.67 rax))
          (begin
            (begin
              (return-point L.rp.17
                (begin
                  (set! rsi 1200)
                  (set! rdi 1088)
                  (set! r15 L.rp.17)
                  (jump L.-.7 rbp r15 rdi rsi)))
              (set! tmp.68 rax))
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi tmp.68)
                  (set! rdi tmp.67)
                  (set! r15 L.rp.18)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! fixnum3.1 rax))))
        (begin (set! r15 tmp-ra.73) (jump L.fun/ascii-char9350.4 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.55 (bitwise-and tmp.23 7)) (= tmp.55 0))
                  (set! tmp.54 14)
                  (set! tmp.54 6))
                (!= tmp.54 6))
            (if (begin
                  (if (begin (set! tmp.57 (bitwise-and tmp.22 7)) (= tmp.57 0))
                    (set! tmp.56 14)
                    (set! tmp.56 6))
                  (!= tmp.56 6))
              (begin (set! rax (- tmp.22 tmp.23)) (jump tmp-ra.69 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.69 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.69 rbp rax))))))
    (define L.*.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.70 r15)
        (begin
          (set! tmp.18 rdi)
          (set! tmp.19 rsi)
          (if (begin
                (if (begin (set! tmp.59 (bitwise-and tmp.19 7)) (= tmp.59 0))
                  (set! tmp.58 14)
                  (set! tmp.58 6))
                (!= tmp.58 6))
            (if (begin
                  (if (begin (set! tmp.61 (bitwise-and tmp.18 7)) (= tmp.61 0))
                    (set! tmp.60 14)
                    (set! tmp.60 6))
                  (!= tmp.60 6))
              (begin
                (set! tmp.62 (arithmetic-shift-right tmp.19 3))
                (begin (set! rax (* tmp.18 tmp.62)) (jump tmp-ra.70 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.70 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.70 rbp rax))))))
    (define L.+.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.71 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.21 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.20 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin (set! rax (+ tmp.20 tmp.21)) (jump tmp-ra.71 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.71 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.71 rbp rax))))))
    (define L.fun/empty9463.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.72 rbp rax)))))
    (define L.fun/empty9462.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.73 rbp rax)))))
    (define L.fun/void9461.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.74 rbp rax)))))
    (begin
      (set! tmp-ra.75 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/void9461.6 rbp r15)))
          (set! void0.4 rax))
        (begin
          (begin
            (return-point L.rp.12
              (begin
                (set! rsi 1440)
                (set! rdi 1464)
                (set! r15 L.rp.12)
                (jump L.+.8 rbp r15 rdi rsi)))
            (set! tmp.67 rax))
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 944)
                  (set! rdi 1904)
                  (set! r15 L.rp.13)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! tmp.68 rax))
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi tmp.68)
                  (set! rdi tmp.67)
                  (set! r15 L.rp.14)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum1.3 rax))))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/empty9462.5 rbp r15)))
          (set! empty2.2 rax))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/empty9463.4 rbp r15)))
          (set! empty3.1 rax))
        (begin (set! rax void0.4) (jump tmp-ra.75 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (())))
    (define L.-.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin
          (set! tmp.33 rdi)
          (set! tmp.34 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.34 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.33 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.33 tmp.34)) (jump tmp-ra.72 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.72 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.72 rbp rax))))))
    (define L.fun/ascii-char9530.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin
          (set! oprand0.3 rdi)
          (set! oprand1.2 rsi)
          (set! oprand2.1 rdx)
          (begin (set! rax 25390) (jump tmp-ra.73 rbp rax)))))
    (begin
      (set! tmp-ra.74 r15)
      (if (begin
            (set! fixnum0.7 1064)
            (set! empty1.6 22)
            (set! fixnum2.5 680)
            (set! error3.4 28222)
            (!= 6 6))
        (begin
          (begin
            (set! void0.11 30)
            (set! error1.10 40510)
            (set! void2.9 30)
            (set! boolean3.8 6)
            (set! tmp.69 1440))
          (begin
            (begin
              (return-point L.rp.7
                (begin
                  (set! rsi 32)
                  (set! rdi 1288)
                  (set! r15 L.rp.7)
                  (jump L.-.6 rbp r15 rdi rsi)))
              (set! tmp.70 rax))
            (begin
              (if (!= 14 6) (set! tmp.71 30) (set! tmp.71 30))
              (begin
                (set! rdx tmp.71)
                (set! rsi tmp.70)
                (set! rdi tmp.69)
                (set! r15 tmp-ra.74)
                (jump L.fun/ascii-char9530.4 rbp r15 rdi rsi rdx)))))
        (begin
          (set! ascii-char0.15 25390)
          (set! fixnum1.14 1560)
          (set! empty2.13 22)
          (set! void3.12 30)
          (begin (set! rax 25390) (jump tmp-ra.74 rbp rax)))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (if (!= 14 6) (!= 6 6) (!= 14 6))
        (if (!= 6 6)
          (begin (set! rax 46654) (jump tmp-ra.50 rbp rax))
          (begin (set! rax 42046) (jump tmp-ra.50 rbp rax)))
        (if (!= 14 6)
          (begin (set! rax 33598) (jump tmp-ra.50 rbp rax))
          (begin (set! rax 58174) (jump tmp-ra.50 rbp rax)))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.*.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin
          (set! tmp.18 rdi)
          (set! tmp.19 rsi)
          (if (begin
                (if (begin (set! tmp.55 (bitwise-and tmp.19 7)) (= tmp.55 0))
                  (set! tmp.54 14)
                  (set! tmp.54 6))
                (!= tmp.54 6))
            (if (begin
                  (if (begin (set! tmp.57 (bitwise-and tmp.18 7)) (= tmp.57 0))
                    (set! tmp.56 14)
                    (set! tmp.56 6))
                  (!= tmp.56 6))
              (begin
                (set! tmp.58 (arithmetic-shift-right tmp.19 3))
                (begin (set! rax (* tmp.18 tmp.58)) (jump tmp-ra.65 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.65 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.65 rbp rax))))))
    (define L.-.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.23 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.22 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (- tmp.22 tmp.23)) (jump tmp-ra.66 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.66 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.66 rbp rax))))))
    (define L.fun/error9595.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin (begin (set! rax 9022) (jump tmp-ra.67 rbp rax)))))
    (define L.fun/void9594.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.68 rbp rax)))))
    (define L.fun/empty9593.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.69 rbp rax)))))
    (begin
      (set! tmp-ra.70 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/empty9593.6 rbp r15)))
          (set! empty0.4 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/void9594.5 rbp r15)))
          (set! void1.3 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/error9595.4 rbp r15)))
          (set! error2.2 rax))
        (begin
          (begin
            (return-point L.rp.13
              (begin
                (set! rsi 1392)
                (set! rdi 1368)
                (set! r15 L.rp.13)
                (jump L.-.8 rbp r15 rdi rsi)))
            (set! tmp.63 rax))
          (begin
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi 400)
                  (set! rdi 1288)
                  (set! r15 L.rp.14)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! tmp.64 rax))
            (begin
              (return-point L.rp.15
                (begin
                  (set! rsi tmp.64)
                  (set! rdi tmp.63)
                  (set! r15 L.rp.15)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! fixnum3.1 rax))))
        (begin (set! rax void1.3) (jump tmp-ra.70 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.void?.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.54 (bitwise-and tmp.44 255)) (= tmp.54 30))
            (begin (set! rax 14) (jump tmp-ra.56 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.56 rbp rax))))))
    (define L.fun/error9648.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 2622) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/error9646.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 58430) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/any9647.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/error9649.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 28222) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/void9650.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.61 rbp rax)))))
    (begin
      (set! tmp-ra.62 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/error9646.5 rbp r15)))
          (set! error0.4 rax))
        (begin
          (begin
            (return-point L.rp.12
              (begin (set! r15 L.rp.12) (jump L.fun/any9647.6 rbp r15)))
            (set! tmp.55 rax))
          (begin
            (return-point L.rp.13
              (begin
                (set! rdi tmp.55)
                (set! r15 L.rp.13)
                (jump L.void?.10 rbp r15 rdi)))
            (set! boolean1.3 rax)))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/error9648.4 rbp r15)))
          (set! error2.2 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/error9649.7 rbp r15)))
          (set! error3.1 rax))
        (begin (set! r15 tmp-ra.62) (jump L.fun/void9650.8 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.ascii-char?.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin
          (set! tmp.46 rdi)
          (if (begin (set! tmp.55 (bitwise-and tmp.46 255)) (= tmp.55 46))
            (begin (set! rax 14) (jump tmp-ra.57 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.57 rbp rax))))))
    (define L.fun/error10696.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 55102) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/ascii-char10693.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/void10697.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/any10694.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.61 rbp rax)))))
    (define L.fun/void10695.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.62 rbp rax)))))
    (begin
      (set! tmp-ra.63 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char10693.5 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (begin
            (return-point L.rp.12
              (begin (set! r15 L.rp.12) (jump L.fun/any10694.7 rbp r15)))
            (set! tmp.56 rax))
          (begin
            (return-point L.rp.13
              (begin
                (set! rdi tmp.56)
                (set! r15 L.rp.13)
                (jump L.ascii-char?.10 rbp r15 rdi)))
            (set! boolean1.4 rax)))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/void10695.8 rbp r15)))
          (set! void2.3 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/error10696.4 rbp r15)))
          (set! error3.2 rax))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/void10697.6 rbp r15)))
          (set! void4.1 rax))
        (begin (set! rax void4.1) (jump tmp-ra.63 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.fun/empty12161.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/error12159.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 1854) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/ascii-char12162.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/error12163.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 10814) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/error12160.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 27198) (jump tmp-ra.59 rbp rax)))))
    (begin
      (set! tmp-ra.60 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/error12159.5 rbp r15)))
          (set! error0.5 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/error12160.8 rbp r15)))
          (set! error1.4 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/empty12161.4 rbp r15)))
          (set! empty2.3 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/ascii-char12162.6 rbp r15)))
          (set! ascii-char3.2 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/error12163.7 rbp r15)))
          (set! error4.1 rax))
        (begin (set! rax empty2.3) (jump tmp-ra.60 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () ())))
    (define L.-.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.76 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.76 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.77 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.77 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.77 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.78 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.78 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.78 rbp rax))))))
    (define L.empty?.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.68 (bitwise-and tmp.44 255)) (= tmp.68 22))
            (begin (set! rax 14) (jump tmp-ra.79 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.79 rbp rax))))))
    (define L.fun/void12536.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/any12537.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.81 rbp rax)))))
    (define L.fun/error12539.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin (begin (set! rax 29502) (jump tmp-ra.82 rbp rax)))))
    (define L.fun/error12538.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 24894) (jump tmp-ra.83 rbp rax)))))
    (begin
      (set! tmp-ra.84 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/void12536.4 rbp r15)))
          (set! void0.5 rax))
        (begin
          (begin
            (return-point L.rp.14
              (begin (set! r15 L.rp.14) (jump L.fun/any12537.5 rbp r15)))
            (set! tmp.69 rax))
          (begin
            (return-point L.rp.15
              (begin
                (set! rdi tmp.69)
                (set! r15 L.rp.15)
                (jump L.empty?.9 rbp r15 rdi)))
            (set! boolean1.4 rax)))
        (begin
          (begin
            (begin
              (return-point L.rp.16
                (begin
                  (set! rsi 384)
                  (set! rdi 784)
                  (set! r15 L.rp.16)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! tmp.71 rax))
            (begin
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rsi 912)
                    (set! rdi 104)
                    (set! r15 L.rp.17)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi tmp.72)
                    (set! rdi tmp.71)
                    (set! r15 L.rp.18)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.70 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 1288)
                    (set! rdi 1776)
                    (set! r15 L.rp.19)
                    (jump L.-.12 rbp r15 rdi rsi)))
                (set! tmp.74 rax))
              (begin
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi 760)
                      (set! rdi 792)
                      (set! r15 L.rp.20)
                      (jump L.*.10 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi tmp.75)
                      (set! rdi tmp.74)
                      (set! r15 L.rp.21)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))))
            (begin
              (return-point L.rp.22
                (begin
                  (set! rsi tmp.73)
                  (set! rdi tmp.70)
                  (set! r15 L.rp.22)
                  (jump L.-.12 rbp r15 rdi rsi)))
              (set! fixnum2.3 rax))))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/error12538.7 rbp r15)))
          (set! error3.2 rax))
        (begin
          (return-point L.rp.24
            (begin (set! r15 L.rp.24) (jump L.fun/error12539.6 rbp r15)))
          (set! error4.1 rax))
        (begin (set! rax error4.1) (jump tmp-ra.84 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () () () () () () () ())))
    (define L.pair?.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! tmp.48 rdi)
          (if (begin (set! tmp.55 (bitwise-and tmp.48 7)) (= tmp.55 1))
            (begin (set! rax 14) (jump tmp-ra.82 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.82 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.22 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.21 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.83 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.24 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.23 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.84 rbp rax))))))
    (define L.*.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.20 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.19 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.68)) (jump tmp-ra.85 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/any13289.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/empty13290.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.87 rbp rax)))))
    (define L.fun/error13288.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin (begin (set! rax 48958) (jump tmp-ra.88 rbp rax)))))
    (define L.fun/ascii-char13291.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.89 rbp rax)))))
    (begin
      (set! tmp-ra.90 r15)
      (begin
        (begin
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 1576)
                  (set! rdi 1360)
                  (set! r15 L.rp.13)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! tmp.70 rax))
            (begin
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi 296)
                    (set! rdi 1888)
                    (set! r15 L.rp.14)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi tmp.71)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.15)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 1808)
                    (set! rdi 1880)
                    (set! r15 L.rp.16)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 704)
                      (set! rdi 2008)
                      (set! r15 L.rp.17)
                      (jump L.*.9 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.18)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (return-point L.rp.19
                (begin
                  (set! rsi tmp.72)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.19)
                  (jump L.+.11 rbp r15 rdi rsi)))
              (set! fixnum0.5 rax))))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/error13288.6 rbp r15)))
          (set! error1.4 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.21
                (begin
                  (set! rsi 1192)
                  (set! rdi 1488)
                  (set! r15 L.rp.21)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! tmp.76 rax))
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 1480)
                    (set! rdi 1160)
                    (set! r15 L.rp.22)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.77 rax))
              (begin
                (return-point L.rp.23
                  (begin
                    (set! rsi tmp.77)
                    (set! rdi tmp.76)
                    (set! r15 L.rp.23)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.75 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi 1240)
                    (set! rdi 24)
                    (set! r15 L.rp.24)
                    (jump L.*.9 rbp r15 rdi rsi)))
                (set! tmp.79 rax))
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1752)
                      (set! rdi 1808)
                      (set! r15 L.rp.25)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.26)
                      (jump L.*.9 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.27
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.75)
                  (set! r15 L.rp.27)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum2.3 rax))))
        (begin
          (begin
            (return-point L.rp.28
              (begin (set! r15 L.rp.28) (jump L.fun/any13289.4 rbp r15)))
            (set! tmp.81 rax))
          (begin
            (return-point L.rp.29
              (begin
                (set! rdi tmp.81)
                (set! r15 L.rp.29)
                (jump L.pair?.12 rbp r15 rdi)))
            (set! boolean3.2 rax)))
        (begin
          (return-point L.rp.30
            (begin (set! r15 L.rp.30) (jump L.fun/empty13290.5 rbp r15)))
          (set! empty4.1 rax))
        (begin (set! r15 tmp-ra.90) (jump L.fun/ascii-char13291.7 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () ())))
    (define L.fixnum?.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.42 rdi)
          (if (begin (set! tmp.55 (bitwise-and tmp.42 7)) (= tmp.55 0))
            (begin (set! rax 14) (jump tmp-ra.76 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.76 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.22 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.21 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.77 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.77 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.77 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.20 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.19 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.64)) (jump tmp-ra.78 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.78 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.78 rbp rax))))))
    (define L.-.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.24 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.23 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.79 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.79 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.79 rbp rax))))))
    (define L.fun/ascii-char13294.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/any13295.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.81 rbp rax)))))
    (define L.fun/error13297.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin (begin (set! rax 46910) (jump tmp-ra.82 rbp rax)))))
    (define L.fun/ascii-char13296.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.83 rbp rax)))))
    (begin
      (set! tmp-ra.84 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/ascii-char13294.4 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi 920)
                  (set! rdi 272)
                  (set! r15 L.rp.14)
                  (jump L.-.9 rbp r15 rdi rsi)))
              (set! tmp.70 rax))
            (begin
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi 1528)
                    (set! rdi 1312)
                    (set! r15 L.rp.15)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi tmp.71)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.16)
                    (jump L.-.9 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rsi 608)
                    (set! rdi 928)
                    (set! r15 L.rp.17)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi 1288)
                      (set! rdi 224)
                      (set! r15 L.rp.18)
                      (jump L.-.9 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.19)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (return-point L.rp.20
                (begin
                  (set! rsi tmp.72)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.20)
                  (jump L.+.11 rbp r15 rdi rsi)))
              (set! fixnum1.4 rax))))
        (begin
          (begin
            (return-point L.rp.21
              (begin (set! r15 L.rp.21) (jump L.fun/any13295.5 rbp r15)))
            (set! tmp.75 rax))
          (begin
            (return-point L.rp.22
              (begin
                (set! rdi tmp.75)
                (set! r15 L.rp.22)
                (jump L.fixnum?.12 rbp r15 rdi)))
            (set! boolean2.3 rax)))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/ascii-char13296.7 rbp r15)))
          (set! ascii-char3.2 rax))
        (begin
          (return-point L.rp.24
            (begin (set! r15 L.rp.24) (jump L.fun/error13297.6 rbp r15)))
          (set! error4.1 rax))
        (begin (set! rax ascii-char0.5) (jump tmp-ra.84 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.ascii-char?.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin
          (set! tmp.46 rdi)
          (if (begin (set! tmp.55 (bitwise-and tmp.46 255)) (= tmp.55 46))
            (begin (set! rax 14) (jump tmp-ra.57 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.57 rbp rax))))))
    (define L.fun/ascii-char13919.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/error13917.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 34878) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/any13918.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 14) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/void13916.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.61 rbp rax)))))
    (define L.fun/ascii-char13915.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.62 rbp rax)))))
    (begin
      (set! tmp-ra.63 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char13915.8 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/void13916.7 rbp r15)))
          (set! void1.4 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/error13917.5 rbp r15)))
          (set! error2.3 rax))
        (begin
          (begin
            (return-point L.rp.14
              (begin (set! r15 L.rp.14) (jump L.fun/any13918.6 rbp r15)))
            (set! tmp.56 rax))
          (begin
            (return-point L.rp.15
              (begin
                (set! rdi tmp.56)
                (set! r15 L.rp.15)
                (jump L.ascii-char?.10 rbp r15 rdi)))
            (set! boolean3.2 rax)))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/ascii-char13919.4 rbp r15)))
          (set! ascii-char4.1 rax))
        (begin (set! rax void1.4) (jump tmp-ra.63 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () () () () () () ())))
    (define L.+.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.22 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.21 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.80 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.80 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.80 rbp rax))))))
    (define L.-.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.24 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.81 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.81 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.81 rbp rax))))))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.82 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.82 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.82 rbp rax))))))
    (define L.fun/empty14259.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.83 rbp rax)))))
    (define L.fun/ascii-char14257.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.84 rbp rax)))))
    (define L.fun/error14258.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin (begin (set! rax 21310) (jump tmp-ra.85 rbp rax)))))
    (begin
      (set! tmp-ra.86 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char14257.5 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.12
                (begin
                  (set! rsi 1776)
                  (set! rdi 1648)
                  (set! r15 L.rp.12)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.13
                  (begin
                    (set! rsi 1328)
                    (set! rdi 1808)
                    (set! r15 L.rp.13)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.14)
                    (jump L.-.9 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi 1872)
                    (set! rdi 1456)
                    (set! r15 L.rp.15)
                    (jump L.-.9 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.16
                    (begin
                      (set! rsi 624)
                      (set! rdi 432)
                      (set! r15 L.rp.16)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.17)
                      (jump L.-.9 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.18)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! fixnum1.4 rax))))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/error14258.6 rbp r15)))
          (set! error2.3 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/empty14259.4 rbp r15)))
          (set! empty3.2 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.21
                (begin
                  (set! rsi 544)
                  (set! rdi 1448)
                  (set! r15 L.rp.21)
                  (jump L.+.10 rbp r15 rdi rsi)))
              (set! tmp.75 rax))
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 1936)
                    (set! rdi 504)
                    (set! r15 L.rp.22)
                    (jump L.+.10 rbp r15 rdi rsi)))
                (set! tmp.76 rax))
              (begin
                (return-point L.rp.23
                  (begin
                    (set! rsi tmp.76)
                    (set! rdi tmp.75)
                    (set! r15 L.rp.23)
                    (jump L.-.9 rbp r15 rdi rsi)))
                (set! tmp.74 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi 624)
                    (set! rdi 832)
                    (set! r15 L.rp.24)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.78 rax))
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 256)
                      (set! rdi 816)
                      (set! r15 L.rp.25)
                      (jump L.*.8 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi tmp.79)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.26)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.27
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.74)
                  (set! r15 L.rp.27)
                  (jump L.-.9 rbp r15 rdi rsi)))
              (set! fixnum4.1 rax))))
        (begin (set! rax empty3.2) (jump tmp-ra.86 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.fun/empty15571.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/ascii-char15572.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/ascii-char15569.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/empty15570.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/void15568.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.59 rbp rax)))))
    (begin
      (set! tmp-ra.60 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/void15568.8 rbp r15)))
          (set! void0.5 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char15569.6 rbp r15)))
          (set! ascii-char1.4 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/empty15570.7 rbp r15)))
          (set! empty2.3 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/empty15571.4 rbp r15)))
          (set! empty3.2 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/ascii-char15572.5 rbp r15)))
          (set! ascii-char4.1 rax))
        (begin (set! rax empty3.2) (jump tmp-ra.60 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.+.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.22 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.21 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.74 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.74 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.74 rbp rax))))))
    (define L.-.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.24 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.75 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.75 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.75 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.76 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.76 rbp rax))))))
    (define L.fun/ascii-char15704.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.77 rbp rax)))))
    (define L.fun/void15705.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.78 rbp rax)))))
    (define L.fun/ascii-char15707.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.79 rbp rax)))))
    (define L.fun/error15708.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 36158) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/empty15706.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.81 rbp rax)))))
    (begin
      (set! tmp-ra.82 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/ascii-char15704.4 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/void15705.5 rbp r15)))
          (set! void1.4 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.15
                (begin
                  (set! rsi 424)
                  (set! rdi 1192)
                  (set! r15 L.rp.15)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 1368)
                    (set! rdi 1600)
                    (set! r15 L.rp.16)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.17)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 536)
                    (set! rdi 1936)
                    (set! r15 L.rp.18)
                    (jump L.+.12 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 1704)
                      (set! rdi 112)
                      (set! r15 L.rp.19)
                      (jump L.*.10 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.20)
                      (jump L.*.10 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.21
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.21)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! fixnum2.3 rax))))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/empty15706.8 rbp r15)))
          (set! empty3.2 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/ascii-char15707.6 rbp r15)))
          (set! ascii-char4.1 rax))
        (begin (set! r15 tmp-ra.82) (jump L.fun/error15708.7 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () () ())))
    (define L.empty?.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.55 (bitwise-and tmp.44 255)) (= tmp.55 22))
            (begin (set! rax 14) (jump tmp-ra.78 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.78 rbp rax))))))
    (define L.ascii-char?.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin
          (set! tmp.46 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.46 255)) (= tmp.56 46))
            (begin (set! rax 14) (jump tmp-ra.79 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.79 rbp rax))))))
    (define L.*.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.20 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.19 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin
                (set! tmp.61 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.61)) (jump tmp-ra.80 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.80 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.80 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.63 (bitwise-and tmp.24 7)) (= tmp.63 0))
                  (set! tmp.62 14)
                  (set! tmp.62 6))
                (!= tmp.62 6))
            (if (begin
                  (if (begin (set! tmp.65 (bitwise-and tmp.23 7)) (= tmp.65 0))
                    (set! tmp.64 14)
                    (set! tmp.64 6))
                  (!= tmp.64 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.81 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.81 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.81 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.67 (bitwise-and tmp.22 7)) (= tmp.67 0))
                  (set! tmp.66 14)
                  (set! tmp.66 6))
                (!= tmp.66 6))
            (if (begin
                  (if (begin (set! tmp.69 (bitwise-and tmp.21 7)) (= tmp.69 0))
                    (set! tmp.68 14)
                    (set! tmp.68 6))
                  (!= tmp.68 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.82 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.82 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.82 rbp rax))))))
    (define L.fun/void15911.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.83 rbp rax)))))
    (define L.fun/any15910.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.84 rbp rax)))))
    (define L.fun/any15908.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.85 rbp rax)))))
    (define L.fun/ascii-char15909.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.86 rbp rax)))))
    (begin
      (set! tmp-ra.87 r15)
      (begin
        (begin
          (begin
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi 152)
                  (set! rdi 1608)
                  (set! r15 L.rp.14)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! tmp.71 rax))
            (begin
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi 256)
                    (set! rdi 1064)
                    (set! r15 L.rp.15)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi tmp.72)
                    (set! rdi tmp.71)
                    (set! r15 L.rp.16)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.70 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rsi 1016)
                    (set! rdi 2032)
                    (set! r15 L.rp.17)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.74 rax))
              (begin
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi 1496)
                      (set! rdi 544)
                      (set! r15 L.rp.18)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi tmp.75)
                      (set! rdi tmp.74)
                      (set! r15 L.rp.19)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))))
            (begin
              (return-point L.rp.20
                (begin
                  (set! rsi tmp.73)
                  (set! rdi tmp.70)
                  (set! r15 L.rp.20)
                  (jump L.*.11 rbp r15 rdi rsi)))
              (set! fixnum0.5 rax))))
        (begin
          (begin
            (return-point L.rp.21
              (begin (set! r15 L.rp.21) (jump L.fun/any15908.6 rbp r15)))
            (set! tmp.76 rax))
          (begin
            (return-point L.rp.22
              (begin
                (set! rdi tmp.76)
                (set! r15 L.rp.22)
                (jump L.ascii-char?.12 rbp r15 rdi)))
            (set! boolean1.4 rax)))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/ascii-char15909.7 rbp r15)))
          (set! ascii-char2.3 rax))
        (begin
          (begin
            (return-point L.rp.24
              (begin (set! r15 L.rp.24) (jump L.fun/any15910.5 rbp r15)))
            (set! tmp.77 rax))
          (begin
            (return-point L.rp.25
              (begin
                (set! rdi tmp.77)
                (set! r15 L.rp.25)
                (jump L.empty?.13 rbp r15 rdi)))
            (set! boolean3.2 rax)))
        (begin
          (return-point L.rp.26
            (begin (set! r15 L.rp.26) (jump L.fun/void15911.4 rbp r15)))
          (set! void4.1 rax))
        (begin (set! rax void4.1) (jump tmp-ra.87 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.fun/error16257.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 57150) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/ascii-char16255.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/empty16258.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/empty16256.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/ascii-char16259.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.59 rbp rax)))))
    (begin
      (set! tmp-ra.60 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/ascii-char16255.5 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/empty16256.7 rbp r15)))
          (set! empty1.4 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/error16257.4 rbp r15)))
          (set! error2.3 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/empty16258.6 rbp r15)))
          (set! empty3.2 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/ascii-char16259.8 rbp r15)))
          (set! ascii-char4.1 rax))
        (begin (set! rax error2.3) (jump tmp-ra.60 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () () () () () () ())))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.80 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.80 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.80 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.81 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.81 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.81 rbp rax))))))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.82 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.82 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.82 rbp rax))))))
    (define L.fun/void17748.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.83 rbp rax)))))
    (define L.fun/ascii-char17750.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.84 rbp rax)))))
    (define L.fun/error17749.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin (begin (set! rax 36158) (jump tmp-ra.85 rbp rax)))))
    (begin
      (set! tmp-ra.86 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/void17748.4 rbp r15)))
          (set! void0.5 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.12
                (begin
                  (set! rsi 1496)
                  (set! rdi 2000)
                  (set! r15 L.rp.12)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.13
                  (begin
                    (set! rsi 464)
                    (set! rdi 1416)
                    (set! r15 L.rp.13)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.14)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi 1352)
                    (set! rdi 912)
                    (set! r15 L.rp.15)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.16
                    (begin
                      (set! rsi 1184)
                      (set! rdi 1176)
                      (set! r15 L.rp.16)
                      (jump L.+.9 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.17)
                      (jump L.+.9 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.18)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! fixnum1.4 rax))))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/error17749.6 rbp r15)))
          (set! error2.3 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/ascii-char17750.5 rbp r15)))
          (set! ascii-char3.2 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.21
                (begin
                  (set! rsi 608)
                  (set! rdi 1920)
                  (set! r15 L.rp.21)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.75 rax))
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 1456)
                    (set! rdi 1400)
                    (set! r15 L.rp.22)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.76 rax))
              (begin
                (return-point L.rp.23
                  (begin
                    (set! rsi tmp.76)
                    (set! rdi tmp.75)
                    (set! r15 L.rp.23)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.74 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi 400)
                    (set! rdi 1480)
                    (set! r15 L.rp.24)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.78 rax))
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1320)
                      (set! rdi 848)
                      (set! r15 L.rp.25)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi tmp.79)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.26)
                      (jump L.+.9 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.27
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.74)
                  (set! r15 L.rp.27)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum4.1 rax))))
        (begin (set! rax void0.5) (jump tmp-ra.86 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.+.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.22 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.21 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.74 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.74 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.74 rbp rax))))))
    (define L.*.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.20 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.19 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin
                (set! tmp.63 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.63)) (jump tmp-ra.75 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.75 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.75 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.24 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.23 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.76 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.76 rbp rax))))))
    (define L.fun/empty17803.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.77 rbp rax)))))
    (define L.fun/error17806.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin (begin (set! rax 38462) (jump tmp-ra.78 rbp rax)))))
    (define L.fun/void17802.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.79 rbp rax)))))
    (define L.fun/void17805.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/empty17804.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.81 rbp rax)))))
    (begin
      (set! tmp-ra.82 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/void17802.6 rbp r15)))
          (set! void0.5 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/empty17803.4 rbp r15)))
          (set! empty1.4 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/empty17804.8 rbp r15)))
          (set! empty2.3 rax))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/void17805.7 rbp r15)))
          (set! void3.2 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.17
                (begin
                  (set! rsi 992)
                  (set! rdi 936)
                  (set! r15 L.rp.17)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 416)
                    (set! rdi 792)
                    (set! r15 L.rp.18)
                    (jump L.*.11 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.19)
                    (jump L.*.11 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi 792)
                    (set! rdi 1936)
                    (set! r15 L.rp.20)
                    (jump L.+.12 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 1904)
                      (set! rdi 472)
                      (set! r15 L.rp.21)
                      (jump L.*.11 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.22)
                      (jump L.*.11 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.23
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.23)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum4.1 rax))))
        (begin (set! r15 tmp-ra.82) (jump L.fun/error17806.5 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (())))
    (define L.*.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.20 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.19 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin
                (set! tmp.59 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.59)) (jump tmp-ra.67 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.67 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.67 rbp rax))))))
    (define L.void?.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin
          (set! tmp.45 rdi)
          (if (begin (set! tmp.60 (bitwise-and tmp.45 255)) (= tmp.60 30))
            (begin (set! rax 14) (jump tmp-ra.68 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.68 rbp rax))))))
    (begin
      (set! tmp-ra.69 r15)
      (if (begin
            (begin
              (begin
                (set! boolean0.5 14)
                (set! error1.4 830)
                (set! void2.3 30)
                (set! fixnum3.2 1512)
                (set! fixnum4.1 88)
                (set! tmp.62 30))
              (begin
                (return-point L.rp.7
                  (begin
                    (set! rdi tmp.62)
                    (set! r15 L.rp.7)
                    (jump L.void?.5 rbp r15 rdi)))
                (set! tmp.61 rax)))
            (!= tmp.61 6))
        (begin
          (if (!= 14 6) (set! tmp.63 400) (set! tmp.63 1200))
          (begin
            (if (!= 14 6) (set! tmp.64 1000) (set! tmp.64 1256))
            (begin
              (set! rsi tmp.64)
              (set! rdi tmp.63)
              (set! r15 tmp-ra.69)
              (jump L.*.6 rbp r15 rdi rsi))))
        (begin
          (if (!= 6 6) (set! tmp.65 1464) (set! tmp.65 360))
          (begin
            (if (!= 14 6) (set! tmp.66 72) (set! tmp.66 816))
            (begin
              (set! rsi tmp.66)
              (set! rdi tmp.65)
              (set! r15 tmp-ra.69)
              (jump L.*.6 rbp r15 rdi rsi))))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.22 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.21 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.74 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.74 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.74 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.24 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.75 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.75 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.75 rbp rax))))))
    (define L.*.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.76 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.76 rbp rax))))))
    (define L.fun/void20503.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.77 rbp rax)))))
    (define L.fun/empty20501.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.78 rbp rax)))))
    (define L.fun/ascii-char20504.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.79 rbp rax)))))
    (define L.fun/void20502.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.80 rbp rax)))))
    (begin
      (set! tmp-ra.81 r15)
      (begin
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/empty20501.5 rbp r15)))
          (set! empty0.5 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/void20502.7 rbp r15)))
          (set! void1.4 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/void20503.4 rbp r15)))
          (set! void2.3 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/ascii-char20504.6 rbp r15)))
          (set! ascii-char3.2 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.16
                (begin
                  (set! rsi 1000)
                  (set! rdi 144)
                  (set! r15 L.rp.16)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rsi 1840)
                    (set! rdi 1248)
                    (set! r15 L.rp.17)
                    (jump L.*.9 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.18)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 1216)
                    (set! rdi 88)
                    (set! r15 L.rp.19)
                    (jump L.*.9 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi 784)
                      (set! rdi 512)
                      (set! r15 L.rp.20)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.21)
                      (jump L.*.9 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.22
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.22)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! fixnum4.1 rax))))
        (begin (set! rax ascii-char3.2) (jump tmp-ra.81 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.83 rbp rax))))))
    (define L.+.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.23 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.22 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.84 rbp rax))))))
    (define L.*.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.85 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/error28491.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 45118) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/empty28492.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin (set! r15 tmp-ra.87) (jump L.fun/empty28493.6 rbp r15)))))
    (define L.fun/empty28493.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.88 rbp rax)))))
    (define L.fun/empty28494.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (begin (set! r15 tmp-ra.89) (jump L.fun/empty28495.12 rbp r15)))))
    (define L.fun/ascii-char28488.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin
            (set! r15 tmp-ra.90)
            (jump L.fun/ascii-char28489.13 rbp r15)))))
    (define L.fun/error28490.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin (set! r15 tmp-ra.91) (jump L.fun/error28491.4 rbp r15)))))
    (define L.fun/void28497.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/void28496.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin (set! r15 tmp-ra.93) (jump L.fun/void28497.10 rbp r15)))))
    (define L.fun/empty28495.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/ascii-char28489.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.95 rbp rax)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 1616)
                    (set! rdi 456)
                    (set! r15 L.rp.18)
                    (jump L.*.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 144)
                      (set! rdi 912)
                      (set! r15 L.rp.19)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.20)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 928)
                      (set! rdi 1048)
                      (set! r15 L.rp.21)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 224)
                        (set! rdi 1272)
                        (set! r15 L.rp.22)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.23)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.24)
                    (jump L.+.16 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 224)
                      (set! rdi 1872)
                      (set! r15 L.rp.25)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1528)
                        (set! rdi 912)
                        (set! r15 L.rp.26)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.27)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 320)
                        (set! rdi 1672)
                        (set! r15 L.rp.28)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.29
                        (begin
                          (set! rsi 1280)
                          (set! rdi 1352)
                          (set! r15 L.rp.29)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.30)
                          (jump L.+.16 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.31)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.32
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.32)
                  (jump L.-.17 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (return-point L.rp.33
            (begin (set! r15 L.rp.33) (jump L.fun/ascii-char28488.8 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (return-point L.rp.34
            (begin (set! r15 L.rp.34) (jump L.fun/error28490.9 rbp r15)))
          (set! error2.4 rax))
        (begin
          (return-point L.rp.35
            (begin (set! r15 L.rp.35) (jump L.fun/empty28492.5 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/empty28494.7 rbp r15)))
          (set! empty4.2 rax))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/void28496.11 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax error2.4) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.fun/ascii-char28930.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin
          (begin
            (set! r15 tmp-ra.56)
            (jump L.fun/ascii-char28931.10 rbp r15)))))
    (define L.fun/error28940.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin
          (begin (set! r15 tmp-ra.57) (jump L.fun/error28941.15 rbp r15)))))
    (define L.fun/ascii-char28935.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/void28936.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin
          (begin (set! r15 tmp-ra.59) (jump L.fun/void28937.11 rbp r15)))))
    (define L.fun/void28938.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin
          (begin (set! r15 tmp-ra.60) (jump L.fun/void28939.14 rbp r15)))))
    (define L.fun/error28933.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 44606) (jump tmp-ra.61 rbp rax)))))
    (define L.fun/ascii-char28931.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.62 rbp rax)))))
    (define L.fun/void28937.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.63 rbp rax)))))
    (define L.fun/ascii-char28934.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin
          (begin
            (set! r15 tmp-ra.64)
            (jump L.fun/ascii-char28935.6 rbp r15)))))
    (define L.fun/error28932.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin
          (begin (set! r15 tmp-ra.65) (jump L.fun/error28933.9 rbp r15)))))
    (define L.fun/void28939.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.66 rbp rax)))))
    (define L.fun/error28941.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin (begin (set! rax 62526) (jump tmp-ra.67 rbp rax)))))
    (begin
      (set! tmp-ra.68 r15)
      (begin
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/ascii-char28930.4 rbp r15)))
          (set! ascii-char0.6 rax))
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/error28932.13 rbp r15)))
          (set! error1.5 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/ascii-char28934.12 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/void28936.7 rbp r15)))
          (set! void3.3 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void28938.8 rbp r15)))
          (set! void4.2 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/error28940.5 rbp r15)))
          (set! error5.1 rax))
        (begin (set! rax void4.2) (jump tmp-ra.68 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.+.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.83 rbp rax))))))
    (define L.*.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.84 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.84 rbp rax))))))
    (define L.-.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/empty30828.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/empty30827.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin (set! r15 tmp-ra.87) (jump L.fun/empty30828.4 rbp r15)))))
    (define L.fun/void30829.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin (set! r15 tmp-ra.88) (jump L.fun/void30830.12 rbp r15)))))
    (define L.fun/error30825.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (begin (set! r15 tmp-ra.89) (jump L.fun/error30826.11 rbp r15)))))
    (define L.fun/error30824.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 31294) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/empty30822.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/error30823.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin (set! r15 tmp-ra.92) (jump L.fun/error30824.8 rbp r15)))))
    (define L.fun/error30826.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 28734) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/void30830.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/empty30821.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin
          (begin (set! r15 tmp-ra.95) (jump L.fun/empty30822.9 rbp r15)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/empty30821.13 rbp r15)))
          (set! empty0.6 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/error30823.10 rbp r15)))
          (set! error1.5 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/error30825.7 rbp r15)))
          (set! error2.4 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/empty30827.5 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 1424)
                    (set! rdi 1560)
                    (set! r15 L.rp.22)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 1440)
                      (set! rdi 488)
                      (set! r15 L.rp.23)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.24)
                      (jump L.*.16 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 432)
                      (set! rdi 1008)
                      (set! r15 L.rp.25)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 664)
                        (set! rdi 760)
                        (set! r15 L.rp.26)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.27)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.28
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.28)
                    (jump L.+.17 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi 408)
                      (set! rdi 1368)
                      (set! r15 L.rp.29)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi 2016)
                        (set! rdi 552)
                        (set! r15 L.rp.30)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.31)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 1168)
                        (set! rdi 528)
                        (set! r15 L.rp.32)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.33
                        (begin
                          (set! rsi 1424)
                          (set! rdi 1872)
                          (set! r15 L.rp.33)
                          (jump L.+.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.34
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.34)
                          (jump L.+.17 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.35)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.36
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.36)
                  (jump L.-.15 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/void30829.6 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax void5.1) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.-.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.111 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.111 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.111 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.111 rbp rax))))))
    (define L.*.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.112 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.112 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.112 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.112 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.113 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.23 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.22 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.113 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.113 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.113 rbp rax))))))
    (define L.fun/void31698.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.114 r15)
        (begin
          (begin (set! r15 tmp-ra.114) (jump L.fun/void31699.8 rbp r15)))))
    (define L.fun/void31701.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.115 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.115 rbp rax)))))
    (define L.fun/void31700.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.116 r15)
        (begin
          (begin (set! r15 tmp-ra.116) (jump L.fun/void31701.5 rbp r15)))))
    (define L.fun/empty31697.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.117 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.117 rbp rax)))))
    (define L.fun/void31699.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.118 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.118 rbp rax)))))
    (define L.fun/empty31696.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.119 r15)
        (begin
          (begin (set! r15 tmp-ra.119) (jump L.fun/empty31697.7 rbp r15)))))
    (begin
      (set! tmp-ra.120 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi 1264)
                    (set! rdi 608)
                    (set! r15 L.rp.14)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.15
                    (begin
                      (set! rsi 1424)
                      (set! rdi 800)
                      (set! r15 L.rp.15)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.16
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.16)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 360)
                      (set! rdi 1392)
                      (set! r15 L.rp.17)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.18
                      (begin
                        (set! rsi 1936)
                        (set! rdi 1416)
                        (set! r15 L.rp.18)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.19
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.19)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.20)
                    (jump L.*.12 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 1840)
                      (set! rdi 760)
                      (set! r15 L.rp.21)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 1968)
                        (set! rdi 1616)
                        (set! r15 L.rp.22)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.23)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 1792)
                        (set! rdi 360)
                        (set! r15 L.rp.24)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.25
                        (begin
                          (set! rsi 1480)
                          (set! rdi 896)
                          (set! r15 L.rp.25)
                          (jump L.*.12 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.26
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.26)
                          (jump L.+.11 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.27)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.28
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.28)
                  (jump L.*.12 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.29
                  (begin
                    (set! rsi 968)
                    (set! rdi 1584)
                    (set! r15 L.rp.29)
                    (jump L.*.12 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.30
                    (begin
                      (set! rsi 1624)
                      (set! rdi 928)
                      (set! r15 L.rp.30)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.31)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi 1432)
                      (set! rdi 656)
                      (set! r15 L.rp.32)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi 208)
                        (set! rdi 784)
                        (set! r15 L.rp.33)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.34)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.35
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.35)
                    (jump L.*.12 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.36
                    (begin
                      (set! rsi 1192)
                      (set! rdi 192)
                      (set! r15 L.rp.36)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.37
                      (begin
                        (set! rsi 1872)
                        (set! rdi 1424)
                        (set! r15 L.rp.37)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.38
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.38)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi 88)
                        (set! rdi 176)
                        (set! r15 L.rp.39)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.40
                        (begin
                          (set! rsi 1120)
                          (set! rdi 912)
                          (set! r15 L.rp.40)
                          (jump L.*.12 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.41
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.41)
                          (jump L.-.13 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.42
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.42)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.43
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.43)
                  (jump L.+.11 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (return-point L.rp.44
            (begin (set! r15 L.rp.44) (jump L.fun/empty31696.9 rbp r15)))
          (set! empty2.4 rax))
        (begin
          (return-point L.rp.45
            (begin (set! r15 L.rp.45) (jump L.fun/void31698.4 rbp r15)))
          (set! void3.3 rax))
        (begin
          (return-point L.rp.46
            (begin (set! r15 L.rp.46) (jump L.fun/void31700.6 rbp r15)))
          (set! void4.2 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.47
                  (begin
                    (set! rsi 896)
                    (set! rdi 640)
                    (set! r15 L.rp.47)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.99 rax))
              (begin
                (begin
                  (return-point L.rp.48
                    (begin
                      (set! rsi 88)
                      (set! rdi 1112)
                      (set! r15 L.rp.48)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.100 rax))
                (begin
                  (return-point L.rp.49
                    (begin
                      (set! rsi tmp.100)
                      (set! rdi tmp.99)
                      (set! r15 L.rp.49)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.98 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.50
                    (begin
                      (set! rsi 1928)
                      (set! rdi 1952)
                      (set! r15 L.rp.50)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.102 rax))
                (begin
                  (begin
                    (return-point L.rp.51
                      (begin
                        (set! rsi 1384)
                        (set! rdi 1552)
                        (set! r15 L.rp.51)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.103 rax))
                  (begin
                    (return-point L.rp.52
                      (begin
                        (set! rsi tmp.103)
                        (set! rdi tmp.102)
                        (set! r15 L.rp.52)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.101 rax))))
              (begin
                (return-point L.rp.53
                  (begin
                    (set! rsi tmp.101)
                    (set! rdi tmp.98)
                    (set! r15 L.rp.53)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.97 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.54
                    (begin
                      (set! rsi 960)
                      (set! rdi 272)
                      (set! r15 L.rp.54)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.106 rax))
                (begin
                  (begin
                    (return-point L.rp.55
                      (begin
                        (set! rsi 1392)
                        (set! rdi 1216)
                        (set! r15 L.rp.55)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.107 rax))
                  (begin
                    (return-point L.rp.56
                      (begin
                        (set! rsi tmp.107)
                        (set! rdi tmp.106)
                        (set! r15 L.rp.56)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.105 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.57
                      (begin
                        (set! rsi 1432)
                        (set! rdi 1152)
                        (set! r15 L.rp.57)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.109 rax))
                  (begin
                    (begin
                      (return-point L.rp.58
                        (begin
                          (set! rsi 832)
                          (set! rdi 1216)
                          (set! r15 L.rp.58)
                          (jump L.-.13 rbp r15 rdi rsi)))
                      (set! tmp.110 rax))
                    (begin
                      (return-point L.rp.59
                        (begin
                          (set! rsi tmp.110)
                          (set! rdi tmp.109)
                          (set! r15 L.rp.59)
                          (jump L.+.11 rbp r15 rdi rsi)))
                      (set! tmp.108 rax))))
                (begin
                  (return-point L.rp.60
                    (begin
                      (set! rsi tmp.108)
                      (set! rdi tmp.105)
                      (set! r15 L.rp.60)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.104 rax))))
            (begin
              (return-point L.rp.61
                (begin
                  (set! rsi tmp.104)
                  (set! rdi tmp.97)
                  (set! r15 L.rp.61)
                  (jump L.*.12 rbp r15 rdi rsi)))
              (set! fixnum5.1 rax))))
        (begin (set! rax void3.3) (jump tmp-ra.120 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.pair?.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.49 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.49 7)) (= tmp.56 1))
            (begin (set! rax 14) (jump tmp-ra.99 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.99 rbp rax))))))
    (define L.-.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.25 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.24 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.100 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.100 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.100 rbp rax))))))
    (define L.+.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.22 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.101 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.101 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.101 rbp rax))))))
    (define L.*.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.21 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.20 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin
                (set! tmp.69 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.69)) (jump tmp-ra.102 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.102 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.102 rbp rax))))))
    (define L.fun/void46565.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin
          (begin (set! r15 tmp-ra.103) (jump L.fun/void46566.6 rbp r15)))))
    (define L.fun/error46563.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin
          (begin (set! r15 tmp-ra.104) (jump L.fun/error46564.8 rbp r15)))))
    (define L.fun/void46566.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/empty46562.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.106 rbp rax)))))
    (define L.fun/error46564.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin (begin (set! rax 49982) (jump tmp-ra.107 rbp rax)))))
    (define L.fun/empty46561.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.108 r15)
        (begin
          (begin (set! r15 tmp-ra.108) (jump L.fun/empty46562.7 rbp r15)))))
    (define L.fun/any46567.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.109 r15)
        (begin (begin (set! rax 1696) (jump tmp-ra.109 rbp rax)))))
    (begin
      (set! tmp-ra.110 r15)
      (begin
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/empty46561.9 rbp r15)))
          (set! empty0.6 rax))
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/error46563.5 rbp r15)))
          (set! error1.5 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 1432)
                    (set! rdi 152)
                    (set! r15 L.rp.18)
                    (jump L.*.12 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 288)
                      (set! rdi 640)
                      (set! r15 L.rp.19)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.20)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 424)
                      (set! rdi 608)
                      (set! r15 L.rp.21)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 1016)
                        (set! rdi 1656)
                        (set! r15 L.rp.22)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.76 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.76)
                        (set! rdi tmp.75)
                        (set! r15 L.rp.23)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.74 rax))))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.74)
                    (set! rdi tmp.71)
                    (set! r15 L.rp.24)
                    (jump L.-.14 rbp r15 rdi rsi)))
                (set! tmp.70 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 600)
                      (set! rdi 1392)
                      (set! r15 L.rp.25)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 856)
                        (set! rdi 280)
                        (set! r15 L.rp.26)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.80 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.80)
                        (set! rdi tmp.79)
                        (set! r15 L.rp.27)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.78 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 344)
                        (set! rdi 760)
                        (set! r15 L.rp.28)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.82 rax))
                  (begin
                    (begin
                      (return-point L.rp.29
                        (begin
                          (set! rsi 1496)
                          (set! rdi 704)
                          (set! r15 L.rp.29)
                          (jump L.+.13 rbp r15 rdi rsi)))
                      (set! tmp.83 rax))
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi tmp.83)
                          (set! rdi tmp.82)
                          (set! r15 L.rp.30)
                          (jump L.+.13 rbp r15 rdi rsi)))
                      (set! tmp.81 rax))))
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi tmp.81)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.31)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.32
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.70)
                  (set! r15 L.rp.32)
                  (jump L.*.12 rbp r15 rdi rsi)))
              (set! fixnum2.4 rax))))
        (begin
          (return-point L.rp.33
            (begin (set! r15 L.rp.33) (jump L.fun/void46565.4 rbp r15)))
          (set! void3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.34
                  (begin
                    (set! rsi 544)
                    (set! rdi 1784)
                    (set! r15 L.rp.34)
                    (jump L.*.12 rbp r15 rdi rsi)))
                (set! tmp.86 rax))
              (begin
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi 352)
                      (set! rdi 1800)
                      (set! r15 L.rp.35)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.87 rax))
                (begin
                  (return-point L.rp.36
                    (begin
                      (set! rsi tmp.87)
                      (set! rdi tmp.86)
                      (set! r15 L.rp.36)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.85 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi 392)
                      (set! rdi 592)
                      (set! r15 L.rp.37)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.89 rax))
                (begin
                  (begin
                    (return-point L.rp.38
                      (begin
                        (set! rsi 520)
                        (set! rdi 1544)
                        (set! r15 L.rp.38)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.90 rax))
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi tmp.90)
                        (set! rdi tmp.89)
                        (set! r15 L.rp.39)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.88 rax))))
              (begin
                (return-point L.rp.40
                  (begin
                    (set! rsi tmp.88)
                    (set! rdi tmp.85)
                    (set! r15 L.rp.40)
                    (jump L.-.14 rbp r15 rdi rsi)))
                (set! tmp.84 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.41
                    (begin
                      (set! rsi 896)
                      (set! rdi 1288)
                      (set! r15 L.rp.41)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.93 rax))
                (begin
                  (begin
                    (return-point L.rp.42
                      (begin
                        (set! rsi 384)
                        (set! rdi 912)
                        (set! r15 L.rp.42)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.94 rax))
                  (begin
                    (return-point L.rp.43
                      (begin
                        (set! rsi tmp.94)
                        (set! rdi tmp.93)
                        (set! r15 L.rp.43)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.92 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.44
                      (begin
                        (set! rsi 400)
                        (set! rdi 680)
                        (set! r15 L.rp.44)
                        (jump L.*.12 rbp r15 rdi rsi)))
                    (set! tmp.96 rax))
                  (begin
                    (begin
                      (return-point L.rp.45
                        (begin
                          (set! rsi 480)
                          (set! rdi 664)
                          (set! r15 L.rp.45)
                          (jump L.+.13 rbp r15 rdi rsi)))
                      (set! tmp.97 rax))
                    (begin
                      (return-point L.rp.46
                        (begin
                          (set! rsi tmp.97)
                          (set! rdi tmp.96)
                          (set! r15 L.rp.46)
                          (jump L.-.14 rbp r15 rdi rsi)))
                      (set! tmp.95 rax))))
                (begin
                  (return-point L.rp.47
                    (begin
                      (set! rsi tmp.95)
                      (set! rdi tmp.92)
                      (set! r15 L.rp.47)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.91 rax))))
            (begin
              (return-point L.rp.48
                (begin
                  (set! rsi tmp.91)
                  (set! rdi tmp.84)
                  (set! r15 L.rp.48)
                  (jump L.*.12 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (begin
            (return-point L.rp.49
              (begin (set! r15 L.rp.49) (jump L.fun/any46567.10 rbp r15)))
            (set! tmp.98 rax))
          (begin
            (return-point L.rp.50
              (begin
                (set! rdi tmp.98)
                (set! r15 L.rp.50)
                (jump L.pair?.15 rbp r15 rdi)))
            (set! boolean5.1 rax)))
        (begin (set! rax void3.3) (jump tmp-ra.110 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.97 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.97 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.97 rbp rax))))))
    (define L.-.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.25 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.24 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.98 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.98 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.98 rbp rax))))))
    (define L.*.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.99 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.99 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.99 rbp rax))))))
    (define L.fun/error50281.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin (begin (set! rax 16446) (jump tmp-ra.100 rbp rax)))))
    (define L.fun/error50280.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin
          (begin (set! r15 tmp-ra.101) (jump L.fun/error50281.4 rbp r15)))))
    (define L.fun/void50283.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.102 rbp rax)))))
    (define L.fun/ascii-char50284.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin
          (begin
            (set! r15 tmp-ra.103)
            (jump L.fun/ascii-char50285.9 rbp r15)))))
    (define L.fun/void50278.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin
          (begin (set! r15 tmp-ra.104) (jump L.fun/void50279.10 rbp r15)))))
    (define L.fun/ascii-char50285.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/void50279.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.106 rbp rax)))))
    (define L.fun/void50282.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin
          (begin (set! r15 tmp-ra.107) (jump L.fun/void50283.6 rbp r15)))))
    (begin
      (set! tmp-ra.108 r15)
      (begin
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/void50278.8 rbp r15)))
          (set! void0.6 rax))
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/error50280.5 rbp r15)))
          (set! error1.5 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 1120)
                    (set! rdi 616)
                    (set! r15 L.rp.18)
                    (jump L.*.13 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 192)
                      (set! rdi 1456)
                      (set! r15 L.rp.19)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.20)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 424)
                      (set! rdi 1400)
                      (set! r15 L.rp.21)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 1392)
                        (set! rdi 1416)
                        (set! r15 L.rp.22)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.23)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.24)
                    (jump L.*.13 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1424)
                      (set! rdi 1960)
                      (set! r15 L.rp.25)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 128)
                        (set! rdi 336)
                        (set! r15 L.rp.26)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.27)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 1640)
                        (set! rdi 1152)
                        (set! r15 L.rp.28)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.29
                        (begin
                          (set! rsi 312)
                          (set! rdi 1088)
                          (set! r15 L.rp.29)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.30)
                          (jump L.-.14 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.31)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.32
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.32)
                  (jump L.+.15 rbp r15 rdi rsi)))
              (set! fixnum2.4 rax))))
        (begin
          (return-point L.rp.33
            (begin (set! r15 L.rp.33) (jump L.fun/void50282.11 rbp r15)))
          (set! void3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.34
                  (begin
                    (set! rsi 608)
                    (set! rdi 1672)
                    (set! r15 L.rp.34)
                    (jump L.-.14 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi 400)
                      (set! rdi 1736)
                      (set! r15 L.rp.35)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.36
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.36)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi 1520)
                      (set! rdi 1496)
                      (set! r15 L.rp.37)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.38
                      (begin
                        (set! rsi 1136)
                        (set! rdi 304)
                        (set! r15 L.rp.38)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.39)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.40
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.40)
                    (jump L.-.14 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.41
                    (begin
                      (set! rsi 192)
                      (set! rdi 1640)
                      (set! r15 L.rp.41)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.42
                      (begin
                        (set! rsi 1224)
                        (set! rdi 1816)
                        (set! r15 L.rp.42)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.43
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.43)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.44
                      (begin
                        (set! rsi 1312)
                        (set! rdi 520)
                        (set! r15 L.rp.44)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.45
                        (begin
                          (set! rsi 1176)
                          (set! rdi 120)
                          (set! r15 L.rp.45)
                          (jump L.*.13 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.46
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.46)
                          (jump L.-.14 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.47
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.47)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.48
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.48)
                  (jump L.*.13 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.49
            (begin (set! r15 L.rp.49) (jump L.fun/ascii-char50284.7 rbp r15)))
          (set! ascii-char5.1 rax))
        (begin (set! rax fixnum2.4) (jump tmp-ra.108 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.97 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.97 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.97 rbp rax))))))
    (define L.*.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.98 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.98 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.98 rbp rax))))))
    (define L.-.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.99 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.99 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.99 rbp rax))))))
    (define L.fun/ascii-char51656.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (begin
            (set! r15 tmp-ra.100)
            (jump L.fun/ascii-char51657.5 rbp r15)))))
    (define L.fun/ascii-char51657.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.101 rbp rax)))))
    (define L.fun/error51658.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin
          (begin (set! r15 tmp-ra.102) (jump L.fun/error51659.11 rbp r15)))))
    (define L.fun/void51654.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin
          (begin (set! r15 tmp-ra.103) (jump L.fun/void51655.9 rbp r15)))))
    (define L.fun/error51653.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin (begin (set! rax 20030) (jump tmp-ra.104 rbp rax)))))
    (define L.fun/void51655.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/error51652.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin
          (begin (set! r15 tmp-ra.106) (jump L.fun/error51653.8 rbp r15)))))
    (define L.fun/error51659.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin (begin (set! rax 45374) (jump tmp-ra.107 rbp rax)))))
    (begin
      (set! tmp-ra.108 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 32)
                    (set! rdi 696)
                    (set! r15 L.rp.16)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1768)
                      (set! rdi 1576)
                      (set! r15 L.rp.17)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.18)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 408)
                      (set! rdi 1632)
                      (set! r15 L.rp.19)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.20
                      (begin
                        (set! rsi 976)
                        (set! rdi 632)
                        (set! r15 L.rp.20)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.21
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.21)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.22)
                    (jump L.*.14 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 608)
                      (set! rdi 168)
                      (set! r15 L.rp.23)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 400)
                        (set! rdi 1912)
                        (set! r15 L.rp.24)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.25)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1256)
                        (set! rdi 992)
                        (set! r15 L.rp.26)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.27
                        (begin
                          (set! rsi 424)
                          (set! rdi 1144)
                          (set! r15 L.rp.27)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.28
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.28)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.29)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.30
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.30)
                  (jump L.*.14 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (return-point L.rp.31
            (begin (set! r15 L.rp.31) (jump L.fun/error51652.10 rbp r15)))
          (set! error1.5 rax))
        (begin
          (return-point L.rp.32
            (begin (set! r15 L.rp.32) (jump L.fun/void51654.7 rbp r15)))
          (set! void2.4 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.33
                  (begin
                    (set! rsi 1560)
                    (set! rdi 1472)
                    (set! r15 L.rp.33)
                    (jump L.*.14 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi 1624)
                      (set! rdi 592)
                      (set! r15 L.rp.34)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.35)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.36
                    (begin
                      (set! rsi 1352)
                      (set! rdi 536)
                      (set! r15 L.rp.36)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.37
                      (begin
                        (set! rsi 848)
                        (set! rdi 888)
                        (set! r15 L.rp.37)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.38
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.38)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.39
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.39)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.40
                    (begin
                      (set! rsi 56)
                      (set! rdi 1120)
                      (set! r15 L.rp.40)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.41
                      (begin
                        (set! rsi 1016)
                        (set! rdi 824)
                        (set! r15 L.rp.41)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.42
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.42)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.43
                      (begin
                        (set! rsi 1056)
                        (set! rdi 16)
                        (set! r15 L.rp.43)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.44
                        (begin
                          (set! rsi 144)
                          (set! rdi 904)
                          (set! r15 L.rp.44)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.45
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.45)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.46
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.46)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.47
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.47)
                  (jump L.*.14 rbp r15 rdi rsi)))
              (set! fixnum3.3 rax))))
        (begin
          (return-point L.rp.48
            (begin (set! r15 L.rp.48) (jump L.fun/ascii-char51656.4 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (return-point L.rp.49
            (begin (set! r15 L.rp.49) (jump L.fun/error51658.6 rbp r15)))
          (set! error5.1 rax))
        (begin (set! rax void2.4) (jump tmp-ra.108 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () ())))
    (define L.vector?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin
          (set! tmp.50 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.50 7)) (= tmp.56 3))
            (begin (set! rax 14) (jump tmp-ra.58 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.58 rbp rax))))))
    (define L.fun/error54326.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin
          (begin (set! r15 tmp-ra.59) (jump L.fun/error54327.8 rbp r15)))))
    (define L.fun/error54322.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 18238) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/empty54331.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.61 rbp rax)))))
    (define L.fun/void54329.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.62 rbp rax)))))
    (define L.fun/error54327.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin (begin (set! rax 17214) (jump tmp-ra.63 rbp rax)))))
    (define L.fun/error54325.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin (begin (set! rax 8766) (jump tmp-ra.64 rbp rax)))))
    (define L.fun/error54321.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin
          (begin (set! r15 tmp-ra.65) (jump L.fun/error54322.5 rbp r15)))))
    (define L.fun/any54323.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.66 rbp rax)))))
    (define L.fun/void54328.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin (begin (set! r15 tmp-ra.67) (jump L.fun/void54329.7 rbp r15)))))
    (define L.fun/error54324.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin
          (begin (set! r15 tmp-ra.68) (jump L.fun/error54325.9 rbp r15)))))
    (define L.fun/empty54330.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (begin (set! r15 tmp-ra.69) (jump L.fun/empty54331.6 rbp r15)))))
    (begin
      (set! tmp-ra.70 r15)
      (begin
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/error54321.10 rbp r15)))
          (set! error0.6 rax))
        (begin
          (begin
            (return-point L.rp.18
              (begin (set! r15 L.rp.18) (jump L.fun/any54323.11 rbp r15)))
            (set! tmp.57 rax))
          (begin
            (return-point L.rp.19
              (begin
                (set! rdi tmp.57)
                (set! r15 L.rp.19)
                (jump L.vector?.16 rbp r15 rdi)))
            (set! boolean1.5 rax)))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/error54324.13 rbp r15)))
          (set! error2.4 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/error54326.4 rbp r15)))
          (set! error3.3 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/void54328.12 rbp r15)))
          (set! void4.2 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/empty54330.14 rbp r15)))
          (set! empty5.1 rax))
        (begin (set! rax void4.2) (jump tmp-ra.70 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () () ())))
    (define L.*.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.21 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.20 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin
                (set! tmp.60 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.60)) (jump tmp-ra.85 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.85 rbp rax))))))
    (define L.+.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.22 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.86 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.86 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.86 rbp rax))))))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.87 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.87 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.87 rbp rax))))))
    (define L.boolean?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.69 (bitwise-and tmp.44 247)) (= tmp.69 6))
            (begin (set! rax 14) (jump tmp-ra.88 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.88 rbp rax))))))
    (define L.fun/ascii-char64319.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/error64313.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 26430) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/empty64317.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/ascii-char64318.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin
            (set! r15 tmp-ra.92)
            (jump L.fun/ascii-char64319.4 rbp r15)))))
    (define L.fun/void64310.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/empty64315.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/any64311.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 40766) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/error64312.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin
          (begin (set! r15 tmp-ra.96) (jump L.fun/error64313.5 rbp r15)))))
    (define L.fun/empty64316.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (begin (set! r15 tmp-ra.97) (jump L.fun/empty64317.6 rbp r15)))))
    (define L.fun/empty64314.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (begin (set! r15 tmp-ra.98) (jump L.fun/empty64315.9 rbp r15)))))
    (define L.fun/void64309.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin (begin (set! r15 tmp-ra.99) (jump L.fun/void64310.8 rbp r15)))))
    (begin
      (set! tmp-ra.100 r15)
      (begin
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/void64309.14 rbp r15)))
          (set! void0.6 rax))
        (begin
          (begin
            (return-point L.rp.21
              (begin (set! r15 L.rp.21) (jump L.fun/any64311.10 rbp r15)))
            (set! tmp.70 rax))
          (begin
            (return-point L.rp.22
              (begin
                (set! rdi tmp.70)
                (set! r15 L.rp.22)
                (jump L.boolean?.16 rbp r15 rdi)))
            (set! boolean1.5 rax)))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/error64312.11 rbp r15)))
          (set! error2.4 rax))
        (begin
          (return-point L.rp.24
            (begin (set! r15 L.rp.24) (jump L.fun/empty64314.13 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (return-point L.rp.25
            (begin (set! r15 L.rp.25) (jump L.fun/empty64316.12 rbp r15)))
          (set! empty4.2 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.26
                  (begin
                    (set! rsi 1520)
                    (set! rdi 1160)
                    (set! r15 L.rp.26)
                    (jump L.-.17 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi 512)
                      (set! rdi 1104)
                      (set! r15 L.rp.27)
                      (jump L.+.18 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.28
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.28)
                      (jump L.+.18 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi 1808)
                      (set! rdi 1440)
                      (set! r15 L.rp.29)
                      (jump L.*.19 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))
                (begin
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi 1592)
                        (set! rdi 696)
                        (set! r15 L.rp.30)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi tmp.77)
                        (set! rdi tmp.76)
                        (set! r15 L.rp.31)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))))
              (begin
                (return-point L.rp.32
                  (begin
                    (set! rsi tmp.75)
                    (set! rdi tmp.72)
                    (set! r15 L.rp.32)
                    (jump L.-.17 rbp r15 rdi rsi)))
                (set! tmp.71 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.33
                    (begin
                      (set! rsi 976)
                      (set! rdi 184)
                      (set! r15 L.rp.33)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi 880)
                        (set! rdi 8)
                        (set! r15 L.rp.34)
                        (jump L.*.19 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi tmp.81)
                        (set! rdi tmp.80)
                        (set! r15 L.rp.35)
                        (jump L.*.19 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.36
                      (begin
                        (set! rsi 1104)
                        (set! rdi 1112)
                        (set! r15 L.rp.36)
                        (jump L.*.19 rbp r15 rdi rsi)))
                    (set! tmp.83 rax))
                  (begin
                    (begin
                      (return-point L.rp.37
                        (begin
                          (set! rsi 1528)
                          (set! rdi 992)
                          (set! r15 L.rp.37)
                          (jump L.+.18 rbp r15 rdi rsi)))
                      (set! tmp.84 rax))
                    (begin
                      (return-point L.rp.38
                        (begin
                          (set! rsi tmp.84)
                          (set! rdi tmp.83)
                          (set! r15 L.rp.38)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))))
                (begin
                  (return-point L.rp.39
                    (begin
                      (set! rsi tmp.82)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.39)
                      (jump L.*.19 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.40
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.71)
                  (set! r15 L.rp.40)
                  (jump L.+.18 rbp r15 rdi rsi)))
              (set! fixnum5.1 rax))))
        (begin
          (set! r15 tmp-ra.100)
          (jump L.fun/ascii-char64318.7 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.*.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.21 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.20 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin
                (set! tmp.60 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.60)) (jump tmp-ra.83 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.83 rbp rax))))))
    (define L.+.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.22 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.84 rbp rax))))))
    (define L.-.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/void75654.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/empty75657.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin (set! r15 tmp-ra.87) (jump L.fun/empty75658.10 rbp r15)))))
    (define L.fun/ascii-char75662.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.88 rbp rax)))))
    (define L.fun/ascii-char75660.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/empty75656.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/ascii-char75661.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin
            (set! r15 tmp-ra.91)
            (jump L.fun/ascii-char75662.6 rbp r15)))))
    (define L.fun/empty75658.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/empty75655.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin (set! r15 tmp-ra.93) (jump L.fun/empty75656.8 rbp r15)))))
    (define L.fun/void75653.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! r15 tmp-ra.94) (jump L.fun/void75654.4 rbp r15)))))
    (define L.fun/ascii-char75659.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin
          (begin
            (set! r15 tmp-ra.95)
            (jump L.fun/ascii-char75660.7 rbp r15)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 8)
                    (set! rdi 1632)
                    (set! r15 L.rp.18)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 448)
                      (set! rdi 824)
                      (set! r15 L.rp.19)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.20)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 8)
                      (set! rdi 1680)
                      (set! r15 L.rp.21)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 1232)
                        (set! rdi 1912)
                        (set! r15 L.rp.22)
                        (jump L.*.17 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.23)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.24)
                    (jump L.+.16 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1320)
                      (set! rdi 552)
                      (set! r15 L.rp.25)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1840)
                        (set! rdi 1744)
                        (set! r15 L.rp.26)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.27)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 1096)
                        (set! rdi 1744)
                        (set! r15 L.rp.28)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.29
                        (begin
                          (set! rsi 408)
                          (set! rdi 1680)
                          (set! r15 L.rp.29)
                          (jump L.*.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.30)
                          (jump L.-.15 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.31)
                      (jump L.*.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.32
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.32)
                  (jump L.-.15 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (return-point L.rp.33
            (begin (set! r15 L.rp.33) (jump L.fun/void75653.12 rbp r15)))
          (set! void1.5 rax))
        (begin
          (return-point L.rp.34
            (begin (set! r15 L.rp.34) (jump L.fun/empty75655.11 rbp r15)))
          (set! empty2.4 rax))
        (begin
          (return-point L.rp.35
            (begin (set! r15 L.rp.35) (jump L.fun/empty75657.5 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/ascii-char75659.13 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/ascii-char75661.9 rbp r15)))
          (set! ascii-char5.1 rax))
        (begin (set! rax ascii-char5.1) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () () ())))
    (define L.+.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.85 rbp rax))))))
    (define L.*.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.86 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.86 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.86 rbp rax))))))
    (define L.-.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.87 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.87 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.87 rbp rax))))))
    (define L.error?.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.48 rdi)
          (if (begin (set! tmp.69 (bitwise-and tmp.48 255)) (= tmp.69 62))
            (begin (set! rax 14) (jump tmp-ra.88 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.88 rbp rax))))))
    (define L.fun/error83230.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (begin (set! r15 tmp-ra.89) (jump L.fun/error83231.10 rbp r15)))))
    (define L.fun/empty83233.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin (set! r15 tmp-ra.90) (jump L.fun/empty83234.9 rbp r15)))))
    (define L.fun/void83236.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/ascii-char83229.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/void83235.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! r15 tmp-ra.93) (jump L.fun/void83236.6 rbp r15)))))
    (define L.fun/empty83234.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/error83231.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 6974) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/any83232.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/ascii-char83228.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (begin
            (set! r15 tmp-ra.97)
            (jump L.fun/ascii-char83229.7 rbp r15)))))
    (begin
      (set! tmp-ra.98 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/ascii-char83228.12 rbp r15)))
          (set! ascii-char0.6 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/error83230.4 rbp r15)))
          (set! error1.5 rax))
        (begin
          (begin
            (return-point L.rp.20
              (begin (set! r15 L.rp.20) (jump L.fun/any83232.11 rbp r15)))
            (set! tmp.70 rax))
          (begin
            (return-point L.rp.21
              (begin
                (set! rdi tmp.70)
                (set! r15 L.rp.21)
                (jump L.error?.14 rbp r15 rdi)))
            (set! boolean2.4 rax)))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/empty83233.5 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/void83235.8 rbp r15)))
          (set! void4.2 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi 1816)
                    (set! rdi 72)
                    (set! r15 L.rp.24)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 440)
                      (set! rdi 1344)
                      (set! r15 L.rp.25)
                      (jump L.*.16 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.26)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi 192)
                      (set! rdi 912)
                      (set! r15 L.rp.27)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 88)
                        (set! rdi 608)
                        (set! r15 L.rp.28)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi tmp.77)
                        (set! rdi tmp.76)
                        (set! r15 L.rp.29)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))))
              (begin
                (return-point L.rp.30
                  (begin
                    (set! rsi tmp.75)
                    (set! rdi tmp.72)
                    (set! r15 L.rp.30)
                    (jump L.*.16 rbp r15 rdi rsi)))
                (set! tmp.71 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi 136)
                      (set! rdi 1432)
                      (set! r15 L.rp.31)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 984)
                        (set! rdi 824)
                        (set! r15 L.rp.32)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi tmp.81)
                        (set! rdi tmp.80)
                        (set! r15 L.rp.33)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi 504)
                        (set! rdi 208)
                        (set! r15 L.rp.34)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.83 rax))
                  (begin
                    (begin
                      (return-point L.rp.35
                        (begin
                          (set! rsi 720)
                          (set! rdi 1128)
                          (set! r15 L.rp.35)
                          (jump L.+.17 rbp r15 rdi rsi)))
                      (set! tmp.84 rax))
                    (begin
                      (return-point L.rp.36
                        (begin
                          (set! rsi tmp.84)
                          (set! rdi tmp.83)
                          (set! r15 L.rp.36)
                          (jump L.*.16 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))))
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi tmp.82)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.37)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.38
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.71)
                  (set! r15 L.rp.38)
                  (jump L.-.15 rbp r15 rdi rsi)))
              (set! fixnum5.1 rax))))
        (begin (set! rax void4.2) (jump tmp-ra.98 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () ())))
    (define L.error?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin
          (set! tmp.48 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.48 255)) (= tmp.56 62))
            (begin (set! rax 14) (jump tmp-ra.58 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.58 rbp rax))))))
    (define L.fun/ascii-char87923.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin
          (begin
            (set! r15 tmp-ra.59)
            (jump L.fun/ascii-char87924.13 rbp r15)))))
    (define L.fun/empty87931.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/ascii-char87928.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin
          (begin
            (set! r15 tmp-ra.61)
            (jump L.fun/ascii-char87929.7 rbp r15)))))
    (define L.fun/ascii-char87929.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.62 rbp rax)))))
    (define L.fun/ascii-char87926.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.63 rbp rax)))))
    (define L.fun/empty87930.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin
          (begin (set! r15 tmp-ra.64) (jump L.fun/empty87931.5 rbp r15)))))
    (define L.fun/void87922.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.65 rbp rax)))))
    (define L.fun/any87927.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.66 rbp rax)))))
    (define L.fun/ascii-char87925.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin
          (begin
            (set! r15 tmp-ra.67)
            (jump L.fun/ascii-char87926.8 rbp r15)))))
    (define L.fun/ascii-char87924.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.68 rbp rax)))))
    (define L.fun/void87921.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (begin (set! r15 tmp-ra.69) (jump L.fun/void87922.10 rbp r15)))))
    (begin
      (set! tmp-ra.70 r15)
      (begin
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/void87921.14 rbp r15)))
          (set! void0.6 rax))
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/ascii-char87923.4 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/ascii-char87925.12 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (begin
            (return-point L.rp.20
              (begin (set! r15 L.rp.20) (jump L.fun/any87927.11 rbp r15)))
            (set! tmp.57 rax))
          (begin
            (return-point L.rp.21
              (begin
                (set! rdi tmp.57)
                (set! r15 L.rp.21)
                (jump L.error?.16 rbp r15 rdi)))
            (set! boolean3.3 rax)))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/ascii-char87928.6 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/empty87930.9 rbp r15)))
          (set! empty5.1 rax))
        (begin (set! rax empty5.1) (jump tmp-ra.70 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.*.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.21 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.20 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin
                (set! tmp.60 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.60)) (jump tmp-ra.83 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.83 rbp rax))))))
    (define L.-.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.25 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.24 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.84 rbp rax))))))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.23 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.22 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/void88023.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/empty88027.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.87 rbp rax)))))
    (define L.fun/error88024.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin (set! r15 tmp-ra.88) (jump L.fun/error88025.11 rbp r15)))))
    (define L.fun/error88021.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 56894) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/empty88026.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin (set! r15 tmp-ra.90) (jump L.fun/empty88027.5 rbp r15)))))
    (define L.fun/error88029.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 50494) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/error88020.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin (set! r15 tmp-ra.92) (jump L.fun/error88021.7 rbp r15)))))
    (define L.fun/error88025.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 54334) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/void88022.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! r15 tmp-ra.94) (jump L.fun/void88023.4 rbp r15)))))
    (define L.fun/error88028.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin
          (begin (set! r15 tmp-ra.95) (jump L.fun/error88029.9 rbp r15)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/error88020.10 rbp r15)))
          (set! error0.6 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/void88022.12 rbp r15)))
          (set! void1.5 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/error88024.6 rbp r15)))
          (set! error2.4 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.21
                  (begin
                    (set! rsi 608)
                    (set! rdi 440)
                    (set! r15 L.rp.21)
                    (jump L.+.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 0)
                      (set! rdi 184)
                      (set! r15 L.rp.22)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.23)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi 168)
                      (set! rdi 144)
                      (set! r15 L.rp.24)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi 1448)
                        (set! rdi 1224)
                        (set! r15 L.rp.25)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.26)
                        (jump L.*.17 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.27
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.27)
                    (jump L.-.16 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.28
                    (begin
                      (set! rsi 864)
                      (set! rdi 32)
                      (set! r15 L.rp.28)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi 872)
                        (set! rdi 752)
                        (set! r15 L.rp.29)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.30)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi 1912)
                        (set! rdi 1912)
                        (set! r15 L.rp.31)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.32
                        (begin
                          (set! rsi 784)
                          (set! rdi 1704)
                          (set! r15 L.rp.32)
                          (jump L.*.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.33
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.33)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.34)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.35
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.35)
                  (jump L.+.15 rbp r15 rdi rsi)))
              (set! fixnum3.3 rax))))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/empty88026.8 rbp r15)))
          (set! empty4.2 rax))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/error88028.13 rbp r15)))
          (set! error5.1 rax))
        (begin (set! rax error0.6) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.99 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.99 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.99 rbp rax))))))
    (define L.*.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.100 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.100 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.100 rbp rax))))))
    (define L.-.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.101 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.101 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.101 rbp rax))))))
    (define L.pair?.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin
          (set! tmp.49 rdi)
          (if (begin (set! tmp.69 (bitwise-and tmp.49 7)) (= tmp.69 1))
            (begin (set! rax 14) (jump tmp-ra.102 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.102 rbp rax))))))
    (define L.fun/ascii-char95245.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.103 rbp rax)))))
    (define L.fun/empty95247.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.104 rbp rax)))))
    (define L.fun/void95242.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/void95241.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin
          (begin (set! r15 tmp-ra.106) (jump L.fun/void95242.6 rbp r15)))))
    (define L.fun/any95243.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.107 rbp rax)))))
    (define L.fun/ascii-char95244.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.108 r15)
        (begin
          (begin
            (set! r15 tmp-ra.108)
            (jump L.fun/ascii-char95245.4 rbp r15)))))
    (define L.fun/empty95246.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.109 r15)
        (begin
          (begin (set! r15 tmp-ra.109) (jump L.fun/empty95247.5 rbp r15)))))
    (begin
      (set! tmp-ra.110 r15)
      (begin
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/void95241.7 rbp r15)))
          (set! void0.6 rax))
        (begin
          (begin
            (return-point L.rp.17
              (begin (set! r15 L.rp.17) (jump L.fun/any95243.8 rbp r15)))
            (set! tmp.70 rax))
          (begin
            (return-point L.rp.18
              (begin
                (set! rdi tmp.70)
                (set! r15 L.rp.18)
                (jump L.pair?.12 rbp r15 rdi)))
            (set! boolean1.5 rax)))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/ascii-char95244.9 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi 1928)
                    (set! rdi 336)
                    (set! r15 L.rp.20)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 1904)
                      (set! rdi 776)
                      (set! r15 L.rp.21)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.22)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 376)
                      (set! rdi 1720)
                      (set! r15 L.rp.23)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 256)
                        (set! rdi 912)
                        (set! r15 L.rp.24)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi tmp.77)
                        (set! rdi tmp.76)
                        (set! r15 L.rp.25)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))))
              (begin
                (return-point L.rp.26
                  (begin
                    (set! rsi tmp.75)
                    (set! rdi tmp.72)
                    (set! r15 L.rp.26)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.71 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi 1400)
                      (set! rdi 136)
                      (set! r15 L.rp.27)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 1648)
                        (set! rdi 1472)
                        (set! r15 L.rp.28)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi tmp.81)
                        (set! rdi tmp.80)
                        (set! r15 L.rp.29)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi 528)
                        (set! rdi 1240)
                        (set! r15 L.rp.30)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.83 rax))
                  (begin
                    (begin
                      (return-point L.rp.31
                        (begin
                          (set! rsi 728)
                          (set! rdi 1200)
                          (set! r15 L.rp.31)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.84 rax))
                    (begin
                      (return-point L.rp.32
                        (begin
                          (set! rsi tmp.84)
                          (set! rdi tmp.83)
                          (set! r15 L.rp.32)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))))
                (begin
                  (return-point L.rp.33
                    (begin
                      (set! rsi tmp.82)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.33)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.34
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.71)
                  (set! r15 L.rp.34)
                  (jump L.*.14 rbp r15 rdi rsi)))
              (set! fixnum3.3 rax))))
        (begin
          (return-point L.rp.35
            (begin (set! r15 L.rp.35) (jump L.fun/empty95246.10 rbp r15)))
          (set! empty4.2 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.36
                  (begin
                    (set! rsi 1368)
                    (set! rdi 168)
                    (set! r15 L.rp.36)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.87 rax))
              (begin
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi 1400)
                      (set! rdi 1368)
                      (set! r15 L.rp.37)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (return-point L.rp.38
                    (begin
                      (set! rsi tmp.88)
                      (set! rdi tmp.87)
                      (set! r15 L.rp.38)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.39
                    (begin
                      (set! rsi 72)
                      (set! rdi 696)
                      (set! r15 L.rp.39)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))
                (begin
                  (begin
                    (return-point L.rp.40
                      (begin
                        (set! rsi 128)
                        (set! rdi 1616)
                        (set! r15 L.rp.40)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))
                  (begin
                    (return-point L.rp.41
                      (begin
                        (set! rsi tmp.91)
                        (set! rdi tmp.90)
                        (set! r15 L.rp.41)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))))
              (begin
                (return-point L.rp.42
                  (begin
                    (set! rsi tmp.89)
                    (set! rdi tmp.86)
                    (set! r15 L.rp.42)
                    (jump L.+.15 rbp r15 rdi rsi)))
                (set! tmp.85 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.43
                    (begin
                      (set! rsi 896)
                      (set! rdi 360)
                      (set! r15 L.rp.43)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.94 rax))
                (begin
                  (begin
                    (return-point L.rp.44
                      (begin
                        (set! rsi 1472)
                        (set! rdi 1968)
                        (set! r15 L.rp.44)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (return-point L.rp.45
                      (begin
                        (set! rsi tmp.95)
                        (set! rdi tmp.94)
                        (set! r15 L.rp.45)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.46
                      (begin
                        (set! rsi 1136)
                        (set! rdi 1360)
                        (set! r15 L.rp.46)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.97 rax))
                  (begin
                    (begin
                      (return-point L.rp.47
                        (begin
                          (set! rsi 728)
                          (set! rdi 1096)
                          (set! r15 L.rp.47)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.98 rax))
                    (begin
                      (return-point L.rp.48
                        (begin
                          (set! rsi tmp.98)
                          (set! rdi tmp.97)
                          (set! r15 L.rp.48)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))))
                (begin
                  (return-point L.rp.49
                    (begin
                      (set! rsi tmp.96)
                      (set! rdi tmp.93)
                      (set! r15 L.rp.49)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))))
            (begin
              (return-point L.rp.50
                (begin
                  (set! rsi tmp.92)
                  (set! rdi tmp.85)
                  (set! r15 L.rp.50)
                  (jump L.-.13 rbp r15 rdi rsi)))
              (set! fixnum5.1 rax))))
        (begin (set! rax ascii-char2.4) (jump tmp-ra.110 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.*.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.21 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.20 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin
                (set! tmp.60 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.60)) (jump tmp-ra.83 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.83 rbp rax))))))
    (define L.+.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.22 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.84 rbp rax))))))
    (define L.-.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/ascii-char101002.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/void101005.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin (set! r15 tmp-ra.87) (jump L.fun/void101006.12 rbp r15)))))
    (define L.fun/ascii-char101007.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin
            (set! r15 tmp-ra.88)
            (jump L.fun/ascii-char101008.10 rbp r15)))))
    (define L.fun/ascii-char101000.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/void101004.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/ascii-char100999.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin
            (set! r15 tmp-ra.91)
            (jump L.fun/ascii-char101000.7 rbp r15)))))
    (define L.fun/ascii-char101008.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/void101003.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin (set! r15 tmp-ra.93) (jump L.fun/void101004.8 rbp r15)))))
    (define L.fun/void101006.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/ascii-char101001.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin
          (begin
            (set! r15 tmp-ra.95)
            (jump L.fun/ascii-char101002.4 rbp r15)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/ascii-char100999.9 rbp r15)))
          (set! ascii-char0.6 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 720)
                    (set! rdi 1032)
                    (set! r15 L.rp.19)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi 1568)
                      (set! rdi 1368)
                      (set! r15 L.rp.20)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.21)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 952)
                      (set! rdi 576)
                      (set! r15 L.rp.22)
                      (jump L.*.17 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi 976)
                        (set! rdi 112)
                        (set! r15 L.rp.23)
                        (jump L.*.17 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.24)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.25
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.25)
                    (jump L.+.16 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 1048)
                      (set! rdi 1704)
                      (set! r15 L.rp.26)
                      (jump L.*.17 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi 16)
                        (set! rdi 728)
                        (set! r15 L.rp.27)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.28)
                        (jump L.*.17 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi 1072)
                        (set! rdi 784)
                        (set! r15 L.rp.29)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi 1152)
                          (set! rdi 1656)
                          (set! r15 L.rp.30)
                          (jump L.*.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.31
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.31)
                          (jump L.*.17 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.32)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.33
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.33)
                  (jump L.+.16 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (return-point L.rp.34
            (begin
              (set! r15 L.rp.34)
              (jump L.fun/ascii-char101001.13 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (return-point L.rp.35
            (begin (set! r15 L.rp.35) (jump L.fun/void101003.11 rbp r15)))
          (set! void3.3 rax))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/void101005.5 rbp r15)))
          (set! void4.2 rax))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/ascii-char101007.6 rbp r15)))
          (set! ascii-char5.1 rax))
        (begin (set! rax ascii-char5.1) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.83 rbp rax))))))
    (define L.+.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.23 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.22 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.84 rbp rax))))))
    (define L.*.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.85 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/error102221.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 2110) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/void102218.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin (set! r15 tmp-ra.87) (jump L.fun/void102219.11 rbp r15)))))
    (define L.fun/empty102216.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin (set! r15 tmp-ra.88) (jump L.fun/empty102217.9 rbp r15)))))
    (define L.fun/ascii-char102223.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/void102214.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin (set! r15 tmp-ra.90) (jump L.fun/void102215.13 rbp r15)))))
    (define L.fun/empty102217.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/error102220.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin (set! r15 tmp-ra.92) (jump L.fun/error102221.4 rbp r15)))))
    (define L.fun/void102219.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/ascii-char102222.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin
            (set! r15 tmp-ra.94)
            (jump L.fun/ascii-char102223.7 rbp r15)))))
    (define L.fun/void102215.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.95 rbp rax)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/void102214.8 rbp r15)))
          (set! void0.6 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/empty102216.6 rbp r15)))
          (set! empty1.5 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/void102218.5 rbp r15)))
          (set! void2.4 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/error102220.10 rbp r15)))
          (set! error3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 184)
                    (set! rdi 1688)
                    (set! r15 L.rp.22)
                    (jump L.*.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 936)
                      (set! rdi 1192)
                      (set! r15 L.rp.23)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.24)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1936)
                      (set! rdi 1264)
                      (set! r15 L.rp.25)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1968)
                        (set! rdi 656)
                        (set! r15 L.rp.26)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.27)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.28
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.28)
                    (jump L.+.16 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi 1256)
                      (set! rdi 632)
                      (set! r15 L.rp.29)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi 520)
                        (set! rdi 1456)
                        (set! r15 L.rp.30)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.31)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 1832)
                        (set! rdi 568)
                        (set! r15 L.rp.32)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.33
                        (begin
                          (set! rsi 24)
                          (set! rdi 1672)
                          (set! r15 L.rp.33)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.34
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.34)
                          (jump L.+.16 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.35)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.36
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.36)
                  (jump L.-.17 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.37
            (begin
              (set! r15 L.rp.37)
              (jump L.fun/ascii-char102222.12 rbp r15)))
          (set! ascii-char5.1 rax))
        (begin (set! rax error3.3) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () () ())))
    (define L.-.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.85 rbp rax))))))
    (define L.*.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.86 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.86 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.86 rbp rax))))))
    (define L.+.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.23 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.22 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.87 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.87 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.87 rbp rax))))))
    (define L.vector?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.50 rdi)
          (if (begin (set! tmp.69 (bitwise-and tmp.50 7)) (= tmp.69 3))
            (begin (set! rax 14) (jump tmp-ra.88 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.88 rbp rax))))))
    (define L.fun/void102243.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (begin (set! r15 tmp-ra.89) (jump L.fun/void102244.10 rbp r15)))))
    (define L.fun/any102238.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/ascii-char102236.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin
            (set! r15 tmp-ra.91)
            (jump L.fun/ascii-char102237.13 rbp r15)))))
    (define L.fun/void102241.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin (set! r15 tmp-ra.92) (jump L.fun/void102242.11 rbp r15)))))
    (define L.fun/ascii-char102239.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin
            (set! r15 tmp-ra.93)
            (jump L.fun/ascii-char102240.12 rbp r15)))))
    (define L.fun/ascii-char102234.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin
            (set! r15 tmp-ra.94)
            (jump L.fun/ascii-char102235.14 rbp r15)))))
    (define L.fun/void102244.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/void102242.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/ascii-char102240.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.97 rbp rax)))))
    (define L.fun/ascii-char102237.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.98 rbp rax)))))
    (define L.fun/ascii-char102235.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.99 rbp rax)))))
    (begin
      (set! tmp-ra.100 r15)
      (begin
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/ascii-char102234.9 rbp r15)))
          (set! ascii-char0.6 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/ascii-char102236.6 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (begin
            (return-point L.rp.22
              (begin (set! r15 L.rp.22) (jump L.fun/any102238.5 rbp r15)))
            (set! tmp.70 rax))
          (begin
            (return-point L.rp.23
              (begin
                (set! rdi tmp.70)
                (set! r15 L.rp.23)
                (jump L.vector?.16 rbp r15 rdi)))
            (set! boolean2.4 rax)))
        (begin
          (return-point L.rp.24
            (begin (set! r15 L.rp.24) (jump L.fun/ascii-char102239.8 rbp r15)))
          (set! ascii-char3.3 rax))
        (begin
          (return-point L.rp.25
            (begin (set! r15 L.rp.25) (jump L.fun/void102241.7 rbp r15)))
          (set! void4.2 rax))
        (begin
          (return-point L.rp.26
            (begin (set! r15 L.rp.26) (jump L.fun/void102243.4 rbp r15)))
          (set! void5.1 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.27
                  (begin
                    (set! rsi 1032)
                    (set! rdi 1848)
                    (set! r15 L.rp.27)
                    (jump L.+.17 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.28
                    (begin
                      (set! rsi 1088)
                      (set! rdi 488)
                      (set! r15 L.rp.28)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.29)
                      (jump L.*.18 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.30
                    (begin
                      (set! rsi 264)
                      (set! rdi 1440)
                      (set! r15 L.rp.30)
                      (jump L.-.19 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))
                (begin
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi 1248)
                        (set! rdi 1976)
                        (set! r15 L.rp.31)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi tmp.77)
                        (set! rdi tmp.76)
                        (set! r15 L.rp.32)
                        (jump L.*.18 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))))
              (begin
                (return-point L.rp.33
                  (begin
                    (set! rsi tmp.75)
                    (set! rdi tmp.72)
                    (set! r15 L.rp.33)
                    (jump L.*.18 rbp r15 rdi rsi)))
                (set! tmp.71 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi 1616)
                      (set! rdi 1904)
                      (set! r15 L.rp.34)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi 312)
                        (set! rdi 576)
                        (set! r15 L.rp.35)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (return-point L.rp.36
                      (begin
                        (set! rsi tmp.81)
                        (set! rdi tmp.80)
                        (set! r15 L.rp.36)
                        (jump L.-.19 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.37
                      (begin
                        (set! rsi 1400)
                        (set! rdi 784)
                        (set! r15 L.rp.37)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.83 rax))
                  (begin
                    (begin
                      (return-point L.rp.38
                        (begin
                          (set! rsi 1936)
                          (set! rdi 592)
                          (set! r15 L.rp.38)
                          (jump L.*.18 rbp r15 rdi rsi)))
                      (set! tmp.84 rax))
                    (begin
                      (return-point L.rp.39
                        (begin
                          (set! rsi tmp.84)
                          (set! rdi tmp.83)
                          (set! r15 L.rp.39)
                          (jump L.-.19 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))))
                (begin
                  (return-point L.rp.40
                    (begin
                      (set! rsi tmp.82)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.40)
                      (jump L.-.19 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (set! rsi tmp.78)
              (set! rdi tmp.71)
              (set! r15 tmp-ra.100)
              (jump L.*.18 rbp r15 rdi rsi))))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.+.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.83 rbp rax))))))
    (define L.-.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.25 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.24 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.84 rbp rax))))))
    (define L.*.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.85 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/empty104399.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin
          (begin (set! r15 tmp-ra.86) (jump L.fun/empty104400.8 rbp r15)))))
    (define L.fun/void104397.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin (set! r15 tmp-ra.87) (jump L.fun/void104398.12 rbp r15)))))
    (define L.fun/error104406.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin (begin (set! rax 52798) (jump tmp-ra.88 rbp rax)))))
    (define L.fun/empty104404.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/empty104400.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/error104405.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin (set! r15 tmp-ra.91) (jump L.fun/error104406.6 rbp r15)))))
    (define L.fun/empty104395.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin (set! r15 tmp-ra.92) (jump L.fun/empty104396.14 rbp r15)))))
    (define L.fun/void104401.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin (set! r15 tmp-ra.93) (jump L.fun/void104402.13 rbp r15)))))
    (define L.fun/void104398.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/void104402.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/empty104396.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/empty104403.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (begin (set! r15 tmp-ra.97) (jump L.fun/empty104404.7 rbp r15)))))
    (begin
      (set! tmp-ra.98 r15)
      (begin
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/empty104395.10 rbp r15)))
          (set! empty0.6 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void104397.5 rbp r15)))
          (set! void1.5 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/empty104399.4 rbp r15)))
          (set! empty2.4 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/void104401.11 rbp r15)))
          (set! void3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi 984)
                    (set! rdi 1080)
                    (set! r15 L.rp.24)
                    (jump L.*.17 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1576)
                      (set! rdi 128)
                      (set! r15 L.rp.25)
                      (jump L.*.17 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.26)
                      (jump L.-.18 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi 576)
                      (set! rdi 1344)
                      (set! r15 L.rp.27)
                      (jump L.-.18 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 560)
                        (set! rdi 832)
                        (set! r15 L.rp.28)
                        (jump L.*.17 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.29)
                        (jump L.*.17 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.30
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.30)
                    (jump L.*.17 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi 568)
                      (set! rdi 1424)
                      (set! r15 L.rp.31)
                      (jump L.+.19 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 776)
                        (set! rdi 1512)
                        (set! r15 L.rp.32)
                        (jump L.+.19 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.33)
                        (jump L.-.18 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi 1344)
                        (set! rdi 208)
                        (set! r15 L.rp.34)
                        (jump L.*.17 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.35
                        (begin
                          (set! rsi 1296)
                          (set! rdi 608)
                          (set! r15 L.rp.35)
                          (jump L.-.18 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.36
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.36)
                          (jump L.+.19 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.37)
                      (jump L.*.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.38
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.38)
                  (jump L.*.17 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.39
            (begin (set! r15 L.rp.39) (jump L.fun/empty104403.15 rbp r15)))
          (set! empty5.1 rax))
        (begin (set! r15 tmp-ra.98) (jump L.fun/error104405.9 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () ())))
    (define L.boolean?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.44 247)) (= tmp.56 6))
            (begin (set! rax 14) (jump tmp-ra.58 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.58 rbp rax))))))
    (define L.fun/any105578.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 1504) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/error105579.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin
          (begin (set! r15 tmp-ra.60) (jump L.fun/error105580.11 rbp r15)))))
    (define L.fun/error105575.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 18494) (jump tmp-ra.61 rbp rax)))))
    (define L.fun/ascii-char105573.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.62 rbp rax)))))
    (define L.fun/error105576.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin
          (begin (set! r15 tmp-ra.63) (jump L.fun/error105577.13 rbp r15)))))
    (define L.fun/ascii-char105572.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin
          (begin
            (set! r15 tmp-ra.64)
            (jump L.fun/ascii-char105573.7 rbp r15)))))
    (define L.fun/void105581.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin
          (begin (set! r15 tmp-ra.65) (jump L.fun/void105582.14 rbp r15)))))
    (define L.fun/error105580.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin (begin (set! rax 31038) (jump tmp-ra.66 rbp rax)))))
    (define L.fun/error105574.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin
          (begin (set! r15 tmp-ra.67) (jump L.fun/error105575.6 rbp r15)))))
    (define L.fun/error105577.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin (begin (set! rax 55358) (jump tmp-ra.68 rbp rax)))))
    (define L.fun/void105582.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.69 rbp rax)))))
    (begin
      (set! tmp-ra.70 r15)
      (begin
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/ascii-char105572.9 rbp r15)))
          (set! ascii-char0.6 rax))
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/error105574.12 rbp r15)))
          (set! error1.5 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/error105576.8 rbp r15)))
          (set! error2.4 rax))
        (begin
          (begin
            (return-point L.rp.20
              (begin (set! r15 L.rp.20) (jump L.fun/any105578.4 rbp r15)))
            (set! tmp.57 rax))
          (begin
            (return-point L.rp.21
              (begin
                (set! rdi tmp.57)
                (set! r15 L.rp.21)
                (jump L.boolean?.16 rbp r15 rdi)))
            (set! boolean3.3 rax)))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/error105579.5 rbp r15)))
          (set! error4.2 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/void105581.10 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax void5.1) (jump tmp-ra.70 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () ())))
    (define L.ascii-char?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin
          (set! tmp.47 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.47 255)) (= tmp.56 46))
            (begin (set! rax 14) (jump tmp-ra.58 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.58 rbp rax))))))
    (define L.fun/void121813.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/ascii-char121818.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin
          (begin
            (set! r15 tmp-ra.60)
            (jump L.fun/ascii-char121819.11 rbp r15)))))
    (define L.fun/void121814.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin
          (begin (set! r15 tmp-ra.61) (jump L.fun/void121815.12 rbp r15)))))
    (define L.fun/ascii-char121816.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin
          (begin
            (set! r15 tmp-ra.62)
            (jump L.fun/ascii-char121817.9 rbp r15)))))
    (define L.fun/void121812.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin
          (begin (set! r15 tmp-ra.63) (jump L.fun/void121813.4 rbp r15)))))
    (define L.fun/ascii-char121817.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.64 rbp rax)))))
    (define L.fun/ascii-char121811.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.65 rbp rax)))))
    (define L.fun/ascii-char121819.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.66 rbp rax)))))
    (define L.fun/void121815.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.67 rbp rax)))))
    (define L.fun/ascii-char121810.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin
          (begin
            (set! r15 tmp-ra.68)
            (jump L.fun/ascii-char121811.10 rbp r15)))))
    (define L.fun/any121809.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.69 rbp rax)))))
    (begin
      (set! tmp-ra.70 r15)
      (begin
        (begin
          (begin
            (return-point L.rp.17
              (begin (set! r15 L.rp.17) (jump L.fun/any121809.14 rbp r15)))
            (set! tmp.57 rax))
          (begin
            (return-point L.rp.18
              (begin
                (set! rdi tmp.57)
                (set! r15 L.rp.18)
                (jump L.ascii-char?.16 rbp r15 rdi)))
            (set! boolean0.6 rax)))
        (begin
          (return-point L.rp.19
            (begin
              (set! r15 L.rp.19)
              (jump L.fun/ascii-char121810.13 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/void121812.8 rbp r15)))
          (set! void2.4 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void121814.6 rbp r15)))
          (set! void3.3 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/ascii-char121816.7 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/ascii-char121818.5 rbp r15)))
          (set! ascii-char5.1 rax))
        (begin (set! rax ascii-char4.2) (jump tmp-ra.70 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.97 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.97 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.97 rbp rax))))))
    (define L.*.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.98 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.98 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.98 rbp rax))))))
    (define L.-.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.99 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.99 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.99 rbp rax))))))
    (define L.fun/error128005.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (begin (set! r15 tmp-ra.100) (jump L.fun/error128006.9 rbp r15)))))
    (define L.fun/empty128002.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.101 rbp rax)))))
    (define L.fun/void128004.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.102 rbp rax)))))
    (define L.fun/empty128001.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin
          (begin (set! r15 tmp-ra.103) (jump L.fun/empty128002.5 rbp r15)))))
    (define L.fun/empty128008.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.104 rbp rax)))))
    (define L.fun/error128006.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 9278) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/empty128007.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin
          (begin (set! r15 tmp-ra.106) (jump L.fun/empty128008.8 rbp r15)))))
    (define L.fun/void128003.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin
          (begin (set! r15 tmp-ra.107) (jump L.fun/void128004.6 rbp r15)))))
    (begin
      (set! tmp-ra.108 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 1144)
                    (set! rdi 1312)
                    (set! r15 L.rp.16)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 2016)
                      (set! rdi 1080)
                      (set! r15 L.rp.17)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.18)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 1384)
                      (set! rdi 40)
                      (set! r15 L.rp.19)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.20
                      (begin
                        (set! rsi 1832)
                        (set! rdi 1872)
                        (set! r15 L.rp.20)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.21
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.21)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.22)
                    (jump L.*.14 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 264)
                      (set! rdi 1008)
                      (set! r15 L.rp.23)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 952)
                        (set! rdi 1376)
                        (set! r15 L.rp.24)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.25)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 464)
                        (set! rdi 1376)
                        (set! r15 L.rp.26)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.27
                        (begin
                          (set! rsi 656)
                          (set! rdi 776)
                          (set! r15 L.rp.27)
                          (jump L.-.13 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.28
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.28)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.29)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.30
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.30)
                  (jump L.-.13 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (return-point L.rp.31
            (begin (set! r15 L.rp.31) (jump L.fun/empty128001.7 rbp r15)))
          (set! empty1.5 rax))
        (begin
          (return-point L.rp.32
            (begin (set! r15 L.rp.32) (jump L.fun/void128003.11 rbp r15)))
          (set! void2.4 rax))
        (begin
          (return-point L.rp.33
            (begin (set! r15 L.rp.33) (jump L.fun/error128005.4 rbp r15)))
          (set! error3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.34
                  (begin
                    (set! rsi 928)
                    (set! rdi 816)
                    (set! r15 L.rp.34)
                    (jump L.*.14 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi 824)
                      (set! rdi 1728)
                      (set! r15 L.rp.35)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.36
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.36)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi 1736)
                      (set! rdi 1480)
                      (set! r15 L.rp.37)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.38
                      (begin
                        (set! rsi 776)
                        (set! rdi 1816)
                        (set! r15 L.rp.38)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.39)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.40
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.40)
                    (jump L.+.15 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.41
                    (begin
                      (set! rsi 1104)
                      (set! rdi 1272)
                      (set! r15 L.rp.41)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.42
                      (begin
                        (set! rsi 1264)
                        (set! rdi 1912)
                        (set! r15 L.rp.42)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.43
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.43)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.44
                      (begin
                        (set! rsi 848)
                        (set! rdi 1968)
                        (set! r15 L.rp.44)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.45
                        (begin
                          (set! rsi 8)
                          (set! rdi 1560)
                          (set! r15 L.rp.45)
                          (jump L.-.13 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.46
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.46)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.47
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.47)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.48
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.48)
                  (jump L.-.13 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.49
            (begin (set! r15 L.rp.49) (jump L.fun/empty128007.10 rbp r15)))
          (set! empty5.1 rax))
        (begin (set! rax empty5.1) (jump tmp-ra.108 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.+.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.83 rbp rax))))))
    (define L.*.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.84 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.84 rbp rax))))))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/ascii-char138303.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/empty138301.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.87 rbp rax)))))
    (define L.fun/empty138304.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin (set! r15 tmp-ra.88) (jump L.fun/empty138305.13 rbp r15)))))
    (define L.fun/empty138307.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/empty138306.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin (set! r15 tmp-ra.90) (jump L.fun/empty138307.7 rbp r15)))))
    (define L.fun/ascii-char138302.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin
            (set! r15 tmp-ra.91)
            (jump L.fun/ascii-char138303.4 rbp r15)))))
    (define L.fun/ascii-char138308.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin
            (set! r15 tmp-ra.92)
            (jump L.fun/ascii-char138309.12 rbp r15)))))
    (define L.fun/error138310.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin (set! r15 tmp-ra.93) (jump L.fun/error138311.15 rbp r15)))))
    (define L.fun/ascii-char138309.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/empty138305.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/empty138300.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin
          (begin (set! r15 tmp-ra.96) (jump L.fun/empty138301.5 rbp r15)))))
    (define L.fun/error138311.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin (begin (set! rax 28990) (jump tmp-ra.97 rbp rax)))))
    (begin
      (set! tmp-ra.98 r15)
      (begin
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/empty138300.14 rbp r15)))
          (set! empty0.6 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/ascii-char138302.9 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/empty138304.6 rbp r15)))
          (set! empty2.4 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/empty138306.8 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi 1600)
                    (set! rdi 2024)
                    (set! r15 L.rp.24)
                    (jump L.-.17 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 376)
                      (set! rdi 1728)
                      (set! r15 L.rp.25)
                      (jump L.*.18 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.26)
                      (jump L.+.19 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi 272)
                      (set! rdi 1504)
                      (set! r15 L.rp.27)
                      (jump L.*.18 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 72)
                        (set! rdi 360)
                        (set! r15 L.rp.28)
                        (jump L.*.18 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.29)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.30
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.30)
                    (jump L.*.18 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi 1448)
                      (set! rdi 1984)
                      (set! r15 L.rp.31)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 24)
                        (set! rdi 1992)
                        (set! r15 L.rp.32)
                        (jump L.*.18 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.33)
                        (jump L.*.18 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi 1576)
                        (set! rdi 856)
                        (set! r15 L.rp.34)
                        (jump L.+.19 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.35
                        (begin
                          (set! rsi 256)
                          (set! rdi 1424)
                          (set! r15 L.rp.35)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.36
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.36)
                          (jump L.*.18 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.37)
                      (jump L.+.19 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.38
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.38)
                  (jump L.*.18 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.39
            (begin
              (set! r15 L.rp.39)
              (jump L.fun/ascii-char138308.10 rbp r15)))
          (set! ascii-char5.1 rax))
        (begin (set! r15 tmp-ra.98) (jump L.fun/error138310.11 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () () () ())))
    (define L.boolean?.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.44 247)) (= tmp.56 6))
            (begin (set! rax 14) (jump tmp-ra.87 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.87 rbp rax))))))
    (define L.+.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.88 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.88 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.88 rbp rax))))))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.25 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.24 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.89 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.89 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.89 rbp rax))))))
    (define L.*.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.21 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.20 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin
                (set! tmp.69 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.69)) (jump tmp-ra.90 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.90 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.90 rbp rax))))))
    (define L.fixnum?.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (set! tmp.43 rdi)
          (if (begin (set! tmp.70 (bitwise-and tmp.43 7)) (= tmp.70 0))
            (begin (set! rax 14) (jump tmp-ra.91 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.91 rbp rax))))))
    (define L.fun/error139284.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 15934) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/ascii-char139281.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin
            (set! r15 tmp-ra.93)
            (jump L.fun/ascii-char139282.10 rbp r15)))))
    (define L.fun/error139283.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/error139284.4 rbp r15)))))
    (define L.fun/void139290.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/any139288.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 13886) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/empty139287.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.97 rbp rax)))))
    (define L.fun/ascii-char139282.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.98 rbp rax)))))
    (define L.fun/any139285.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.99 rbp rax)))))
    (define L.fun/void139289.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (begin (set! r15 tmp-ra.100) (jump L.fun/void139290.7 rbp r15)))))
    (define L.fun/empty139286.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin
          (begin (set! r15 tmp-ra.101) (jump L.fun/empty139287.9 rbp r15)))))
    (begin
      (set! tmp-ra.102 r15)
      (begin
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/ascii-char139281.5 rbp r15)))
          (set! ascii-char0.6 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/error139283.6 rbp r15)))
          (set! error1.5 rax))
        (begin
          (begin
            (return-point L.rp.22
              (begin (set! r15 L.rp.22) (jump L.fun/any139285.11 rbp r15)))
            (set! tmp.71 rax))
          (begin
            (return-point L.rp.23
              (begin
                (set! rdi tmp.71)
                (set! r15 L.rp.23)
                (jump L.fixnum?.15 rbp r15 rdi)))
            (set! boolean2.4 rax)))
        (begin
          (return-point L.rp.24
            (begin (set! r15 L.rp.24) (jump L.fun/empty139286.13 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.25
                  (begin
                    (set! rsi 1832)
                    (set! rdi 1960)
                    (set! r15 L.rp.25)
                    (jump L.*.16 rbp r15 rdi rsi)))
                (set! tmp.74 rax))
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 768)
                      (set! rdi 1680)
                      (set! r15 L.rp.26)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi tmp.75)
                      (set! rdi tmp.74)
                      (set! r15 L.rp.27)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.28
                    (begin
                      (set! rsi 1520)
                      (set! rdi 856)
                      (set! r15 L.rp.28)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))
                (begin
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi 1096)
                        (set! rdi 1264)
                        (set! r15 L.rp.29)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.78 rax))
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi tmp.78)
                        (set! rdi tmp.77)
                        (set! r15 L.rp.30)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.76 rax))))
              (begin
                (return-point L.rp.31
                  (begin
                    (set! rsi tmp.76)
                    (set! rdi tmp.73)
                    (set! r15 L.rp.31)
                    (jump L.+.18 rbp r15 rdi rsi)))
                (set! tmp.72 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi 96)
                      (set! rdi 160)
                      (set! r15 L.rp.32)
                      (jump L.+.18 rbp r15 rdi rsi)))
                  (set! tmp.81 rax))
                (begin
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi 240)
                        (set! rdi 696)
                        (set! r15 L.rp.33)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.82 rax))
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi tmp.82)
                        (set! rdi tmp.81)
                        (set! r15 L.rp.34)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.80 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi 1704)
                        (set! rdi 880)
                        (set! r15 L.rp.35)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.84 rax))
                  (begin
                    (begin
                      (return-point L.rp.36
                        (begin
                          (set! rsi 1640)
                          (set! rdi 1696)
                          (set! r15 L.rp.36)
                          (jump L.*.16 rbp r15 rdi rsi)))
                      (set! tmp.85 rax))
                    (begin
                      (return-point L.rp.37
                        (begin
                          (set! rsi tmp.85)
                          (set! rdi tmp.84)
                          (set! r15 L.rp.37)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.83 rax))))
                (begin
                  (return-point L.rp.38
                    (begin
                      (set! rsi tmp.83)
                      (set! rdi tmp.80)
                      (set! r15 L.rp.38)
                      (jump L.+.18 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))))
            (begin
              (return-point L.rp.39
                (begin
                  (set! rsi tmp.79)
                  (set! rdi tmp.72)
                  (set! r15 L.rp.39)
                  (jump L.+.18 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (begin
            (return-point L.rp.40
              (begin (set! r15 L.rp.40) (jump L.fun/any139288.8 rbp r15)))
            (set! tmp.86 rax))
          (begin
            (return-point L.rp.41
              (begin
                (set! rdi tmp.86)
                (set! r15 L.rp.41)
                (jump L.boolean?.19 rbp r15 rdi)))
            (set! boolean5.1 rax)))
        (begin (set! r15 tmp-ra.102) (jump L.fun/void139289.12 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.-.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.125 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.125 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.125 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.125 rbp rax))))))
    (define L.+.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.126 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.23 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.22 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.126 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.126 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.126 rbp rax))))))
    (define L.*.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.127 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.127 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.127 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.127 rbp rax))))))
    (define L.fun/void140574.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.128 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.128 rbp rax)))))
    (define L.fun/empty140576.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.129 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.129 rbp rax)))))
    (define L.fun/empty140575.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.130 r15)
        (begin
          (begin (set! r15 tmp-ra.130) (jump L.fun/empty140576.5 rbp r15)))))
    (define L.fun/void140573.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.131 r15)
        (begin
          (begin (set! r15 tmp-ra.131) (jump L.fun/void140574.4 rbp r15)))))
    (begin
      (set! tmp-ra.132 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.12
                  (begin
                    (set! rsi 336)
                    (set! rdi 1776)
                    (set! r15 L.rp.12)
                    (jump L.*.9 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.13
                    (begin
                      (set! rsi 728)
                      (set! rdi 728)
                      (set! r15 L.rp.13)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.14
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.14)
                      (jump L.*.9 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.15
                    (begin
                      (set! rsi 1264)
                      (set! rdi 1176)
                      (set! r15 L.rp.15)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.16
                      (begin
                        (set! rsi 984)
                        (set! rdi 496)
                        (set! r15 L.rp.16)
                        (jump L.-.11 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.17
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.17)
                        (jump L.*.9 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.18)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 1616)
                      (set! rdi 1520)
                      (set! r15 L.rp.19)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.20
                      (begin
                        (set! rsi 1392)
                        (set! rdi 24)
                        (set! r15 L.rp.20)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.21
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.21)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 1456)
                        (set! rdi 824)
                        (set! r15 L.rp.22)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.23
                        (begin
                          (set! rsi 240)
                          (set! rdi 1952)
                          (set! r15 L.rp.23)
                          (jump L.+.10 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.24
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.24)
                          (jump L.-.11 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.25)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.26
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.26)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.27
                  (begin
                    (set! rsi 816)
                    (set! rdi 1424)
                    (set! r15 L.rp.27)
                    (jump L.*.9 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.28
                    (begin
                      (set! rsi 1520)
                      (set! rdi 784)
                      (set! r15 L.rp.28)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.29)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.30
                    (begin
                      (set! rsi 1680)
                      (set! rdi 2032)
                      (set! r15 L.rp.30)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi 328)
                        (set! rdi 1776)
                        (set! r15 L.rp.31)
                        (jump L.*.9 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.32)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.33
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.33)
                    (jump L.*.9 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi 1216)
                      (set! rdi 2000)
                      (set! r15 L.rp.34)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi 1280)
                        (set! rdi 1928)
                        (set! r15 L.rp.35)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.36
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.36)
                        (jump L.-.11 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.37
                      (begin
                        (set! rsi 488)
                        (set! rdi 120)
                        (set! r15 L.rp.37)
                        (jump L.-.11 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.38
                        (begin
                          (set! rsi 280)
                          (set! rdi 1032)
                          (set! r15 L.rp.38)
                          (jump L.-.11 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.39
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.39)
                          (jump L.-.11 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.40
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.40)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.41
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.41)
                  (jump L.+.10 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.42
                  (begin
                    (set! rsi 1984)
                    (set! rdi 888)
                    (set! r15 L.rp.42)
                    (jump L.+.10 rbp r15 rdi rsi)))
                (set! tmp.99 rax))
              (begin
                (begin
                  (return-point L.rp.43
                    (begin
                      (set! rsi 888)
                      (set! rdi 1024)
                      (set! r15 L.rp.43)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.100 rax))
                (begin
                  (return-point L.rp.44
                    (begin
                      (set! rsi tmp.100)
                      (set! rdi tmp.99)
                      (set! r15 L.rp.44)
                      (jump L.*.9 rbp r15 rdi rsi)))
                  (set! tmp.98 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.45
                    (begin
                      (set! rsi 8)
                      (set! rdi 504)
                      (set! r15 L.rp.45)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.102 rax))
                (begin
                  (begin
                    (return-point L.rp.46
                      (begin
                        (set! rsi 1544)
                        (set! rdi 512)
                        (set! r15 L.rp.46)
                        (jump L.*.9 rbp r15 rdi rsi)))
                    (set! tmp.103 rax))
                  (begin
                    (return-point L.rp.47
                      (begin
                        (set! rsi tmp.103)
                        (set! rdi tmp.102)
                        (set! r15 L.rp.47)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.101 rax))))
              (begin
                (return-point L.rp.48
                  (begin
                    (set! rsi tmp.101)
                    (set! rdi tmp.98)
                    (set! r15 L.rp.48)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.97 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.49
                    (begin
                      (set! rsi 760)
                      (set! rdi 288)
                      (set! r15 L.rp.49)
                      (jump L.*.9 rbp r15 rdi rsi)))
                  (set! tmp.106 rax))
                (begin
                  (begin
                    (return-point L.rp.50
                      (begin
                        (set! rsi 960)
                        (set! rdi 880)
                        (set! r15 L.rp.50)
                        (jump L.-.11 rbp r15 rdi rsi)))
                    (set! tmp.107 rax))
                  (begin
                    (return-point L.rp.51
                      (begin
                        (set! rsi tmp.107)
                        (set! rdi tmp.106)
                        (set! r15 L.rp.51)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.105 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.52
                      (begin
                        (set! rsi 1288)
                        (set! rdi 1000)
                        (set! r15 L.rp.52)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.109 rax))
                  (begin
                    (begin
                      (return-point L.rp.53
                        (begin
                          (set! rsi 48)
                          (set! rdi 1744)
                          (set! r15 L.rp.53)
                          (jump L.+.10 rbp r15 rdi rsi)))
                      (set! tmp.110 rax))
                    (begin
                      (return-point L.rp.54
                        (begin
                          (set! rsi tmp.110)
                          (set! rdi tmp.109)
                          (set! r15 L.rp.54)
                          (jump L.*.9 rbp r15 rdi rsi)))
                      (set! tmp.108 rax))))
                (begin
                  (return-point L.rp.55
                    (begin
                      (set! rsi tmp.108)
                      (set! rdi tmp.105)
                      (set! r15 L.rp.55)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.104 rax))))
            (begin
              (return-point L.rp.56
                (begin
                  (set! rsi tmp.104)
                  (set! rdi tmp.97)
                  (set! r15 L.rp.56)
                  (jump L.*.9 rbp r15 rdi rsi)))
              (set! fixnum2.4 rax))))
        (begin
          (return-point L.rp.57
            (begin (set! r15 L.rp.57) (jump L.fun/void140573.7 rbp r15)))
          (set! void3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.58
                  (begin
                    (set! rsi 1960)
                    (set! rdi 1272)
                    (set! r15 L.rp.58)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.113 rax))
              (begin
                (begin
                  (return-point L.rp.59
                    (begin
                      (set! rsi 648)
                      (set! rdi 1632)
                      (set! r15 L.rp.59)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.114 rax))
                (begin
                  (return-point L.rp.60
                    (begin
                      (set! rsi tmp.114)
                      (set! rdi tmp.113)
                      (set! r15 L.rp.60)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.112 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.61
                    (begin
                      (set! rsi 1632)
                      (set! rdi 712)
                      (set! r15 L.rp.61)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.116 rax))
                (begin
                  (begin
                    (return-point L.rp.62
                      (begin
                        (set! rsi 552)
                        (set! rdi 216)
                        (set! r15 L.rp.62)
                        (jump L.*.9 rbp r15 rdi rsi)))
                    (set! tmp.117 rax))
                  (begin
                    (return-point L.rp.63
                      (begin
                        (set! rsi tmp.117)
                        (set! rdi tmp.116)
                        (set! r15 L.rp.63)
                        (jump L.-.11 rbp r15 rdi rsi)))
                    (set! tmp.115 rax))))
              (begin
                (return-point L.rp.64
                  (begin
                    (set! rsi tmp.115)
                    (set! rdi tmp.112)
                    (set! r15 L.rp.64)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.111 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.65
                    (begin
                      (set! rsi 1296)
                      (set! rdi 1944)
                      (set! r15 L.rp.65)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.120 rax))
                (begin
                  (begin
                    (return-point L.rp.66
                      (begin
                        (set! rsi 1872)
                        (set! rdi 1872)
                        (set! r15 L.rp.66)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.121 rax))
                  (begin
                    (return-point L.rp.67
                      (begin
                        (set! rsi tmp.121)
                        (set! rdi tmp.120)
                        (set! r15 L.rp.67)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.119 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.68
                      (begin
                        (set! rsi 1744)
                        (set! rdi 1128)
                        (set! r15 L.rp.68)
                        (jump L.+.10 rbp r15 rdi rsi)))
                    (set! tmp.123 rax))
                  (begin
                    (begin
                      (return-point L.rp.69
                        (begin
                          (set! rsi 480)
                          (set! rdi 944)
                          (set! r15 L.rp.69)
                          (jump L.*.9 rbp r15 rdi rsi)))
                      (set! tmp.124 rax))
                    (begin
                      (return-point L.rp.70
                        (begin
                          (set! rsi tmp.124)
                          (set! rdi tmp.123)
                          (set! r15 L.rp.70)
                          (jump L.-.11 rbp r15 rdi rsi)))
                      (set! tmp.122 rax))))
                (begin
                  (return-point L.rp.71
                    (begin
                      (set! rsi tmp.122)
                      (set! rdi tmp.119)
                      (set! r15 L.rp.71)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.118 rax))))
            (begin
              (return-point L.rp.72
                (begin
                  (set! rsi tmp.118)
                  (set! rdi tmp.111)
                  (set! r15 L.rp.72)
                  (jump L.-.11 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.73
            (begin (set! r15 L.rp.73) (jump L.fun/empty140575.6 rbp r15)))
          (set! empty5.1 rax))
        (begin (set! rax void3.3) (jump tmp-ra.132 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.-.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.111 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.111 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.111 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.111 rbp rax))))))
    (define L.*.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.112 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.112 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.112 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.112 rbp rax))))))
    (define L.+.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.113 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.23 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.22 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.113 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.113 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.113 rbp rax))))))
    (define L.fun/error143927.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.114 r15)
        (begin
          (begin (set! r15 tmp-ra.114) (jump L.fun/error143928.6 rbp r15)))))
    (define L.fun/ascii-char143929.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.115 r15)
        (begin
          (begin
            (set! r15 tmp-ra.115)
            (jump L.fun/ascii-char143930.11 rbp r15)))))
    (define L.fun/error143928.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.116 r15)
        (begin (begin (set! rax 10814) (jump tmp-ra.116 rbp rax)))))
    (define L.fun/void143926.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.117 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.117 rbp rax)))))
    (define L.fun/void143924.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.118 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.118 rbp rax)))))
    (define L.fun/void143923.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.119 r15)
        (begin
          (begin (set! r15 tmp-ra.119) (jump L.fun/void143924.8 rbp r15)))))
    (define L.fun/void143925.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.120 r15)
        (begin
          (begin (set! r15 tmp-ra.120) (jump L.fun/void143926.7 rbp r15)))))
    (define L.fun/ascii-char143930.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.121 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.121 rbp rax)))))
    (begin
      (set! tmp-ra.122 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 1392)
                    (set! rdi 544)
                    (set! r15 L.rp.16)
                    (jump L.+.13 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1344)
                      (set! rdi 360)
                      (set! r15 L.rp.17)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.18)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 288)
                      (set! rdi 1888)
                      (set! r15 L.rp.19)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.20
                      (begin
                        (set! rsi 800)
                        (set! rdi 968)
                        (set! r15 L.rp.20)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.21
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.21)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.22)
                    (jump L.*.14 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 912)
                      (set! rdi 32)
                      (set! r15 L.rp.23)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 1744)
                        (set! rdi 688)
                        (set! r15 L.rp.24)
                        (jump L.+.13 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.25)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 568)
                        (set! rdi 304)
                        (set! r15 L.rp.26)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.27
                        (begin
                          (set! rsi 256)
                          (set! rdi 1344)
                          (set! r15 L.rp.27)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.28
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.28)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.29)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.30
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.30)
                  (jump L.*.14 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.31
                  (begin
                    (set! rsi 1384)
                    (set! rdi 1648)
                    (set! r15 L.rp.31)
                    (jump L.+.13 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi 488)
                      (set! rdi 1664)
                      (set! r15 L.rp.32)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.33
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.33)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi 192)
                      (set! rdi 1624)
                      (set! r15 L.rp.34)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi 416)
                        (set! rdi 1080)
                        (set! r15 L.rp.35)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.36
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.36)
                        (jump L.+.13 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.37
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.37)
                    (jump L.+.13 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.38
                    (begin
                      (set! rsi 1496)
                      (set! rdi 1936)
                      (set! r15 L.rp.38)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi 80)
                        (set! rdi 344)
                        (set! r15 L.rp.39)
                        (jump L.+.13 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.40
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.40)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.41
                      (begin
                        (set! rsi 1656)
                        (set! rdi 1728)
                        (set! r15 L.rp.41)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.42
                        (begin
                          (set! rsi 1408)
                          (set! rdi 1040)
                          (set! r15 L.rp.42)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.43
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.43)
                          (jump L.-.15 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.44
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.44)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.45
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.45)
                  (jump L.*.14 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (return-point L.rp.46
            (begin (set! r15 L.rp.46) (jump L.fun/void143923.9 rbp r15)))
          (set! void2.4 rax))
        (begin
          (return-point L.rp.47
            (begin (set! r15 L.rp.47) (jump L.fun/void143925.10 rbp r15)))
          (set! void3.3 rax))
        (begin
          (return-point L.rp.48
            (begin (set! r15 L.rp.48) (jump L.fun/error143927.4 rbp r15)))
          (set! error4.2 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.49
                  (begin
                    (set! rsi 1064)
                    (set! rdi 464)
                    (set! r15 L.rp.49)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.99 rax))
              (begin
                (begin
                  (return-point L.rp.50
                    (begin
                      (set! rsi 1200)
                      (set! rdi 664)
                      (set! r15 L.rp.50)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.100 rax))
                (begin
                  (return-point L.rp.51
                    (begin
                      (set! rsi tmp.100)
                      (set! rdi tmp.99)
                      (set! r15 L.rp.51)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.98 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.52
                    (begin
                      (set! rsi 848)
                      (set! rdi 1152)
                      (set! r15 L.rp.52)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.102 rax))
                (begin
                  (begin
                    (return-point L.rp.53
                      (begin
                        (set! rsi 1976)
                        (set! rdi 448)
                        (set! r15 L.rp.53)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.103 rax))
                  (begin
                    (return-point L.rp.54
                      (begin
                        (set! rsi tmp.103)
                        (set! rdi tmp.102)
                        (set! r15 L.rp.54)
                        (jump L.+.13 rbp r15 rdi rsi)))
                    (set! tmp.101 rax))))
              (begin
                (return-point L.rp.55
                  (begin
                    (set! rsi tmp.101)
                    (set! rdi tmp.98)
                    (set! r15 L.rp.55)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.97 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.56
                    (begin
                      (set! rsi 128)
                      (set! rdi 16)
                      (set! r15 L.rp.56)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.106 rax))
                (begin
                  (begin
                    (return-point L.rp.57
                      (begin
                        (set! rsi 88)
                        (set! rdi 1704)
                        (set! r15 L.rp.57)
                        (jump L.+.13 rbp r15 rdi rsi)))
                    (set! tmp.107 rax))
                  (begin
                    (return-point L.rp.58
                      (begin
                        (set! rsi tmp.107)
                        (set! rdi tmp.106)
                        (set! r15 L.rp.58)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.105 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.59
                      (begin
                        (set! rsi 472)
                        (set! rdi 1280)
                        (set! r15 L.rp.59)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.109 rax))
                  (begin
                    (begin
                      (return-point L.rp.60
                        (begin
                          (set! rsi 752)
                          (set! rdi 472)
                          (set! r15 L.rp.60)
                          (jump L.+.13 rbp r15 rdi rsi)))
                      (set! tmp.110 rax))
                    (begin
                      (return-point L.rp.61
                        (begin
                          (set! rsi tmp.110)
                          (set! rdi tmp.109)
                          (set! r15 L.rp.61)
                          (jump L.-.15 rbp r15 rdi rsi)))
                      (set! tmp.108 rax))))
                (begin
                  (return-point L.rp.62
                    (begin
                      (set! rsi tmp.108)
                      (set! rdi tmp.105)
                      (set! r15 L.rp.62)
                      (jump L.+.13 rbp r15 rdi rsi)))
                  (set! tmp.104 rax))))
            (begin
              (return-point L.rp.63
                (begin
                  (set! rsi tmp.104)
                  (set! rdi tmp.97)
                  (set! r15 L.rp.63)
                  (jump L.+.13 rbp r15 rdi rsi)))
              (set! fixnum5.1 rax))))
        (begin
          (set! r15 tmp-ra.122)
          (jump L.fun/ascii-char143929.5 rbp r15))))))
