...
(require rackunit
         cpsc411/langs/v7
         (only-in "../impose-calling-conventions.rkt" impose-calling-conventions))

(define (check-proc-imp-cmf-lang-v7 p)
  (if (proc-imp-cmf-lang-v7? p) p #f))

(define (check-imp-cmf-lang-v7 p)
  (if (imp-cmf-lang-v7? p) p #f))

(define-syntax-rule (check-by-interp p)
  (check-equal? (interp-proc-imp-cmf-lang-v7 (check-proc-imp-cmf-lang-v7 p))
                (interp-imp-cmf-lang-v7 (check-imp-cmf-lang-v7 (impose-calling-conventions p)))))
...
(check-by-interp '(module (define L.+.1
                            (lambda (tmp.3 tmp.4)
                              (if (begin
                                    (if (begin
                                          (set! tmp.25 (bitwise-and tmp.4 7))
                                          (= tmp.25 0))
                                        (set! tmp.24 14)
                                        (set! tmp.24 6))
                                    (!= tmp.24 6))
                                  (if (begin
                                        (if (begin
                                              (set! tmp.27 (bitwise-and tmp.3 7))
                                              (= tmp.27 0))
                                            (set! tmp.26 14)
                                            (set! tmp.26 6))
                                        (!= tmp.26 6))
...
(check-by-interp '(module (define L.fixnum?.1
                            (lambda (tmp.17)
                              (if (begin
                                    (set! tmp.24 (bitwise-and tmp.17 7))
                                    (= tmp.24 0))
                                  14
                                  6)))
                          (call L.fixnum?.1 14)
...
(check-by-interp '(module (define L.error?.1
                            (lambda (tmp.22)
                              (if (begin
                                    (set! tmp.24 (bitwise-and tmp.22 255))
                                    (= tmp.24 62))
                                  14
                                  6)))
                          (call L.error?.1 30)
...
(check-by-interp '(module (define L.ascii-char?.1
                            (lambda (tmp.21)
                              (if (begin
                                    (set! tmp.24 (bitwise-and tmp.21 255))
                                    (= tmp.24 46))
                                  14
                                  6)))
...
(check-by-interp '(module (define L.boolean?.1
                            (lambda (tmp.18)
                              (if (begin
                                    (set! tmp.24 (bitwise-and tmp.18 247))
                                    (= tmp.24 6))
                                  14
                                  6)))
                          (call L.boolean?.1 30)
...
(check-by-interp '(module (define L.void?.2
                            (lambda (tmp.20)
                              (if (begin
                                    (set! tmp.24 (bitwise-and tmp.20 255))
                                    (= tmp.24 30))
                                  14
                                  6)))
...
(check-by-interp '(module (define L.<=.2
                            (lambda (tmp.10 tmp.11)
                              (if (begin
                                    (if (begin
                                          (set! tmp.26 (bitwise-and tmp.11 7))
                                          (= tmp.26 0))
                                        (set! tmp.25 14)
                                        (set! tmp.25 6))
                                    (!= tmp.25 6))
                                  (if (begin
                                        (if (begin
                                              (set! tmp.28 (bitwise-and tmp.10 7))
                                              (= tmp.28 0))
                                            (set! tmp.27 14)
                                            (set! tmp.27 6))
                                        (!= tmp.27 6))
...
(check-by-interp '(module (define L.*.2
                            (lambda (tmp.6 tmp.7)
                              (if (begin
                                    (if (begin
                                          (set! tmp.30 (bitwise-and tmp.7 7))
                                          (= tmp.30 0))
                                        (set! tmp.29 14)
                                        (set! tmp.29 6))
                                    (!= tmp.29 6))
                                  (if (begin
                                        (if (begin
                                              (set! tmp.32 (bitwise-and tmp.6 7))
                                              (= tmp.32 0))
                                            (set! tmp.31 14)
                                            (set! tmp.31 6))
                                        (!= tmp.31 6))
                                      (begin
                                        (set! tmp.33 (arithmetic-shift-right tmp.7 3))
                                        (* tmp.6 tmp.33))
                                      318)
                                  318)))
                          (define L.func.0.1
...
(check-by-interp '(module (define L.void?.3
                            (lambda (tmp.28)
                              (if (begin
                                    (set! tmp.32 (bitwise-and tmp.28 255))
                                    (= tmp.32 30))
                                  14
                                  6)))
...
(check-by-interp '(module (define L.boolean?.3
                            (lambda (tmp.25)
                              (if (begin
                                    (set! tmp.31 (bitwise-and tmp.25 247))
                                    (= tmp.31 6))
                                  14
                                  6)))
...
(check-by-interp '(module (define L.ascii-char?.2
                            (lambda (tmp.25)
                              (if (begin
                                    (set! tmp.28 (bitwise-and tmp.25 255))
                                    (= tmp.28 46))
                                  14
                                  6)))
                          (define L.eq?.1 (lambda (tmp.19 tmp.20) (if (= tmp.19 tmp.20) 14 6)))
                    (begin
                      (set! foo.4.3 (call L.eq?.1 22 0))
                      (set! foobar.1.2 6)
...
(check-by-interp '(module (define L.ascii-char?.3
                            (lambda (tmp.28)
                              (if (begin
                                    (set! tmp.31 (bitwise-and tmp.28 255))
                                    (= tmp.31 46))
                                  14
                                  6)))
                          (define L.func.0.1
                            (lambda (foobar.8.5 ball.0.4 foobar.2.3 foobar.6.2 bat.9.1) 6))
...
(check-by-interp '(module (define L.ascii-char?.3
                            (lambda (tmp.30)
                              (if (begin
                                    (set! tmp.33 (bitwise-and tmp.30 255))
                                    (= tmp.33 46))
                                  14
                                  6)))
...
(check-by-interp
 '(module (define L.boolean?.3
            (lambda (tmp.30)
              (if (begin
                    (set! tmp.36 (bitwise-and tmp.30 247))
                    (= tmp.36 6))
                  14
                  6)))
...
(check-by-interp '(module (define L.ascii-char?.3
                            (lambda (tmp.32)
                              (if (begin
                                    (set! tmp.35 (bitwise-and tmp.32 255))
                                    (= tmp.35 46))
                                  14
                                  6)))
...
(check-by-interp '(module (define L.*.4
                            (lambda (tmp.9 tmp.10)
                              (if (begin
                                    (if (begin
                                          (set! tmp.33 (bitwise-and tmp.10 7))
                                          (= tmp.33 0))
                                        (set! tmp.32 14)
                                        (set! tmp.32 6))
                                    (!= tmp.32 6))
                                  (if (begin
                                        (if (begin
                                              (set! tmp.35 (bitwise-and tmp.9 7))
                                              (= tmp.35 0))
                                            (set! tmp.34 14)
                                            (set! tmp.34 6))
                                        (!= tmp.34 6))
                                      (begin
                                        (set! tmp.36 (arithmetic-shift-right tmp.10 3))
                                        (* tmp.9 tmp.36))
                                      318)
                                  318)))
...
(check-by-interp
 '(module (define L.empty?.3
            (lambda (tmp.33)
              (if (begin
                    (set! tmp.38 (bitwise-and tmp.33 255))
                    (= tmp.38 22))
                  14
                  6)))
          (define L.tmp.0.1
...
(check-by-interp '(module (define L.boolean?.4
                            (lambda (tmp.31)
                              (if (begin
                                    (set! tmp.37 (bitwise-and tmp.31 247))
                                    (= tmp.37 6))
                                  14
                                  6)))
                    ...
                    (define L.x.2.3
                      (lambda (bar.6.13 bat.2.12 foobar.7.11 foo.1.10 ball.9.9 foobar.0.8) 8))
                    (call L.boolean?.4 6)))
(check-by-interp '(module (define L.ascii-char?.4
                            (lambda (tmp.37)
                              (if (begin
                                    (set! tmp.40 (bitwise-and tmp.37 255))
                                    (= tmp.40 46))
                                  14
                                  6)))
                          (define L.tmp.0.1
...
(check-by-interp '(module (define L.<.5
                            (lambda (tmp.20 tmp.21)
                              (if (begin
                                    (if (begin
                                          (set! tmp.38 (bitwise-and tmp.21 7))
                                          (= tmp.38 0))
                                        (set! tmp.37 14)
                                        (set! tmp.37 6))
                                    (!= tmp.37 6))
                                  (if (begin
                                        (if (begin
                                              (set! tmp.40 (bitwise-and tmp.20 7))
                                              (= tmp.40 0))
                                            (set! tmp.39 14)
                                            (set! tmp.39 6))
                                        (!= tmp.39 6))
                    ...
                    (begin
                      (set! bat.5.13 (call L.eq?.4 8 6))
                      (call L.<.5 -2229142488 8))))
(check-by-interp
 '(module (define L.empty?.4
            (lambda (tmp.35)
              (if (begin
                    (set! tmp.40 (bitwise-and tmp.35 255))
                    (= tmp.40 22))
                  14
                  6)))
    ...
    (if (!= 30 6)
        (begin
          (set! ball.1.16 (call L.empty?.4 6))
          ball.1.16)
        8)))
(check-by-interp
 '(module (define L.fixnum?.4
            (lambda (tmp.37)
              (if (begin
                    (set! tmp.44 (bitwise-and tmp.37 7))
                    (= tmp.44 0))
                  14
                  6)))
          (define L.x.0.1
