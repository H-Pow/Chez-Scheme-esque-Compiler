...
(require cpsc411/compiler-lib
         cpsc411/langs/v8
         (only-in "../resolve-predicates.rkt"))
...
(check-by-interp
 '(module
    ...
    (define L.__nested.6 (begin (set! rax 57406) (jump r15)))))
(check-by-interp
 '(module
    (define L.__main.7
      (begin
...
(check-by-interp
 '(module
    ...
    (define L.__nested.6 (begin (set! rax 224) (jump r15)))))
(check-by-interp
 '(module
    (define L.__main.5
      (begin
...
(check-by-interp
 '(module
    ...
    (define L.tmp.10 (begin (set! r14 43582) (jump L.tmp.12)))
 ...
 '(module
    (define L.__main.18
      (begin
        (set! (rbp - 0) r15)
        (set! r15 6)
        (if (false) (jump L.tmp.15) (jump L.tmp.16))))
    (define L.*.6
      (begin
        (set! r15 r15)
        (set! r14 rdi)
    ...
    (define L.tmp.15 (begin (set! r15 1712) (jump L.tmp.17)))
    (define L.tmp.16 (begin (set! r15 688) (jump L.tmp.17)))
    (define L.tmp.17
      (begin (set! r14 14) (if (true) (jump L.tmp.12) (jump L.tmp.13))))
    (define L.tmp.12 (begin (set! r14 1480) (jump L.tmp.14)))
    (define L.tmp.13 (begin (set! r14 128) (jump L.tmp.14)))
    (define L.tmp.14
      (begin
        (set! rbp (- rbp 16))
        (set! rsi r14)
    ...
    (define L.rp.7
      (begin
        ...
        (set! (rbp - 8) rax)
        (set! r15 6)
        (if (false) (jump L.tmp.9) (jump L.tmp.10))))
    (define L.tmp.9 (begin (set! r15 352) (jump L.tmp.11)))
...
(check-by-interp
 '(module
    ...
    (define L.tmp.33 (begin (set! (rbp - 8) 22) (jump L.tmp.34)))
    (define L.tmp.34 (jump L.tmp.31))
    (define L.rp.18
      (begin (set! rbp (+ rbp 24)) (set! (rbp - 8) rax) (jump L.tmp.31)))
    (define L.tmp.29
      (begin (set! r15 6) (if (false) (jump L.tmp.32) (jump L.tmp.33))))
    (define L.tmp.30
      (begin
        (set! rbp (- rbp 24))
        (set! rdx 14)
 ...
 '(module
    ...
    (define L.tmp.10
      (begin
        (set! r14 14)
        (if (true) (jump L.__nested.5) (jump L.__nested.6))))
    (define L.__nested.7 (begin (set! rax 1448) (jump r15)))
...
(check-by-interp
 '(module
    ...
    (define L.__nested.8 (begin (set! rax 22) (jump r15)))
    (define L.__nested.5
      (begin
        (set! r14 14)
