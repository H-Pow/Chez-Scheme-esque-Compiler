...
(check-by-interp
 '(module
    ...
    (let ((tmp.53 (let ((ascii-char0.3 25390) (boolean1.2 6)) 14)))
...
(check-by-interp
 '(module
    ...
    (let ((tmp.53 (let ((void0.3 30) (error1.2 38974)) error1.2)))
      (call L.fun/error8412.4 tmp.53))))
(check-by-interp
 '(module
    (define L.fun/ascii-char8415.4 (lambda (oprand0.1) 25390))
    (let ((tmp.53 (let ((ascii-char0.3 25390) (error1.2 29502)) 25390)))
...
(check-by-interp
 '(module
    ...
    (let ((tmp.59
      ...
      (let ((tmp.60
             (let ((tmp.61 (call L.fun/void8469.5 30 6)))
               (let ((tmp.62 (if (!= 6 6) 43582 65086)))
    ...
    (define L.-.5
      (lambda (tmp.21 tmp.22)
        (if (let ((tmp.58
          ...
          830)))
    (let ((tmp.62
           (let ((tmp.63 (if (!= 6 6) 1712 688)))
             (let ((tmp.64 (if (!= 14 6) 1480 128)))
               (call L.-.5 tmp.63 tmp.64)))))
      (let ((tmp.65
             (let ((tmp.66 (if (!= 6 6) 352 264)))
               (let ((tmp.67
                      (let ((ascii-char0.3 25390)
                            (error1.2 13118)
                            (empty2.1 22))
                        608)))
...
(check-by-interp
 '(module
    ...
    (define L.vector?.6
      (lambda (tmp.47)
        (if (let ((tmp.62 (bitwise-and tmp.47 7))) (= tmp.62 3)) 14 6)))
    (define L.fun/any8734.4 (lambda (oprand0.3 oprand1.2 oprand2.1) 41278))
    (if (let ((tmp.63
               (let ((tmp.64 (call L.fun/any8734.4 5950 22 25390)))
                 (call L.vector?.6 tmp.64))))
          (!= tmp.63 6))
      (let ((tmp.65 (call L.-.7 1480 528)))
        (let ((tmp.66 (if (!= 14 6) 1488 1392))) (call L.*.8 tmp.65 tmp.66)))
      (if (!= 14 6) 1552 1376))))
(check-by-interp
 '(module
...
(check-by-interp
 '(module
    ...
    (let ((empty0.4 (call L.fun/empty8806.4))
          (fixnum1.3
           (let ((tmp.67 (call L.-.8 1608 864)))
             (let ((tmp.68 (call L.+.9 328 352))) (call L.-.8 tmp.67 tmp.68))))
          (fixnum2.2
           (let ((tmp.69 (call L.*.10 320 1048)))
             (let ((tmp.70 (call L.*.10 1096 968)))
...
(check-by-interp
 '(module
    (define L.error?.16
      (lambda (tmp.59)
        (if (let ((tmp.67 (bitwise-and tmp.59 255))) (= tmp.67 62)) 14 6)))
    (define L.fixnum?.15
      (lambda (tmp.54)
        (if (let ((tmp.68 (bitwise-and tmp.54 7))) (= tmp.68 0)) 14 6)))
    (define L.+.14
      (lambda (tmp.33 tmp.34)
        (if (let ((tmp.69
              ...
              (!= tmp.69 6))
          (if (let ((tmp.71
                     (if (let ((tmp.72 (bitwise-and tmp.33 7))) (= tmp.72 0))
                       14
                       6)))
                (!= tmp.71 6))
            (+ tmp.33 tmp.34)
    ...
    (define L.vector?.13
      (lambda (tmp.61)
        (if (let ((tmp.73 (bitwise-and tmp.61 7))) (= tmp.73 3)) 14 6)))
    ...
    (let ((tmp.74
           (if (let ((tmp.75 (call L.fun/boolean9033.11 25390 44094 25390)))
                 (!= tmp.75 6))
             (if (!= 6 6) 22 22)
...
(check-by-interp
 '(module
    (define L.boolean?.10
      (lambda (tmp.42)
        (if (let ((tmp.54 (bitwise-and tmp.42 247))) (= tmp.54 6)) 14 6)))
    ...
    (let ((empty0.4 (call L.fun/empty9362.8))
      ...
      (let ((tmp.55 (call L.fun/any9366.6))) (call L.boolean?.10 tmp.55)))))
(check-by-interp
 '(module
...
(check-by-interp
 '(module
    ...
    (let ((void0.4 (call L.fun/void9902.5))
          ...
          (fixnum2.2
           (let ((tmp.63 (call L.*.8 504 1592)))
             (let ((tmp.64 (call L.*.8 552 1320)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.5
           (let ((tmp.68
                  (let ((tmp.69 (call L.+.9 1112 1560)))
                    (let ((tmp.70 (call L.-.10 904 704)))
                      (call L.+.9 tmp.69 tmp.70)))))
             (let ((tmp.71
                    (let ((tmp.72 (call L.+.9 216 744)))
                      (let ((tmp.73 (call L.-.10 1832 1952)))
          ...
          (fixnum4.1
           (let ((tmp.74
                  (let ((tmp.75 (call L.*.11 760 1024)))
                    (let ((tmp.76 (call L.+.9 1344 160)))
                      (call L.-.10 tmp.75 tmp.76)))))
             (let ((tmp.77
                    (let ((tmp.78 (call L.*.11 1136 648)))
                      (let ((tmp.79 (call L.*.11 736 224)))
...
(check-by-interp
 '(module
    ...
    (define L.ascii-char?.9
      (lambda (tmp.46)
        (if (let ((tmp.63 (bitwise-and tmp.46 255))) (= tmp.63 46)) 14 6)))
    ...
    (let ((ascii-char0.5 (call L.fun/ascii-char10854.5))
          (boolean1.4
           (let ((tmp.64 (call L.fun/any10855.4)))
             (call L.ascii-char?.9 tmp.64)))
          (empty2.3 (call L.fun/empty10856.6))
          (fixnum3.2
           (let ((tmp.65
                  (let ((tmp.66 (call L.+.10 1352 784)))
                    (let ((tmp.67 (call L.-.11 8 1792)))
                      (call L.-.11 tmp.66 tmp.67)))))
             (let ((tmp.68
                    (let ((tmp.69 (call L.-.11 112 992)))
                      (let ((tmp.70 (call L.+.10 1104 48)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.5
           (let ((tmp.68
                  (let ((tmp.69 (call L.*.8 1512 1328)))
                    (let ((tmp.70 (call L.*.8 1088 1096)))
                      (call L.+.9 tmp.69 tmp.70)))))
             (let ((tmp.71
                    (let ((tmp.72 (call L.-.10 1528 832)))
                      (let ((tmp.73 (call L.*.8 192 1744)))
                        (call L.*.8 tmp.72 tmp.73)))))
               (call L.+.9 tmp.68 tmp.71))))
          (ascii-char1.4 (call L.fun/ascii-char14227.6))
          (fixnum2.3
           (let ((tmp.74
                  (let ((tmp.75 (call L.+.9 880 1440)))
                    (let ((tmp.76 (call L.-.10 632 320)))
                      (call L.-.10 tmp.75 tmp.76)))))
             (let ((tmp.77
                    (let ((tmp.78 (call L.-.10 1688 360)))
                      (let ((tmp.79 (call L.+.9 1800 1952)))
                        (call L.-.10 tmp.78 tmp.79)))))
...
(check-by-interp
 '(module
    ...
    (let ((error0.5 (call L.fun/error14542.8))
          ...
          (fixnum2.3
           (let ((tmp.68
                  (let ((tmp.69 (call L.*.10 1248 288)))
                    (let ((tmp.70 (call L.*.10 856 552)))
                      (call L.*.10 tmp.69 tmp.70)))))
             (let ((tmp.71
                    (let ((tmp.72 (call L.*.10 824 1056)))
                      (let ((tmp.73 (call L.+.11 608 1720)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.5
           (let ((tmp.64
                  (let ((tmp.65 (call L.*.10 1304 1392)))
                    (let ((tmp.66 (call L.*.10 1800 368)))
                      (call L.-.11 tmp.65 tmp.66)))))
             (let ((tmp.67
                    (let ((tmp.68 (call L.*.10 1672 1944)))
                      (let ((tmp.69 (call L.-.11 1184 1864)))
...
(check-by-interp
 '(module
    ...
    (define L.*.8
      (lambda (tmp.19 tmp.20)
        (if (let ((tmp.64
              ...
              (!= tmp.64 6))
          (if (let ((tmp.66
                     (if (let ((tmp.67 (bitwise-and tmp.19 7))) (= tmp.67 0))
                       14
                       6)))
                (!= tmp.66 6))
            (let ((tmp.68 (arithmetic-shift-right tmp.20 3)))
              (* tmp.19 tmp.68))
            318)
          318)))
    ...
    (let ((empty0.5 (call L.fun/empty16025.4))
          (fixnum1.4
           (let ((tmp.69
                  (let ((tmp.70 (call L.*.8 56 224)))
                    (let ((tmp.71 (call L.+.9 336 384)))
                      (call L.-.10 tmp.70 tmp.71)))))
             (let ((tmp.72
                    (let ((tmp.73 (call L.-.10 1936 672)))
                      (let ((tmp.74 (call L.-.10 1096 1760)))
                        (call L.*.8 tmp.73 tmp.74)))))
               (call L.-.10 tmp.69 tmp.72))))
          (fixnum2.3
           (let ((tmp.75
                  (let ((tmp.76 (call L.*.8 56 1408)))
                    (let ((tmp.77 (call L.*.8 8 528)))
                      (call L.*.8 tmp.76 tmp.77)))))
             (let ((tmp.78
                    (let ((tmp.79 (call L.+.9 472 376)))
                      (let ((tmp.80 (call L.-.10 1000 656)))
                        (call L.*.8 tmp.79 tmp.80)))))
               (call L.*.8 tmp.75 tmp.78))))
          (fixnum3.2
           (let ((tmp.81
                  (let ((tmp.82 (call L.*.8 1064 464)))
                    (let ((tmp.83 (call L.*.8 1088 1504)))
                      (call L.*.8 tmp.82 tmp.83)))))
             (let ((tmp.84
                    (let ((tmp.85 (call L.+.9 1328 1048)))
                      (let ((tmp.86 (call L.*.8 1832 560)))
                        (call L.*.8 tmp.85 tmp.86)))))
               (call L.+.9 tmp.81 tmp.84))))
          (boolean4.1
           (let ((tmp.87 (call L.fun/any16026.6)))
...
(check-by-interp
 '(module
    ...
    (let ((void0.5 (call L.fun/void19235.7))
      ...
      (let ((tmp.68
             (let ((tmp.69 (call L.-.10 1752 160)))
               (let ((tmp.70 (call L.+.11 456 1232)))
                 (call L.-.10 tmp.69 tmp.70)))))
        (let ((tmp.71
               (let ((tmp.72 (call L.-.10 792 1432)))
                 (let ((tmp.73 (call L.-.10 1848 1552)))
...
(check-by-interp
 '(module
    ...
    (let ((empty0.5 (call L.fun/empty20764.7))
      ...
      (let ((tmp.68
             (let ((tmp.69 (call L.-.10 1504 1184)))
               (let ((tmp.70 (call L.+.11 1616 1296)))
                 (call L.-.10 tmp.69 tmp.70)))))
        (let ((tmp.71
               (let ((tmp.72 (call L.-.10 8 1272)))
                 (let ((tmp.73 (call L.*.12 888 528)))
                   (call L.*.12 tmp.72 tmp.73)))))
          (call L.*.12 tmp.68 tmp.71))))))
(check-by-interp
 '(module
    ...
    (let ((ascii-char0.5 (call L.fun/ascii-char21096.4))
          (fixnum1.4
           (let ((tmp.68
                  (let ((tmp.69 (call L.+.9 1208 1976)))
                    (let ((tmp.70 (call L.*.10 1432 32)))
                      (call L.+.9 tmp.69 tmp.70)))))
             (let ((tmp.71
                    (let ((tmp.72 (call L.*.10 1400 456)))
                      (let ((tmp.73 (call L.-.11 888 1672)))
          ...
          (fixnum4.1
           (let ((tmp.74
                  (let ((tmp.75 (call L.*.10 1032 1800)))
                    (let ((tmp.76 (call L.+.9 552 560)))
                      (call L.-.11 tmp.75 tmp.76)))))
             (let ((tmp.77
                    (let ((tmp.78 (call L.-.11 1424 368)))
                      (let ((tmp.79 (call L.-.11 864 296)))
...
(check-by-interp
 '(module
    ...
    (let ((ascii-char0.5 (call L.fun/ascii-char21254.7))
          ...
          (fixnum2.3
           (let ((tmp.63
                  (let ((tmp.64 (call L.+.9 1728 736)))
                    (let ((tmp.65 (call L.-.10 1624 192)))
                      (call L.-.10 tmp.64 tmp.65)))))
             (let ((tmp.66
                    (let ((tmp.67 (call L.-.10 160 888)))
                      (let ((tmp.68 (call L.-.10 1968 1920)))
...
(check-by-interp
 '(module
    ...
    (let ((empty0.5 (call L.fun/empty21523.4))
      ...
      (let ((tmp.68
             (let ((tmp.69 (call L.*.10 1072 1448)))
               (let ((tmp.70 (call L.-.11 496 1416)))
                 (call L.-.11 tmp.69 tmp.70)))))
        (let ((tmp.71
               (let ((tmp.72 (call L.-.11 80 1208)))
                 (let ((tmp.73 (call L.-.11 888 1032)))
                   (call L.+.12 tmp.72 tmp.73)))))
          (call L.+.12 tmp.68 tmp.71))))))
(check-by-interp
 '(module
 ...
 '(module
    ...
    (let ((ascii-char0.6 (call L.fun/ascii-char35206.4))
          ...
          (fixnum4.2
           (let ((tmp.69
                  (let ((tmp.70
                         (let ((tmp.71 (call L.*.15 24 112)))
                           (let ((tmp.72 (call L.+.16 1176 808)))
                             (call L.+.16 tmp.71 tmp.72)))))
                    (let ((tmp.73
                           (let ((tmp.74 (call L.-.17 1752 1560)))
                             (let ((tmp.75 (call L.*.15 40 1400)))
                               (call L.+.16 tmp.74 tmp.75)))))
                      (call L.-.17 tmp.70 tmp.73)))))
             (let ((tmp.76
                    (let ((tmp.77
                           (let ((tmp.78 (call L.-.17 680 1136)))
                             (let ((tmp.79 (call L.-.17 1016 1040)))
                               (call L.-.17 tmp.78 tmp.79)))))
                      (let ((tmp.80
                             (let ((tmp.81 (call L.-.17 256 288)))
                               (let ((tmp.82 (call L.+.16 280 1544)))
...
(check-by-interp
 '(module
    ...
    (let ((empty0.6 (call L.fun/empty43333.9))
          ...
          (boolean2.4
           (let ((tmp.71 (call L.fun/any43337.6))) (call L.void?.15 tmp.71)))
          (fixnum3.3
           (let ((tmp.72
                  (let ((tmp.73
                         (let ((tmp.74 (call L.*.16 832 704)))
                           (let ((tmp.75 (call L.-.17 1040 1832)))
                             (call L.*.16 tmp.74 tmp.75)))))
                    (let ((tmp.76
                           (let ((tmp.77 (call L.-.17 952 568)))
                             (let ((tmp.78 (call L.+.18 1392 1880)))
                               (call L.+.18 tmp.77 tmp.78)))))
                      (call L.+.18 tmp.73 tmp.76)))))
             (let ((tmp.79
                    (let ((tmp.80
                           (let ((tmp.81 (call L.*.16 616 456)))
                             (let ((tmp.82 (call L.-.17 1928 432)))
                               (call L.+.18 tmp.81 tmp.82)))))
                      (let ((tmp.83
                             (let ((tmp.84 (call L.+.18 400 1376)))
                               (let ((tmp.85 (call L.*.16 576 1920)))
                                 (call L.*.16 tmp.84 tmp.85)))))
                        (call L.*.16 tmp.80 tmp.83)))))
               (call L.+.18 tmp.72 tmp.79))))
          (empty4.2 (call L.fun/empty43338.10))
          (boolean5.1
           (let ((tmp.86 (call L.fun/any43340.7))) (call L.error?.19 tmp.86))))
 ...
 '(module
    ...
    (let ((empty0.6 (call L.fun/empty44111.6))
          ...
          (fixnum4.2
           (let ((tmp.69
                  (let ((tmp.70
                         (let ((tmp.71 (call L.*.15 488 1768)))
                           (let ((tmp.72 (call L.*.15 992 1968)))
                             (call L.+.16 tmp.71 tmp.72)))))
                    (let ((tmp.73
                           (let ((tmp.74 (call L.*.15 1304 120)))
                             (let ((tmp.75 (call L.*.15 296 336)))
                               (call L.+.16 tmp.74 tmp.75)))))
                      (call L.+.16 tmp.70 tmp.73)))))
             (let ((tmp.76
                    (let ((tmp.77
                           (let ((tmp.78 (call L.*.15 200 136)))
                             (let ((tmp.79 (call L.*.15 544 1016)))
                               (call L.*.15 tmp.78 tmp.79)))))
                      (let ((tmp.80
                             (let ((tmp.81 (call L.*.15 1440 712)))
                               (let ((tmp.82 (call L.-.17 1984 1408)))
      ...
      (let ((tmp.83
             (let ((tmp.84
                    (let ((tmp.85 (call L.+.16 416 736)))
                      (let ((tmp.86 (call L.-.17 fixnum4.2 136)))
                        (call L.-.17 tmp.85 tmp.86)))))
               (let ((tmp.87
                      (let ((tmp.88 (call L.+.16 520 192)))
                        (let ((tmp.89 (call L.*.15 fixnum4.2 fixnum4.2)))
                          (call L.-.17 tmp.88 tmp.89)))))
                 (call L.+.16 tmp.84 tmp.87)))))
        (let ((tmp.90
               (let ((tmp.91
                      (let ((tmp.92 (call L.*.15 440 fixnum4.2)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.6
           (let ((tmp.70
                  (let ((tmp.71
                         (let ((tmp.72 (call L.-.12 2024 1192)))
                           (let ((tmp.73 (call L.*.13 472 1848)))
                             (call L.-.12 tmp.72 tmp.73)))))
                    (let ((tmp.74
                           (let ((tmp.75 (call L.-.12 1496 200)))
                             (let ((tmp.76 (call L.*.13 936 400)))
                               (call L.-.12 tmp.75 tmp.76)))))
                      (call L.-.12 tmp.71 tmp.74)))))
             (let ((tmp.77
                    (let ((tmp.78
                           (let ((tmp.79 (call L.-.12 1400 512)))
                             (let ((tmp.80 (call L.+.14 304 568)))
                               (call L.*.13 tmp.79 tmp.80)))))
                      (let ((tmp.81
                             (let ((tmp.82 (call L.-.12 1000 1904)))
                               (let ((tmp.83 (call L.*.13 528 448)))
          ...
          (fixnum1.5
           (let ((tmp.84
                  (let ((tmp.85
                         (let ((tmp.86 (call L.-.12 888 8)))
                           (let ((tmp.87 (call L.*.13 1576 1296)))
                             (call L.+.14 tmp.86 tmp.87)))))
                    (let ((tmp.88
                           (let ((tmp.89 (call L.-.12 1288 2000)))
                             (let ((tmp.90 (call L.+.14 488 1088)))
                               (call L.*.13 tmp.89 tmp.90)))))
                      (call L.+.14 tmp.85 tmp.88)))))
             (let ((tmp.91
                    (let ((tmp.92
                           (let ((tmp.93 (call L.-.12 1392 1032)))
                             (let ((tmp.94 (call L.+.14 1776 536)))
                               (call L.+.14 tmp.93 tmp.94)))))
                      (let ((tmp.95
                             (let ((tmp.96 (call L.*.13 1016 2016)))
                               (let ((tmp.97 (call L.-.12 1680 1736)))
          ...
          (boolean5.1
           (let ((tmp.98 (call L.fun/any58880.7)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.6
           (let ((tmp.70
                  (let ((tmp.71
                         (let ((tmp.72 (call L.*.10 224 2024)))
                           (let ((tmp.73 (call L.*.10 1520 1720)))
                             (call L.+.11 tmp.72 tmp.73)))))
                    (let ((tmp.74
                           (let ((tmp.75 (call L.+.11 1768 680)))
                             (let ((tmp.76 (call L.-.12 1488 840)))
                               (call L.*.10 tmp.75 tmp.76)))))
                      (call L.+.11 tmp.71 tmp.74)))))
             (let ((tmp.77
                    (let ((tmp.78
                           (let ((tmp.79 (call L.-.12 1920 1392)))
                             (let ((tmp.80 (call L.+.11 1632 1024)))
                               (call L.-.12 tmp.79 tmp.80)))))
                      (let ((tmp.81
                             (let ((tmp.82 (call L.*.10 1096 1008)))
                               (let ((tmp.83 (call L.+.11 416 416)))
          ...
          (fixnum2.4
           (let ((tmp.84
                  (let ((tmp.85
                         (let ((tmp.86 (call L.+.11 424 120)))
                           (let ((tmp.87 (call L.-.12 792 64)))
                             (call L.+.11 tmp.86 tmp.87)))))
                    (let ((tmp.88
                           (let ((tmp.89 (call L.*.10 936 240)))
                             (let ((tmp.90 (call L.*.10 1192 376)))
                               (call L.*.10 tmp.89 tmp.90)))))
                      (call L.-.12 tmp.85 tmp.88)))))
             (let ((tmp.91
                    (let ((tmp.92
                           (let ((tmp.93 (call L.-.12 1392 1872)))
                             (let ((tmp.94 (call L.+.11 1024 304)))
                               (call L.-.12 tmp.93 tmp.94)))))
                      (let ((tmp.95
                             (let ((tmp.96 (call L.-.12 1776 688)))
                               (let ((tmp.97 (call L.+.11 552 1240)))
          ...
          (fixnum3.3
           (let ((tmp.98
                  (let ((tmp.99
                         (let ((tmp.100 (call L.+.11 872 1848)))
                           (let ((tmp.101 (call L.+.11 128 1936)))
                             (call L.+.11 tmp.100 tmp.101)))))
                    (let ((tmp.102
                           (let ((tmp.103 (call L.-.12 1336 256)))
                             (let ((tmp.104 (call L.*.10 2000 1576)))
                               (call L.*.10 tmp.103 tmp.104)))))
                      (call L.*.10 tmp.99 tmp.102)))))
             (let ((tmp.105
                    (let ((tmp.106
                           (let ((tmp.107 (call L.+.11 1528 1152)))
                             (let ((tmp.108 (call L.+.11 1792 816)))
                               (call L.+.11 tmp.107 tmp.108)))))
                      (let ((tmp.109
                             (let ((tmp.110 (call L.+.11 512 2032)))
                               (let ((tmp.111 (call L.*.10 1032 360)))
                                 (call L.*.10 tmp.110 tmp.111)))))
                        (call L.*.10 tmp.106 tmp.109)))))
               (call L.-.12 tmp.98 tmp.105))))
          (boolean4.2
           (let ((tmp.112 (call L.fun/any63954.8))) (call L.void?.13 tmp.112)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.6
           (let ((tmp.69
                  (let ((tmp.70
                         (let ((tmp.71 (call L.+.15 104 336)))
                           (let ((tmp.72 (call L.+.15 888 1800)))
                             (call L.*.16 tmp.71 tmp.72)))))
                    (let ((tmp.73
                           (let ((tmp.74 (call L.-.17 872 1344)))
                             (let ((tmp.75 (call L.+.15 1360 680)))
                               (call L.-.17 tmp.74 tmp.75)))))
                      (call L.+.15 tmp.70 tmp.73)))))
             (let ((tmp.76
                    (let ((tmp.77
                           (let ((tmp.78 (call L.+.15 72 1248)))
                             (let ((tmp.79 (call L.+.15 896 1360)))
                               (call L.-.17 tmp.78 tmp.79)))))
                      (let ((tmp.80
                             (let ((tmp.81 (call L.*.16 680 152)))
                               (let ((tmp.82 (call L.-.17 1104 1392)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.6
           (let ((tmp.69
                  (let ((tmp.70
                         (let ((tmp.71 (call L.*.13 1888 2024)))
                           (let ((tmp.72 (call L.*.13 696 1464)))
                             (call L.-.14 tmp.71 tmp.72)))))
                    (let ((tmp.73
                           (let ((tmp.74 (call L.*.13 1344 416)))
                             (let ((tmp.75 (call L.-.14 464 88)))
                               (call L.+.15 tmp.74 tmp.75)))))
                      (call L.-.14 tmp.70 tmp.73)))))
             (let ((tmp.76
                    (let ((tmp.77
                           (let ((tmp.78 (call L.*.13 40 1760)))
                             (let ((tmp.79 (call L.-.14 104 1688)))
                               (call L.+.15 tmp.78 tmp.79)))))
                      (let ((tmp.80
                             (let ((tmp.81 (call L.+.15 1952 952)))
                               (let ((tmp.82 (call L.*.13 1264 584)))
          ...
          (fixnum4.2
           (let ((tmp.83
                  (let ((tmp.84
                         (let ((tmp.85 (call L.-.14 2008 1504)))
                           (let ((tmp.86 (call L.+.15 1768 1856)))
                             (call L.-.14 tmp.85 tmp.86)))))
                    (let ((tmp.87
                           (let ((tmp.88 (call L.-.14 880 1728)))
                             (let ((tmp.89 (call L.*.13 1992 152)))
                               (call L.-.14 tmp.88 tmp.89)))))
                      (call L.*.13 tmp.84 tmp.87)))))
             (let ((tmp.90
                    (let ((tmp.91
                           (let ((tmp.92 (call L.*.13 840 672)))
                             (let ((tmp.93 (call L.+.15 1704 368)))
                               (call L.*.13 tmp.92 tmp.93)))))
                      (let ((tmp.94
                             (let ((tmp.95 (call L.*.13 624 1992)))
                               (let ((tmp.96 (call L.-.14 1976 920)))
 ...
 '(module
    ...
    (let ((void0.6 (call L.fun/void77838.5))
          (fixnum1.5
           (let ((tmp.69
                  (let ((tmp.70
                         (let ((tmp.71 (call L.*.15 1560 800)))
                           (let ((tmp.72 (call L.-.16 1880 1936)))
                             (call L.+.17 tmp.71 tmp.72)))))
                    (let ((tmp.73
                           (let ((tmp.74 (call L.*.15 216 752)))
                             (let ((tmp.75 (call L.-.16 1208 800)))
                               (call L.*.15 tmp.74 tmp.75)))))
                      (call L.+.17 tmp.70 tmp.73)))))
             (let ((tmp.76
                    (let ((tmp.77
                           (let ((tmp.78 (call L.-.16 8 752)))
                             (let ((tmp.79 (call L.-.16 1448 1208)))
                               (call L.-.16 tmp.78 tmp.79)))))
                      (let ((tmp.80
                             (let ((tmp.81 (call L.+.17 1232 1784)))
                               (let ((tmp.82 (call L.-.16 1288 1256)))
                                 (call L.*.15 tmp.81 tmp.82)))))
 ...
 '(module
    ...
    (let ((error0.6 (call L.fun/error78092.6))
          (fixnum1.5
           (let ((tmp.71
                  (let ((tmp.72
                         (let ((tmp.73 (call L.-.13 1192 480)))
                           (let ((tmp.74 (call L.*.14 1584 56)))
                             (call L.-.13 tmp.73 tmp.74)))))
                    (let ((tmp.75
                           (let ((tmp.76 (call L.+.15 1488 128)))
                             (let ((tmp.77 (call L.+.15 488 1720)))
                               (call L.-.13 tmp.76 tmp.77)))))
                      (call L.+.15 tmp.72 tmp.75)))))
             (let ((tmp.78
                    (let ((tmp.79
                           (let ((tmp.80 (call L.*.14 1312 1640)))
                             (let ((tmp.81 (call L.*.14 1184 864)))
                               (call L.-.13 tmp.80 tmp.81)))))
                      (let ((tmp.82
                             (let ((tmp.83 (call L.*.14 208 296)))
                               (let ((tmp.84 (call L.*.14 800 32)))
                                 (call L.*.14 tmp.83 tmp.84)))))
                        (call L.-.13 tmp.79 tmp.82)))))
               (call L.-.13 tmp.71 tmp.78))))
          (boolean2.4
           (let ((tmp.85 (call L.fun/any78094.7)))
             (call L.boolean?.16 tmp.85)))
          (boolean3.3
           (let ((tmp.86 (call L.fun/any78095.4)))
             (call L.ascii-char?.17 tmp.86)))
          (fixnum4.2
           (let ((tmp.87
                  (let ((tmp.88
                         (let ((tmp.89 (call L.*.14 136 1384)))
                           (let ((tmp.90 (call L.-.13 1784 592)))
                             (call L.+.15 tmp.89 tmp.90)))))
                    (let ((tmp.91
                           (let ((tmp.92 (call L.+.15 1536 584)))
                             (let ((tmp.93 (call L.*.14 1528 1504)))
                               (call L.*.14 tmp.92 tmp.93)))))
                      (call L.+.15 tmp.88 tmp.91)))))
             (let ((tmp.94
                    (let ((tmp.95
                           (let ((tmp.96 (call L.*.14 1536 1824)))
                             (let ((tmp.97 (call L.*.14 1760 840)))
                               (call L.-.13 tmp.96 tmp.97)))))
                      (let ((tmp.98
                             (let ((tmp.99 (call L.-.13 1864 1032)))
                               (let ((tmp.100 (call L.-.13 88 688)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.6
           (let ((tmp.70
                  (let ((tmp.71
                         (let ((tmp.72 (call L.+.14 1216 248)))
                           (let ((tmp.73 (call L.+.14 1128 528)))
                             (call L.*.15 tmp.72 tmp.73)))))
                    (let ((tmp.74
                           (let ((tmp.75 (call L.-.16 176 1032)))
                             (let ((tmp.76 (call L.-.16 968 1320)))
                               (call L.-.16 tmp.75 tmp.76)))))
                      (call L.*.15 tmp.71 tmp.74)))))
             (let ((tmp.77
                    (let ((tmp.78
                           (let ((tmp.79 (call L.-.16 424 1872)))
                             (let ((tmp.80 (call L.+.14 480 1288)))
                               (call L.+.14 tmp.79 tmp.80)))))
                      (let ((tmp.81
                             (let ((tmp.82 (call L.-.16 1368 1400)))
                               (let ((tmp.83 (call L.+.14 704 544)))
                                 (call L.-.16 tmp.82 tmp.83)))))
                        (call L.-.16 tmp.78 tmp.81)))))
               (call L.+.14 tmp.70 tmp.77))))
          (boolean1.5
           (let ((tmp.84 (call L.fun/any101991.10)))
...
(check-by-interp
 '(module
    ...
    (let ((boolean0.6
           (let ((tmp.70 (call L.fun/any122156.11)))
             (call L.boolean?.16 tmp.70)))
          (empty1.5 (call L.fun/empty122157.12))
          (fixnum2.4
           (let ((tmp.71
                  (let ((tmp.72
                         (let ((tmp.73 (call L.-.17 1368 96)))
                           (let ((tmp.74 (call L.-.17 1560 448)))
                             (call L.-.17 tmp.73 tmp.74)))))
                    (let ((tmp.75
                           (let ((tmp.76 (call L.-.17 952 224)))
                             (let ((tmp.77 (call L.*.18 1856 1680)))
                               (call L.+.19 tmp.76 tmp.77)))))
                      (call L.-.17 tmp.72 tmp.75)))))
             (let ((tmp.78
                    (let ((tmp.79
                           (let ((tmp.80 (call L.-.17 104 304)))
                             (let ((tmp.81 (call L.*.18 1432 1296)))
                               (call L.+.19 tmp.80 tmp.81)))))
                      (let ((tmp.82
                             (let ((tmp.83 (call L.+.19 728 1160)))
                               (let ((tmp.84 (call L.-.17 416 464)))
 ...
 '(module
    ...
    (let ((error0.6 (call L.fun/error124205.15))
          ...
          (fixnum5.1
           (let ((tmp.69
                  (let ((tmp.70
                         (let ((tmp.71 (call L.+.17 1640 16)))
                           (let ((tmp.72 (call L.+.17 1384 120)))
                             (call L.*.18 tmp.71 tmp.72)))))
                    (let ((tmp.73
                           (let ((tmp.74 (call L.+.17 712 1400)))
                             (let ((tmp.75 (call L.+.17 936 912)))
                               (call L.+.17 tmp.74 tmp.75)))))
                      (call L.+.17 tmp.70 tmp.73)))))
             (let ((tmp.76
                    (let ((tmp.77
                           (let ((tmp.78 (call L.*.18 1344 952)))
                             (let ((tmp.79 (call L.-.19 648 264)))
                               (call L.*.18 tmp.78 tmp.79)))))
                      (let ((tmp.80
                             (let ((tmp.81 (call L.-.19 560 1552)))
                               (let ((tmp.82 (call L.+.17 592 952)))
                                 (call L.+.17 tmp.81 tmp.82)))))
                        (call L.+.17 tmp.77 tmp.80)))))
...
(check-by-interp
 '(module
    ...
    (let ((void0.6 (call L.fun/void156379.5))
          (boolean1.5
           (let ((tmp.70 (call L.fun/any156381.8))) (call L.pair?.14 tmp.70)))
          (fixnum2.4
           (let ((tmp.71
                  (let ((tmp.72
                         (let ((tmp.73 (call L.-.15 232 2016)))
                           (let ((tmp.74 (call L.-.15 504 304)))
                             (call L.-.15 tmp.73 tmp.74)))))
                    (let ((tmp.75
                           (let ((tmp.76 (call L.-.15 368 1152)))
                             (let ((tmp.77 (call L.-.15 1752 424)))
                               (call L.-.15 tmp.76 tmp.77)))))
                      (call L.-.15 tmp.72 tmp.75)))))
             (let ((tmp.78
                    (let ((tmp.79
                           (let ((tmp.80 (call L.-.15 1736 1616)))
                             (let ((tmp.81 (call L.*.16 320 568)))
                               (call L.-.15 tmp.80 tmp.81)))))
                      (let ((tmp.82
                             (let ((tmp.83 (call L.+.17 192 1224)))
                               (let ((tmp.84 (call L.-.15 1976 2016)))
      ...
      (let ((tmp.85
             (let ((tmp.86
                    (let ((tmp.87 (call L.+.17 912 1944)))
                      (let ((tmp.88 (call L.+.17 1896 fixnum2.4)))
...
(check-by-interp
 '(module
    ...
    (let ((fixnum0.6
           (let ((tmp.69
                  (let ((tmp.70
                         (let ((tmp.71 (call L.*.13 2008 1576)))
                           (let ((tmp.72 (call L.*.13 1088 1128)))
                             (call L.+.14 tmp.71 tmp.72)))))
                    (let ((tmp.73
                           (let ((tmp.74 (call L.-.15 1968 48)))
                             (let ((tmp.75 (call L.*.13 592 624)))
                               (call L.+.14 tmp.74 tmp.75)))))
                      (call L.-.15 tmp.70 tmp.73)))))
             (let ((tmp.76
                    (let ((tmp.77
                           (let ((tmp.78 (call L.*.13 1184 672)))
                             (let ((tmp.79 (call L.+.14 1320 1584)))
                               (call L.-.15 tmp.78 tmp.79)))))
                      (let ((tmp.80
                             (let ((tmp.81 (call L.+.14 1768 904)))
                               (let ((tmp.82 (call L.*.13 928 104)))
                                 (call L.-.15 tmp.81 tmp.82)))))
                        (call L.+.14 tmp.77 tmp.80)))))
               (call L.-.15 tmp.69 tmp.76))))
          (fixnum1.5
           (let ((tmp.83
                  (let ((tmp.84
                         (let ((tmp.85 (call L.*.13 1160 240)))
                           (let ((tmp.86 (call L.-.15 1520 256)))
                             (call L.*.13 tmp.85 tmp.86)))))
                    (let ((tmp.87
                           (let ((tmp.88 (call L.+.14 960 240)))
                             (let ((tmp.89 (call L.+.14 624 728)))
                               (call L.+.14 tmp.88 tmp.89)))))
                      (call L.-.15 tmp.84 tmp.87)))))
             (let ((tmp.90
                    (let ((tmp.91
                           (let ((tmp.92 (call L.-.15 1880 1080)))
                             (let ((tmp.93 (call L.*.13 440 1000)))
                               (call L.+.14 tmp.92 tmp.93)))))
                      (let ((tmp.94
                             (let ((tmp.95 (call L.-.15 936 608)))
                               (let ((tmp.96 (call L.-.15 568 984)))
