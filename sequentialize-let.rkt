...
(module+ test
  ...
  (define-syntax-rule (check-by-interp vulv5)
    (check-equal? (interp-values-unique-lang-v5 vulv5)
                  (interp-imp-mf-lang-v5 (peek (sequentialize-let vulv5) "imp-mf-lang:"))
                  (peek/pretty vulv5 "values-unique:")))
  (define-syntax-rule (check-by-interp-v6 p)
    (check-equal? (interp-values-unique-lang-v6 p) 
                  (interp-imp-mf-lang-v6 (peek (sequentialize-let p) "v6: imp-mf-lang:"))
                  (peek/pretty p "v6: values-unique:")))
...
(check-by-interp-v6 '(module
   (let ((foo.7.1 (let ((foo.5.2 1)) (let ((foo.5.3 foo.5.2)) foo.5.3))))
     foo.7.1)))
(check-by-interp-v6 '(module
   (define L.func.0.1
...
(check-by-interp-v6 '(module
   ...
   (let ((foo.5.13
          (let ((bat.0.14 (if (<= 9223372036854775807 0) 0 93609173)))
            (let ((foo.6.15 bat.0.14)) foo.6.15))))
     foo.5.13)))
(check-by-interp-v6 '(module
   (let ((bar.7.1
          (let ((bat.1.2 -9223372036854775808))
...
(check-by-interp-v6 '(module
   ...
   (define L.fn.2.3
     (lambda (bat.9.6) (let ((bar.6.7 (* 1 0))) (call L.fn.1.2))))
   (if (true)
...
(check-by-interp-v6 '(module
   (define L.x.0.1
     (lambda (bar.6.5 foobar.5.4 foo.9.3 foobar.7.2 ball.4.1)
       (let ((bat.3.6
              (let ((foobar.7.7 (call L.x.0.1 0 ball.4.1 ball.4.1 foo.9.3 1)))
...
(check-by-interp-v6 '(module
   ...
   (let ((bat.8.9 (if (true) 480297521 (- 1 0))))
     (if (= bat.8.9 0)
       ...
       (if (> 9223372036854775807 -1225978347)
         ...
         (let ((foo.3.8 9223372036854775807))
           (call
            ...
            foo.6.6
            0)))))
   (if (>= 1 9223372036854775807) (let ((bar.8.9 1)) bar.8.9) -32529780)))
(check-by-interp-v6 '(module
   (define L.proc.0.1
...
(check-by-interp-v6 '(module
   ...
   (let ((bat.9.10 -477286222) (bar.7.9 0)) 643069821)))
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda (bar.9.1)
       (if (!= bar.9.1 bar.9.1)
         ...
         (let ((bat.2.3 (- bar.9.1 bar.9.1))
               (bat.0.2
                (let ((bar.9.6 bar.9.1) (bat.0.5 bar.9.1) (ball.4.4 bar.9.1))
                  ball.4.4)))
   ...
   (define L.func.2.3
     (lambda (bar.0.17 foobar.7.16 foo.4.15 foo.6.14)
       (let ((foo.9.18 foobar.7.16))
         (let ((foo.4.19
                (let ((foo.6.21 foo.6.14) (foo.9.20 foo.6.14)) foobar.7.16)))
     ...
     (lambda (bar.0.14
       ...
       (let ((foobar.9.16
         ...
         (call
          ...
          bar.0.14
          bar.4.11))))
   (let ((bar.0.18 0) (foo.8.17 1029279872)) bar.0.18)))
(check-by-interp-v6 '(module
   (if (if (let ((bat.2.2 1) (foobar.0.1 -1606724555)) (= bat.2.2 bat.2.2))
         (let ((ball.5.5 1) (bat.1.4 1) (bar.8.3 -9223372036854775808))
           (<= ball.5.5 ball.5.5))
   ...
   (define L.proc.0.1
     (lambda (bar.6.3 bar.3.2 foo.1.1)
       (let ((bar.3.6 (call L.proc.0.1 9223372036854775807 foo.1.1 foo.1.1))
             ...
             (bar.6.4
              (let ((ball.2.9 (call L.proc.0.1 foo.1.1 foo.1.1 bar.6.3))
                    (ball.9.8 bar.3.2)
         ...
         (if (> bar.6.4 foo.1.1)
           ...
           (let ((ball.4.11 -2087609688) (foo.8.10 bar.3.6)) ball.9.5)))))
   (let ((ball.9.13 1) (foo.1.12 -9223372036854775808)) foo.1.12)))
(check-by-interp-v6 '(module
   (define L.fn.0.1
     (lambda (bar.0.1)
       (if (true)
   ...
   (define L.tmp.1.2
     (lambda (foobar.6.7 foobar.3.6 foo.5.5 bar.2.4 ball.8.3 bar.0.2)
       (let ((bar.0.10 9223372036854775807)
             ...
             (foobar.6.8
              (let ((ball.1.11 (+ bar.2.4 foobar.3.6))) foobar.3.6)))
         (call L.fn.0.1 0))))
   (define L.func.2.3
...
(check-by-interp-v6 '(module (let ((foobar.1.2 0) (bat.0.1 1)) bat.0.1)))
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda ()
...
(check-by-interp-v6 '(module
   ...
   (let () 1)))
(check-by-interp-v6 '(module
   (let ((ball.3.2 -550916464) (foobar.9.1 9223372036854775807)) foobar.9.1)))
(check-by-interp-v6 '(module
   (if (false)
   ...
   (define L.tmp.0.1
     (lambda (bar.1.4 bat.6.3 ball.4.2 bar.0.1)
       (let ((ball.7.6 (call L.tmp.0.1 bat.6.3 bat.6.3 bar.0.1 bar.1.4))
         ...
         (let ((foobar.8.9 (let ((foobar.2.11 bat.6.3) (bar.1.10 1)) bar.0.1))
               (bar.0.8 bar.1.4)
...
(check-by-interp-v6 '(module
   (if (true)
     ...
     (if (false)
       (let ((foo.5.2 1032709229) (bat.6.1 1)) foo.5.2)
...
(check-by-interp-v6 '(module
   ...
   (define L.fn.1.2
     (lambda (foo.6.7 foobar.2.6 foobar.1.5 bat.3.4 foobar.5.3)
       (let ((bar.0.8
              (let ((foobar.7.10 (let () foobar.5.3))
       ...
       (if (<= bat.2.4 9223372036854775807)
         (if (true)
           ...
           (call
            ...
            foo.1.1
            bat.6.5))
         (let ((bar.3.7 foo.1.1) (bar.4.6 (let () bat.6.5)))
...
(check-by-interp-v6 '(module
   (let ((ball.1.1 -9223372036854775808))
     (let ((bar.0.4
            (let ((foobar.8.7 0) (bar.7.6 ball.1.1) (ball.9.5 ball.1.1))
              ball.9.5))
           (foo.3.3 ball.1.1)
...
(check-by-interp-v6 '(module
   (let ((foo.7.1 1))
     (let ((foobar.3.4
            (if (= foo.7.1 foo.7.1)
              (let ((foo.8.7 foo.7.1) (foobar.5.6 foo.7.1) (foo.4.5 foo.7.1))
                foo.7.1)
              (let ((foo.4.9 foo.7.1) (foobar.1.8 foo.7.1)) foo.7.1)))
           (foo.8.3 foo.7.1)
           (foobar.1.2 (+ foo.7.1 foo.7.1)))
       (let ()
         (let ((foobar.3.11 foobar.3.4) (foobar.1.10 1648274049)) foo.7.1))))))
(check-by-interp-v6 '(module
...
(check-by-interp-v6 '(module
   (if (if (!= 9223372036854775807 2124101395)
     ...
     (let ((bat.2.2
            (let ((bat.2.3 -9223372036854775808)) -9223372036854775808))
           (bar.8.1 (* 9223372036854775807 -9223372036854775808)))
       bat.2.2)
     (let ((bar.9.4 (+ 1865158198 1)))
...
(check-by-interp-v6 '(module
   (let ((bat.2.2 (- -9223372036854775808 0)) (bat.0.1 -9223372036854775808))
     (let ((ball.3.5 1969620648) (foo.5.4 bat.2.2) (bat.0.3 bat.0.1))
       foo.5.4))))
...
(check-by-interp '(module
   (define L.fn.0.1 (lambda (foo.3.2 bat.7.1) (call L.tmp.1.2 0 bat.7.1)))
   (define L.tmp.1.2
...
(check-by-interp '(module
   (define L.tmp.0.1
     (lambda (bat.8.2 foo.1.1)
       (if (if (if (>= bat.8.2 foo.1.1) (< bat.8.2 0) (>= bat.8.2 foo.1.1))
             (not (>= -9223372036854775808 bat.8.2))
             (< 9223372036854775807 9223372036854775807))
         (call L.proc.1.2 9223372036854775807)
         (call L.tmp.2.3 bat.8.2 bat.8.2))))
   (define L.proc.1.2
     (lambda (foobar.3.3) (if (false) foobar.3.3 (* foobar.3.3 foobar.3.3))))
   ...
   (define L.fn.0.1
     (lambda (bat.1.2 foo.0.1)
       (let ((bat.5.3
         ...
         (let ((ball.4.6
                (if (<= -1657890681 9223372036854775807) bat.1.2 bat.1.2)))
...
(check-by-interp '(module
   (define L.x.0.1
     (lambda (bat.1.1)
       (let ((ball.5.2
              (let ((bat.6.3 (+ -1473118393 -9223372036854775808)))
                (let ((foo.4.4 9223372036854775807)) 218975616))))
         (* bat.1.1 bat.1.1))))
   (define L.tmp.1.2
     (lambda ()
       (let ((bat.1.5
              (if (not (<= 1 1)) (* -753830259 -9223372036854775808) 0)))
...
(check-by-interp '(module
   (define L.func.0.1
     (lambda (bar.6.1) (if (false) bar.6.1 (call L.func.0.1 bar.6.1))))
...
(check-by-interp '(module
   (define L.x.0.1 (lambda (bat.2.2 bar.0.1) (call L.x.0.1 1 203659231)))
   (define L.tmp.1.2
     (lambda ()
       (let ((foo.3.3
              (let ((bat.2.4
                     (if (= -9223372036854775808 1) 9223372036854775807 0)))
                (if (>= -9223372036854775808 784027956)
                  9223372036854775807
                  0))))
         (if (false)
...
(check-by-interp '(module
   ...
   (let ((ball.0.4.8 (* -9223372036854775808 -9223372036854775808)))
     (+ ball.0.4.8 0))))
(check-by-interp '(module
...
(check-by-interp '(module
   (define L.fn.0.1
     (lambda ()
       (let ((bat.9.1 (if (false) 1 (let ((foo.5.2 1)) 1))))
         (if (if (>= 9223372036854775807 -9223372036854775808)
...
(check-by-interp '(module
   ...
   (define L.x.1.2
     (lambda (bar.0.3)
       (let ((ball.1.4
              (let ((bat.9.5 (let ((bat.2.6 1757280127)) bat.2.6)))
...
(check-by-interp '(module
   (define L.fn.0.1
     (lambda (foobar.3.2 ball.7.1)
       (let ((bar.9.3
              (let ((bat.4.4 (* 1 9223372036854775807)))
   ...
   (define L.x.1.2
     (lambda (bat.2.6)
       (let ((foobar.3.7
              (let ((bat.4.8 (+ -1410706204 bat.2.6)))
...
(check-by-interp '(module
   (define L.proc.0.1
     (lambda (foobar.1.6 bar.0.5 foobar.2.4 ball.8.3 bat.3.2 bar.5.1)
       (let ((bar.9.7
              (if (let ((bar.7.8 -669410514)) (< bat.3.2 bat.3.2))
                (+ bar.5.1 ball.8.3)
                foobar.2.4)))
...
(check-by-interp '(module
     ...
     (lambda (bat.7.7 ball.9.6 foobar.3.5 ball.5.4 ball.4.3 bar.0.2 ball.2.1)
       (if (not
            (if (>= 388494724 ball.9.6)
              ...
              (< foobar.3.5 ball.5.4)))
         (if (<= ball.4.3 bar.0.2) 0 (let ((ball.4.8 ball.5.4)) 1887946265))
         foobar.3.5)))
   (define L.tmp.1.2
   ...
   (define L.tmp.2.3
     (lambda (bat.7.23 ball.2.22 foo.6.21 foobar.3.20 bar.8.19 bar.0.18)
       (let ((foobar.3.24
              (if (let ((ball.2.25 258314756)) (!= bar.8.19 0))
                bar.8.19
...
(check-by-interp '(module
   ...
   (define L.fn.2.3
     (lambda ()
       (let ((ball.7.10 (+ 1 1)))
         (if (false) ball.7.10 (let ((foobar.1.11 ball.7.10)) 1969054361)))))
   (define L.x.3.4
     (lambda (foobar.4.18
              foobar.5.17
              bar.9.16
   ...
   (define L.x.5.6
     (lambda (foo.6.25 foo.3.24 foobar.4.23 bar.9.22 foobar.1.21 foobar.5.20)
       (let ((foobar.5.26
              (let ((foobar.1.27 bar.9.22))
                (let ((foo.6.28 foo.3.24)) foobar.1.27))))
   ...
   (define L.func.6.7
     (lambda (foobar.2.36
              ...
              foobar.5.30)
       (if (if (true)
             (let ((ball.0.37 foobar.2.36))
               (> -1497437069 -9223372036854775808))
             (not (!= -1101838227 -1416967818)))
         (let ((ball.8.38 (+ foobar.2.36 foobar.2.36)))
           (let ((foobar.1.39 0)) foobar.1.39))
         1)))
   (define L.fn.7.8
     (lambda ()
       (if (true)
   ...
   (if (let ((foo.6.40
              (if (let ((foobar.2.41 -9223372036854775808)) (true))
                (let ((foobar.2.42
         ...
         (if (true) (let ((bar.9.44 0)) (true)) (< 858519747 foo.6.40)))
     1
