...
(check-by-interp '(module (define L.fixnum?.1
                            (lambda (tmp.17) (if (= (bitwise-and tmp.17 7) 0) 14 6)))
                          (call L.fixnum?.1 14)
                    ))
                            ...
                            (lambda (tmp.6 tmp.7)
                              (if (!= (if (= (bitwise-and tmp.7 7) 0) 14 6) 6)
                                  (if (!= (if (= (bitwise-and tmp.6 7) 0) 14 6) 6)
                                      (* tmp.6 (arithmetic-shift-right tmp.7 3))
                                      318)
                                  318)))
                          (define L.func.0.1
...
(check-by-interp '(module (define L.tmp.0.1
                    ...
                    (if (!= 25902 6) 30 22)))
(check-by-interp
 '(module (define L.void?.3 (lambda (tmp.28) (if (= (bitwise-and tmp.28 255) 30) 14 6)))
...
(check-by-interp '(module (define L.boolean?.3
                            (lambda (tmp.25) (if (= (bitwise-and tmp.25 247) 6) 14 6)))
...
(check-by-interp '(module (define L.ascii-char?.2
                            (lambda (tmp.25) (if (= (bitwise-and tmp.25 255) 46) 14 6)))
...
(check-by-interp '(module (define L.fn.0.1 (lambda (foo.4.3 bat.8.2 bat.6.1) bat.8.2))
                    ...
                    (if (!= 30 6) 30 29230)))
(check-by-interp
 '(module (define L.ascii-char?.3 (lambda (tmp.28) (if (= (bitwise-and tmp.28 255) 46) 14 6)))
...
(check-by-interp '(module (define L.ascii-char?.3
                            (lambda (tmp.30) (if (= (bitwise-and tmp.30 255) 46) 14 6)))
...
(check-by-interp '(module (define L.x.0.1 (lambda (ball.3.1) 0))
                    ...
                    (let ([bat.7.8 22]
                      ...
                      22)))
(check-by-interp
 '(module (define L.boolean?.3 (lambda (tmp.30) (if (= (bitwise-and tmp.30 247) 6) 14 6)))
    ...
    (call L.boolean?.3 30)))
(check-by-interp
 '(module (define L.ascii-char?.3 (lambda (tmp.32) (if (= (bitwise-and tmp.32 255) 46) 14 6)))
                            ...
                            (lambda (tmp.9 tmp.10)
                              (if (!= (if (= (bitwise-and tmp.10 7) 0) 14 6) 6)
                                  (if (!= (if (= (bitwise-and tmp.9 7) 0) 14 6) 6)
                                      (* tmp.9 (arithmetic-shift-right tmp.10 3))
                                      318)
                                  318)))
...
(check-by-interp '(module (define L.fn.0.1
                    ...
                    22))
(check-by-interp
 '(module (define L.empty?.3 (lambda (tmp.33) (if (= (bitwise-and tmp.33 255) 22) 14 6)))
          (define L.tmp.0.1
...
(check-by-interp '(module (define L.proc.0.1
                    ...
                    (if (!= 6 6) 14 22)))
(check-by-interp
 '(module (define L.ascii-char?.4 (lambda (tmp.32) (if (= (bitwise-and tmp.32 255) 46) 14 6)))
          (define L.fn.0.1
...
(check-by-interp
 '(module (define L.boolean?.4 (lambda (tmp.31) (if (= (bitwise-and tmp.31 247) 6) 14 6)))
    ...
    (call L.boolean?.4 6)))
(check-by-interp
 '(module (define L.ascii-char?.4 (lambda (tmp.37) (if (= (bitwise-and tmp.37 255) 46) 14 6)))
          (define L.tmp.0.1
...
(check-by-interp
 '(module (define L.empty?.4 (lambda (tmp.35) (if (= (bitwise-and tmp.35 255) 22) 14 6)))
    ...
    (if (!= 30 6)
        ...
        8)))
(check-by-interp
 '(module (define L.fixnum?.4 (lambda (tmp.37) (if (= (bitwise-and tmp.37 7) 0) 14 6)))
          (define L.x.0.1
