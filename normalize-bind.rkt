...
(define-syntax-rule (check-by-interp-v6 v6)
  (check-equal? (interp-imp-mf-lang-v6 (peek v6)) (interp-proc-imp-cmf-lang-v6 (peek (normalize-bind v6)))))


(check-by-interp-v6 '(module
   (begin
     (set! foobar.3.1 (begin (set! bat.9.2 -422317085) bat.9.2))
     (begin (set! ball.2.3 foobar.3.1) foobar.3.1))))
(check-by-interp-v6 '(module
   (define L.func.0.1
   ...
   (begin
     (set! foobar.9.10
       (if (false) 9223372036854775807 (* 1 -9223372036854775808)))
     (if (not (> foobar.9.10 foobar.9.10))
       (begin (set! ball.6.11 foobar.9.10) -1510146984)
     ...
     (set! bat.9.1
       (if (begin (set! bat.8.2 -1418594624) (false))
         (begin
           ...
           (begin (set! bat.9.4 767736686) bat.9.4))
         (if (true)
           (begin (set! bat.3.5 1942655457) bat.3.5)
...
(check-by-interp-v6 '(module
   ...
   (begin
     (set! foobar.5.7
       (if (< 824269768 9223372036854775807) -9223372036854775808 709343632))
     foobar.5.7)))
(check-by-interp-v6 '(module (begin (set! foo.6.1 -356902212) (+ -979281755 9223372036854775807))))
(check-by-interp-v6 '(module
   (define L.tmp.0.1
   ...
   (define L.x.0.1
     (lambda (ball.3.5 bar.7.4 bat.6.3 ball.4.2 bat.5.1)
       (if (not (<= bat.6.3 9223372036854775807))
         (begin
           (set! foobar.9.6 (begin (set! bat.6.7 -18835826) bat.6.7))
           (if (>= bat.6.3 ball.4.2) bat.6.3 bar.7.4))
         (begin
           (set! bat.5.8
             (call L.x.0.1 bat.5.1 9223372036854775807 bat.6.3 bat.5.1 0))
   ...
   (define L.func.1.2
     (lambda (bar.8.10 ball.4.9)
       (begin
         ...
         (begin
           (set! foobar.9.12
             (if (>= -1133252869 9223372036854775807) 479665611 bar.8.10))
           (begin (set! bat.5.13 bar.7.11) bar.7.11)))))
...
(check-by-interp-v6 '(module
   (define L.fn.0.1
     (lambda (ball.9.2 bat.0.1) (begin (set! foobar.4.3 ball.9.2) (* 0 0))))
   (define L.fn.1.2
     (lambda ()
   ...
   (define L.fn.2.3
     (lambda (bat.5.11 bat.0.10 ball.9.9 foobar.4.8 bar.7.7 bat.3.6 bar.2.5)
       (if (not (true))
         ...
         (if (true)
           (* bat.3.6 foobar.4.8)
           (begin (set! bat.6.13 bat.5.11) 674342291)))))
   (if (begin
         (set! bat.0.14
           (begin
             (set! bar.2.15 (begin (set! bar.2.16 1) -9223372036854775808))
             (begin (set! foobar.8.17 bar.2.15) 9223372036854775807)))
         (if (true)
           (if (= bat.0.14 bat.0.14)
             (>= -1195644570 bat.0.14)
             (= bat.0.14 bat.0.14))
           (if (> bat.0.14 bat.0.14) (>= bat.0.14 bat.0.14) (= 0 bat.0.14))))
     (call
      L.fn.2.3
      1453047515
      -1663007716
   ...
   (begin
     (set! bat.4.1
       (if (> -9223372036854775808 9223372036854775807)
         (begin
           (set! bar.5.2
             (if (!= 1 9223372036854775807) 9223372036854775807 463110926))
           0)
         (begin (set! bar.3.3 0) (- -1584028825 -9223372036854775808))))
     (if (not (> bat.4.1 1))
   ...
   (define L.func.1.2
     (lambda (bat.3.9 foo.8.8 bar.0.7 foo.9.6 bat.4.5)
       (call
        ...
        -9223372036854775808)))
   (define L.tmp.2.3
     (lambda (foo.7.16 bat.3.15 bat.5.14 bat.4.13 foo.8.12 foo.2.11 foo.6.10)
       bat.5.14))
   (if (false)
     (begin (set! foo.9.17 1281469771) 0)
     (call
      L.tmp.2.3
      1
      9223372036854775807
     ...
     (lambda (foo.9.5 foo.0.4 foobar.7.3 bat.2.2 foo.1.1)
       (if (false)
         (begin
           ...
           (begin (set! foo.9.7 9223372036854775807) 9223372036854775807))
         1)))
   (call
    L.func.0.1
    -9223372036854775808
   ...
   (define L.x.0.1
     (lambda (bar.3.4 ball.0.3 bat.2.2 bat.9.1)
       (begin
         ...
         (begin
           (set! foobar.7.6 (begin (set! bar.3.7 0) bar.3.7))
           (- bar.3.4 0)))))
     ...
     (begin
       (set! bar.7.5
         (if (if (> 9223372036854775807 1428972274)
           ...
           -9223372036854775808))
       (if (if (<= 0 bar.7.5) (!= bar.7.5 2009350954) (!= 1170635915 bar.7.5))
         (begin (set! foobar.1.6 bar.7.5) 0)
         (begin (set! foobar.1.7 -378459323) bar.7.5)))
     (if (false) 0 (- 1326448876 360169641)))))
(check-by-interp-v6 '(module
   (begin
     (set! foo.7.1
       (begin (set! foo.5.2 1) (begin (set! foo.5.3 foo.5.2) foo.5.3)))
     foo.7.1)))
(check-by-interp-v6 '(module
   (define L.func.0.1
     ...
     (lambda ()
       (if (<= 0 1773437967)
         (begin
           ...
           (begin (set! bar.5.2 -9223372036854775808) 1))
...
(check-by-interp-v6 '(module
   (define L.fn.0.1
     (lambda (foobar.4.2 foo.5.1)
       (begin
         (set! bat.1.3
           (begin (set! foo.5.4 foobar.4.2) (+ foobar.4.2 foo.5.4)))
         (call L.fn.0.1 0 foo.5.1))))
   (define L.tmp.1.2
     (lambda (bat.0.9 foo.2.8 foobar.4.7 bat.1.6 bat.9.5)
       (if (if (true)
             (< bat.9.5 bat.0.9)
             (begin (set! bar.7.10 bat.0.9) (<= 0 -508327908)))
         (begin
   ...
   (begin
     (set! foo.5.13
       (begin
         ...
         (begin (set! foo.6.15 bat.0.14) foo.6.15)))
     foo.5.13)))
(check-by-interp-v6 '(module
   (begin
     (set! bar.7.1
       (begin
         ...
         (begin (set! foobar.4.3 0) foobar.4.3)))
     (if (begin (set! foo.0.4 bar.7.1) (<= 1 -9223372036854775808))
       bar.7.1
       (begin (set! foobar.4.5 bar.7.1) 1974766267)))))
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda (bar.0.6 foobar.2.5 foo.1.4 foobar.9.3 bat.5.2 foobar.8.1)
       (begin
         ...
         (if (false)
           (begin (set! foo.1.8 -9223372036854775808) foo.1.8)
           (begin (set! foo.1.9 310790521) foobar.8.1)))))
   (begin (set! foo.1.10 -1379426851) 488563830)))
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda (ball.2.3 foo.5.2 ball.0.1)
       (if (if (= 0 ball.2.3)
         ...
         (if (true)
           (begin (set! foo.5.4 ball.2.3) foo.5.4)
...
(check-by-interp-v6 '(module
   (define L.tmp.0.1
     (lambda (bar.0.3 bar.4.2 bar.9.1) (call L.fn.1.2 9223372036854775807)))
   (define L.fn.1.2
     (lambda (bar.9.4)
       (if (> 1 1)
         ...
         (if (if (> bar.9.4 9223372036854775807)
           ...
           -9223372036854775808))))
   (define L.x.2.3
     (lambda (foobar.6.7 ball.8.6 bar.4.5)
       (call L.x.2.3 foobar.6.7 -845528888 bar.4.5)))
   (begin (set! bat.1.8 0) bat.1.8)))
(check-by-interp-v6 '(module
   (define L.x.0.1
     (lambda (foobar.4.5 bar.5.4 foobar.6.3 ball.7.2 bat.1.1)
       (if (false)
         (if (begin (set! bar.9.6 foobar.6.3) (< 0 foobar.6.3))
           (call L.x.1.2 0 -9223372036854775808)
           (begin (set! foobar.2.7 bat.1.1) foobar.4.5))
         (if (true) (if (!= -9223372036854775808 1) 0 849007740) foobar.4.5))))
   (define L.x.1.2
...
(check-by-interp-v6 '(module
   (define L.tmp.0.1
     (lambda (bat.3.5 bar.8.4 ball.4.3 bar.2.2 foo.6.1) foo.6.1))
   (begin
     (set! foo.5.6
       (call L.tmp.0.1 1 -9223372036854775808 0 1 -9223372036854775808))
       ...
       (begin
         (set! bar.2.2
           (if (begin
             ...
             (- ball.1.1 -1702731989)
             (begin (set! bar.2.4 ball.1.1) ball.1.1)))
         (if (not (= -9223372036854775808 bar.2.2))
           (begin (set! bar.6.5 1) bar.2.2)
           (call L.proc.0.1 0)))))
   (define L.fn.1.2 (lambda () (+ 9223372036854775807 1)))
   (define L.fn.2.3
     (lambda (bat.9.6) (begin (set! bar.6.7 (* 1 0)) (call L.fn.1.2))))
   (if (true)
     (begin (set! bat.3.8 (call L.fn.2.3 1)) (begin (set! bar.7.9 bat.3.8) 1))
     (begin (set! bar.5.10 -1579827217) (call L.fn.2.3 bar.5.10)))))
(check-by-interp-v6 '(module
   (define L.x.0.1
     ...
     (lambda (foo.2.12 foobar.1.11 foo.8.10 ball.4.9 foobar.7.8)
       (if (false)
         ...
         (if (false)
           ...
           (begin (set! ball.0.14 foo.8.10) 1)))))
   (define L.tmp.2.3
     (lambda ()
       (begin
         (set! foobar.5.15 (begin (set! foo.8.16 1) foo.8.16))
     ...
     (lambda (bar.4.6 foobar.5.5 bar.0.4 foo.7.3 ball.3.2 ball.8.1)
       (if (not
         ...
         (if (true)
           ...
           (begin (set! bar.0.7 foo.7.3) foobar.5.5))
         (call
          L.proc.0.1
          1
          ball.8.1
       ...
       (begin
         (set! ball.3.11
           (if (<= -408033154 bar.1.10)
             (+ ball.8.9 bar.1.10)
             (begin (set! bar.4.12 ball.8.9) bar.4.12)))
       ...
       (begin
         (set! ball.7.5
           (if (begin
             ...
             (begin (set! foo.0.7 1) foo.3.2)
             foo.3.2))
         (if (true)
           (begin (set! foo.3.8 foo.0.3) ball.7.5)
           (call L.x.0.1 foo.3.2 foo.3.2 -9223372036854775808 foo.0.3)))))
   (begin
     (set! bat.8.9 (if (true) 480297521 (- 1 0)))
     (if (= bat.8.9 0)
       (begin
         (set! foo.4.10 (begin (set! bat.9.11 bat.8.9) 0))
       ...
       (if (begin (set! bat.9.12 1) (>= bat.8.9 1434817833))
         ...
         (begin (set! bat.2.13 -9223372036854775808) 1398310587))))))
(check-by-interp-v6 '(module
   (define L.func.0.1
       ...
       (if (> 9223372036854775807 -1225978347)
         ...
         (begin
           ...
           (call
            ...
            foo.6.6
            0)))))
   (if (>= 1 9223372036854775807) (begin (set! bar.8.9 1) bar.8.9) -32529780)))
(check-by-interp-v6 '(module
   (define L.proc.0.1
...
(check-by-interp-v6 '(module
   ...
   (begin (set! bat.9.10 -477286222) (set! bar.7.9 0) 643069821)))
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda (bar.9.1)
       (if (!= bar.9.1 bar.9.1)
...
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda (foo.2.6 ball.1.5 foo.4.4 foobar.7.3 foo.5.2 foo.9.1)
       (- foo.2.6 foo.4.4)))
   (define L.proc.1.2
     (lambda ()
       (begin
         ...
         (begin
           (set! foo.6.10
             (call L.func.0.1 0 foo.6.7 foo.6.7 foo.6.7 foo.6.7 foo.6.7))
...
(check-by-interp-v6 '(module
   ...
   (begin
     ...
     (set! bat.6.19
       (if (not (>= -915730633 0))
         (begin (set! bat.8.21 -532660031) -248685178)
         0))
     ...
     (lambda (bar.0.14
       ...
       (begin
         ...
         (call
          ...
          bar.0.14
          bar.4.11))))
   (begin (set! bar.0.18 0) (set! foo.8.17 1029279872) bar.0.18)))
(check-by-interp-v6 '(module
   (if (if (begin
             (set! bat.2.2 1)
         ...
         (begin
           ...
           (<= ball.5.5 ball.5.5))
         (begin (!= -1879829070 1981475003)))
     (- -634246165 -684848771)
     0)))
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda (bat.2.3 foo.3.2 bar.6.1)
       (call L.proc.1.2 bat.2.3 -1803672217 bat.2.3)))
   (define L.proc.1.2
     (lambda (foo.3.6 ball.0.5 bat.1.4) (call L.proc.1.2 1 ball.0.5 bat.1.4)))
   (begin (set! bar.9.7 0) bar.9.7)))
(check-by-interp-v6 '(module
   (begin
     (begin
       (set! ball.0.1
       ...
       (begin
         ...
         (if (> bar.6.4 foo.1.1)
           ...
           (begin
             ...
             ball.9.5)))))
   (begin (set! ball.9.13 1) (set! foo.1.12 -9223372036854775808) foo.1.12)))
(check-by-interp-v6 '(module
   (define L.fn.0.1
     (lambda (bar.0.1)
       (if (true)
   ...
   (define L.tmp.1.2
     (lambda (foobar.6.7 foobar.3.6 foo.5.5 bar.2.4 ball.8.3 bar.0.2)
       (begin
         ...
         (set! foobar.6.8
           (begin (set! ball.1.11 (+ bar.2.4 foobar.3.6)) foobar.3.6))
         (call L.fn.0.1 0))))
   (define L.func.2.3
     (lambda (ball.9.15 ball.8.14 bar.0.13 foobar.6.12)
       (call
        ...
        bar.0.13
        9223372036854775807)))
   (begin (set! bat.4.16 (+ 1 9223372036854775807)) bat.4.16)))
(check-by-interp-v6 '(module (begin (set! foobar.1.2 0) (set! bat.0.1 1) bat.0.1)))
(check-by-interp-v6 '(module
   (define L.func.0.1
     (lambda ()
       (call L.x.1.2 38797657 1 9223372036854775807 9223372036854775807 1 0)))
   (define L.x.1.2
   ...
   (if (false)
     (if (if (< 9223372036854775807 1646335033)
       ...
       (if (!= -9223372036854775808 -9223372036854775808)
         -9223372036854775808
         1282320164)
       (begin (set! bar.1.1 -9223372036854775808) bar.1.1))
     (- 9223372036854775807 1))))
(check-by-interp-v6 '(module
   (define L.tmp.0.1
     (lambda (bar.1.4 bat.6.3 ball.4.2 bar.0.1)
       (begin
         ...
         (set! ball.4.5 (begin (- bat.6.3 bar.0.1)))
         (begin
           (set! foobar.8.9
             (begin (set! foobar.2.11 bat.6.3) (set! bar.1.10 1) bar.0.1))
           (set! bar.0.8 bar.1.4)
         ...
         (begin
           ...
           (set! foo.2.8
             (call
              ...
              -9223372036854775808
              foo.6.1))
           (begin (set! foo.2.11 foo.1.6) ball.0.2))
         (begin -774243080))))
   (define L.fn.1.2
...
(check-by-interp-v6 '(module
   (if (true)
     ...
     (if (false)
       (begin (set! foo.5.2 1032709229) (set! bat.6.1 1) foo.5.2)
       (begin (set! bat.8.3 9223372036854775807) bat.8.3)))))
(check-by-interp-v6 '(module
   (if (< 0 9223372036854775807)
     1620518798
   ...
   (define L.fn.1.2
     (lambda (foo.6.7 foobar.2.6 foobar.1.5 bat.3.4 foobar.5.3)
       (begin
         (set! bar.0.8
           (begin
             (set! foobar.7.10 (begin foobar.5.3))
...
(check-by-interp-v6 '(module
   ...
   (define L.fn.1.2
     (lambda (bat.0.9 foobar.7.8 bar.2.7 bat.6.6 foobar.4.5)
       (call L.x.2.3 foobar.7.8)))
   (define L.x.2.3
     (lambda (foobar.7.10)
       (begin
         (set! foo.9.11
...
(check-by-interp-v6 '(module
   (define L.x.0.1
     (lambda (foo.3.6 foobar.0.5 bat.2.4 bar.7.3 bar.5.2 bat.6.1)
       (call L.x.1.2 bar.7.3 1)))
   (define L.x.1.2
   ...
   (define L.tmp.0.1
     (lambda (bat.6.5 bat.2.4 foobar.8.3 bar.3.2 foo.1.1)
       (if (<= bat.2.4 9223372036854775807)
         ...
         (begin
           (set! bar.3.7 foo.1.1)
           (set! bar.4.6 (begin bat.6.5))
           (begin (set! bat.2.8 foobar.8.3) bat.2.8)))))
   (define L.tmp.1.2
     (lambda (bar.3.15
              foo.1.14
              foobar.8.13
...
(check-by-interp-v6 '(module
   (begin
     ...
     (set! bat.6.2 (begin -9223372036854775808))
...
(check-by-interp-v6 '(module
   (begin
     ...
     (begin
       ...
       (begin (set! ball.9.8 bar.0.4) ball.1.1)))))
(check-by-interp-v6 '(module
   (begin
     (set! foo.7.1 1)
     (begin
       (set! foobar.3.4
         (if (= foo.7.1 foo.7.1)
           (begin
             ...
             (set! foo.4.5 foo.7.1)
             foo.7.1)
           (begin (set! foo.4.9 foo.7.1) (set! foobar.1.8 foo.7.1) foo.7.1)))
       (set! foo.8.3 foo.7.1)
       ...
       (begin
         (begin
           ...
           foo.7.1))))))
(check-by-interp-v6 '(module
   (define L.func.0.1 (lambda (bat.7.2 bat.8.1) (call L.func.0.1 1 bat.8.1)))
   0))
(check-by-interp-v6 '(module
   ...
   (define L.func.1.2
     (lambda (ball.1.11 bat.4.10 foo.5.9 ball.9.8 foo.8.7 ball.3.6)
       (if (if (if (< foo.8.7 foo.8.7)
                 ...
                 (!= bat.4.10 foo.8.7))
             (= ball.3.6 0)
             (if (> 0 ball.3.6) (<= 1 ball.9.8) (>= 529266316 foo.8.7)))
         (if (begin (set! bat.4.12 foo.5.9) (<= ball.9.8 bat.4.12))
           (begin (set! foo.8.13 ball.3.6) ball.3.6)
           (call
            L.func.1.2
            foo.5.9
            ball.1.11
            ...
            ball.3.6))
         (begin
           (set! ball.1.14 (begin (set! ball.3.15 foo.5.9) ball.3.15))
           (call
            L.func.1.2
            ball.1.14
            ball.3.6
...
(check-by-interp-v6 '(module
   (if (if (!= 9223372036854775807 2124101395)
     ...
     (begin
       (set! bat.2.2
         (begin (set! bat.2.3 -9223372036854775808) -9223372036854775808))
...
(check-by-interp '(module
   (define L.L.func.0.1.4 (lambda () 0))
   ...
   (define L.L.func.2.3.6
     (lambda ()
       (begin
         (set! bar.4.2.6
           (if (true)
             (begin (set! foo.2.3.7 1) foo.2.3.7)
             (* -9223372036854775808 1806293504)))
         (begin (set! foobar.6.4.8 (if (> 1 0) 156890122 bar.4.2.6)) 0))))
   (call L.L.tmp.1.2.5 -1860620182)))
(check-by-interp '(module
   (define L.L.tmp.0.1.4
     (lambda ()
       (if (>= 9223372036854775807 -1020514810) (call L.L.x.2.3.6) (* 1 0))))
   ...
   (define L.L.x.2.3.6
     (lambda ()
       (if (if (true) (not (> 0 1)) (true))
         (if (> 0 0)
           (if (< 9223372036854775807 9223372036854775807)
             9223372036854775807
             -260353756)
           (begin (set! ball.0.2.6 0) ball.0.2.6))
         (begin
           (set! bar.3.3.7
             (if (!= -302047143 0) 9223372036854775807 102036653))
           (if (= 0 bar.3.3.7) bar.3.3.7 -362331747)))))
   (begin
     (set! ball.0.4.8 (* -9223372036854775808 -9223372036854775808))
     (+ ball.0.4.8 0))))
(check-by-interp '(module
   (define L.L.proc.0.1.4 (lambda (ball.6.1.5) (call L.L.func.1.2.5)))
     ...
     (lambda (ball.3.2.6)
       (if (true)
         (begin
           ...
           (begin (set! foo.4.4.8 1) 1236904416))
         (call L.L.func.1.2.5))))
...
(check-by-interp '(module
   (define L.fn.0.1
     (lambda ()
       (begin
         (set! bat.9.1 (if (false) 1 (begin (set! foo.5.2 1) 1)))
         (if (if (>= 9223372036854775807 -9223372036854775808)
               (= -248968641 9223372036854775807)
               (!= bat.9.1 -9223372036854775808))
           (begin (set! ball.4.3 bat.9.1) 1622965009)
           (begin (set! foo.5.4 bat.9.1) bat.9.1)))))
   (if (false) (if (>= 995853130 1) 1 -9223372036854775808) (call L.fn.0.1))))
   ...
   (define L.fn.0.1
     (lambda ()
       (begin
         ...
         (begin
           (set! foo.8.2 (begin (set! foo.1.3 bar.2.1) foo.1.3))
           bar.2.1))))
   (define L.func.1.2
     (lambda ()
       (begin
         (set! ball.3.4
           (if (begin (set! foo.8.5 1) (< 1 foo.8.5))
             (begin (set! foo.1.6 406779451) 1200977699)
             (begin (set! foo.8.7 0) foo.8.7)))
         (* ball.3.4 ball.3.4))))
   (begin (set! bat.5.8 1) (begin (set! ball.9.9 0) bat.5.8))))
(check-by-interp '(module
   (define L.x.0.1 (lambda (ball.1.2 foo.0.1) (call L.tmp.1.2)))
   (define L.tmp.1.2
     (lambda ()
       (if (if (if (<= -765445006 0)
             ...
             (true))
         (begin
           (set! ball.4.3 (begin (set! bar.5.4 -9223372036854775808) bar.5.4))
   ...
   (define L.proc.0.1
     (lambda (bar.8.1)
       (begin
         (set! bat.7.2
           (if (false)
             (begin (set! bat.4.3 bar.8.1) 1733388107)
             (begin (set! foobar.2.4 -818241658) foobar.2.4)))
         (begin
   ...
   (define L.proc.0.1
     (lambda (ball.3.1)
       (if (not (false))
         ...
         (begin
           (set! foobar.4.2
             (begin (set! bat.9.3 ball.3.1) 9223372036854775807))
           (+ -9223372036854775808 ball.3.1)))))
   (define L.x.1.2 (lambda () 1))
   (begin
     (set! foobar.4.4 (begin (set! ball.1.5 453798193) ball.1.5))
     (if (>= 9223372036854775807 foobar.4.4) 0 -1617493587))))
(check-by-interp '(module
   (define L.func.0.1
     (lambda (bat.8.1)
       (if (begin (set! bat.2.2 (* 9223372036854775807 bat.8.1)) (true))
         (if (true)
   ...
   (if (false) (* 1337690701 9223372036854775807) (call L.proc.1.2))))
(check-by-interp '(module
   (if (not (> 1 9223372036854775807))
   ...
   (define L.x.1.2
     (lambda (bar.0.3)
       (begin
         (set! ball.1.4
           (begin
             (set! bat.9.5 (begin (set! bat.2.6 1757280127) bat.2.6))
             (begin (set! ball.5.7 1) -1128483887)))
         (if (begin (set! foo.7.8 bar.0.3) (> foo.7.8 foo.7.8))
           ball.1.4
           (+ ball.1.4 bar.0.3)))))
   (define L.fn.2.3
     (lambda ()
       (if (begin
         ...
         (begin
           ...
           (begin (set! foo.4.11 ball.1.10) 770292232)))))
       ...
       (begin
         (set! bar.3.10
           (begin
             ...
             (begin (set! foobar.5.12 ball.1.9) -9223372036854775808)))
         (call L.proc.0.1 bar.3.10 -780648786 bar.3.10))))
   (begin (set! bat.0.13 -579691794) 953357957)))
(check-by-interp '(module
   (begin
     (set! bar.5.1 (begin (set! ball.2.2 9223372036854775807) -546026276))
     (if (!= bar.5.1 1) 9223372036854775807 2063023986))))
(check-by-interp '(module
   (define L.func.0.1
     (lambda (foo.8.2 bar.2.1)
       (if (not (begin (set! foo.8.3 foo.8.2) (= foo.8.3 0)))
         (begin (set! foobar.6.4 (+ foo.8.2 0)) (* 0 9223372036854775807))
         (begin
       ...
       (begin
         (set! foobar.3.7
           (begin
             ...
             (begin (set! bat.2.9 1) -9223372036854775808)))
         (* -152436426 0))))
   (define L.proc.2.3
     (lambda (foo.5.13 ball.7.12 foo.0.11 ball.8.10)
       (begin (set! foobar.1.14 foo.5.13) (+ 956544411 1))))
...
(check-by-interp '(module
   (define L.proc.0.1
     (lambda (foobar.1.6 bar.0.5 foobar.2.4 ball.8.3 bat.3.2 bar.5.1)
       (begin
         (set! bar.9.7
           (if (begin (set! bar.7.8 -669410514) (< bat.3.2 bat.3.2))
             (+ bar.5.1 ball.8.3)
             foobar.2.4))
         (begin
           (set! ball.6.9 (begin (set! foo.4.10 ball.8.3) 1))
...
(check-by-interp '(module
   (define L.x.0.1
     (lambda (bat.7.7 ball.9.6 foobar.3.5 ball.5.4 ball.4.3 bar.0.2 ball.2.1)
       (if (not
            (if (>= 388494724 ball.9.6)
              (> ball.5.4 -9223372036854775808)
              (< foobar.3.5 ball.5.4)))
         (if (<= ball.4.3 bar.0.2)
           0
           (begin (set! ball.4.8 ball.5.4) 1887946265))
         foobar.3.5)))
   (define L.tmp.1.2
     (lambda (bat.7.14 ball.9.13 ball.5.12 foo.6.11 bar.0.10 ball.2.9)
       (if (true)
         (* foo.6.11 9223372036854775807)
         (if (begin (set! foobar.3.15 1) (<= bar.0.10 ball.5.12))
           (begin (set! ball.9.16 -9223372036854775808) ball.9.16)
           (begin (set! bat.7.17 1) 1)))))
   (define L.tmp.2.3
     (lambda (bat.7.23 ball.2.22 foo.6.21 foobar.3.20 bar.8.19 bar.0.18)
       (begin
         (set! foobar.3.24
           (if (begin (set! ball.2.25 258314756) (!= bar.8.19 0))
             bar.8.19
             (begin (set! foo.6.26 -1809848824) 9223372036854775807)))
         (if (< 1525021420 1)
           (call
            L.tmp.1.2
            foobar.3.24
...
(check-by-interp '(module
   ...
   (define L.func.1.2
     (lambda (foo.1.12 bar.2.11 foo.5.10 bat.3.9 bar.9.8)
       (begin (set! bat.3.13 1) 9223372036854775807)))
   (define L.fn.2.3
   ...
   (call
    ...
    1
    -9223372036854775808)))
(check-by-interp '(module (if (true) (+ 1 -9223372036854775808) (if (> 1383245321 0) 0 1))))
(check-by-interp '(module
   (define L.x.0.1
     (lambda (ball.0.7
              foobar.1.6
   ...
   (define L.fn.2.3
     (lambda ()
       (begin
         ...
         (if (false)
           ball.7.10
           (begin (set! foobar.1.11 ball.7.10) 1969054361)))))
   (define L.x.3.4
     (lambda (foobar.4.18
              foobar.5.17
              bar.9.16
       ...
       (begin
         (set! foobar.5.26
           (begin
             ...
             (begin (set! foo.6.28 foo.3.24) foobar.1.27)))
         (if (begin (set! ball.8.29 bar.9.22) (> foo.3.24 foo.6.25))
           (call
            L.x.0.1
            -1863740769
            9223372036854775807
     ...
     (lambda (foobar.2.36
       ...
       (if (if (true)
         ...
         (begin
           ...
           (begin (set! foobar.1.39 0) foobar.1.39))
         1)))
   (define L.fn.7.8
     (lambda ()
       (if (true)
   ...
   (if (begin
         (set! foo.6.40
           (if (begin (set! foobar.2.41 -9223372036854775808) (true))
             (begin
               (set! foobar.2.42
               ...
               foobar.2.42)
             (if (true)
               (begin (set! foobar.5.43 9223372036854775807) foobar.5.43)
               (if (= -9223372036854775808 1173781558) 0 1))))
         (if (true) (begin (set! bar.9.44 0) (true)) (< 858519747 foo.6.40)))
     1
     (if (< 1 -1323259230) (call L.fn.2.3) (call L.fn.2.3)))))
