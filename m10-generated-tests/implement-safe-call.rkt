...
(check-by-interp
 '(module
    ...
    (let ((g42027422.7 #\b))
      (if (call error?.65 g42027422.7)
        g42027422.7
        (let ((g42027423.8 #\_))
          (if (call error?.65 g42027423.8)
            g42027423.8
            (let ((g42027424.9 #\c))
              (if (call error?.65 g42027424.9)
                g42027424.9
                (let ((g42027425.10 #\q))
                  (if (call error?.65 g42027425.10)
                    g42027425.10
                    (let ((g42027426.11 #\V))
                      (if (call error?.65 g42027426.11)
                        g42027426.11
                        (let ((g42027427.12 #\p))
                          (if (call error?.65 g42027427.12)
                            g42027427.12
                            #\R))))))))))))))
(check-by-interp
 '(module
    (define error?.60 (lambda (tmp.50) (error? tmp.50)))
    (let ((g42031243.7 (error 172)))
      (if (call error?.60 g42031243.7) g42031243.7 (error 172)))))
(check-by-interp
...
(check-by-interp
 '(module
    ...
    (let ((g42050319.7
           (let ((g42050320.8 #\V))
             (if (call error?.66 g42050320.8)
               g42050320.8
               (let ((g42050321.9 #\f))
                 (if (call error?.66 g42050321.9)
                   g42050321.9
                   (let ((g42050322.10 #\E))
                     (if (call error?.66 g42050322.10)
                       g42050322.10
                       (let ((g42050323.11 #\n))
                         (if (call error?.66 g42050323.11)
                           g42050323.11
                           #\\))))))))))
      (if (call error?.66 g42050319.7)
        g42050319.7
        (let ((g42050324.12 (let () #\I)))
          (if (call error?.66 g42050324.12)
            g42050324.12
            (let ((void0.13 (void))) #\k)))))))
(check-by-interp
 '(module
    (define error?.84 (lambda (tmp.72) (error? tmp.72)))
    ...
    (define vector-set!.83
      (lambda (tmp.60 tmp.61 tmp.62)
        (if (fixnum? tmp.61)
          (if (vector? tmp.60)
            (call unsafe-vector-set!.5 tmp.60 tmp.61 tmp.62)
            (error 10))
          (error 10))))
    (define vector-init-loop.32
      (lambda (len.33 i.35 vec.34)
        (if (eq? len.33 i.35)
          vec.34
          (begin
            (unsafe-vector-set! vec.34 i.35 0)
            (call vector-init-loop.32 len.33 (unsafe-fx+ i.35 1) vec.34)))))
    (define make-init-vector.4
      (lambda (tmp.30)
        (let ((tmp.31 (unsafe-make-vector tmp.30)))
          (call vector-init-loop.32 tmp.30 0 tmp.31))))
    (define make-vector.82
      (lambda (tmp.58)
        (if (fixnum? tmp.58) (call make-init-vector.4 tmp.58) (error 8))))
    (define fun/ascii-char8506.13 (lambda () #\L))
    (define fun/ascii-char8507.14 (lambda () #\i))
    (let ((tmp.7.15
           (let ((procedure0.16
                  (lambda ()
                    (let ((tmp.8.17 (call make-vector.82 8)))
                      (let ((g42054139.18 (call vector-set!.83 tmp.8.17 0 1)))
                        (if (call error?.84 g42054139.18)
                          g42054139.18
                          (let ((g42054140.19
                                 (call vector-set!.83 tmp.8.17 1 2)))
                            (if (call error?.84 g42054140.19)
                              g42054140.19
                              (let ((g42054141.20
                                     (call vector-set!.83 tmp.8.17 2 3)))
                                (if (call error?.84 g42054141.20)
                                  g42054141.20
                                  (let ((g42054142.21
                                         (call vector-set!.83 tmp.8.17 3 4)))
                                    (if (call error?.84 g42054142.21)
                                      g42054142.21
                                      (let ((g42054143.22
                                             (call
                                              vector-set!.83
 ...
 '(module
    ...
    (if (let ((error0.8 (error 194))) (error 152))
      (let ((g42119027.9 (if #f (error 108) (error 68))))
        (if (call error?.69 g42119027.9)
          g42119027.9
          (let ((g42119028.10 (error 192)))
            (if (call error?.69 g42119028.10)
              g42119028.10
              (let ((g42119029.11 (if #f (error 126) (error 57))))
                (if (call error?.69 g42119029.11)
                  g42119029.11
                  (let ((g42119030.12
                         (let ((g42119031.13 (error 31)))
                           (if (call error?.69 g42119031.13)
                             g42119031.13
                             (let ((g42119032.14 (error 219)))
                               (if (call error?.69 g42119032.14)
                                 g42119032.14
                                 (error 86)))))))
                    (if (call error?.69 g42119030.12)
                      g42119030.12
                      (let ((g42119033.15 (if #t (error 147) (error 181))))
                        (if (call error?.69 g42119033.15)
                          g42119033.15
                          (let ((g42119034.16 (call fun/error8557.7)))
                            (if (call error?.69 g42119034.16)
                              g42119034.16
                              (error 211)))))))))))))
      #f)))
(check-by-interp
 '(module
    ...
    (if (call fun/boolean8560.9)
      ...
      (call fun/ascii-char8562.7 (call cons.64 31 (call cons.64 309 empty))))))
(check-by-interp
 '(module
...
(check-by-interp
 '(module
    (define pair?.61 (lambda (tmp.52) (pair? tmp.52)))
    (call
     pair?.61
...
(check-by-interp
 '(module
    ...
    (define fun/vector8594.11
      (lambda ()
        (let ((tmp.7.14 (call make-vector.76 8)))
          (let ((g42161003.15 (call vector-set!.77 tmp.7.14 0 1)))
            (if (call error?.78 g42161003.15)
              g42161003.15
              (let ((g42161004.16 (call vector-set!.77 tmp.7.14 1 2)))
                (if (call error?.78 g42161004.16)
                  g42161004.16
                  (let ((g42161005.17 (call vector-set!.77 tmp.7.14 2 3)))
                    (if (call error?.78 g42161005.17)
                      g42161005.17
                      (let ((g42161006.18 (call vector-set!.77 tmp.7.14 3 4)))
                        (if (call error?.78 g42161006.18)
                          g42161006.18
                          (let ((g42161007.19
                                 (call vector-set!.77 tmp.7.14 4 5)))
                            (if (call error?.78 g42161007.19)
                              g42161007.19
                              (let ((g42161008.20
                                     (call vector-set!.77 tmp.7.14 5 6)))
                                (if (call error?.78 g42161008.20)
                                  g42161008.20
                                  (let ((g42161009.21
                                         (call vector-set!.77 tmp.7.14 6 7)))
                                    (if (call error?.78 g42161009.21)
                                      g42161009.21
                                      (let ((g42161010.22
                                             (call
                                              vector-set!.77
...
(check-by-interp
 '(module
    (define error?.107 (lambda (tmp.96) (error? tmp.96)))
    (define cons.106 (lambda (tmp.101 tmp.102) (cons tmp.101 tmp.102)))
    ...
    (let ((g42168642.25
           (let ((tmp.7.26
                  (let ((pair0.28
                         (call cons.106 141 (call cons.106 498 empty)))
                        (boolean1.27 #f))
                    #\B)))
             (if tmp.7.26
               tmp.7.26
               (let ((tmp.8.29
                      (let ((g42168643.30 #\V))
                        (if (call error?.107 g42168643.30) g42168643.30 #\B))))
                 (if tmp.8.29
                   tmp.8.29
                   (let ((tmp.9.31
                       ...
                       (let ((tmp.10.33 (call fun/ascii-char8602.22)))
                         (if tmp.10.33
                           ...
                           (let ((fixnum0.35 12) (ascii-char1.34 #\R))
                             #\e)))))))))))
      (if (call error?.107 g42168642.25)
        g42168642.25
        (let ((g42168644.36 (call fun/ascii-char8603.24)))
          (if (call error?.107 g42168644.36)
            g42168644.36
            (let ((g42168645.37
                                                     ...
                                                     (if tmp.20.49
                                                       ...
                                                       (let ((tmp.21.50 #\_))
                                                         (if tmp.21.50
                                                           tmp.21.50
                                                           #\F))))))))))))))))))))))
              (if (call error?.107 g42168645.37)
                g42168645.37
                (let ((g42168646.51 (let ((fixnum0.52 45)) #\j)))
                  (if (call error?.107 g42168646.51)
                    g42168646.51
                    (let ((g42168647.53 (if #f #\A #\c)))
                      (if (call error?.107 g42168647.53)
                        g42168647.53
                        (if #f #\U #\e)))))))))))))
(check-by-interp
 '(module
...
(check-by-interp
 '(module
    ...
    (let ((g42183917.7 (let ((ascii-char0.8 #\r)) #\C)))
      (if (call error?.63 g42183917.7)
        g42183917.7
        (let ((g42183918.9 (if #f #\G #\R)))
          (if (call error?.63 g42183918.9)
            g42183918.9
            (let ((g42183919.10 (let () #\t)))
              (if (call error?.63 g42183919.10)
                g42183919.10
                (let () #\t)))))))))
(check-by-interp
 '(module
      ...
      (lambda (tmp.68 tmp.69 tmp.70)
        (if (unsafe-fx< tmp.69 (unsafe-vector-length tmp.68))
          (if (unsafe-fx>= tmp.69 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.117
      (lambda (tmp.92 tmp.93 tmp.94)
        (if (fixnum? tmp.93)
          (if (vector? tmp.92)
            (call unsafe-vector-set!.5 tmp.92 tmp.93 tmp.94)
            (error 10))
          (error 10))))
    (define vector-init-loop.64
      (lambda (len.65 i.67 vec.66)
        (if (eq? len.65 i.67)
          vec.66
          (begin
            (unsafe-vector-set! vec.66 i.67 0)
            (call vector-init-loop.64 len.65 (unsafe-fx+ i.67 1) vec.66)))))
    (define make-init-vector.4
      (lambda (tmp.62)
        (let ((tmp.63 (unsafe-make-vector tmp.62)))
          (call vector-init-loop.64 tmp.62 0 tmp.63))))
    (define make-vector.116
      (lambda (tmp.90)
        (if (fixnum? tmp.90) (call make-init-vector.4 tmp.90) (error 8))))
    (define cons.115 (lambda (tmp.109 tmp.110) (cons tmp.109 tmp.110)))
    (define error?.114 (lambda (tmp.104) (error? tmp.104)))
          ...
          (if (if #t empty empty)
            (if (if (let ((tmp.7.30 empty))
              ...
              (let ((g42195365.36 (call fun/empty8629.25 #f)))
                (if (call error?.114 g42195365.36)
                  g42195365.36
                  (let ((g42195366.37 (let ((fixnum0.38 174)) empty)))
                    (if (call error?.114 g42195366.37)
                      g42195366.37
                      (let ((g42195367.39 (call fun/empty8630.24)))
                        (if (call error?.114 g42195367.39)
                          g42195367.39
                          (let ((g42195368.40
                                 (let ((tmp.13.41 empty))
                                   (if tmp.13.41
                                       ...
                                       (if tmp.14.42
                                         ...
                                         (let ((tmp.15.43 empty))
                                           (if tmp.15.43
                                             tmp.15.43
                                             empty))))))))
                            (if (call error?.114 g42195368.40)
                              g42195368.40
                              (let ((g42195369.44
                                     (let ((pair0.46
                                            (call
                                             cons.115
                                             202
                                             (call cons.115 322 empty)))
                                           (vector1.45
                                            (let ((tmp.16.47
                                                   (call make-vector.116 8)))
                                              (let ((g42195370.48
                                                     (call
                                                      vector-set!.117
                                                      tmp.16.47
                                                                        ...
                                                                        (if (call
                                                                          ...
                                                                          (let ((g42195377.55
                                                                            ...
                                                                            (if (call
                                                                              ...
                                                                              tmp.16.47)))))))))))))))))))
                                       empty)))
                                (if (call error?.114 g42195369.44)
                                  g42195369.44
                                  (let ((tmp.17.56 empty))
                                    (if tmp.17.56
                                      tmp.17.56
...
(check-by-interp
 '(module
    (define error?.62 (lambda (tmp.52) (error? tmp.52)))
    (if (if #t #t #t)
      (let ((ascii-char0.7 #\H)) 50)
      (let ((g42210650.8 184))
        (if (call error?.62 g42210650.8)
          g42210650.8
          (let ((g42210651.9 213))
            (if (call error?.62 g42210651.9) g42210651.9 114)))))))
(check-by-interp
 '(module
    ...
    (if (let ((void0.10 (void)) (empty1.9 empty)) (error 173))
      (let ((tmp.7.11 (let ((boolean0.12 #t)) (error 149))))
        (if tmp.7.11
          tmp.7.11
          (let ((tmp.8.13
                 (let ((pair0.14 (call cons.67 227 (call cons.67 467 empty))))
                   (error 230))))
            (if tmp.8.13 tmp.8.13 (if #t (error 139) (error 80))))))
      #f)))
(check-by-interp
 '(module
    (define cons.82 (lambda (tmp.76 tmp.77) (cons tmp.76 tmp.77)))
                 ...
                 (if (let () empty)
                   (if (let ((tmp.12.23 empty))
                     ...
                     (if (let ((procedure0.26
                                (lambda ()
                                  (call cons.82 98 (call cons.82 276 empty))))
                               (boolean1.25 #f))
                           empty)
                       (if #t empty empty)
                       #f)
                   ...
                   #f)
                 #f)))
          (if (call error?.81 g42218287.22)
            g42218287.22
            (let ((g42218288.27 (call fun/empty8650.14)))
              (if (call error?.81 g42218288.27)
                g42218288.27
                (let ((g42218289.28 (if empty empty #f)))
                  (if (call error?.81 g42218289.28)
                    g42218289.28
                    (if #t empty empty)))))))))))
(check-by-interp
 '(module
...
(check-by-interp
 '(module
    (define error?.68 (lambda (tmp.58) (error? tmp.58)))
    ...
    (let ((g42225918.13 (call fun/ascii-char8658.8)))
      (if (call error?.68 g42225918.13)
        g42225918.13
        (let ((g42225919.14 (let () #\Q)))
          (if (call error?.68 g42225919.14)
            g42225919.14
            (let ((g42225920.15 (call fun/ascii-char8660.10 empty)))
              (if (call error?.68 g42225920.15)
                g42225920.15
...
(check-by-interp
 '(module
    ...
    (if (let ((g42233549.10 (let () #t)))
          (if (call error?.63 g42233549.10) g42233549.10 (let () #t)))
      (call error?.63 (call fun/any7325.7 #\K (lambda () 713)))
      (if #f #f #t))))
(check-by-interp
 '(module
    (define error?.79 (lambda (tmp.65) (error? tmp.65)))
    (define unsafe-vector-set!.5
    ...
    (define cons.76 (lambda (tmp.70 tmp.71) (cons tmp.70 tmp.71)))
    (define void?.75 (lambda (tmp.63) (void? tmp.63)))
    (define fun/any8476.8 (lambda (oprand0.9) 169))
    (if (call void?.75 (call fun/any8476.8 (lambda () 1020)))
      (let ((procedure0.12
             (lambda () (call cons.76 215 (call cons.76 403 empty))))
            (error1.11 (error 137))
            (error2.10 (error 179)))
        #f)
      (let ((vector0.13
             (let ((tmp.7.14 (call make-vector.77 8)))
               (let ((g42237367.15 (call vector-set!.78 tmp.7.14 0 0)))
                 (if (call error?.79 g42237367.15)
                   g42237367.15
                   (let ((g42237368.16 (call vector-set!.78 tmp.7.14 1 1)))
                     (if (call error?.79 g42237368.16)
                       g42237368.16
                       (let ((g42237369.17 (call vector-set!.78 tmp.7.14 2 2)))
                         (if (call error?.79 g42237369.17)
                           g42237369.17
                           (let ((g42237370.18
                                  (call vector-set!.78 tmp.7.14 3 3)))
                             (if (call error?.79 g42237370.18)
                               g42237370.18
                               (let ((g42237371.19
                                      (call vector-set!.78 tmp.7.14 4 4)))
                                 (if (call error?.79 g42237371.19)
                                   g42237371.19
                                   (let ((g42237372.20
                                          (call vector-set!.78 tmp.7.14 5 5)))
                                     (if (call error?.79 g42237372.20)
                                       g42237372.20
                                       (let ((g42237373.21
                                              (call
                                               vector-set!.78
                                         ...
                                         (if (call error?.79 g42237373.21)
                                           ...
                                           (let ((g42237374.22
                                                  (call
                                                   ...
                                                   7
                                                   7)))
                                             (if (call error?.79 g42237374.22)
                                               g42237374.22
                                               tmp.7.14)))))))))))))))))))
        #t))))
(check-by-interp
 '(module
    (define cons.75 (lambda (tmp.66 tmp.67) (cons tmp.66 tmp.67)))
    (define fixnum?.74 (lambda (tmp.56) (fixnum? tmp.56)))
    ...
    (if (call
         ...
         (if #t
           194
           (let ((tmp.7.8 (call make-vector.71 8)))
             (let ((g42241192.9 (call vector-set!.72 tmp.7.8 0 1)))
               (if (call error?.73 g42241192.9)
                 g42241192.9
                 (let ((g42241193.10 (call vector-set!.72 tmp.7.8 1 2)))
                   (if (call error?.73 g42241193.10)
                     g42241193.10
                     (let ((g42241194.11 (call vector-set!.72 tmp.7.8 2 3)))
                       (if (call error?.73 g42241194.11)
                         g42241194.11
                         (let ((g42241195.12
                                (call vector-set!.72 tmp.7.8 3 4)))
                           (if (call error?.73 g42241195.12)
                             g42241195.12
                             (let ((g42241196.13
                                    (call vector-set!.72 tmp.7.8 4 5)))
                               (if (call error?.73 g42241196.13)
                                 g42241196.13
                                 (let ((g42241197.14
                                        (call vector-set!.72 tmp.7.8 5 6)))
                                   (if (call error?.73 g42241197.14)
                                     g42241197.14
                                     (let ((g42241198.15
                                            (call vector-set!.72 tmp.7.8 6 7)))
                                       (if (call error?.73 g42241198.15)
                                         g42241198.15
                                         (let ((g42241199.16
                                                (call
                                                 vector-set!.72
                                                 ...
                                                 7
                                                 8)))
                                           (if (call error?.73 g42241199.16)
                                             g42241199.16
                                             tmp.7.8)))))))))))))))))))
      (let ((pair0.17 (call cons.75 82 (call cons.75 500 empty)))) empty)
...
(check-by-interp
 '(module
    (define error?.111 (lambda (tmp.98) (error? tmp.98)))
    (define unsafe-vector-set!.5
    ...
    (define vector-set!.110
      (lambda (tmp.86 tmp.87 tmp.88)
        (if (fixnum? tmp.87)
          (if (vector? tmp.86)
            (call unsafe-vector-set!.5 tmp.86 tmp.87 tmp.88)
            (error 10))
          (error 10))))
    (define vector-init-loop.58
      (lambda (len.59 i.61 vec.60)
        (if (eq? len.59 i.61)
          vec.60
          (begin
            (unsafe-vector-set! vec.60 i.61 0)
            (call vector-init-loop.58 len.59 (unsafe-fx+ i.61 1) vec.60)))))
    (define make-init-vector.4
      (lambda (tmp.56)
        (let ((tmp.57 (unsafe-make-vector tmp.56)))
          (call vector-init-loop.58 tmp.56 0 tmp.57))))
    (define make-vector.109
      (lambda (tmp.84)
        (if (fixnum? tmp.84) (call make-init-vector.4 tmp.84) (error 8))))
    (define cons.108 (lambda (tmp.103 tmp.104) (cons tmp.103 tmp.104)))
    ...
    (define fun/pair8497.11
      (lambda (oprand0.21)
        (let ((boolean0.22 #t))
          (call cons.108 243 (call cons.108 369 empty)))))
    ...
    (define fun/pair8498.16
      (lambda () (call cons.108 164 (call cons.108 306 empty))))
    ...
    (call
     ...
     (if (call
          ...
          (let ((vector0.27
                 (let ((tmp.7.28 (call make-vector.109 8)))
                   (let ((g42252645.29 (call vector-set!.110 tmp.7.28 0 1)))
                     (if (call error?.111 g42252645.29)
                       g42252645.29
                       (let ((g42252646.30
                              (call vector-set!.110 tmp.7.28 1 2)))
                         (if (call error?.111 g42252646.30)
                           g42252646.30
                           (let ((g42252647.31
                                  (call vector-set!.110 tmp.7.28 2 3)))
                             (if (call error?.111 g42252647.31)
                               g42252647.31
                               (let ((g42252648.32
                                      (call vector-set!.110 tmp.7.28 3 4)))
                                 (if (call error?.111 g42252648.32)
                                   g42252648.32
                                   (let ((g42252649.33
                                          (call vector-set!.110 tmp.7.28 4 5)))
                                     (if (call error?.111 g42252649.33)
                                       g42252649.33
                                       (let ((g42252650.34
                                              (call
                                               vector-set!.110
                                               ...
                                               5
                                               6)))
                                         (if (call error?.111 g42252650.34)
                                           g42252650.34
                                           (let ((g42252651.35
                                                  (call
                                                   vector-set!.110
                                                   ...
                                                   6
                                                   7)))
                                             (if (call error?.111 g42252651.35)
                                               g42252651.35
                                               (let ((g42252652.36
                                                      (call
                                                       vector-set!.110
       ...
       (if (if #t #\j #\G)
         (if (let ((g42252653.37
               ...
               (if (call error?.111 g42252653.37)
                 g42252653.37
                 (let ((g42252654.39
                        (let ((pair0.41
                               (call cons.108 222 (call cons.108 394 empty)))
                              (error1.40 (error 102)))
                          #\E)))
                   (if (call error?.111 g42252654.39)
                     g42252654.39
                     (let ((g42252655.42
                            (let ((boolean0.45 #f)
                                  (void1.44 (void))
                                  (boolean2.43 #f))
                              #\s)))
                       (if (call error?.111 g42252655.42)
                         g42252655.42
                         (let ((g42252656.46
                                (let ((boolean0.49 #f)
                                      (pair1.48
                                       (call
                                        cons.108
                                        165
                                        (call cons.108 270 empty)))
                                      (fixnum2.47 165))
                                  #\k)))
                           (if (call error?.111 g42252656.46)
                             g42252656.46
                             (let ((g42252657.50 (call fun/ascii-char8494.14)))
                               (if (call error?.111 g42252657.50)
                                 g42252657.50
                                 (let ((pair0.53
                                        (call
                                         cons.108
                                         123
                                         (call cons.108 483 empty)))
                                       (void1.52 (void))
                                       (void2.51 (void)))
                                   #\D)))))))))))
           (let () #\\)
    ...
    (let ()
      (if (if #f empty empty)
        (if (let ((g42256477.10 empty))
              (if (call error?.65 g42256477.10)
                g42256477.10
                (let ((g42256478.11 empty))
                  (if (call error?.65 g42256478.11)
                    g42256478.11
                    (let ((g42256479.12 empty))
                      (if (call error?.65 g42256479.12)
                        g42256479.12
                        empty))))))
...
(check-by-interp
 '(module
    ...
    (let ((void0.7
           (let ((pair0.9 (call cons.62 107 (call cons.62 481 empty)))
                 (boolean1.8 #t))
             (void))))
      (if #f 61 215))))
(check-by-interp
...
(check-by-interp
 '(module
    (define error?.69 (lambda (tmp.58) (error? tmp.58)))
    (define <.68
    ...
    (call
     ...
     (if (call <.68 248 54)
       (let ((g42302032.11 (void)))
         (if (call error?.69 g42302032.11)
           g42302032.11
           (let ((g42302033.12 (void)))
             (if (call error?.69 g42302033.12)
               g42302033.12
               (let ((g42302034.13 (void)))
                 (if (call error?.69 g42302034.13)
                   g42302034.13
                   (let ((g42302035.14 (void)))
                     (if (call error?.69 g42302035.14)
                       g42302035.14
                       (void)))))))))
       (let ((error0.15 (error 120))) (void))))))
(check-by-interp
 '(module
    (define cons.90 (lambda (tmp.82 tmp.83) (cons tmp.82 tmp.83)))
    (define error?.89 (lambda (tmp.77) (error? tmp.77)))
    (define unsafe-vector-set!.5
    ...
    (define vector-set!.88
      (lambda (tmp.65 tmp.66 tmp.67)
        (if (fixnum? tmp.66)
          (if (vector? tmp.65)
            (call unsafe-vector-set!.5 tmp.65 tmp.66 tmp.67)
            (error 10))
          (error 10))))
    (define vector-init-loop.37
      (lambda (len.38 i.40 vec.39)
        (if (eq? len.38 i.40)
          vec.39
          (begin
            (unsafe-vector-set! vec.39 i.40 0)
            (call vector-init-loop.37 len.38 (unsafe-fx+ i.40 1) vec.39)))))
    (define make-init-vector.4
      (lambda (tmp.35)
        (let ((tmp.36 (unsafe-make-vector tmp.35)))
          (call vector-init-loop.37 tmp.35 0 tmp.36))))
    (define make-vector.87
      (lambda (tmp.63)
        (if (fixnum? tmp.63) (call make-init-vector.4 tmp.63) (error 8))))
    ...
    (define fun/void8488.10
      (lambda (oprand0.13)
        (call
         ...
         oprand0.13
         3
         (let ((tmp.7.14 (call make-vector.87 8)))
           (let ((g42305850.15 (call vector-set!.88 tmp.7.14 0 0)))
             (if (call error?.89 g42305850.15)
               g42305850.15
               (let ((g42305851.16 (call vector-set!.88 tmp.7.14 1 1)))
                 (if (call error?.89 g42305851.16)
                   g42305851.16
                   (let ((g42305852.17 (call vector-set!.88 tmp.7.14 2 2)))
                     (if (call error?.89 g42305852.17)
                       g42305852.17
                       (let ((g42305853.18 (call vector-set!.88 tmp.7.14 3 3)))
                         (if (call error?.89 g42305853.18)
                           g42305853.18
                           (let ((g42305854.19
                                  (call vector-set!.88 tmp.7.14 4 4)))
                             (if (call error?.89 g42305854.19)
                               g42305854.19
                               (let ((g42305855.20
                                      (call vector-set!.88 tmp.7.14 5 5)))
                                 (if (call error?.89 g42305855.20)
                                   g42305855.20
                                   (let ((g42305856.21
                                          (call vector-set!.88 tmp.7.14 6 6)))
                                     (if (call error?.89 g42305856.21)
                                       g42305856.21
                                       (let ((g42305857.22
                                              (call
                                               vector-set!.88
                                               ...
                                               7
                                               7)))
                                         (if (call error?.89 g42305857.22)
                                           g42305857.22
                                           tmp.7.14))))))))))))))))))))
    (let ()
      (if (let ((ascii-char0.25 #\j)
                (empty1.24 empty)
                (pair2.23 (call cons.90 146 (call cons.90 308 empty))))
            (void))
        (if (call fun/void8487.9 148 (void))
          (if (call
               fun/void8488.10
               (let ((tmp.8.26 (call make-vector.87 8)))
                 (let ((g42305858.27 (call vector-set!.88 tmp.8.26 0 1)))
                   (if (call error?.89 g42305858.27)
                     g42305858.27
                     (let ((g42305859.28 (call vector-set!.88 tmp.8.26 1 2)))
                       (if (call error?.89 g42305859.28)
                         g42305859.28
                         (let ((g42305860.29
                                (call vector-set!.88 tmp.8.26 2 3)))
                           (if (call error?.89 g42305860.29)
                             g42305860.29
                             (let ((g42305861.30
                                    (call vector-set!.88 tmp.8.26 3 4)))
                               (if (call error?.89 g42305861.30)
                                 g42305861.30
                                 (let ((g42305862.31
                                        (call vector-set!.88 tmp.8.26 4 5)))
                                   (if (call error?.89 g42305862.31)
                                     g42305862.31
                                     (let ((g42305863.32
                                            (call
                                             vector-set!.88
...
(check-by-interp
 '(module
    (define error?.142 (lambda (tmp.129) (error? tmp.129)))
    (define vector-init-loop.89
      (lambda (len.90 i.92 vec.91)
        (if (eq? len.90 i.92)
          vec.91
          (begin
            (unsafe-vector-set! vec.91 i.92 0)
            (call vector-init-loop.89 len.90 (unsafe-fx+ i.92 1) vec.91)))))
    (define make-init-vector.4
      (lambda (tmp.87)
        (let ((tmp.88 (unsafe-make-vector tmp.87)))
          (call vector-init-loop.89 tmp.87 0 tmp.88))))
    (define make-vector.141
      (lambda (tmp.115)
        (if (fixnum? tmp.115) (call make-init-vector.4 tmp.115) (error 8))))
    (define unsafe-vector-set!.5
      (lambda (tmp.93 tmp.94 tmp.95)
        (if (unsafe-fx< tmp.94 (unsafe-vector-length tmp.93))
          (if (unsafe-fx>= tmp.94 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.140
      (lambda (tmp.117 tmp.118 tmp.119)
        (if (fixnum? tmp.118)
          (if (vector? tmp.117)
            (call unsafe-vector-set!.5 tmp.117 tmp.118 tmp.119)
            (error 10))
          (error 10))))
    (define cons.139 (lambda (tmp.134 tmp.135) (cons tmp.134 tmp.135)))
    (define fun/pair8504.16
      (lambda (oprand0.26 oprand1.25)
        (call cons.139 175 (call cons.139 296 empty))))
    (define fun/void8499.17 (lambda (oprand0.27) (void)))
    (define fun/void8501.18
      (lambda (oprand0.29 oprand1.28) (call vector-set!.140 oprand0.29 5 #\I)))
    (define fun/void8498.19 (lambda () (void)))
    (define fun/fixnum8502.20 (lambda (oprand0.30) (let () 229)))
    (define fun/pair8503.21
      (lambda () (call cons.139 187 (call cons.139 408 empty))))
    ...
    (call
     ...
     (if (call
          ...
          (if #f 182 177)
          (if #f
            (call cons.139 128 (call cons.139 289 empty))
            (call cons.139 177 (call cons.139 385 empty))))
       (if (if #f (void) (void))
         (if (let ((procedure0.38
                    (lambda () (call cons.139 6 (call cons.139 507 empty))))
                   (ascii-char1.37 #\M)
                   (fixnum2.36 199))
               (void))
           (if (if #f (void) (void))
             (if (let ((empty0.40 empty)
                       (procedure1.39
                        (lambda ()
                          (let ((tmp.7.41 (call make-vector.141 8)))
                            (let ((g42328608.42
                                   (call vector-set!.140 tmp.7.41 0 1)))
                              (if (call error?.142 g42328608.42)
                                g42328608.42
                                (let ((g42328609.43
                                       (call vector-set!.140 tmp.7.41 1 2)))
                                  (if (call error?.142 g42328609.43)
                                    g42328609.43
                                    (let ((g42328610.44
                                           (call
                                            vector-set!.140
                                            ...
                                            2
                                            3)))
                                      (if (call error?.142 g42328610.44)
                                        g42328610.44
                                        (let ((g42328611.45
                                               (call
                                                vector-set!.140
                                                ...
                                                3
                                                4)))
                                          (if (call error?.142 g42328611.45)
                                            g42328611.45
                                            (let ((g42328612.46
                                                   (call
                                                    vector-set!.140
                                        ...
                                        (let ((tmp.14.59
                                          ...
                                          (let ((g42328616.60
                                                 (call
                                                  ...
                                                  0
                                                  1)))
                                            (if (call error?.142 g42328616.60)
                                              g42328616.60
                                              (let ((g42328617.61
                                                     (call
                                                      vector-set!.140
     ...
     (call
      fun/fixnum8502.20
      (let ((g42328624.68 (call cons.139 234 (call cons.139 272 empty))))
        (if (call error?.142 g42328624.68)
          g42328624.68
          (let ((g42328625.69
                 (let ((error0.72 (error 104))
                       (procedure1.71
                        (lambda ()
                          (let ((tmp.15.73 (call make-vector.141 8)))
                            (let ((g42328626.74
                                   (call vector-set!.140 tmp.15.73 0 0)))
                              (if (call error?.142 g42328626.74)
                                g42328626.74
                                (let ((g42328627.75
                                       (call vector-set!.140 tmp.15.73 1 1)))
                                  (if (call error?.142 g42328627.75)
                                    g42328627.75
                                    (let ((g42328628.76
                                           (call
                                            vector-set!.140
                                            ...
                                            2
                                            2)))
                                      (if (call error?.142 g42328628.76)
                                        g42328628.76
                                        (let ((g42328629.77
                                               (call
                                                vector-set!.140
                                                ...
                                                3
                                                3)))
                                          (if (call error?.142 g42328629.77)
                                            g42328629.77
                                            (let ((g42328630.78
                                                   (call
                                                    vector-set!.140
                                                      ...
                                                      (if (call
                                                        ...
                                                        (let ((g42328633.81
                                                          ...
                                                          (if (call
                                                            ...
                                                            tmp.15.73)))))))))))))))))))
                       (ascii-char2.70 #\M))
                   (call cons.139 43 (call cons.139 459 empty)))))
            (if (call error?.142 g42328625.69)
              g42328625.69
              (let ((g42328634.82
                     (call cons.139 92 (call cons.139 487 empty))))
                (if (call error?.142 g42328634.82)
                  g42328634.82
                  (let ((g42328635.83
                         (let ((g42328636.84
                                (call cons.139 23 (call cons.139 399 empty))))
                           (if (call error?.142 g42328636.84)
                             g42328636.84
                             (call cons.139 163 (call cons.139 314 empty))))))
                    (if (call error?.142 g42328635.83)
                      g42328635.83
                      (let ((g42328637.85 (call fun/pair8503.21)))
                        (if (call error?.142 g42328637.85)
                          g42328637.85
                          (let ((g42328638.86
                                 (if #t
                                   (call cons.139 59 (call cons.139 331 empty))
                                   (call
                                    cons.139
                                    18
                                    (call cons.139 310 empty)))))
                            (if (call error?.142 g42328638.86)
                              g42328638.86
                              (call
                               fun/pair8504.16
                               (void)
...
(check-by-interp
 '(module
    (define error?.95 (lambda (tmp.81) (error? tmp.81)))
    (define unsafe-vector-set!.5
    ...
    (define <.92
      (lambda (tmp.59 tmp.60)
        (if (fixnum? tmp.60)
          ...
          (error 4))))
    (define cons.91 (lambda (tmp.86 tmp.87) (cons tmp.86 tmp.87)))
    ...
    (call
     ...
     (if (if (call
              fun/empty8510.8
              (error 189)
              (call cons.91 54 (call cons.91 464 empty)))
     ...
     (if (call <.92 93 14)
       (let ((boolean0.27 #t)
             (vector1.26
              (let ((tmp.7.28 (call make-vector.93 8)))
                (let ((g42336237.29 (call vector-set!.94 tmp.7.28 0 1)))
                  (if (call error?.95 g42336237.29)
                    g42336237.29
                    (let ((g42336238.30 (call vector-set!.94 tmp.7.28 1 2)))
                      (if (call error?.95 g42336238.30)
                        g42336238.30
                        (let ((g42336239.31
                               (call vector-set!.94 tmp.7.28 2 3)))
                          (if (call error?.95 g42336239.31)
                            g42336239.31
                            (let ((g42336240.32
                                   (call vector-set!.94 tmp.7.28 3 4)))
                              (if (call error?.95 g42336240.32)
                                g42336240.32
                                (let ((g42336241.33
                                       (call vector-set!.94 tmp.7.28 4 5)))
                                  (if (call error?.95 g42336241.33)
                                    g42336241.33
                                    (let ((g42336242.34
                                           (call vector-set!.94 tmp.7.28 5 6)))
                                      (if (call error?.95 g42336242.34)
                                        g42336242.34
                                        (let ((g42336243.35
                                               (call
                                                vector-set!.94
...
(check-by-interp
 '(module
    ...
    (if (call fun/boolean8517.8 (lambda () 918))
      #\L
      (let ((vector0.12
             (let ((tmp.7.13 (call make-vector.74 8)))
               (let ((g42343841.14 (call vector-set!.75 tmp.7.13 0 1)))
                 (if (call error?.76 g42343841.14)
                   g42343841.14
                   (let ((g42343842.15 (call vector-set!.75 tmp.7.13 1 2)))
                     (if (call error?.76 g42343842.15)
                       g42343842.15
                       (let ((g42343843.16 (call vector-set!.75 tmp.7.13 2 3)))
                         (if (call error?.76 g42343843.16)
                           g42343843.16
                           (let ((g42343844.17
                                  (call vector-set!.75 tmp.7.13 3 4)))
                             (if (call error?.76 g42343844.17)
                               g42343844.17
                               (let ((g42343845.18
                                      (call vector-set!.75 tmp.7.13 4 5)))
                                 (if (call error?.76 g42343845.18)
                                   g42343845.18
                                   (let ((g42343846.19
                                          (call vector-set!.75 tmp.7.13 5 6)))
                                     (if (call error?.76 g42343846.19)
                                       g42343846.19
                                       (let ((g42343847.20
                                              (call
                                               vector-set!.75
    ...
    (define *.80
      (lambda (tmp.38 tmp.39)
        (if (fixnum? tmp.39)
          ...
          (error 1))))
    (define cons.79 (lambda (tmp.71 tmp.72) (cons tmp.71 tmp.72)))
    (define error?.78 (lambda (tmp.66) (error? tmp.66)))
    (define unsafe-vector-set!.5
      (lambda (tmp.30 tmp.31 tmp.32)
        (if (unsafe-fx< tmp.31 (unsafe-vector-length tmp.30))
      ...
      (call
       ...
       (let ()
         (let ((tmp.7.13 (call make-vector.76 8)))
           (let ((g42347666.14 (call vector-set!.77 tmp.7.13 0 1)))
             (if (call error?.78 g42347666.14)
               g42347666.14
               (let ((g42347667.15 (call vector-set!.77 tmp.7.13 1 2)))
                 (if (call error?.78 g42347667.15)
                   g42347667.15
                   (let ((g42347668.16 (call vector-set!.77 tmp.7.13 2 3)))
                     (if (call error?.78 g42347668.16)
                       g42347668.16
                       (let ((g42347669.17 (call vector-set!.77 tmp.7.13 3 4)))
                         (if (call error?.78 g42347669.17)
                           g42347669.17
                           (let ((g42347670.18
                                  (call vector-set!.77 tmp.7.13 4 5)))
                             (if (call error?.78 g42347670.18)
                               g42347670.18
                               (let ((g42347671.19
                                      (call vector-set!.77 tmp.7.13 5 6)))
                                 (if (call error?.78 g42347671.19)
                                   g42347671.19
                                   (let ((g42347672.20
                                          (call vector-set!.77 tmp.7.13 6 7)))
                                     (if (call error?.78 g42347672.20)
                                       g42347672.20
                                       (let ((g42347673.21
                                              (call
                                               vector-set!.77
                                               ...
                                               7
                                               8)))
                                         (if (call error?.78 g42347673.21)
                                           g42347673.21
                                           tmp.7.13))))))))))))))))))
       (if #f
         (call cons.79 25 (call cons.79 278 empty))
         (call cons.79 168 (call cons.79 487 empty)))))
     (if (let () #f)
       (let ((g42347674.22 232))
         (if (call error?.78 g42347674.22)
           g42347674.22
           (let ((g42347675.23 165))
             (if (call error?.78 g42347675.23) g42347675.23 174))))
       (if #f 106 28)))))
(check-by-interp
 '(module
 ...
 '(module
    ...
    (call
     ...
     (let ((ascii-char0.16 (call fun/ascii-char8535.12))
       ...
       (if #f
         (let ((tmp.7.17 (call make-vector.87 8)))
           (let ((g42366676.18 (call vector-set!.88 tmp.7.17 0 1)))
             (if (call error?.89 g42366676.18)
               g42366676.18
               (let ((g42366677.19 (call vector-set!.88 tmp.7.17 1 2)))
                 (if (call error?.89 g42366677.19)
                   g42366677.19
                   (let ((g42366678.20 (call vector-set!.88 tmp.7.17 2 3)))
                     (if (call error?.89 g42366678.20)
                       g42366678.20
                       (let ((g42366679.21 (call vector-set!.88 tmp.7.17 3 4)))
                         (if (call error?.89 g42366679.21)
                           g42366679.21
                           (let ((g42366680.22
                                  (call vector-set!.88 tmp.7.17 4 5)))
                             (if (call error?.89 g42366680.22)
                               g42366680.22
                               (let ((g42366681.23
                                      (call vector-set!.88 tmp.7.17 5 6)))
                                 (if (call error?.89 g42366681.23)
                                   g42366681.23
                                   (let ((g42366682.24
                                          (call vector-set!.88 tmp.7.17 6 7)))
                                     (if (call error?.89 g42366682.24)
                                       g42366682.24
                                       (let ((g42366683.25
                                              (call
                                               vector-set!.88
                                               ...
                                               7
                                               8)))
                                         (if (call error?.89 g42366683.25)
                                           g42366683.25
                                           tmp.7.17)))))))))))))))))
         (let ((tmp.8.26 (call make-vector.87 8)))
           (let ((g42366684.27 (call vector-set!.88 tmp.8.26 0 1)))
             (if (call error?.89 g42366684.27)
               g42366684.27
               (let ((g42366685.28 (call vector-set!.88 tmp.8.26 1 2)))
                 (if (call error?.89 g42366685.28)
                   g42366685.28
                   (let ((g42366686.29 (call vector-set!.88 tmp.8.26 2 3)))
                     (if (call error?.89 g42366686.29)
                       g42366686.29
                       (let ((g42366687.30 (call vector-set!.88 tmp.8.26 3 4)))
                         (if (call error?.89 g42366687.30)
                           g42366687.30
                           (let ((g42366688.31
                                  (call vector-set!.88 tmp.8.26 4 5)))
                             (if (call error?.89 g42366688.31)
                               g42366688.31
                               (let ((g42366689.32
                                      (call vector-set!.88 tmp.8.26 5 6)))
                                 (if (call error?.89 g42366689.32)
                                   g42366689.32
                                   (let ((g42366690.33
                                          (call vector-set!.88 tmp.8.26 6 7)))
                                     (if (call error?.89 g42366690.33)
                                       g42366690.33
                                       (let ((g42366691.34
                                              (call
                                               vector-set!.88
                                               ...
                                               7
                                               8)))
                                         (if (call error?.89 g42366691.34)
                                           g42366691.34
                                           tmp.8.26))))))))))))))))))))))
(check-by-interp
 '(module
    (define cons.221 (lambda (tmp.212 tmp.213) (cons tmp.212 tmp.213)))
    (define unsafe-vector-set!.5
      (lambda (tmp.171 tmp.172 tmp.173)
        (if (unsafe-fx< tmp.172 (unsafe-vector-length tmp.171))
          (if (unsafe-fx>= tmp.172 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.220
      (lambda (tmp.195 tmp.196 tmp.197)
        (if (fixnum? tmp.196)
          (if (vector? tmp.195)
            (call unsafe-vector-set!.5 tmp.195 tmp.196 tmp.197)
            (error 10))
          (error 10))))
    (define vector-init-loop.167
    ...
    (define make-init-vector.4
      (lambda (tmp.165)
        (let ((tmp.166 (unsafe-make-vector tmp.165)))
          (call vector-init-loop.167 tmp.165 0 tmp.166))))
    (define make-vector.219
      (lambda (tmp.193)
        (if (fixnum? tmp.193) (call make-init-vector.4 tmp.193) (error 8))))
    (define error?.218 (lambda (tmp.207) (error? tmp.207)))
    (define >=.217
    ...
    (let ((g42374287.48
           (if (call >=.217 2 186)
             ...
             (let ((procedure0.49 (lambda (oprand0.51 oprand1.50) #\_)))
               empty))))
      (if (call error?.218 g42374287.48)
        g42374287.48
        (let ((g42374288.52
               (let ((fixnum0.55
                      (let ((tmp.7.56 131))
                     ...
                     (vector1.54
                      (let ((tmp.10.59 (call make-vector.219 8)))
                        (let ((g42374289.60
                               (call
                                vector-set!.220
                                tmp.10.59
                                0
                                (call (call make-vector.219 8)))))
                          (if (call error?.218 g42374289.60)
                            g42374289.60
                            (let ((g42374290.61
                                   (call
                                    vector-set!.220
                                    ...
                                    1
                                    (call #t))))
                              (if (call error?.218 g42374290.61)
                                g42374290.61
                                (let ((g42374291.62
                                       (call
                                        vector-set!.220
                                        ...
                                        2
                                        (call #t))))
                                  (if (call error?.218 g42374291.62)
                                    g42374291.62
                                    (let ((g42374292.63
                                           (call
                                            vector-set!.220
                                            ...
                                            3
                                            (call empty))))
                                      (if (call error?.218 g42374292.63)
                                        g42374292.63
                                        (let ((g42374293.64
                                               (call
                                                vector-set!.220
                                                ...
                                                4
                                                (call #\_))))
                                          (if (call error?.218 g42374293.64)
                                            g42374293.64
                                            (let ((g42374294.65
                                                   (call
                                                    vector-set!.220
                 ...
                 (if empty
                   (if empty (if empty (if empty empty #f) #f) #f)
                   #f))))
          (if (call error?.218 g42374288.52)
            g42374288.52
            (let ((g42374297.71
              ...
              (if (call error?.218 g42374297.71)
                g42374297.71
                (let ((g42374298.75
                       (if (if #f empty empty)
                         (if (let ((boolean0.77 #t)
                                   (pair1.76
                                    (call
                                     cons.221
                                     42
                                     (call cons.221 383 empty))))
                               empty)
                           (if (if #t empty empty)
                             (if (let () empty)
                               (if (let ((vector0.80
                                          (let ((tmp.14.81
                                                 (call make-vector.219 8)))
                                            (let ((g42374299.82
                                                   (call
                                                    vector-set!.220
                                                    tmp.14.81
                           ...
                           #f)
                         #f)))
                  (if (call error?.218 g42374298.75)
                    g42374298.75
                    (let ((g42374307.90
                           (let ((g42374308.91 (if #f empty empty)))
                             (if (call error?.218 g42374308.91)
                               g42374308.91
                               (let ((g42374309.92
                                      (let ((tmp.15.93 empty))
                                        (if tmp.15.93
                                                ...
                                                (if tmp.17.95
                                                  ...
                                                  (let ((tmp.18.96 empty))
                                                    (if tmp.18.96
                                                      tmp.18.96
                                                      empty))))))))))
                                 (if (call error?.218 g42374309.92)
                                   g42374309.92
                                   (let ((g42374310.97
                                          (let ((boolean0.98 #f)) empty)))
                                     (if (call error?.218 g42374310.97)
                                       g42374310.97
                                       (let ((g42374311.99
                                              (let ((pair0.102
                                                     (call
                                                    ...
                                                    (pair2.100
                                                     (call
                                                      ...
                                                      (call
                                                       ...
                                                       empty))))
                                                empty)))
                                         (if (call error?.218 g42374311.99)
                                           g42374311.99
                                           (let ((g42374312.103
                                                  (call
                                                   fun/empty8542.34
                                             ...
                                             (if (call
                                               ...
                                               g42374312.103
                                               (if empty empty #f)))))))))))))
                      (if (call error?.218 g42374307.90)
                        g42374307.90
                        (let ((g42374313.104
                               (let ((vector0.107
                                      (let ((tmp.19.108
                                        ...
                                        (let ((g42374314.109
                                               (call
                                                ...
                                                0
                                                0)))
                                          (if (call error?.218 g42374314.109)
                                            g42374314.109
                                            (let ((g42374315.110
                                                   (call
                                                    vector-set!.220
                                     ...
                                     (pair1.106
                                      (call
                                       cons.221
                                       168
                                       (call cons.221 348 empty)))
                                     (error2.105 (error 250)))
                                 empty)))
                          (if (call error?.218 g42374313.104)
                            g42374313.104
                            (let ((g42374322.117
                                   (let ((g42374323.118
                                          (let ((g42374324.119 empty))
                                            (if (call error?.218 g42374324.119)
                                              g42374324.119
                                              empty))))
                                     (if (call error?.218 g42374323.118)
                                       g42374323.118
                                       (let ((g42374325.120
                                              (if #f empty empty)))
                                         (if (call error?.218 g42374325.120)
                                           g42374325.120
                                           (let ((g42374326.121
                                                  (call
                                                   fun/empty8543.42
                                                   ...
                                                   (let ((g42374328.127
                                                     ...
                                                     (if (call
                                                       ...
                                                       (if #f
                                                         empty
                                                         empty)))))))))))))
                              (if (call error?.218 g42374322.117)
                                g42374322.117
                                (let ((g42374329.128 (call fun/empty8544.35)))
                                  (if (call error?.218 g42374329.128)
                                    g42374329.128
                                    (let ((g42374330.129
                                           (let ((tmp.24.130
                                                               ...
                                                               (let ((fixnum0.164
                                                                     ...
                                                                     (error1.163
                                                                      (error
                                                                       182)))
                                                                 empty)))))))))))))
                                      (if (call error?.218 g42374330.129)
                                        g42374330.129
                                        (call
                                         fun/empty8547.38)))))))))))))))))))))
(check-by-interp
 '(module
    (define error?.73 (lambda (tmp.62) (error? tmp.62)))
    ...
    (if (call
         fun/boolean8550.8
         (if #t #\C #\o)
         (let ((g42378163.14 (call cons.72 107 (call cons.72 348 empty))))
           (if (call error?.73 g42378163.14)
             g42378163.14
             (let ((g42378164.15 (call cons.72 203 (call cons.72 496 empty))))
               (if (call error?.73 g42378164.15)
                 g42378164.15
                 (let ((g42378165.16
                        (call cons.72 79 (call cons.72 267 empty))))
                   (if (call error?.73 g42378165.16)
                     g42378165.16
                     (let ((g42378166.17
                            (call cons.72 239 (call cons.72 409 empty))))
                       (if (call error?.73 g42378166.17)
                         g42378166.17
                         (let ((g42378167.18
                                (call cons.72 77 (call cons.72 484 empty))))
                           (if (call error?.73 g42378167.18)
                             g42378167.18
                             (let ((g42378168.19
                                    (call
                                     cons.72
                                     220
                                     (call cons.72 290 empty))))
                               (if (call error?.73 g42378168.19)
                                 g42378168.19
                                 (call
                                  cons.72
                                  89
                                  (call cons.72 469 empty)))))))))))))))
      (call fun/fixnum8552.7)
      (let () 134))))
(check-by-interp
 '(module
    (define error?.68 (lambda (tmp.57) (error? tmp.57)))
    (define cons.67 (lambda (tmp.62 tmp.63) (cons tmp.62 tmp.63)))
    (define fun/fixnum8556.7 (lambda (oprand0.9 oprand1.8) (if #t 71 190)))
    (if (let ((pair0.12 (call cons.67 177 (call cons.67 360 empty)))
              (empty1.11 empty)
              (boolean2.10 #f))
          #t)
      (let ((pair0.13 (call cons.67 210 (call cons.67 266 empty)))) 198)
      (call
       fun/fixnum8556.7
       (if #t empty empty)
       (let ((g42381986.14 (call cons.67 103 (call cons.67 399 empty))))
         (if (call error?.68 g42381986.14)
           g42381986.14
           (call cons.67 19 (call cons.67 508 empty))))))))
(check-by-interp
 '(module
    (define error?.86 (lambda (tmp.73) (error? tmp.73)))
    (define unsafe-vector-set!.5
      (lambda (tmp.37 tmp.38 tmp.39)
        (if (unsafe-fx< tmp.38 (unsafe-vector-length tmp.37))
          (if (unsafe-fx>= tmp.38 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.85
      (lambda (tmp.61 tmp.62 tmp.63)
        (if (fixnum? tmp.62)
          (if (vector? tmp.61)
            (call unsafe-vector-set!.5 tmp.61 tmp.62 tmp.63)
            (error 10))
          (error 10))))
      ...
      (lambda (len.34 i.36 vec.35)
        (if (eq? len.34 i.36)
          ...
          (begin
            ...
            (call vector-init-loop.33 len.34 (unsafe-fx+ i.36 1) vec.35)))))
    (define make-init-vector.4
      (lambda (tmp.31)
        (let ((tmp.32 (unsafe-make-vector tmp.31)))
          (call vector-init-loop.33 tmp.31 0 tmp.32))))
    (define make-vector.84
      (lambda (tmp.59)
        (if (fixnum? tmp.59) (call make-init-vector.4 tmp.59) (error 8))))
    (define cons.83 (lambda (tmp.78 tmp.79) (cons tmp.78 tmp.79)))
    (define fun/pair8560.12
      (lambda (oprand0.14) (call cons.83 5 (call cons.83 421 empty))))
    (define fun/procedure8559.13 (lambda () (lambda () 550)))
    (let ((vector0.15
           (let ((tmp.7.16 (call make-vector.84 8)))
             (let ((g42385801.17
                    (call vector-set!.85 tmp.7.16 0 (call (let () (void))))))
               (if (call error?.86 g42385801.17)
                 g42385801.17
                 (let ((g42385802.18
                        (call vector-set!.85 tmp.7.16 1 (call (let () 84)))))
                   (if (call error?.86 g42385802.18)
                     g42385802.18
                     (let ((g42385803.19
                            (call
                             vector-set!.85
                             ...
                             (call
                              (let ((pair0.21
                                     (call
                                      cons.83
                                      115
                                      (call cons.83 428 empty)))
                                    (empty1.20 empty))
                                (void))))))
                       (if (call error?.86 g42385803.19)
                         g42385803.19
                         (let ((g42385804.22
                                (call
                                 vector-set!.85
                                     ...
                                     (let ((g42385807.29
                                       ...
                                       (if (call error?.86 g42385807.29)
                                         ...
                                         (let ((g42385808.30
                                           ...
                                           (if (call error?.86 g42385808.30)
                                             g42385808.30
                                             tmp.7.16)))))))))))))))))))
      (if #f #\J #\`))))
(check-by-interp
 '(module
    (define ascii-char?.102 (lambda (tmp.88) (ascii-char? tmp.88)))
    (define error?.101 (lambda (tmp.89) (error? tmp.89)))
    (define unsafe-vector-set!.5
      (lambda (tmp.53 tmp.54 tmp.55)
        (if (unsafe-fx< tmp.54 (unsafe-vector-length tmp.53))
          (if (unsafe-fx>= tmp.54 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.100
      (lambda (tmp.77 tmp.78 tmp.79)
        (if (fixnum? tmp.78)
          (if (vector? tmp.77)
            (call unsafe-vector-set!.5 tmp.77 tmp.78 tmp.79)
            (error 10))
          (error 10))))
      ...
      (lambda (len.50 i.52 vec.51)
        (if (eq? len.50 i.52)
          ...
          (begin
            ...
            (call vector-init-loop.49 len.50 (unsafe-fx+ i.52 1) vec.51)))))
    (define make-init-vector.4
      (lambda (tmp.47)
        (let ((tmp.48 (unsafe-make-vector tmp.47)))
          (call vector-init-loop.49 tmp.47 0 tmp.48))))
    (define make-vector.99
      (lambda (tmp.75)
        (if (fixnum? tmp.75) (call make-init-vector.4 tmp.75) (error 8))))
    (define fun/error7326.14 (lambda () (if #f (error 41) (error 41))))
    (define fun/any7325.15
      (lambda ()
        (let ((tmp.7.17 (call make-vector.99 8)))
          (let ((g42409226.18 (call vector-set!.100 tmp.7.17 0 1)))
            (if (call error?.101 g42409226.18)
              g42409226.18
              (let ((g42409227.19 (call vector-set!.100 tmp.7.17 1 2)))
                (if (call error?.101 g42409227.19)
                  g42409227.19
                  (let ((g42409228.20 (call vector-set!.100 tmp.7.17 2 3)))
                    (if (call error?.101 g42409228.20)
                      g42409228.20
                      (let ((g42409229.21 (call vector-set!.100 tmp.7.17 3 4)))
                        (if (call error?.101 g42409229.21)
                          g42409229.21
                          (let ((g42409230.22
                                 (call vector-set!.100 tmp.7.17 4 5)))
                            (if (call error?.101 g42409230.22)
                              g42409230.22
                              (let ((g42409231.23
                                     (call vector-set!.100 tmp.7.17 5 6)))
                                (if (call error?.101 g42409231.23)
                                  g42409231.23
                                  (let ((g42409232.24
                                         (call vector-set!.100 tmp.7.17 6 7)))
                                    (if (call error?.101 g42409232.24)
                                      g42409232.24
                                      (let ((g42409233.25
                                             (call
                                              vector-set!.100
                                              ...
                                              7
                                              8)))
                                        (if (call error?.101 g42409233.25)
                                          g42409233.25
                                          tmp.7.17)))))))))))))))))))
    (define fun/error7327.16 (lambda (oprand0.27 oprand1.26) (error 150)))
    (if (call ascii-char?.102 (call fun/any7325.15))
        ...
        (if tmp.8.28
          ...
          (let ((tmp.9.29
                 (let ((g42409234.30 (error 136)))
                   (if (call error?.101 g42409234.30)
                     g42409234.30
                     (let ((g42409235.31 (error 134)))
                       (if (call error?.101 g42409235.31)
                         g42409235.31
                         (let ((g42409236.32 (error 216)))
                           (if (call error?.101 g42409236.32)
                             g42409236.32
                             (error 132)))))))))
            (if tmp.9.29
              tmp.9.29
                ...
                (if tmp.10.33
                  ...
                  (let ((tmp.11.34
                         (call
                          fun/error7327.16
                          (void)
                          (let ((tmp.12.35 (call make-vector.99 8)))
                            (let ((g42409237.36
                                   (call vector-set!.100 tmp.12.35 0 1)))
                              (if (call error?.101 g42409237.36)
                                g42409237.36
                                (let ((g42409238.37
                                       (call vector-set!.100 tmp.12.35 1 2)))
                                  (if (call error?.101 g42409238.37)
                                    g42409238.37
                                    (let ((g42409239.38
                                           (call
                                            vector-set!.100
                                            ...
                                            2
                                            3)))
                                      (if (call error?.101 g42409239.38)
                                        g42409239.38
                                        (let ((g42409240.39
                                               (call
                                                vector-set!.100
                                                ...
                                                3
                                                4)))
                                          (if (call error?.101 g42409240.39)
                                            g42409240.39
                                            (let ((g42409241.40
                                                   (call
                                                    vector-set!.100
                      ...
                      (let ((tmp.13.44 (let ((fixnum0.45 172)) (error 184))))
                        (if tmp.13.44
                          ...
                          (let ((g42409245.46 (error 89)))
                            (if (call error?.101 g42409245.46)
                              g42409245.46
                              (error 219))))))))))))))))
(check-by-interp
 '(module
          ...
          (if tmp.7.11
            ...
            (let ((tmp.8.12 (let () #t)))
              (if tmp.8.12
                ...
                (let ((tmp.9.13
                       (let ((g42413066.14 #t))
                         (if (call error?.72 g42413066.14)
                           g42413066.14
                           (let ((g42413067.15 #t))
                             (if (call error?.72 g42413067.15)
                               g42413067.15
                               (let ((g42413068.16 #f))
                                 (if (call error?.72 g42413068.16)
                                   g42413068.16
                                   (let ((g42413069.17 #t))
                                     (if (call error?.72 g42413069.17)
                                       g42413069.17
                                       (let ((g42413070.18 #f))
                                         (if (call error?.72 g42413070.18)
                                           g42413070.18
                                           (let ((g42413071.19 #f))
                                             (if (call error?.72 g42413071.19)
                                               g42413071.19
                                               #t))))))))))))))
                  (if tmp.9.13
                    tmp.9.13
...
(check-by-interp
 '(module
    (define ascii-char?.60 (lambda (tmp.49) (ascii-char? tmp.49)))
    (let ((boolean0.7 (call ascii-char?.60 (if #f #\l 229))))
      (let () (void)))))
(check-by-interp
 '(module
...
(check-by-interp
 '(module
    (define empty?.77 (lambda (tmp.60) (empty? tmp.60)))
    (define error?.76 (lambda (tmp.63) (error? tmp.63)))
    (define unsafe-vector-set!.5
    ...
    (define fun/pair8489.8
      (lambda () (call cons.73 183 (call cons.73 488 empty))))
    (define fun/pair8488.9
      (lambda () (call cons.73 88 (call cons.73 319 empty))))
    (let ((ascii-char0.11 (if #t #\T #\Q))
          (vector1.10
           (let ((tmp.7.12 (call make-vector.74 8)))
             (let ((g42424517.13
                    (call vector-set!.75 tmp.7.12 0 (call (let () empty)))))
               (if (call error?.76 g42424517.13)
                 g42424517.13
                 (let ((g42424518.14
                        (call
                         vector-set!.75
                         tmp.7.12
                         1
                         (call (call error?.76 (lambda () 656))))))
                   (if (call error?.76 g42424518.14)
                     g42424518.14
                     (let ((g42424519.15
                            (call
                             vector-set!.75
                           ...
                           (if (call error?.76 g42424520.16)
                             ...
                             (let ((g42424521.17
                                    (call
                                     ...
                                     tmp.7.12
                                     4
                                     (call (call empty?.77 (void))))))
                               (if (call error?.76 g42424521.17)
                                 g42424521.17
                                 (let ((g42424522.18
                                        (call
                                         vector-set!.75
 ...
 '(module
    ...
    (let ((g42428345.7
           (let ((empty0.10 empty) (fixnum1.9 21) (ascii-char2.8 #\F))
             (void))))
      (if (call error?.64 g42428345.7)
        g42428345.7
        (let ((g42428346.11 (let () (void))))
          (if (call error?.64 g42428346.11)
            g42428346.11
            (if #f (void) (void))))))))
(check-by-interp
 '(module
 ...
 '(module
    ...
    (define vector-set!.85
      (lambda (tmp.62 tmp.63 tmp.64)
        (if (fixnum? tmp.63)
          (if (vector? tmp.62)
            (call unsafe-vector-set!.5 tmp.62 tmp.63 tmp.64)
            (error 10))
          (error 10))))
      ...
      (lambda (len.35 i.37 vec.36)
        (if (eq? len.35 i.37)
          ...
          (begin
            ...
            (call vector-init-loop.34 len.35 (unsafe-fx+ i.37 1) vec.36)))))
    (define make-init-vector.4
      (lambda (tmp.32)
        (let ((tmp.33 (unsafe-make-vector tmp.32)))
          (call vector-init-loop.34 tmp.32 0 tmp.33))))
    (define make-vector.84
      (lambda (tmp.60)
        (if (fixnum? tmp.60) (call make-init-vector.4 tmp.60) (error 8))))
    ...
    (call
     ...
     (if (call
          fun/boolean8510.12
          (let ((tmp.7.17 (call make-vector.84 8)))
            (let ((g42439795.18 (call vector-set!.85 tmp.7.17 0 0)))
              (if (call error?.86 g42439795.18)
                g42439795.18
                (let ((g42439796.19 (call vector-set!.85 tmp.7.17 1 1)))
                  (if (call error?.86 g42439796.19)
                    g42439796.19
                    (let ((g42439797.20 (call vector-set!.85 tmp.7.17 2 2)))
                      (if (call error?.86 g42439797.20)
                        g42439797.20
                        (let ((g42439798.21
                               (call vector-set!.85 tmp.7.17 3 3)))
                          (if (call error?.86 g42439798.21)
                            g42439798.21
                            (let ((g42439799.22
                                   (call vector-set!.85 tmp.7.17 4 4)))
                              (if (call error?.86 g42439799.22)
                                g42439799.22
                                (let ((g42439800.23
                                       (call vector-set!.85 tmp.7.17 5 5)))
                                  (if (call error?.86 g42439800.23)
                                    g42439800.23
                                    (let ((g42439801.24
                                           (call vector-set!.85 tmp.7.17 6 6)))
                                      (if (call error?.86 g42439801.24)
                                        g42439801.24
                                        (let ((g42439802.25
                                               (call
                                                vector-set!.85
...
(check-by-interp
 '(module
    ...
    (let ((boolean0.9 (call <=.63 (let () 15) (let ((void0.10 (void))) 193))))
      (call fun/empty8514.8))))
(check-by-interp
 '(module
    (define error?.104 (lambda (tmp.91) (error? tmp.91)))
    (define unsafe-vector-set!.5
      (lambda (tmp.55 tmp.56 tmp.57)
        (if (unsafe-fx< tmp.56 (unsafe-vector-length tmp.55))
          (if (unsafe-fx>= tmp.56 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.103
      (lambda (tmp.79 tmp.80 tmp.81)
        (if (fixnum? tmp.80)
          (if (vector? tmp.79)
            (call unsafe-vector-set!.5 tmp.79 tmp.80 tmp.81)
            (error 10))
          (error 10))))
    (define vector-init-loop.51
      (lambda (len.52 i.54 vec.53)
        (if (eq? len.52 i.54)
          vec.53
          (begin
            (unsafe-vector-set! vec.53 i.54 0)
            (call vector-init-loop.51 len.52 (unsafe-fx+ i.54 1) vec.53)))))
    (define make-init-vector.4
      (lambda (tmp.49)
        (let ((tmp.50 (unsafe-make-vector tmp.49)))
          (call vector-init-loop.51 tmp.49 0 tmp.50))))
    (define make-vector.102
      (lambda (tmp.77)
        (if (fixnum? tmp.77) (call make-init-vector.4 tmp.77) (error 8))))
    (define cons.101 (lambda (tmp.96 tmp.97) (cons tmp.96 tmp.97)))
              ...
              (pair1.20
               (call
                ...
                (call
                 ...
                 (call
                  cons.101
                  (call cons.101 118 (call cons.101 358 empty))
                  (call
                   cons.101
                   (call cons.101 16 (call cons.101 483 empty))
                   (call
                    cons.101
                    empty
                    (call
                     cons.101
                     #t
                     (call cons.101 #t (call cons.101 #t empty))))))))))
          (call fun/ascii-char8518.9))
      (if (if (call
               fun/ascii-char8519.14
               #\d
               (if #t
                 (call cons.101 212 (call cons.101 434 empty))
                 (call cons.101 140 (call cons.101 331 empty))))
            (if (let () #\T)
              (if (if #f #\_ #\J)
                (let ((procedure0.23 (lambda (oprand0.24) 53))
                      (pair1.22 (call cons.101 151 (call cons.101 317 empty))))
                  #\b)
                #f)
              #f)
            #f)
        (if (let ((boolean0.25
                   (let ((empty0.27 empty)
                         (procedure1.26
                          (lambda ()
                            (let ((tmp.7.28 (call make-vector.102 8)))
                              (let ((g42447437.29
                                     (call vector-set!.103 tmp.7.28 0 0)))
                                (if (call error?.104 g42447437.29)
                                  g42447437.29
                                  (let ((g42447438.30
                                         (call vector-set!.103 tmp.7.28 1 1)))
                                    (if (call error?.104 g42447438.30)
                                      g42447438.30
                                      (let ((g42447439.31
                                             (call
                                              vector-set!.103
                                              ...
                                              2
                                              2)))
                                        (if (call error?.104 g42447439.31)
                                          g42447439.31
                                          (let ((g42447440.32
                                                 (call
                                                  vector-set!.103
                                                  ...
                                                  3
                                                  3)))
                                            (if (call error?.104 g42447440.32)
                                              g42447440.32
                                              (let ((g42447441.33
                                                     (call
                                                      vector-set!.103
          ...
          (if (call fun/ascii-char8521.13)
            (if (if (let () #\k)
                  (if (let ((vector0.37
                             (let ((tmp.8.38 (call make-vector.102 8)))
                               (let ((g42447445.39
                                      (call vector-set!.103 tmp.8.38 0 1)))
                                 (if (call error?.104 g42447445.39)
                                   g42447445.39
                                   (let ((g42447446.40
                                          (call vector-set!.103 tmp.8.38 1 2)))
                                     (if (call error?.104 g42447446.40)
                                       g42447446.40
                                       (let ((g42447447.41
                                              (call
                                               vector-set!.103
                                               ...
                                               2
                                               3)))
                                         (if (call error?.104 g42447447.41)
                                           g42447447.41
                                           (let ((g42447448.42
                                                  (call
                                                   vector-set!.103
                                                   ...
                                                   3
                                                   4)))
                                             (if (call error?.104 g42447448.42)
                                               g42447448.42
                                               (let ((g42447449.43
                                                      (call
                                                       vector-set!.103
...
(check-by-interp
 '(module
    ...
    (define fun/ascii-char8528.8
      (lambda ()
        (let ((pair0.13 (call cons.71 127 (call cons.71 352 empty)))) #\Y)))
...
(check-by-interp
 '(module
    (define procedure?.136 (lambda (tmp.126) (procedure? tmp.126)))
    (define unsafe-vector-set!.5
      (lambda (tmp.86 tmp.87 tmp.88)
        (if (unsafe-fx< tmp.87 (unsafe-vector-length tmp.86))
          (if (unsafe-fx>= tmp.87 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.135
      (lambda (tmp.110 tmp.111 tmp.112)
        (if (fixnum? tmp.111)
          (if (vector? tmp.110)
            (call unsafe-vector-set!.5 tmp.110 tmp.111 tmp.112)
            (error 10))
          (error 10))))
    (define vector-init-loop.82
      (lambda (len.83 i.85 vec.84)
        (if (eq? len.83 i.85)
          vec.84
          (begin
            (unsafe-vector-set! vec.84 i.85 0)
            (call vector-init-loop.82 len.83 (unsafe-fx+ i.85 1) vec.84)))))
    (define make-init-vector.4
      (lambda (tmp.80)
        (let ((tmp.81 (unsafe-make-vector tmp.80)))
          (call vector-init-loop.82 tmp.80 0 tmp.81))))
    (define make-vector.134
      (lambda (tmp.108)
        (if (fixnum? tmp.108) (call make-init-vector.4 tmp.108) (error 8))))
    (define cons.133 (lambda (tmp.127 tmp.128) (cons tmp.127 tmp.128)))
    (define error?.132 (lambda (tmp.122) (error? tmp.122)))
    ...
    (let ((g42455084.37
           (call
            ...
            (let () empty))))
      (if (call error?.132 g42455084.37)
        g42455084.37
        (let ((g42455085.38
               (if (let ((boolean0.40 #f) (empty1.39 empty)) #f)
                 (let ((error0.42 (error 211)) (ascii-char1.41 #\U)) #\b)
                 (let ((g42455086.43 #\f))
                   (if (call error?.132 g42455086.43)
                     g42455086.43
                     (let ((g42455087.44 #\`))
                       (if (call error?.132 g42455087.44)
                         g42455087.44
                         (let ((g42455088.45 #\a))
                           (if (call error?.132 g42455088.45)
                             g42455088.45
                             (let ((g42455089.46 #\O))
                               (if (call error?.132 g42455089.46)
                                 g42455089.46
                                 (let ((g42455090.47 #\`))
                                   (if (call error?.132 g42455090.47)
                                     g42455090.47
                                     (let ((g42455091.48 #\O))
                                       (if (call error?.132 g42455091.48)
                                         g42455091.48
                                         #\J)))))))))))))))
          (if (call error?.132 g42455085.38)
            g42455085.38
            (let ((g42455092.49
                   (let ((pair0.52
                          (call
                           cons.133
                           (call cons.133 8 (call cons.133 450 empty))
                           (call
                            cons.133
                            (call cons.133 143 (call cons.133 368 empty))
                            (call
                             cons.133
                             110
                             (call
                              ...
                              (call
                               ...
                               (call
                                ...
                                (call
                                 cons.133
                                 #\B
                                 (call cons.133 empty empty)))))))))
                         (vector1.51
                          (let ((tmp.7.53 (call make-vector.134 8)))
                            (let ((g42455093.54
                                   (call
                                    vector-set!.135
                                    tmp.7.53
                                    0
                                    (call (lambda () 902)))))
                              (if (call error?.132 g42455093.54)
                                g42455093.54
                                (let ((g42455094.55
                                       (call
                                        vector-set!.135
                                        ...
                                        1
                                        (call (error 179)))))
                                  (if (call error?.132 g42455094.55)
                                    g42455094.55
                                    (let ((g42455095.56
                                           (call
                                            vector-set!.135
                                            ...
                                            (call
                                             (call
                                              cons.133
                                              17
                                              (call cons.133 299 empty))))))
                                      (if (call error?.132 g42455095.56)
                                        g42455095.56
                                        (let ((g42455096.57
                                               (call
                                                vector-set!.135
                                                ...
                                                3
                                                (call #\E))))
                                          (if (call error?.132 g42455096.57)
                                            g42455096.57
                                            (let ((g42455097.58
                                                   (call
                                                    vector-set!.135
              ...
              (if (call error?.132 g42455092.49)
                g42455092.49
                (let ((g42455109.71
                       (call
                  ...
                  (if (call error?.132 g42455109.71)
                    g42455109.71
                    (let ((g42455110.75
                           (if (if #t #\i #\[)
                             (if (call
                               ...
                               #f)
                             #f)))
                      (if (call error?.132 g42455110.75)
                        g42455110.75
                        (call
                         fun/ascii-char8544.25
                         (call procedure?.136 (if #f (error 129) #\`))
 ...
 '(module
    ...
    (call
     ...
     (if (if #f #f #f)
       ...
       (call
        fun/empty8562.11
        (let ((tmp.7.19 (call make-vector.89 8)))
          (let ((g42462740.20 (call vector-set!.90 tmp.7.19 0 1)))
            (if (call error?.91 g42462740.20)
              g42462740.20
              (let ((g42462741.21 (call vector-set!.90 tmp.7.19 1 2)))
                (if (call error?.91 g42462741.21)
                  g42462741.21
                  (let ((g42462742.22 (call vector-set!.90 tmp.7.19 2 3)))
                    (if (call error?.91 g42462742.22)
                      g42462742.22
                      (let ((g42462743.23 (call vector-set!.90 tmp.7.19 3 4)))
                        (if (call error?.91 g42462743.23)
                          g42462743.23
                          (let ((g42462744.24
                                 (call vector-set!.90 tmp.7.19 4 5)))
                            (if (call error?.91 g42462744.24)
                              g42462744.24
                              (let ((g42462745.25
                                     (call vector-set!.90 tmp.7.19 5 6)))
                                (if (call error?.91 g42462745.25)
                                  g42462745.25
                                  (let ((g42462746.26
                                         (call vector-set!.90 tmp.7.19 6 7)))
                                    (if (call error?.91 g42462746.26)
                                      g42462746.26
                                      (let ((g42462747.27
                                             (call
                                              vector-set!.90
                                              ...
                                              7
                                              8)))
                                        (if (call error?.91 g42462747.27)
                                          g42462747.27
                                          tmp.7.19)))))))))))))))))
        (lambda () 807)))
     (if (call
          fun/boolean8563.12
          (let ((tmp.8.28 (call make-vector.89 8)))
            (let ((g42462748.29 (call vector-set!.90 tmp.8.28 0 1)))
              (if (call error?.91 g42462748.29)
                g42462748.29
                (let ((g42462749.30 (call vector-set!.90 tmp.8.28 1 2)))
                  (if (call error?.91 g42462749.30)
                    g42462749.30
                    (let ((g42462750.31 (call vector-set!.90 tmp.8.28 2 3)))
                      (if (call error?.91 g42462750.31)
                        g42462750.31
                        (let ((g42462751.32
                               (call vector-set!.90 tmp.8.28 3 4)))
                          (if (call error?.91 g42462751.32)
                            g42462751.32
                            (let ((g42462752.33
                                   (call vector-set!.90 tmp.8.28 4 5)))
                              (if (call error?.91 g42462752.33)
                                g42462752.33
                                (let ((g42462753.34
                                       (call vector-set!.90 tmp.8.28 5 6)))
                                  (if (call error?.91 g42462753.34)
                                    g42462753.34
                                    (let ((g42462754.35
                                           (call vector-set!.90 tmp.8.28 6 7)))
                                      (if (call error?.91 g42462754.35)
                                        g42462754.35
                                        (let ((g42462755.36
                                               (call
                                                vector-set!.90
    ...
    (call
     ...
     (let ((error0.12
            (let ((g42470384.13 (error 207)))
              (if (call error?.71 g42470384.13)
                g42470384.13
                (let ((g42470385.14 (error 104)))
                  (if (call error?.71 g42470385.14)
                    g42470385.14
                    (let ((g42470386.15 (error 16)))
                      (if (call error?.71 g42470386.15)
                        g42470386.15
                        (let ((g42470387.16 (error 167)))
                          (if (call error?.71 g42470387.16)
                            g42470387.16
                            (error 61))))))))))
           (error1.11
            (if (error 129)
       ...
       (let ((g42470388.17 (error 161)))
         (if (call error?.71 g42470388.17)
           g42470388.17
           (let ((g42470389.18 (error 194)))
             (if (call error?.71 g42470389.18) g42470389.18 (error 177)))))))))
(check-by-interp
 '(module
    (define error?.114 (lambda (tmp.101) (error? tmp.101)))
    (define unsafe-vector-set!.5
      (lambda (tmp.65 tmp.66 tmp.67)
        (if (unsafe-fx< tmp.66 (unsafe-vector-length tmp.65))
          (if (unsafe-fx>= tmp.66 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.113
      (lambda (tmp.89 tmp.90 tmp.91)
        (if (fixnum? tmp.90)
          (if (vector? tmp.89)
            (call unsafe-vector-set!.5 tmp.89 tmp.90 tmp.91)
            (error 10))
          (error 10))))
    (define vector-init-loop.61
      (lambda (len.62 i.64 vec.63)
        (if (eq? len.62 i.64)
          vec.63
          (begin
            (unsafe-vector-set! vec.63 i.64 0)
            (call vector-init-loop.61 len.62 (unsafe-fx+ i.64 1) vec.63)))))
    (define make-init-vector.4
      (lambda (tmp.59)
        (let ((tmp.60 (unsafe-make-vector tmp.59)))
          (call vector-init-loop.61 tmp.59 0 tmp.60))))
    (define make-vector.112
      (lambda (tmp.87)
        (if (fixnum? tmp.87) (call make-init-vector.4 tmp.87) (error 8))))
    (define cons.111 (lambda (tmp.106 tmp.107) (cons tmp.106 tmp.107)))
    (define fun/error8591.11 (lambda () (error 47)))
    (define fun/pair8592.12
      (lambda (oprand0.15)
        (if #f
          (call cons.111 232 (call cons.111 299 empty))
          (call cons.111 190 (call cons.111 484 empty)))))
    ...
    (call
     ...
     (let ((vector0.21
            (let ((tmp.7.22 (call make-vector.112 8)))
              (let ((g42474204.23
                     (call vector-set!.113 tmp.7.22 0 (call empty))))
                (if (call error?.114 g42474204.23)
                  g42474204.23
                  (let ((g42474205.24
                         (call vector-set!.113 tmp.7.22 1 (call (error 230)))))
                    (if (call error?.114 g42474205.24)
                      g42474205.24
                      (let ((g42474206.25
                             (call vector-set!.113 tmp.7.22 2 (call (void)))))
                        (if (call error?.114 g42474206.25)
                          g42474206.25
                          (let ((g42474207.26
                                 (call
                                  vector-set!.113
                                  ...
                                  (call
                                   (call
                                    cons.111
                                    167
                                    (call cons.111 385 empty))))))
                            (if (call error?.114 g42474207.26)
                              g42474207.26
                              (let ((g42474208.27
                                     (call
                                      vector-set!.113
                                      ...
                                      4
                                      (call (void)))))
                                (if (call error?.114 g42474208.27)
                                  g42474208.27
                                  (let ((g42474209.28
                                         (call
                                          vector-set!.113
                                          ...
                                          5
                                          (call empty))))
                                    (if (call error?.114 g42474209.28)
                                      g42474209.28
                                      (let ((g42474210.29
                                             (call
                                              vector-set!.113
                                              ...
                                              6
                                              (call #\c))))
                                        (if (call error?.114 g42474210.29)
                                          g42474210.29
                                          (let ((g42474211.30
                                                 (call
                                                  vector-set!.113
                                                  ...
                                                  7
                                                  (call #\t))))
                                            (if (call error?.114 g42474211.30)
                                              g42474211.30
                                              tmp.7.22))))))))))))))))))
           (vector1.20
            (let ((tmp.8.31 (call make-vector.112 8)))
              (let ((g42474212.32
                     (call vector-set!.113 tmp.8.31 0 (call (error 207)))))
                (if (call error?.114 g42474212.32)
                  g42474212.32
                  (let ((g42474213.33
                         (call
                          vector-set!.113
                          ...
                          1
                          (call (lambda () 674)))))
                    (if (call error?.114 g42474213.33)
                      g42474213.33
                      (let ((g42474214.34
                             (call
                              vector-set!.113
                              ...
                              2
                              (call
                               (let ((tmp.9.35 (call make-vector.112 8)))
                                 (let ((g42474215.36
                                        (call vector-set!.113 tmp.9.35 0 1)))
                                   (if (call error?.114 g42474215.36)
                                     g42474215.36
                                     (let ((g42474216.37
                                            (call
                                             vector-set!.113
                                             ...
                                             1
                                             2)))
                                       (if (call error?.114 g42474216.37)
                                         g42474216.37
                                         (let ((g42474217.38
                                                (call
                                                 vector-set!.113
                                                 ...
                                                 2
                                                 3)))
                                           (if (call error?.114 g42474217.38)
                                             g42474217.38
                                             (let ((g42474218.39
                                                    (call
                                                     vector-set!.113
                                                           ...
                                                           (if (call
                                                             ...
                                                             (let ((g42474222.43
                                                               ...
                                                               (if (call
                                                                 ...
                                                                 g42474222.43
                                                                 tmp.9.35)))))))))))))))))))))
                        (if (call error?.114 g42474214.34)
                          g42474214.34
                          (let ((g42474223.44
                                 (call
                                  vector-set!.113
                                  ...
                                  3
                                  (call
                                   (let ((tmp.10.45 (call make-vector.112 8)))
                                     (let ((g42474224.46
                                            (call
                                             vector-set!.113
                                             tmp.10.45
                                             0
                                             0)))
                                       (if (call error?.114 g42474224.46)
                                         g42474224.46
                                         (let ((g42474225.47
                                                (call
                                                 vector-set!.113
                                                 ...
                                                 1
                                                 1)))
                                           (if (call error?.114 g42474225.47)
                                             g42474225.47
                                             (let ((g42474226.48
                                                    (call
                                                     vector-set!.113
                                                               ...
                                                               (if (call
                                                                 ...
                                                                 (let ((g42474231.53
                                                                   ...
                                                                   (if (call
                                                                     ...
                                                                     g42474231.53
                                                                     tmp.10.45)))))))))))))))))))))
                            (if (call error?.114 g42474223.44)
                              g42474223.44
                              (let ((g42474232.54
                                     (call
                                      vector-set!.113
                                      ...
                                      4
                                      (call #f))))
                                (if (call error?.114 g42474232.54)
                                  g42474232.54
                                  (let ((g42474233.55
                                         (call
                                          vector-set!.113
                                          ...
                                          (call
                                           (call
                                            cons.111
                                            232
                                            (call cons.111 259 empty))))))
                                    (if (call error?.114 g42474233.55)
                                      g42474233.55
                                      (let ((g42474234.56
                                             (call
                                              vector-set!.113
                                              ...
                                              6
                                              (call #f))))
                                        (if (call error?.114 g42474234.56)
                                          g42474234.56
                                          (let ((g42474235.57
                                                 (call
                                                  vector-set!.113
                                                  ...
                                                  7
                                                  (call (void)))))
                                            (if (call error?.114 g42474235.57)
                                              g42474235.57
                                              tmp.8.31)))))))))))))))))))
       (let ((empty0.58 empty)) #\i)))))
(check-by-interp
 '(module
    (define error?.81 (lambda (tmp.68) (error? tmp.68)))
    (define unsafe-vector-set!.5
      (lambda (tmp.32 tmp.33 tmp.34)
        (if (unsafe-fx< tmp.33 (unsafe-vector-length tmp.32))
          (if (unsafe-fx>= tmp.33 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.80
      (lambda (tmp.56 tmp.57 tmp.58)
        (if (fixnum? tmp.57)
          (if (vector? tmp.56)
            (call unsafe-vector-set!.5 tmp.56 tmp.57 tmp.58)
            (error 10))
          (error 10))))
    (define vector-init-loop.28
      (lambda (len.29 i.31 vec.30)
        (if (eq? len.29 i.31)
          vec.30
          (begin
            (unsafe-vector-set! vec.30 i.31 0)
            (call vector-init-loop.28 len.29 (unsafe-fx+ i.31 1) vec.30)))))
    (define make-init-vector.4
      (lambda (tmp.26)
        (let ((tmp.27 (unsafe-make-vector tmp.26)))
          (call vector-init-loop.28 tmp.26 0 tmp.27))))
    (define make-vector.79
      (lambda (tmp.54)
        (if (fixnum? tmp.54) (call make-init-vector.4 tmp.54) (error 8))))
    (define cons.78 (lambda (tmp.73 tmp.74) (cons tmp.73 tmp.74)))
    ...
    (call
     ...
     (if (if #t #t #f)
       (let ((boolean0.14 #f)
             (empty1.13 empty)
             (pair2.12 (call cons.78 41 (call cons.78 412 empty))))
         165)
       (let ((vector0.16
              (let ((tmp.7.17 (call make-vector.79 8)))
                (let ((g42478053.18 (call vector-set!.80 tmp.7.17 0 1)))
                  (if (call error?.81 g42478053.18)
                    g42478053.18
                    (let ((g42478054.19 (call vector-set!.80 tmp.7.17 1 2)))
                      (if (call error?.81 g42478054.19)
                        g42478054.19
                        (let ((g42478055.20
                               (call vector-set!.80 tmp.7.17 2 3)))
                          (if (call error?.81 g42478055.20)
                            g42478055.20
                            (let ((g42478056.21
                                   (call vector-set!.80 tmp.7.17 3 4)))
                              (if (call error?.81 g42478056.21)
                                g42478056.21
                                (let ((g42478057.22
                                       (call vector-set!.80 tmp.7.17 4 5)))
                                  (if (call error?.81 g42478057.22)
                                    g42478057.22
                                    (let ((g42478058.23
                                           (call vector-set!.80 tmp.7.17 5 6)))
                                      (if (call error?.81 g42478058.23)
                                        g42478058.23
                                        (let ((g42478059.24
                                               (call
                                                vector-set!.80
...
(check-by-interp
 '(module
    (define cons.77 (lambda (tmp.72 tmp.73) (cons tmp.72 tmp.73)))
            ...
            (if tmp.8.21
              ...
              (let ((tmp.9.22
                     (call
                      ...
                      (call cons.77 124 (call cons.77 438 empty))
                      (lambda () 729))))
                (if tmp.9.22
                  tmp.9.22
...
(check-by-interp
 '(module
    (define void?.77 (lambda (tmp.64) (void? tmp.64)))
    ...
    (define fun/pair8619.12
      (lambda () (call cons.76 87 (call cons.76 327 empty))))
    ...
    (let ((tmp.7.21 (call fun/boolean8615.10 (let () #t) (if #f #f #t))))
      (if tmp.7.21
        tmp.7.21
        (let ((tmp.8.22 (call void?.77 (call fun/any8616.15 (lambda () 764)))))
          (if tmp.8.22
            tmp.8.22
            (let ((tmp.9.23
                   (call
    ...
    (define |-.212|
      (lambda (tmp.174 tmp.175)
        (if (fixnum? tmp.175)
          ...
          (error 3))))
    (define cons.211 (lambda (tmp.203 tmp.204) (cons tmp.203 tmp.204)))
    (define error?.210 (lambda (tmp.198) (error? tmp.198)))
    (define unsafe-vector-set!.5
      (lambda (tmp.162 tmp.163 tmp.164)
        (if (unsafe-fx< tmp.163 (unsafe-vector-length tmp.162))
          (if (unsafe-fx>= tmp.163 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.209
      (lambda (tmp.186 tmp.187 tmp.188)
        (if (fixnum? tmp.187)
          (if (vector? tmp.186)
            (call unsafe-vector-set!.5 tmp.186 tmp.187 tmp.188)
            (error 10))
          (error 10))))
    (define vector-init-loop.158
    ...
    (define make-init-vector.4
      (lambda (tmp.156)
        (let ((tmp.157 (unsafe-make-vector tmp.156)))
          (call vector-init-loop.158 tmp.156 0 tmp.157))))
    (define make-vector.208
      (lambda (tmp.184)
        (if (fixnum? tmp.184) (call make-init-vector.4 tmp.184) (error 8))))
    ...
    (call
                                       ...
                                       (let ((tmp.12.59
                                         ...
                                         (let ((g42493326.60
                                                (call
                                                 ...
                                                 0
                                                 0)))
                                           (if (call error?.210 g42493326.60)
                                             g42493326.60
                                             (let ((g42493327.61
                                                    (call
                                                     vector-set!.209
     ...
     (if (let ((tmp.14.74
                (call
                 fun/fixnum8625.38
                 (error 175)
                 (let ((tmp.15.75 (call make-vector.208 8)))
                   (let ((g42493334.76 (call vector-set!.209 tmp.15.75 0 1)))
                     (if (call error?.210 g42493334.76)
                       g42493334.76
                       (let ((g42493335.77
                              (call vector-set!.209 tmp.15.75 1 2)))
                         (if (call error?.210 g42493335.77)
                           g42493335.77
                           (let ((g42493336.78
                                  (call vector-set!.209 tmp.15.75 2 3)))
                             (if (call error?.210 g42493336.78)
                               g42493336.78
                               (let ((g42493337.79
                                      (call vector-set!.209 tmp.15.75 3 4)))
                                 (if (call error?.210 g42493337.79)
                                   g42493337.79
                                   (let ((g42493338.80
                                          (call
                                           vector-set!.209
                                           ...
                                           4
                                           5)))
                                     (if (call error?.210 g42493338.80)
                                       g42493338.80
                                       (let ((g42493339.81
                                              (call
                                               vector-set!.209
                                               ...
                                               5
                                               6)))
                                         (if (call error?.210 g42493339.81)
                                           g42493339.81
                                           (let ((g42493340.82
                                                  (call
                                                   vector-set!.209
                                                   ...
                                                   6
                                                   7)))
                                             (if (call error?.210 g42493340.82)
                                               g42493340.82
                                               (let ((g42493341.83
                                                      (call
                                                       vector-set!.209
       ...
       (if (if 32
             (if (call
                  fun/fixnum8628.37
                  (let ((tmp.19.90 (call make-vector.208 8)))
                    (let ((g42493342.91 (call vector-set!.209 tmp.19.90 0 0)))
                      (if (call error?.210 g42493342.91)
                        g42493342.91
                        (let ((g42493343.92
                               (call vector-set!.209 tmp.19.90 1 1)))
                          (if (call error?.210 g42493343.92)
                            g42493343.92
                            (let ((g42493344.93
                                   (call vector-set!.209 tmp.19.90 2 2)))
                              (if (call error?.210 g42493344.93)
                                g42493344.93
                                (let ((g42493345.94
                                       (call vector-set!.209 tmp.19.90 3 3)))
                                  (if (call error?.210 g42493345.94)
                                    g42493345.94
                                    (let ((g42493346.95
                                           (call
                                            vector-set!.209
                                            ...
                                            4
                                            4)))
                                      (if (call error?.210 g42493346.95)
                                        g42493346.95
                                        (let ((g42493347.96
                                               (call
                                                vector-set!.209
                                                ...
                                                5
                                                5)))
                                          (if (call error?.210 g42493347.96)
                                            g42493347.96
                                            (let ((g42493348.97
                                                   (call
                                                    vector-set!.209
                  ...
                  empty)
               (if (let ((pair0.100
                          (call cons.211 182 (call cons.211 324 empty)))
                         (error1.99 (error 85)))
                     6)
                 (if 113
         ...
         (if (if #t 60 147)
           (if (let ((g42493350.103 (if #f 74 149)))
                 (if (call error?.210 g42493350.103)
                   g42493350.103
                   (let ((g42493351.104
                          (if 18 (if 18 (if 233 (if 91 193 #f) #f) #f) #f)))
                     (if (call error?.210 g42493351.104)
                       g42493351.104
                       (let ((g42493352.105
                              (let ((tmp.20.106 235))
                                (if tmp.20.106 tmp.20.106 137))))
                         (if (call error?.210 g42493352.105)
                           g42493352.105
                           (let ((g42493353.107 (if #t 238 229)))
                             (if (call error?.210 g42493353.107)
                               g42493353.107
                               (let ((vector0.110
                                      (let ((tmp.21.111
                                             (call make-vector.208 8)))
                                        (let ((g42493354.112
                                               (call
                                                ...
                                                0
                                                1)))
                                          (if (call error?.210 g42493354.112)
                                            g42493354.112
                                            (let ((g42493355.113
                                                   (call
                                                    vector-set!.209
                                      ...
                                      (let ((tmp.22.120
                                        ...
                                        (let ((g42493362.121
                                               (call
                                                ...
                                                0
                                                0)))
                                          (if (call error?.210 g42493362.121)
                                            g42493362.121
                                            (let ((g42493363.122
                                                   (call
                                                    vector-set!.209
             ...
             (if (call
                  |-.212|
                  (let ((void0.130 (void)) (pair1.129 (call cons.211 #\O #f)))
                    76)
                  (let ((empty0.133 empty)
                        (boolean1.132 #t)
                        (vector2.131
                         (let ((tmp.23.134 (call make-vector.208 8)))
                           (let ((g42493370.135
                                  (call vector-set!.209 tmp.23.134 0 1)))
                             (if (call error?.210 g42493370.135)
                               g42493370.135
                               (let ((g42493371.136
                                      (call vector-set!.209 tmp.23.134 1 2)))
                                 (if (call error?.210 g42493371.136)
                                   g42493371.136
                                   (let ((g42493372.137
                                          (call
                                           vector-set!.209
                                           ...
                                           2
                                           3)))
                                     (if (call error?.210 g42493372.137)
                                       g42493372.137
                                       (let ((g42493373.138
                                              (call
                                               vector-set!.209
                                               ...
                                               3
                                               4)))
                                         (if (call error?.210 g42493373.138)
                                           g42493373.138
                                           (let ((g42493374.139
                                                  (call
                                                   vector-set!.209
...
(check-by-interp
 '(module
    ...
    (define fun/pair8635.12
      (lambda (oprand0.18) (call cons.76 16 (call cons.76 418 empty))))
    ...
    (if (let ((tmp.7.20 (call fun/boolean8631.11 #\f)))
          (if tmp.7.20
            tmp.7.20
            (let ((tmp.8.21 (call >.77 237 82)))
              (if tmp.8.21
                tmp.8.21
                (let ((tmp.9.22 (let () #t)))
                  (if tmp.9.22
                    tmp.9.22
                    (let ((tmp.10.23 (call <=.78 191 13)))
...
(check-by-interp
 '(module
    (define ascii-char?.64 (lambda (tmp.52) (ascii-char? tmp.52)))
    ...
    (call
     fun/empty8643.7
     (if #t empty empty)
     (call ascii-char?.64 (let ((vector0.10 (call make-vector.63 8))) 146)))))
(check-by-interp
 '(module
    (define >.117
      (lambda (tmp.83 tmp.84)
        (if (fixnum? tmp.84)
          ...
          (error 6))))
    (define cons.116 (lambda (tmp.106 tmp.107) (cons tmp.106 tmp.107)))
    (define unsafe-vector-set!.5
      (lambda (tmp.65 tmp.66 tmp.67)
        (if (unsafe-fx< tmp.66 (unsafe-vector-length tmp.65))
          (if (unsafe-fx>= tmp.66 0)
    ...
    (define |+.112|
      (lambda (tmp.75 tmp.76)
        (if (fixnum? tmp.76)
          ...
          (error 2))))
    (define error?.111 (lambda (tmp.101) (error? tmp.101)))
    ...
    (call
     ...
     (let ((g42516139.26 (call fun/fixnum8646.17 #\q)))
       (if (call error?.111 g42516139.26) g42516139.26 (if #f 81 95)))
     (if (call
          |+.112|
          (if 251
                                     ...
                                     (let ((void0.52 (void))
                                           ...
                                           (pair2.50
                                            (call
                                             cons.116
                                             103
                                             (call cons.116 494 empty))))
                                       245)))))))))))))
           (if (let ((boolean0.55 #t)
                     (pair1.54 (call cons.116 131 (call cons.116 435 empty)))
             ...
             (if (if (if #t 35 220)
                   (if (if #t 36 135)
                     (if (call
                          fun/fixnum8648.19
                          (call cons.116 124 (call cons.116 432 empty))
                          82)
...
(check-by-interp
 '(module
    ...
    (define make-vector.141
      (lambda (tmp.115)
        (if (fixnum? tmp.115) (call make-init-vector.4 tmp.115) (error 8))))
    (define error?.140 (lambda (tmp.129) (error? tmp.129)))
    (define cons.139 (lambda (tmp.134 tmp.135) (cons tmp.134 tmp.135)))
    ...
    (let ((g42519968.23
           (if (let ((boolean0.26 #f)
                     (pair1.25 (call cons.139 247 (call cons.139 312 empty)))
                     (error2.24 (error 55)))
                 #f)
             (call fun/void8652.10 #\O empty)
             (let ((g42519969.27 (void)))
               (if (call error?.140 g42519969.27) g42519969.27 (void))))))
      (if (call error?.140 g42519968.23)
        g42519968.23
        (let ((g42519970.28
               (if (let ((g42519971.29 #t))
                     (if (call error?.140 g42519971.29) g42519971.29 #f))
                 (if #t (void) (void))
                 (let ((g42519972.30 (void)))
                   (if (call error?.140 g42519972.30)
                     g42519972.30
                     (let ((g42519973.31 (void)))
                       (if (call error?.140 g42519973.31)
                         g42519973.31
                         (let ((g42519974.32 (void)))
                           (if (call error?.140 g42519974.32)
                             g42519974.32
                             (let ((g42519975.33 (void)))
                               (if (call error?.140 g42519975.33)
                                 g42519975.33
                                 (let ((g42519976.34 (void)))
                                   (if (call error?.140 g42519976.34)
                                     g42519976.34
                                     (void))))))))))))))
          (if (call error?.140 g42519970.28)
            g42519970.28
            (let ((g42519977.35
                   (call
                    fun/void8653.14
                    (if #t
                      (call cons.139 38 (call cons.139 339 empty))
                      (call cons.139 84 (call cons.139 401 empty)))
                    (let ((error0.37 (error 143)) (error1.36 (error 119)))
                      (error 96)))))
              (if (call error?.140 g42519977.35)
                g42519977.35
                (let ((g42519978.38
                       (let ((g42519979.39
                              (if (let ((ascii-char0.41 #\M)
                                         ...
                                         (let ((tmp.7.42
                                           ...
                                           (let ((g42519980.43
                                                  (call
                                                   ...
                                                   0
                                                   0)))
                                             (if (call error?.140 g42519980.43)
                                               g42519980.43
                                               (let ((g42519981.44
                                                      (call
                                                       vector-set!.142
                                ...
                                (let ((g42519988.51 (void)))
                                  (if (call error?.140 g42519988.51)
                                    g42519988.51
                                    (let ((g42519989.52 (void)))
                                      (if (call error?.140 g42519989.52)
                                        g42519989.52
                                        (void)))))
                                #f)))
                         (if (call error?.140 g42519979.39)
                           g42519979.39
                           (let ((g42519990.53
                                  (let ((vector0.55
                                         (let ((tmp.8.56
                                           ...
                                           (let ((g42519991.57
                                                  (call
                                                   ...
                                                   0
                                                   0)))
                                             (if (call error?.140 g42519991.57)
                                               g42519991.57
                                               (let ((g42519992.58
                                                      (call
                                                       vector-set!.142
                                        ...
                                        (error1.54 (error 186)))
                                    (void))))
                             (if (call error?.140 g42519990.53)
                               g42519990.53
                               (let ((g42519999.65
                                      (let ((procedure0.66 (lambda () 121)))
                                        (void))))
                                 (if (call error?.140 g42519999.65)
                                   g42519999.65
                                   (let ((g42520000.67
                                          (call
                                           fun/void8654.13
                                           (if #t #\a #\r))))
                                     (if (call error?.140 g42520000.67)
                                       g42520000.67
                                       (let ((g42520001.68
                                              (if #t (void) (void))))
                                         (if (call error?.140 g42520001.68)
                                           g42520001.68
                                           (let ((g42520002.69
                                                  (if #f (void) (void))))
                                             (if (call error?.140 g42520002.69)
                                               g42520002.69
                                               (if #f
                                                 (void)
                                                 (void))))))))))))))))
                  (if (call error?.140 g42519978.38)
                    g42519978.38
                    (let ((g42520003.70
                           (let ((void0.71 (call fun/void8655.12)))
                             (let ((pair0.73
                                    (call
                                     cons.139
                                     100
                                     (call cons.139 256 empty)))
                                   (pair1.72
                                    (call
                                     cons.139
                                     1
                                     (call cons.139 424 empty))))
                               (void)))))
                      (if (call error?.140 g42520003.70)
                        g42520003.70
                        (call
                         fun/void8656.11
                         (let ((ascii-char0.75 #\B)
                               (pair1.74
                                (call cons.139 147 (call cons.139 339 empty))))
                           #\N)
                         (let ((vector0.77
                                (let ((tmp.9.78 (call make-vector.141 8)))
                                  (let ((g42520004.79
                                         (call vector-set!.142 tmp.9.78 0 1)))
                                    (if (call error?.140 g42520004.79)
                                      g42520004.79
                                      (let ((g42520005.80
                                             (call
                                              vector-set!.142
                                              ...
                                              1
                                              2)))
                                        (if (call error?.140 g42520005.80)
                                          g42520005.80
                                          (let ((g42520006.81
                                                 (call
                                                  vector-set!.142
                                                  ...
                                                  2
                                                  3)))
                                            (if (call error?.140 g42520006.81)
                                              g42520006.81
                                              (let ((g42520007.82
                                                     (call
                                                      vector-set!.142
    ...
    (let ((void0.9 (let ((ascii-char0.10 #\s)) (void))))
      (if (if #f (void) (void))
        (if (let ((g42523829.11 (void)))
              (if (call error?.64 g42523829.11) g42523829.11 (void)))
          (call fun/void8659.7 #f)
          #f)
        #f))))
(check-by-interp
 '(module
    (define cons.74 (lambda (tmp.67 tmp.68) (cons tmp.67 tmp.68)))
    (define error?.73 (lambda (tmp.62) (error? tmp.62)))
      ...
      (if (let ((empty0.16
                ...
                (pair1.15
                 (call
                  ...
                  (call
                   cons.74
                   (call cons.74 154 (call cons.74 498 empty))
                   (call
                    cons.74
                    95
                    (call
                     ...
                     (call
                      ...
                      (call
                       cons.74
                       (call cons.74 118 (call cons.74 510 empty))
                       (call cons.74 #f (call cons.74 #\] empty))))))))))
...
(check-by-interp
 '(module
    (define error?.88 (lambda (tmp.76) (error? tmp.76)))
    (define unsafe-vector-set!.5
      (lambda (tmp.40 tmp.41 tmp.42)
        (if (unsafe-fx< tmp.41 (unsafe-vector-length tmp.40))
          (if (unsafe-fx>= tmp.41 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.87
      (lambda (tmp.64 tmp.65 tmp.66)
        (if (fixnum? tmp.65)
          (if (vector? tmp.64)
            (call unsafe-vector-set!.5 tmp.64 tmp.65 tmp.66)
            (error 10))
          (error 10))))
      ...
      (lambda (len.37 i.39 vec.38)
        (if (eq? len.37 i.39)
          ...
          (begin
            ...
            (call vector-init-loop.36 len.37 (unsafe-fx+ i.39 1) vec.38)))))
    (define make-init-vector.4
      (lambda (tmp.34)
        (let ((tmp.35 (unsafe-make-vector tmp.34)))
          (call vector-init-loop.36 tmp.34 0 tmp.35))))
    (define make-vector.86
      (lambda (tmp.62)
        (if (fixnum? tmp.62) (call make-init-vector.4 tmp.62) (error 8))))
    ...
    (if (let () #f)
      (call
       fun/empty8665.9
       (let ((vector0.13
              (let ((tmp.7.14 (call make-vector.86 8)))
                (let ((g42531467.15 (call vector-set!.87 tmp.7.14 0 0)))
                  (if (call error?.88 g42531467.15)
                    g42531467.15
                    (let ((g42531468.16 (call vector-set!.87 tmp.7.14 1 1)))
                      (if (call error?.88 g42531468.16)
                        g42531468.16
                        (let ((g42531469.17
                               (call vector-set!.87 tmp.7.14 2 2)))
                          (if (call error?.88 g42531469.17)
                            g42531469.17
                            (let ((g42531470.18
                                   (call vector-set!.87 tmp.7.14 3 3)))
                              (if (call error?.88 g42531470.18)
                                g42531470.18
                                (let ((g42531471.19
                                       (call vector-set!.87 tmp.7.14 4 4)))
                                  (if (call error?.88 g42531471.19)
                                    g42531471.19
                                    (let ((g42531472.20
                                           (call vector-set!.87 tmp.7.14 5 5)))
                                      (if (call error?.88 g42531472.20)
                                        g42531472.20
                                        (let ((g42531473.21
                                               (call
                                                vector-set!.87
                                          ...
                                          (if (call error?.88 g42531473.21)
                                            ...
                                            (let ((g42531474.22
                                                   (call
                                                    ...
                                                    7
                                                    7)))
                                              (if (call error?.88 g42531474.22)
                                                g42531474.22
                                                tmp.7.14))))))))))))))))))
             (empty1.12 empty))
         #\H))
      (let ((vector0.24
             (let ((tmp.8.25 (call make-vector.86 8)))
               (let ((g42531475.26 (call vector-set!.87 tmp.8.25 0 1)))
                 (if (call error?.88 g42531475.26)
                   g42531475.26
                   (let ((g42531476.27 (call vector-set!.87 tmp.8.25 1 2)))
                     (if (call error?.88 g42531476.27)
                       g42531476.27
                       (let ((g42531477.28 (call vector-set!.87 tmp.8.25 2 3)))
                         (if (call error?.88 g42531477.28)
                           g42531477.28
                           (let ((g42531478.29
                                  (call vector-set!.87 tmp.8.25 3 4)))
                             (if (call error?.88 g42531478.29)
                               g42531478.29
                               (let ((g42531479.30
                                      (call vector-set!.87 tmp.8.25 4 5)))
                                 (if (call error?.88 g42531479.30)
                                   g42531479.30
                                   (let ((g42531480.31
                                          (call vector-set!.87 tmp.8.25 5 6)))
                                     (if (call error?.88 g42531480.31)
                                       g42531480.31
                                       (let ((g42531481.32
                                              (call
                                               vector-set!.87
...
(check-by-interp
 '(module
    (define error?.100 (lambda (tmp.87) (error? tmp.87)))
    (define unsafe-vector-set!.5
      (lambda (tmp.51 tmp.52 tmp.53)
        (if (unsafe-fx< tmp.52 (unsafe-vector-length tmp.51))
          (if (unsafe-fx>= tmp.52 0)
            ...
            (error 10))
          (error 10))))
    (define vector-set!.99
      (lambda (tmp.75 tmp.76 tmp.77)
        (if (fixnum? tmp.76)
          (if (vector? tmp.75)
            (call unsafe-vector-set!.5 tmp.75 tmp.76 tmp.77)
            (error 10))
          (error 10))))
    (define vector-init-loop.47
      (lambda (len.48 i.50 vec.49)
        (if (eq? len.48 i.50)
          vec.49
          (begin
            (unsafe-vector-set! vec.49 i.50 0)
            (call vector-init-loop.47 len.48 (unsafe-fx+ i.50 1) vec.49)))))
    (define make-init-vector.4
      (lambda (tmp.45)
        (let ((tmp.46 (unsafe-make-vector tmp.45)))
          (call vector-init-loop.47 tmp.45 0 tmp.46))))
    (define make-vector.98
      (lambda (tmp.73)
        (if (fixnum? tmp.73) (call make-init-vector.4 tmp.73) (error 8))))
    (define cons.97 (lambda (tmp.92 tmp.93) (cons tmp.92 tmp.93)))
    (define fun/fixnum8668.10 (lambda () 84))
    (define fun/pair8669.11
      (lambda () (call cons.97 33 (call cons.97 355 empty))))
    (define fun/fixnum8670.12 (lambda () 116))
    (let ((vector0.14
           (let ((tmp.7.15 (call make-vector.98 8)))
             (let ((g42535303.16
                    (call
                     vector-set!.99
                     tmp.7.15
                     0
                     (call (let () (error 47))))))
               (if (call error?.100 g42535303.16)
                 g42535303.16
                 (let ((g42535304.17
                        (call vector-set!.99 tmp.7.15 1 (call (error 83)))))
                   (if (call error?.100 g42535304.17)
                     g42535304.17
                     (let ((g42535305.18
                            (call
                             vector-set!.99
                             ...
                             (call
                              (if #t
                                (call cons.97 212 (call cons.97 273 empty))
                                (call cons.97 6 (call cons.97 506 empty)))))))
                       (if (call error?.100 g42535305.18)
                         g42535305.18
                         (let ((g42535306.19
                                (call
                                 vector-set!.99
                                 ...
                                 3
                                 (call
                                  (let ((g42535307.20 (call make-vector.98 8)))
                                    (if (call error?.100 g42535307.20)
                                      g42535307.20
                                      (let ((g42535308.21
                                             (let ((tmp.8.22
                                                    (call make-vector.98 8)))
                                               (let ((g42535309.23
                                                      (call
                                                       vector-set!.99
                                                       tmp.8.22
                                                                         ...
                                                                         (if (call
                                                                           ...
                                                                           (let ((g42535316.30
                                                                             ...
                                                                             (if (call
                                                                               ...
                                                                               g42535316.30
                                                                               tmp.8.22)))))))))))))))))))
                                        (if (call error?.100 g42535308.21)
                                          g42535308.21
                                          (let ((tmp.9.31
                                                 (call make-vector.98 8)))
                                            (let ((g42535317.32
                                                                      ...
                                                                      (if (call
                                                                        ...
                                                                        (let ((g42535324.39
                                                                          ...
                                                                          (if (call
                                                                            ...
                                                                            g42535324.39
                                                                            tmp.9.31)))))))))))))))))))))))))
                           (if (call error?.100 g42535306.19)
                             g42535306.19
                             (let ((g42535325.40
                                    (call
                                     vector-set!.99
                               ...
                               (if (call error?.100 g42535325.40)
                                 g42535325.40
                                 (let ((g42535326.41
                                        (call
                                         vector-set!.99
                                   ...
                                   (if (call error?.100 g42535326.41)
                                     g42535326.41
                                     (let ((g42535327.42
                                            (call
                                             vector-set!.99
                                       ...
                                       (if (call error?.100 g42535327.42)
                                         g42535327.42
                                         (let ((g42535328.43
                                                (call
                                                 vector-set!.99
                                           ...
                                           (if (call error?.100 g42535328.43)
                                             g42535328.43
                                             tmp.7.15))))))))))))))))))
          (empty1.13 (let () empty)))
 ...
 '(module
    ...
    (call
     ...
     (call
      fun/boolean8683.15
      (if #f
        (let ((tmp.7.20 (call make-vector.90 8)))
          (let ((g42546779.21 (call vector-set!.91 tmp.7.20 0 1)))
            (if (call error?.92 g42546779.21)
              g42546779.21
              (let ((g42546780.22 (call vector-set!.91 tmp.7.20 1 2)))
                (if (call error?.92 g42546780.22)
                  g42546780.22
                  (let ((g42546781.23 (call vector-set!.91 tmp.7.20 2 3)))
                    (if (call error?.92 g42546781.23)
                      g42546781.23
                      (let ((g42546782.24 (call vector-set!.91 tmp.7.20 3 4)))
                        (if (call error?.92 g42546782.24)
                          g42546782.24
                          (let ((g42546783.25
                                 (call vector-set!.91 tmp.7.20 4 5)))
                            (if (call error?.92 g42546783.25)
                              g42546783.25
                              (let ((g42546784.26
                                     (call vector-set!.91 tmp.7.20 5 6)))
                                (if (call error?.92 g42546784.26)
                                  g42546784.26
                                  (let ((g42546785.27
                                         (call vector-set!.91 tmp.7.20 6 7)))
                                    (if (call error?.92 g42546785.27)
                                      g42546785.27
                                      (let ((g42546786.28
                                             (call
                                              vector-set!.91
                                              ...
                                              7
                                              8)))
                                        (if (call error?.92 g42546786.28)
                                          g42546786.28
                                          tmp.7.20)))))))))))))))))
        (let ((tmp.8.29 (call make-vector.90 8)))
          (let ((g42546787.30 (call vector-set!.91 tmp.8.29 0 1)))
            (if (call error?.92 g42546787.30)
              g42546787.30
              (let ((g42546788.31 (call vector-set!.91 tmp.8.29 1 2)))
                (if (call error?.92 g42546788.31)
                  g42546788.31
                  (let ((g42546789.32 (call vector-set!.91 tmp.8.29 2 3)))
                    (if (call error?.92 g42546789.32)
                      g42546789.32
                      (let ((g42546790.33 (call vector-set!.91 tmp.8.29 3 4)))
                        (if (call error?.92 g42546790.33)
                          g42546790.33
                          (let ((g42546791.34
                                 (call vector-set!.91 tmp.8.29 4 5)))
                            (if (call error?.92 g42546791.34)
                              g42546791.34
                              (let ((g42546792.35
                                     (call vector-set!.91 tmp.8.29 5 6)))
                                (if (call error?.92 g42546792.35)
                                  g42546792.35
                                  (let ((g42546793.36
                                         (call vector-set!.91 tmp.8.29 6 7)))
                                    (if (call error?.92 g42546793.36)
                                      g42546793.36
                                      (let ((g42546794.37
                                             (call
                                              vector-set!.91
 ...
 '(module
    ...
    (if (call fun/boolean8690.8)
      ...
      (call
       ...
       (let ((g42550609.15 #\x))
         (if (call error?.69 g42550609.15)
           g42550609.15
           (let ((g42550610.16 #\U))
             (if (call error?.69 g42550610.16) g42550610.16 #\O))))))))
(check-by-interp
 '(module
    (define cons.98 (lambda (tmp.89 tmp.90) (cons tmp.89 tmp.90)))
    (define void?.97 (lambda (tmp.82) (void? tmp.82)))
    ...
    (let ((vector0.21
           (let ((tmp.7.22 (call make-vector.94 8)))
             (let ((g42554425.23
                    (call
                     vector-set!.95
                     tmp.7.22
                                    ...
                                    (call
                                     ...
                                     (call
                                      (let ((g42554430.28 156))
                                        (if (call error?.96 g42554430.28)
                                          g42554430.28
                                          (let ((g42554431.29 226))
                                            (if (call error?.96 g42554431.29)
                                              g42554431.29
                                              114))))))))
                               (if (call error?.96 g42554429.27)
                                 g42554429.27
                                 (let ((g42554432.30
                                        (call
                                         vector-set!.95
                                             ...
                                             (call
                                              (let ()
                                                (call
                                                 cons.98
                                                 41
                                                 (call cons.98 429 empty)))))))
                                       (if (call error?.96 g42554433.31)
                                         g42554433.31
                                         (let ((g42554434.32
                                                (call
                                                 vector-set!.95
                ...
                (if tmp.11.36
                  ...
                  (let ((tmp.12.37
                         (let ((g42554435.38 empty))
                           (if (call error?.96 g42554435.38)
                             g42554435.38
                             (let ((g42554436.39 empty))
                               (if (call error?.96 g42554436.39)
                                 g42554436.39
                                 empty))))))
                    (if tmp.12.37
                      tmp.12.37
...
(check-by-interp
 '(module
    (define error?.77 (lambda (tmp.67) (error? tmp.67)))
                ...
                (if tmp.9.17
                  ...
                  (let ((tmp.10.20 (if #t #\` #\M)))
                    (if tmp.10.20
                      ...
                      (let ((tmp.11.21
                             (let ((g42562071.22 #\n))
                               (if (call error?.77 g42562071.22)
                                 g42562071.22
                                 (let ((g42562072.23 #\L))
                                   (if (call error?.77 g42562072.23)
                                     g42562072.23
                                     #\c))))))
                        (if tmp.11.21
                          tmp.11.21
