#lang racket
(require cpsc411/compiler-lib
         cpsc411/langs/v8
         (only-in "../select-instructions.rkt"))
(define (fail-if-invalid p)
  (when (not (asm-alloc-lang-v8? p))
    (error
     (~a
      (pretty-format p)
      "\n is not a semantically valid "
      "asm-alloc-lang-v8"
      " program")))
  p)
(define (check-by-interp p)
  (interp-imp-cmf-lang-v8 p)
  (interp-asm-alloc-lang-v8 (fail-if-invalid (select-instructions p))))

(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (begin (set! rax 24894) (jump tmp-ra.50 rbp rax)))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (begin (set! rax 22) (jump tmp-ra.50 rbp rax)))))
(check-by-interp
 '(module
    ((new-frames ()))
    (define L.fun/void8390.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.50 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.50 rbp rax)))))
    (begin
      (set! tmp-ra.51 r15)
      (begin (set! r15 tmp-ra.51) (jump L.fun/void8390.4 rbp r15)))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.51 r15)
      (begin
        (set! empty0.1 22)
        (begin (set! rax 21310) (jump tmp-ra.51 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.52 r15)
      (begin
        (set! void0.2 30)
        (set! ascii-char1.1 25390)
        (begin (set! rax 576) (jump tmp-ra.52 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (define L.fun/void8397.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.54 r15)
        (begin
          (set! oprand0.1 rdi)
          (begin (set! rax 30) (jump tmp-ra.54 rbp rax)))))
    (begin
      (set! tmp-ra.55 r15)
      (begin
        (begin (set! ascii-char0.3 25390) (set! boolean1.2 6) (set! tmp.53 14))
        (begin
          (set! rdi tmp.53)
          (set! r15 tmp-ra.55)
          (jump L.fun/void8397.4 rbp r15 rdi))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (!= 14 6)
        (begin (set! rax 18494) (jump tmp-ra.50 rbp rax))
        (begin (set! rax 57406) (jump tmp-ra.50 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (!= 6 6)
        (begin (set! rax 25390) (jump tmp-ra.50 rbp rax))
        (begin (set! rax 25390) (jump tmp-ra.50 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.50 r15)
      (if (!= 14 6)
        (begin (set! rax 240) (jump tmp-ra.50 rbp rax))
        (begin (set! rax 224) (jump tmp-ra.50 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.52 r15)
      (begin
        (set! empty0.2 22)
        (set! ascii-char1.1 25390)
        (begin (set! rax 30) (jump tmp-ra.52 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (define L.fun/error8412.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.54 r15)
        (begin
          (set! oprand0.1 rdi)
          (begin (set! rax 29758) (jump tmp-ra.54 rbp rax)))))
    (begin
      (set! tmp-ra.55 r15)
      (begin
        (begin (set! void0.3 30) (set! error1.2 38974) (set! tmp.53 error1.2))
        (begin
          (set! rdi tmp.53)
          (set! r15 tmp-ra.55)
          (jump L.fun/error8412.4 rbp r15 rdi))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (define L.fun/ascii-char8415.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.54 r15)
        (begin
          (set! oprand0.1 rdi)
          (begin (set! rax 25390) (jump tmp-ra.54 rbp rax)))))
    (begin
      (set! tmp-ra.55 r15)
      (begin
        (begin
          (set! ascii-char0.3 25390)
          (set! error1.2 29502)
          (set! tmp.53 25390))
        (begin
          (set! rdi tmp.53)
          (set! r15 tmp-ra.55)
          (jump L.fun/ascii-char8415.4 rbp r15 rdi))))))
(check-by-interp
 '(module
    ((new-frames (() () ())))
    (define L.-.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.54 (bitwise-and tmp.22 7)) (= tmp.54 0))
                  (set! tmp.53 14)
                  (set! tmp.53 6))
                (!= tmp.53 6))
            (if (begin
                  (if (begin (set! tmp.56 (bitwise-and tmp.21 7)) (= tmp.56 0))
                    (set! tmp.55 14)
                    (set! tmp.55 6))
                  (!= tmp.55 6))
              (begin (set! rax (- tmp.21 tmp.22)) (jump tmp-ra.61 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.61 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.61 rbp rax))))))
    (define L.+.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.20 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.19 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin (set! rax (+ tmp.19 tmp.20)) (jump tmp-ra.62 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.62 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.62 rbp rax))))))
    (define L.fun/empty8418.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.63 rbp rax)))))
    (define L.fun/ascii-char8419.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.64 rbp rax)))))
    (begin
      (set! tmp-ra.65 r15)
      (begin
        (begin
          (return-point L.rp.9
            (begin
              (set! rsi 912)
              (set! rdi 760)
              (set! r15 L.rp.9)
              (jump L.+.7 rbp r15 rdi rsi)))
          (set! fixnum0.3 rax))
        (begin
          (return-point L.rp.10
            (begin
              (set! rsi 1664)
              (set! rdi 384)
              (set! r15 L.rp.10)
              (jump L.-.8 rbp r15 rdi rsi)))
          (set! fixnum1.2 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/empty8418.4 rbp r15)))
          (set! empty2.1 rax))
        (begin (set! r15 tmp-ra.65) (jump L.fun/ascii-char8419.5 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() ())))
    (define L.fun/ascii-char8467.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin
          (set! oprand0.2 rdi)
          (set! oprand1.1 rsi)
          (begin (set! rax oprand0.2) (jump tmp-ra.63 rbp rax)))))
    (define L.fun/void8469.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin
          (set! oprand0.4 rdi)
          (set! oprand1.3 rsi)
          (begin (set! rax 30) (jump tmp-ra.64 rbp rax)))))
    (define L.fun/empty8468.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin
          (set! oprand0.6 rdi)
          (set! oprand1.5 rsi)
          (begin (set! rax 22) (jump tmp-ra.65 rbp rax)))))
    (begin
      (set! tmp-ra.66 r15)
      (begin
        (begin
          (set! ascii-char0.9 25390)
          (set! empty1.8 22)
          (set! error2.7 15166)
          (set! tmp.59 25390))
        (begin
          (begin
            (begin
              (return-point L.rp.8
                (begin
                  (set! rsi 6)
                  (set! rdi 30)
                  (set! r15 L.rp.8)
                  (jump L.fun/void8469.5 rbp r15 rdi rsi)))
              (set! tmp.61 rax))
            (begin
              (if (!= 6 6) (set! tmp.62 43582) (set! tmp.62 65086))
              (begin
                (return-point L.rp.9
                  (begin
                    (set! rsi tmp.62)
                    (set! rdi tmp.61)
                    (set! r15 L.rp.9)
                    (jump L.fun/empty8468.6 rbp r15 rdi rsi)))
                (set! tmp.60 rax))))
          (begin
            (set! rsi tmp.60)
            (set! rdi tmp.59)
            (set! r15 tmp-ra.66)
            (jump L.fun/ascii-char8467.4 rbp r15 rdi rsi)))))))
(check-by-interp
 '(module
    ((new-frames (() ())))
    (define L.*.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin
          (set! tmp.17 rdi)
          (set! tmp.18 rsi)
          (if (begin
                (if (begin (set! tmp.54 (bitwise-and tmp.18 7)) (= tmp.54 0))
                  (set! tmp.53 14)
                  (set! tmp.53 6))
                (!= tmp.53 6))
            (if (begin
                  (if (begin (set! tmp.56 (bitwise-and tmp.17 7)) (= tmp.56 0))
                    (set! tmp.55 14)
                    (set! tmp.55 6))
                  (!= tmp.55 6))
              (begin
                (set! tmp.57 (arithmetic-shift-right tmp.18 3))
                (begin (set! rax (* tmp.17 tmp.57)) (jump tmp-ra.68 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.68 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.68 rbp rax))))))
    (define L.-.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                  (set! tmp.58 14)
                  (set! tmp.58 6))
                (!= tmp.58 6))
            (if (begin
                  (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                    (set! tmp.60 14)
                    (set! tmp.60 6))
                  (!= tmp.60 6))
              (begin (set! rax (- tmp.21 tmp.22)) (jump tmp-ra.69 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.69 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.69 rbp rax))))))
    (begin
      (set! tmp-ra.70 r15)
      (begin
        (begin
          (if (!= 6 6) (set! tmp.63 1712) (set! tmp.63 688))
          (begin
            (if (!= 14 6) (set! tmp.64 1480) (set! tmp.64 128))
            (begin
              (return-point L.rp.7
                (begin
                  (set! rsi tmp.64)
                  (set! rdi tmp.63)
                  (set! r15 L.rp.7)
                  (jump L.-.5 rbp r15 rdi rsi)))
              (set! tmp.62 rax))))
        (begin
          (begin
            (if (!= 6 6) (set! tmp.66 352) (set! tmp.66 264))
            (begin
              (begin
                (set! ascii-char0.3 25390)
                (set! error1.2 13118)
                (set! empty2.1 22)
                (set! tmp.67 608))
              (begin
                (return-point L.rp.8
                  (begin
                    (set! rsi tmp.67)
                    (set! rdi tmp.66)
                    (set! r15 L.rp.8)
                    (jump L.-.5 rbp r15 rdi rsi)))
                (set! tmp.65 rax))))
          (begin
            (set! rsi tmp.65)
            (set! rdi tmp.62)
            (set! r15 tmp-ra.70)
            (jump L.*.6 rbp r15 rdi rsi)))))))
(check-by-interp
 '(module
    ((new-frames (() () ())))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin
          (set! tmp.17 rdi)
          (set! tmp.18 rsi)
          (if (begin
                (if (begin (set! tmp.54 (bitwise-and tmp.18 7)) (= tmp.54 0))
                  (set! tmp.53 14)
                  (set! tmp.53 6))
                (!= tmp.53 6))
            (if (begin
                  (if (begin (set! tmp.56 (bitwise-and tmp.17 7)) (= tmp.56 0))
                    (set! tmp.55 14)
                    (set! tmp.55 6))
                  (!= tmp.55 6))
              (begin
                (set! tmp.57 (arithmetic-shift-right tmp.18 3))
                (begin (set! rax (* tmp.17 tmp.57)) (jump tmp-ra.67 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.67 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.67 rbp rax))))))
    (define L.-.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                  (set! tmp.58 14)
                  (set! tmp.58 6))
                (!= tmp.58 6))
            (if (begin
                  (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                    (set! tmp.60 14)
                    (set! tmp.60 6))
                  (!= tmp.60 6))
              (begin (set! rax (- tmp.21 tmp.22)) (jump tmp-ra.68 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.68 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.68 rbp rax))))))
    (define L.vector?.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (set! tmp.47 rdi)
          (if (begin (set! tmp.62 (bitwise-and tmp.47 7)) (= tmp.62 3))
            (begin (set! rax 14) (jump tmp-ra.69 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.69 rbp rax))))))
    (define L.fun/any8734.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.70 r15)
        (begin
          (set! oprand0.3 rdi)
          (set! oprand1.2 rsi)
          (set! oprand2.1 rdx)
          (begin (set! rax 41278) (jump tmp-ra.70 rbp rax)))))
    (begin
      (set! tmp-ra.71 r15)
      (if (begin
            (begin
              (begin
                (return-point L.rp.9
                  (begin
                    (set! rdx 25390)
                    (set! rsi 22)
                    (set! rdi 5950)
                    (set! r15 L.rp.9)
                    (jump L.fun/any8734.4 rbp r15 rdi rsi rdx)))
                (set! tmp.64 rax))
              (begin
                (return-point L.rp.10
                  (begin
                    (set! rdi tmp.64)
                    (set! r15 L.rp.10)
                    (jump L.vector?.6 rbp r15 rdi)))
                (set! tmp.63 rax)))
            (!= tmp.63 6))
        (begin
          (begin
            (return-point L.rp.11
              (begin
                (set! rsi 528)
                (set! rdi 1480)
                (set! r15 L.rp.11)
                (jump L.-.7 rbp r15 rdi rsi)))
            (set! tmp.65 rax))
          (begin
            (if (!= 14 6) (set! tmp.66 1488) (set! tmp.66 1392))
            (begin
              (set! rsi tmp.66)
              (set! rdi tmp.65)
              (set! r15 tmp-ra.71)
              (jump L.*.8 rbp r15 rdi rsi))))
        (if (!= 14 6)
          (begin (set! rax 1552) (jump tmp-ra.71 rbp rax))
          (begin (set! rax 1376) (jump tmp-ra.71 rbp rax)))))))
(check-by-interp
 '(module
    ((new-frames (() () () ())))
    (define L.fun/void8780.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.54 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.54 rbp rax)))))
    (define L.fun/ascii-char8782.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/void8781.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/error8783.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 35646) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/void8779.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.58 rbp rax)))))
    (begin
      (set! tmp-ra.59 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/void8779.8 rbp r15)))
          (set! void0.4 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/void8780.4 rbp r15)))
          (set! void1.3 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/void8781.6 rbp r15)))
          (set! void2.2 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/ascii-char8782.5 rbp r15)))
          (set! ascii-char3.1 rax))
        (begin (set! r15 tmp-ra.59) (jump L.fun/error8783.7 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () ())))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.71 r15)
        (begin
          (set! tmp.18 rdi)
          (set! tmp.19 rsi)
          (if (begin
                (if (begin (set! tmp.55 (bitwise-and tmp.19 7)) (= tmp.55 0))
                  (set! tmp.54 14)
                  (set! tmp.54 6))
                (!= tmp.54 6))
            (if (begin
                  (if (begin (set! tmp.57 (bitwise-and tmp.18 7)) (= tmp.57 0))
                    (set! tmp.56 14)
                    (set! tmp.56 6))
                  (!= tmp.56 6))
              (begin
                (set! tmp.58 (arithmetic-shift-right tmp.19 3))
                (begin (set! rax (* tmp.18 tmp.58)) (jump tmp-ra.71 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.71 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.71 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.21 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.20 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (+ tmp.20 tmp.21)) (jump tmp-ra.72 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.72 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.72 rbp rax))))))
    (define L.-.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.23 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.22 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin (set! rax (- tmp.22 tmp.23)) (jump tmp-ra.73 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.73 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.73 rbp rax))))))
    (define L.fun/empty8806.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.74 rbp rax)))))
    (define L.fun/void8807.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.75 rbp rax)))))
    (define L.fun/ascii-char8808.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.76 rbp rax)))))
    (begin
      (set! tmp-ra.77 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/empty8806.4 rbp r15)))
          (set! empty0.4 rax))
        (begin
          (begin
            (return-point L.rp.12
              (begin
                (set! rsi 864)
                (set! rdi 1608)
                (set! r15 L.rp.12)
                (jump L.-.8 rbp r15 rdi rsi)))
            (set! tmp.67 rax))
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 352)
                  (set! rdi 328)
                  (set! r15 L.rp.13)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! tmp.68 rax))
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi tmp.68)
                  (set! rdi tmp.67)
                  (set! r15 L.rp.14)
                  (jump L.-.8 rbp r15 rdi rsi)))
              (set! fixnum1.3 rax))))
        (begin
          (begin
            (return-point L.rp.15
              (begin
                (set! rsi 1048)
                (set! rdi 320)
                (set! r15 L.rp.15)
                (jump L.*.10 rbp r15 rdi rsi)))
            (set! tmp.69 rax))
          (begin
            (begin
              (return-point L.rp.16
                (begin
                  (set! rsi 968)
                  (set! rdi 1096)
                  (set! r15 L.rp.16)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! tmp.70 rax))
            (begin
              (return-point L.rp.17
                (begin
                  (set! rsi tmp.70)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.17)
                  (jump L.-.8 rbp r15 rdi rsi)))
              (set! fixnum2.2 rax))))
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/void8807.5 rbp r15)))
          (set! void3.1 rax))
        (begin (set! r15 tmp-ra.77) (jump L.fun/ascii-char8808.6 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () ())))
    (define L.error?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin
          (set! tmp.59 rdi)
          (if (begin (set! tmp.67 (bitwise-and tmp.59 255)) (= tmp.67 62))
            (begin (set! rax 14) (jump tmp-ra.78 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.78 rbp rax))))))
    (define L.fixnum?.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin
          (set! tmp.54 rdi)
          (if (begin (set! tmp.68 (bitwise-and tmp.54 7)) (= tmp.68 0))
            (begin (set! rax 14) (jump tmp-ra.79 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.79 rbp rax))))))
    (define L.+.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin
          (set! tmp.33 rdi)
          (set! tmp.34 rsi)
          (if (begin
                (if (begin (set! tmp.70 (bitwise-and tmp.34 7)) (= tmp.70 0))
                  (set! tmp.69 14)
                  (set! tmp.69 6))
                (!= tmp.69 6))
            (if (begin
                  (if (begin (set! tmp.72 (bitwise-and tmp.33 7)) (= tmp.72 0))
                    (set! tmp.71 14)
                    (set! tmp.71 6))
                  (!= tmp.71 6))
              (begin (set! rax (+ tmp.33 tmp.34)) (jump tmp-ra.80 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.80 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.80 rbp rax))))))
    (define L.vector?.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin
          (set! tmp.61 rdi)
          (if (begin (set! tmp.73 (bitwise-and tmp.61 7)) (= tmp.73 3))
            (begin (set! rax 14) (jump tmp-ra.81 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.81 rbp rax))))))
    (define L.fun/empty9034.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! oprand0.3 rdi)
          (set! oprand1.2 rsi)
          (set! oprand2.1 rdx)
          (begin (set! rax 22) (jump tmp-ra.82 rbp rax)))))
    (define L.fun/error9036.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 47166) (jump tmp-ra.83 rbp rax)))))
    (define L.fun/error9035.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin (begin (set! rax 36414) (jump tmp-ra.84 rbp rax)))))
    (define L.fun/void9037.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.85 rbp rax)))))
    (define L.fun/ascii-char9032.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/empty9038.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.87 rbp rax)))))
    (define L.fun/ascii-char9031.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! oprand0.6 rdi)
          (set! oprand1.5 rsi)
          (set! oprand2.4 rdx)
          (begin (set! r15 tmp-ra.88) (jump L.fun/ascii-char9032.8 rbp r15)))))
    (define L.fun/boolean9033.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (set! oprand0.9 rdi)
          (set! oprand1.8 rsi)
          (set! oprand2.7 rdx)
          (begin (set! rax 14) (jump tmp-ra.89 rbp rax)))))
    (begin
      (set! tmp-ra.90 r15)
      (begin
        (if (begin
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rdx 25390)
                    (set! rsi 44094)
                    (set! rdi 25390)
                    (set! r15 L.rp.17)
                    (jump L.fun/boolean9033.11 rbp r15 rdi rsi rdx)))
                (set! tmp.75 rax))
              (!= tmp.75 6))
          (if (!= 6 6) (set! tmp.74 22) (set! tmp.74 22))
          (begin
            (return-point L.rp.18
              (begin
                (set! rdx 14)
                (set! rsi 59454)
                (set! rdi 4158)
                (set! r15 L.rp.18)
                (jump L.fun/empty9034.4 rbp r15 rdi rsi rdx)))
            (set! tmp.74 rax)))
        (begin
          (begin
            (begin
              (return-point L.rp.19
                (begin
                  (set! rdi 25390)
                  (set! r15 L.rp.19)
                  (jump L.vector?.13 rbp r15 rdi)))
              (set! boolean0.13 rax))
            (begin
              (return-point L.rp.20
                (begin
                  (set! rsi 192)
                  (set! rdi 224)
                  (set! r15 L.rp.20)
                  (jump L.+.14 rbp r15 rdi rsi)))
              (set! fixnum1.12 rax))
            (begin
              (return-point L.rp.21
                (begin (set! r15 L.rp.21) (jump L.fun/error9035.6 rbp r15)))
              (set! error2.11 rax))
            (begin
              (return-point L.rp.22
                (begin (set! r15 L.rp.22) (jump L.fun/error9036.5 rbp r15)))
              (set! error3.10 rax))
            (begin
              (return-point L.rp.23
                (begin
                  (set! rdi 30)
                  (set! r15 L.rp.23)
                  (jump L.fixnum?.15 rbp r15 rdi)))
              (set! tmp.76 rax)))
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rdx 14)
                    (set! rsi 6)
                    (set! rdi 22)
                    (set! r15 L.rp.24)
                    (jump L.fun/ascii-char9031.10 rbp r15 rdi rsi rdx)))
                (set! ascii-char0.17 rax))
              (begin
                (return-point L.rp.25
                  (begin (set! r15 L.rp.25) (jump L.fun/void9037.7 rbp r15)))
                (set! void1.16 rax))
              (begin
                (return-point L.rp.26
                  (begin
                    (set! rdx 6)
                    (set! rsi 14)
                    (set! rdi 22)
                    (set! r15 L.rp.26)
                    (jump L.fun/ascii-char9031.10 rbp r15 rdi rsi rdx)))
                (set! ascii-char2.15 rax))
              (begin
                (return-point L.rp.27
                  (begin (set! r15 L.rp.27) (jump L.fun/empty9038.9 rbp r15)))
                (set! empty3.14 rax))
              (begin
                (return-point L.rp.28
                  (begin
                    (set! rdi 30)
                    (set! r15 L.rp.28)
                    (jump L.error?.16 rbp r15 rdi)))
                (set! tmp.77 rax)))
            (begin
              (set! rdx tmp.77)
              (set! rsi tmp.76)
              (set! rdi tmp.74)
              (set! r15 tmp-ra.90)
              (jump L.fun/ascii-char9031.10 rbp r15 rdi rsi rdx))))))))
(check-by-interp
 '(module
    ((new-frames (() () () ())))
    (define L.fun/void9076.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.54 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.54 rbp rax)))))
    (define L.fun/ascii-char9078.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/error9075.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 57662) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/empty9077.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.57 rbp rax)))))
    (begin
      (set! tmp-ra.58 r15)
      (begin
        (begin
          (return-point L.rp.9
            (begin (set! r15 L.rp.9) (jump L.fun/error9075.6 rbp r15)))
          (set! error0.4 rax))
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/void9076.4 rbp r15)))
          (set! void1.3 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/empty9077.7 rbp r15)))
          (set! empty2.2 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/ascii-char9078.5 rbp r15)))
          (set! ascii-char3.1 rax))
        (begin (set! rax empty2.2) (jump tmp-ra.58 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.54 r15)
      (if (if (!= 6 6) (!= 14 6) (!= 14 6))
        (begin
          (set! fixnum0.4 1720)
          (set! fixnum1.3 984)
          (set! void2.2 30)
          (set! ascii-char3.1 25390)
          (begin (set! rax fixnum0.4) (jump tmp-ra.54 rbp rax)))
        (if (!= 6 6)
          (begin (set! rax 1448) (jump tmp-ra.54 rbp rax))
          (begin (set! rax 1416) (jump tmp-ra.54 rbp rax)))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.boolean?.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin
          (set! tmp.42 rdi)
          (if (begin (set! tmp.54 (bitwise-and tmp.42 247)) (= tmp.54 6))
            (begin (set! rax 14) (jump tmp-ra.56 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.56 rbp rax))))))
    (define L.fun/error9365.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 62014) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/error9364.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 63806) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/any9366.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 14) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/error9363.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 60990) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/empty9362.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.61 rbp rax)))))
    (begin
      (set! tmp-ra.62 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/empty9362.8 rbp r15)))
          (set! empty0.4 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/error9363.7 rbp r15)))
          (set! error1.3 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/error9364.5 rbp r15)))
          (set! error2.2 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/error9365.4 rbp r15)))
          (set! error3.1 rax))
        (begin
          (begin
            (return-point L.rp.15
              (begin (set! r15 L.rp.15) (jump L.fun/any9366.6 rbp r15)))
            (set! tmp.55 rax))
          (begin
            (set! rdi tmp.55)
            (set! r15 tmp-ra.62)
            (jump L.boolean?.10 rbp r15 rdi)))))))
(check-by-interp
 '(module
    ((new-frames (() () () ())))
    (define L.fun/ascii-char9574.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.54 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.54 rbp rax)))))
    (define L.fun/void9575.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/void9573.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/empty9572.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.57 rbp rax)))))
    (begin
      (set! tmp-ra.58 r15)
      (begin
        (begin
          (return-point L.rp.9
            (begin (set! r15 L.rp.9) (jump L.fun/empty9572.7 rbp r15)))
          (set! empty0.4 rax))
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/void9573.6 rbp r15)))
          (set! void1.3 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char9574.4 rbp r15)))
          (set! ascii-char2.2 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/void9575.5 rbp r15)))
          (set! void3.1 rax))
        (begin (set! rax void1.3) (jump tmp-ra.58 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames ()))
    (begin
      (set! tmp-ra.58 r15)
      (if (begin
            (set! empty0.4 22)
            (set! fixnum1.3 968)
            (set! void2.2 30)
            (set! fixnum3.1 896)
            (!= 6 6))
        (if (!= 14 6)
          (begin (set! rax 22) (jump tmp-ra.58 rbp rax))
          (begin (set! rax 22) (jump tmp-ra.58 rbp rax)))
        (begin
          (set! empty0.8 22)
          (set! error1.7 42302)
          (set! void2.6 30)
          (set! fixnum3.5 992)
          (begin (set! rax 22) (jump tmp-ra.58 rbp rax)))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.-.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.55 (bitwise-and tmp.23 7)) (= tmp.55 0))
                  (set! tmp.54 14)
                  (set! tmp.54 6))
                (!= tmp.54 6))
            (if (begin
                  (if (begin (set! tmp.57 (bitwise-and tmp.22 7)) (= tmp.57 0))
                    (set! tmp.56 14)
                    (set! tmp.56 6))
                  (!= tmp.56 6))
              (begin (set! rax (- tmp.22 tmp.23)) (jump tmp-ra.65 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.65 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.65 rbp rax))))))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin
          (set! tmp.18 rdi)
          (set! tmp.19 rsi)
          (if (begin
                (if (begin (set! tmp.59 (bitwise-and tmp.19 7)) (= tmp.59 0))
                  (set! tmp.58 14)
                  (set! tmp.58 6))
                (!= tmp.58 6))
            (if (begin
                  (if (begin (set! tmp.61 (bitwise-and tmp.18 7)) (= tmp.61 0))
                    (set! tmp.60 14)
                    (set! tmp.60 6))
                  (!= tmp.60 6))
              (begin
                (set! tmp.62 (arithmetic-shift-right tmp.19 3))
                (begin (set! rax (* tmp.18 tmp.62)) (jump tmp-ra.66 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.66 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.66 rbp rax))))))
    (define L.fun/ascii-char9904.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.67 rbp rax)))))
    (define L.fun/void9902.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.68 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.68 rbp rax)))))
    (define L.fun/ascii-char9903.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.69 rbp rax)))))
    (begin
      (set! tmp-ra.70 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/void9902.5 rbp r15)))
          (set! void0.4 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char9903.6 rbp r15)))
          (set! ascii-char1.3 rax))
        (begin
          (begin
            (return-point L.rp.12
              (begin
                (set! rsi 1592)
                (set! rdi 504)
                (set! r15 L.rp.12)
                (jump L.*.8 rbp r15 rdi rsi)))
            (set! tmp.63 rax))
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 1320)
                  (set! rdi 552)
                  (set! r15 L.rp.13)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.64 rax))
            (begin
              (return-point L.rp.14
                (begin
                  (set! rsi tmp.64)
                  (set! rdi tmp.63)
                  (set! r15 L.rp.14)
                  (jump L.-.9 rbp r15 rdi rsi)))
              (set! fixnum2.2 rax))))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/ascii-char9904.4 rbp r15)))
          (set! ascii-char3.1 rax))
        (begin (set! rax void0.4) (jump tmp-ra.70 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () () () () () () ())))
    (define L.*.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.20 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.19 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin
                (set! tmp.59 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.59)) (jump tmp-ra.80 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.80 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.80 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.24 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.23 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.81 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.81 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.81 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.22 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.21 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.82 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.82 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.82 rbp rax))))))
    (define L.fun/void10674.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.83 rbp rax)))))
    (define L.fun/error10675.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin (begin (set! rax 64318) (jump tmp-ra.84 rbp rax)))))
    (define L.fun/void10672.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.85 rbp rax)))))
    (define L.fun/empty10673.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.86 rbp rax)))))
    (begin
      (set! tmp-ra.87 r15)
      (begin
        (begin
          (begin
            (begin
              (return-point L.rp.12
                (begin
                  (set! rsi 1560)
                  (set! rdi 1112)
                  (set! r15 L.rp.12)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.13
                  (begin
                    (set! rsi 704)
                    (set! rdi 904)
                    (set! r15 L.rp.13)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.14)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi 744)
                    (set! rdi 216)
                    (set! r15 L.rp.15)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.16
                    (begin
                      (set! rsi 1952)
                      (set! rdi 1832)
                      (set! r15 L.rp.16)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.17)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.18)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum0.5 rax))))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/void10672.6 rbp r15)))
          (set! void1.4 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/empty10673.7 rbp r15)))
          (set! empty2.3 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void10674.4 rbp r15)))
          (set! void3.2 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.22
                (begin
                  (set! rsi 1024)
                  (set! rdi 760)
                  (set! r15 L.rp.22)
                  (jump L.*.11 rbp r15 rdi rsi)))
              (set! tmp.75 rax))
            (begin
              (begin
                (return-point L.rp.23
                  (begin
                    (set! rsi 160)
                    (set! rdi 1344)
                    (set! r15 L.rp.23)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.76 rax))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.76)
                    (set! rdi tmp.75)
                    (set! r15 L.rp.24)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.74 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.25
                  (begin
                    (set! rsi 648)
                    (set! rdi 1136)
                    (set! r15 L.rp.25)
                    (jump L.*.11 rbp r15 rdi rsi)))
                (set! tmp.78 rax))
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 224)
                      (set! rdi 736)
                      (set! r15 L.rp.26)
                      (jump L.*.11 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi tmp.79)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.27)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.28
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.74)
                  (set! r15 L.rp.28)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum4.1 rax))))
        (begin (set! r15 tmp-ra.87) (jump L.fun/error10675.5 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () ())))
    (define L.-.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.71 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.71 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.71 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.71 rbp rax))))))
    (define L.+.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.72 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.72 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.72 rbp rax))))))
    (define L.ascii-char?.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin
          (set! tmp.46 rdi)
          (if (begin (set! tmp.63 (bitwise-and tmp.46 255)) (= tmp.63 46))
            (begin (set! rax 14) (jump tmp-ra.73 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.73 rbp rax))))))
    (define L.fun/any10855.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.74 rbp rax)))))
    (define L.fun/ascii-char10854.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.75 rbp rax)))))
    (define L.fun/empty10856.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.76 rbp rax)))))
    (define L.fun/void10857.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.77 rbp rax)))))
    (begin
      (set! tmp-ra.78 r15)
      (begin
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/ascii-char10854.5 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (begin
            (return-point L.rp.13
              (begin (set! r15 L.rp.13) (jump L.fun/any10855.4 rbp r15)))
            (set! tmp.64 rax))
          (begin
            (return-point L.rp.14
              (begin
                (set! rdi tmp.64)
                (set! r15 L.rp.14)
                (jump L.ascii-char?.9 rbp r15 rdi)))
            (set! boolean1.4 rax)))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/empty10856.6 rbp r15)))
          (set! empty2.3 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.16
                (begin
                  (set! rsi 784)
                  (set! rdi 1352)
                  (set! r15 L.rp.16)
                  (jump L.+.10 rbp r15 rdi rsi)))
              (set! tmp.66 rax))
            (begin
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rsi 1792)
                    (set! rdi 8)
                    (set! r15 L.rp.17)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.67 rax))
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi tmp.67)
                    (set! rdi tmp.66)
                    (set! r15 L.rp.18)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.65 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 992)
                    (set! rdi 112)
                    (set! r15 L.rp.19)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.69 rax))
              (begin
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi 48)
                      (set! rdi 1104)
                      (set! r15 L.rp.20)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi tmp.70)
                      (set! rdi tmp.69)
                      (set! r15 L.rp.21)
                      (jump L.+.10 rbp r15 rdi rsi)))
                  (set! tmp.68 rax))))
            (begin
              (return-point L.rp.22
                (begin
                  (set! rsi tmp.68)
                  (set! rdi tmp.65)
                  (set! r15 L.rp.22)
                  (jump L.+.10 rbp r15 rdi rsi)))
              (set! fixnum3.2 rax))))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/void10857.7 rbp r15)))
          (set! void4.1 rax))
        (begin (set! rax empty2.3) (jump tmp-ra.78 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.fun/void11402.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/ascii-char11403.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/ascii-char11400.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/error11401.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 15166) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/ascii-char11399.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.59 rbp rax)))))
    (begin
      (set! tmp-ra.60 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/ascii-char11399.8 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char11400.6 rbp r15)))
          (set! ascii-char1.4 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/error11401.7 rbp r15)))
          (set! error2.3 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/void11402.4 rbp r15)))
          (set! void3.2 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/ascii-char11403.5 rbp r15)))
          (set! ascii-char4.1 rax))
        (begin (set! rax void3.2) (jump tmp-ra.60 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.fun/void13621.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/ascii-char13620.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/empty13623.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/error13618.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 22846) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/void13619.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.59 rbp rax)))))
    (define L.fun/error13622.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 25150) (jump tmp-ra.60 rbp rax)))))
    (begin
      (set! tmp-ra.61 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/error13618.7 rbp r15)))
          (set! error0.5 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/void13619.8 rbp r15)))
          (set! void1.4 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/ascii-char13620.5 rbp r15)))
          (set! ascii-char2.3 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/void13621.4 rbp r15)))
          (set! void3.2 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/error13622.9 rbp r15)))
          (set! error4.1 rax))
        (begin (set! r15 tmp-ra.61) (jump L.fun/empty13623.6 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () () () () () () ())))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.80 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.80 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.80 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.81 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.81 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.81 rbp rax))))))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.82 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.82 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.82 rbp rax))))))
    (define L.fun/void14228.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.83 rbp rax)))))
    (define L.fun/empty14229.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.84 rbp rax)))))
    (define L.fun/ascii-char14227.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.85 rbp rax)))))
    (begin
      (set! tmp-ra.86 r15)
      (begin
        (begin
          (begin
            (begin
              (return-point L.rp.11
                (begin
                  (set! rsi 1328)
                  (set! rdi 1512)
                  (set! r15 L.rp.11)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.12
                  (begin
                    (set! rsi 1096)
                    (set! rdi 1088)
                    (set! r15 L.rp.12)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.13
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.13)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi 832)
                    (set! rdi 1528)
                    (set! r15 L.rp.14)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.15
                    (begin
                      (set! rsi 1744)
                      (set! rdi 192)
                      (set! r15 L.rp.15)
                      (jump L.*.8 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.16
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.16)
                      (jump L.*.8 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.17
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.17)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! fixnum0.5 rax))))
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/ascii-char14227.6 rbp r15)))
          (set! ascii-char1.4 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.19
                (begin
                  (set! rsi 1440)
                  (set! rdi 880)
                  (set! r15 L.rp.19)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! tmp.75 rax))
            (begin
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi 320)
                    (set! rdi 632)
                    (set! r15 L.rp.20)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.76 rax))
              (begin
                (return-point L.rp.21
                  (begin
                    (set! rsi tmp.76)
                    (set! rdi tmp.75)
                    (set! r15 L.rp.21)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.74 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 360)
                    (set! rdi 1688)
                    (set! r15 L.rp.22)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.78 rax))
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 1952)
                      (set! rdi 1800)
                      (set! r15 L.rp.23)
                      (jump L.+.9 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi tmp.79)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.24)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.25
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.74)
                  (set! r15 L.rp.25)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! fixnum2.3 rax))))
        (begin
          (return-point L.rp.26
            (begin (set! r15 L.rp.26) (jump L.fun/void14228.4 rbp r15)))
          (set! void3.2 rax))
        (begin
          (return-point L.rp.27
            (begin (set! r15 L.rp.27) (jump L.fun/empty14229.5 rbp r15)))
          (set! empty4.1 rax))
        (begin (set! rax empty4.1) (jump tmp-ra.86 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.-.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.74 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.74 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.74 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.75 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.75 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.75 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.76 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.76 rbp rax))))))
    (define L.fun/void14544.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.77 rbp rax)))))
    (define L.fun/empty14543.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.78 rbp rax)))))
    (define L.fun/ascii-char14546.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.79 rbp rax)))))
    (define L.fun/error14545.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 62) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/error14542.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 38462) (jump tmp-ra.81 rbp rax)))))
    (begin
      (set! tmp-ra.82 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/error14542.8 rbp r15)))
          (set! error0.5 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/empty14543.5 rbp r15)))
          (set! empty1.4 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.15
                (begin
                  (set! rsi 288)
                  (set! rdi 1248)
                  (set! r15 L.rp.15)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 552)
                    (set! rdi 856)
                    (set! r15 L.rp.16)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.17
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.17)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 1056)
                    (set! rdi 824)
                    (set! r15 L.rp.18)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 1720)
                      (set! rdi 608)
                      (set! r15 L.rp.19)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.20)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.21
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.21)
                  (jump L.-.12 rbp r15 rdi rsi)))
              (set! fixnum2.3 rax))))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/void14544.4 rbp r15)))
          (set! void3.2 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/error14545.7 rbp r15)))
          (set! error4.1 rax))
        (begin (set! r15 tmp-ra.82) (jump L.fun/ascii-char14546.6 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.-.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.70 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.70 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.70 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.70 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.71 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.20 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.19 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin
                (set! tmp.63 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.63)) (jump tmp-ra.71 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.71 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.71 rbp rax))))))
    (define L.fun/empty15112.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.72 rbp rax)))))
    (define L.fun/error15114.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin (begin (set! rax 11326) (jump tmp-ra.73 rbp rax)))))
    (define L.fun/empty15111.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.74 rbp rax)))))
    (define L.fun/ascii-char15115.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.75 rbp rax)))))
    (define L.fun/void15113.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.76 rbp rax)))))
    (begin
      (set! tmp-ra.77 r15)
      (begin
        (begin
          (begin
            (begin
              (return-point L.rp.12
                (begin
                  (set! rsi 1392)
                  (set! rdi 1304)
                  (set! r15 L.rp.12)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! tmp.65 rax))
            (begin
              (begin
                (return-point L.rp.13
                  (begin
                    (set! rsi 368)
                    (set! rdi 1800)
                    (set! r15 L.rp.13)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.66 rax))
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi tmp.66)
                    (set! rdi tmp.65)
                    (set! r15 L.rp.14)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.64 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi 1944)
                    (set! rdi 1672)
                    (set! r15 L.rp.15)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.68 rax))
              (begin
                (begin
                  (return-point L.rp.16
                    (begin
                      (set! rsi 1864)
                      (set! rdi 1184)
                      (set! r15 L.rp.16)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.69 rax))
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi tmp.69)
                      (set! rdi tmp.68)
                      (set! r15 L.rp.17)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.67 rax))))
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi tmp.67)
                  (set! rdi tmp.64)
                  (set! r15 L.rp.18)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! fixnum0.5 rax))))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/empty15111.6 rbp r15)))
          (set! empty1.4 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/empty15112.4 rbp r15)))
          (set! empty2.3 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void15113.8 rbp r15)))
          (set! void3.2 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/error15114.5 rbp r15)))
          (set! error4.1 rax))
        (begin (set! r15 tmp-ra.77) (jump L.fun/ascii-char15115.7 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.fixnum?.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.42 rdi)
          (if (begin (set! tmp.55 (bitwise-and tmp.42 7)) (= tmp.55 0))
            (begin (set! rax 14) (jump tmp-ra.88 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.88 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.24 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.23 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.89 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.89 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.89 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.22 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.21 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.90 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.90 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.90 rbp rax))))))
    (define L.*.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.20 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.19 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.68)) (jump tmp-ra.91 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.91 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.91 rbp rax))))))
    (define L.fun/empty16025.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/ascii-char16027.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/any16026.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.94 rbp rax)))))
    (begin
      (set! tmp-ra.95 r15)
      (begin
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/empty16025.4 rbp r15)))
          (set! empty0.5 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 224)
                  (set! rdi 56)
                  (set! r15 L.rp.13)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.70 rax))
            (begin
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi 384)
                    (set! rdi 336)
                    (set! r15 L.rp.14)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi tmp.71)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.15)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 672)
                    (set! rdi 1936)
                    (set! r15 L.rp.16)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1760)
                      (set! rdi 1096)
                      (set! r15 L.rp.17)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.18)
                      (jump L.*.8 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (return-point L.rp.19
                (begin
                  (set! rsi tmp.72)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.19)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum1.4 rax))))
        (begin
          (begin
            (begin
              (return-point L.rp.20
                (begin
                  (set! rsi 1408)
                  (set! rdi 56)
                  (set! r15 L.rp.20)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.76 rax))
            (begin
              (begin
                (return-point L.rp.21
                  (begin
                    (set! rsi 528)
                    (set! rdi 8)
                    (set! r15 L.rp.21)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.77 rax))
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi tmp.77)
                    (set! rdi tmp.76)
                    (set! r15 L.rp.22)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.75 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.23
                  (begin
                    (set! rsi 376)
                    (set! rdi 472)
                    (set! r15 L.rp.23)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.79 rax))
              (begin
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi 656)
                      (set! rdi 1000)
                      (set! r15 L.rp.24)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.25)
                      (jump L.*.8 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.26
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.75)
                  (set! r15 L.rp.26)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! fixnum2.3 rax))))
        (begin
          (begin
            (begin
              (return-point L.rp.27
                (begin
                  (set! rsi 464)
                  (set! rdi 1064)
                  (set! r15 L.rp.27)
                  (jump L.*.8 rbp r15 rdi rsi)))
              (set! tmp.82 rax))
            (begin
              (begin
                (return-point L.rp.28
                  (begin
                    (set! rsi 1504)
                    (set! rdi 1088)
                    (set! r15 L.rp.28)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.83 rax))
              (begin
                (return-point L.rp.29
                  (begin
                    (set! rsi tmp.83)
                    (set! rdi tmp.82)
                    (set! r15 L.rp.29)
                    (jump L.*.8 rbp r15 rdi rsi)))
                (set! tmp.81 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.30
                  (begin
                    (set! rsi 1048)
                    (set! rdi 1328)
                    (set! r15 L.rp.30)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi 560)
                      (set! rdi 1832)
                      (set! r15 L.rp.31)
                      (jump L.*.8 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.32)
                      (jump L.*.8 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (return-point L.rp.33
                (begin
                  (set! rsi tmp.84)
                  (set! rdi tmp.81)
                  (set! r15 L.rp.33)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! fixnum3.2 rax))))
        (begin
          (begin
            (return-point L.rp.34
              (begin (set! r15 L.rp.34) (jump L.fun/any16026.6 rbp r15)))
            (set! tmp.87 rax))
          (begin
            (return-point L.rp.35
              (begin
                (set! rdi tmp.87)
                (set! r15 L.rp.35)
                (jump L.fixnum?.11 rbp r15 rdi)))
            (set! boolean4.1 rax)))
        (begin (set! r15 tmp-ra.95) (jump L.fun/ascii-char16027.5 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.*.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.20 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.19 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin
                (set! tmp.59 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.59)) (jump tmp-ra.74 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.74 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.74 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.22 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.21 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.75 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.75 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.75 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.24 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.23 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.76 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.76 rbp rax))))))
    (define L.fun/ascii-char19237.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.77 rbp rax)))))
    (define L.fun/ascii-char19236.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.78 rbp rax)))))
    (define L.fun/error19239.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin (begin (set! rax 55358) (jump tmp-ra.79 rbp rax)))))
    (define L.fun/void19235.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/error19238.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 21566) (jump tmp-ra.81 rbp rax)))))
    (begin
      (set! tmp-ra.82 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/void19235.7 rbp r15)))
          (set! void0.5 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/ascii-char19236.5 rbp r15)))
          (set! ascii-char1.4 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/ascii-char19237.4 rbp r15)))
          (set! ascii-char2.3 rax))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/error19238.8 rbp r15)))
          (set! error3.2 rax))
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/error19239.6 rbp r15)))
          (set! error4.1 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi 160)
                  (set! rdi 1752)
                  (set! r15 L.rp.18)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 1232)
                    (set! rdi 456)
                    (set! r15 L.rp.19)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.20)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.21
                  (begin
                    (set! rsi 1432)
                    (set! rdi 792)
                    (set! r15 L.rp.21)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 1552)
                      (set! rdi 1848)
                      (set! r15 L.rp.22)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.23)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (set! rsi tmp.71)
              (set! rdi tmp.68)
              (set! r15 tmp-ra.82)
              (jump L.-.10 rbp r15 rdi rsi))))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.*.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.20 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.19 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin
                (set! tmp.59 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.59)) (jump tmp-ra.74 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.74 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.74 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.22 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.21 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.75 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.75 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.75 rbp rax))))))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.24 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.23 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.76 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.76 rbp rax))))))
    (define L.fun/empty20765.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.77 rbp rax)))))
    (define L.fun/ascii-char20767.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.78 rbp rax)))))
    (define L.fun/ascii-char20768.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.79 rbp rax)))))
    (define L.fun/empty20764.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/error20766.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 64574) (jump tmp-ra.81 rbp rax)))))
    (begin
      (set! tmp-ra.82 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/empty20764.7 rbp r15)))
          (set! empty0.5 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/empty20765.4 rbp r15)))
          (set! empty1.4 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/error20766.8 rbp r15)))
          (set! error2.3 rax))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/ascii-char20767.5 rbp r15)))
          (set! ascii-char3.2 rax))
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/ascii-char20768.6 rbp r15)))
          (set! ascii-char4.1 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi 1184)
                  (set! rdi 1504)
                  (set! r15 L.rp.18)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 1296)
                    (set! rdi 1616)
                    (set! r15 L.rp.19)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.20)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.21
                  (begin
                    (set! rsi 1272)
                    (set! rdi 8)
                    (set! r15 L.rp.21)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 528)
                      (set! rdi 888)
                      (set! r15 L.rp.22)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.23)
                      (jump L.*.12 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (set! rsi tmp.71)
              (set! rdi tmp.68)
              (set! r15 tmp-ra.82)
              (jump L.*.12 rbp r15 rdi rsi))))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () () () () () () () ())))
    (define L.-.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.80 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.80 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.80 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.20 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.19 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin
                (set! tmp.63 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.63)) (jump tmp-ra.81 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.81 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.81 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.82 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.22 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.21 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.82 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.82 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.82 rbp rax))))))
    (define L.fun/ascii-char21096.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.83 rbp rax)))))
    (define L.fun/ascii-char21098.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.84 rbp rax)))))
    (define L.fun/error21099.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin (begin (set! rax 33086) (jump tmp-ra.85 rbp rax)))))
    (define L.fun/ascii-char21097.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.86 rbp rax)))))
    (begin
      (set! tmp-ra.87 r15)
      (begin
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/ascii-char21096.4 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 1976)
                  (set! rdi 1208)
                  (set! r15 L.rp.13)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi 32)
                    (set! rdi 1432)
                    (set! r15 L.rp.14)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.15)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 456)
                    (set! rdi 1400)
                    (set! r15 L.rp.16)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1672)
                      (set! rdi 888)
                      (set! r15 L.rp.17)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.18)
                      (jump L.+.9 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (return-point L.rp.19
                (begin
                  (set! rsi tmp.71)
                  (set! rdi tmp.68)
                  (set! r15 L.rp.19)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! fixnum1.4 rax))))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/ascii-char21097.7 rbp r15)))
          (set! ascii-char2.3 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/ascii-char21098.5 rbp r15)))
          (set! ascii-char3.2 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.22
                (begin
                  (set! rsi 1800)
                  (set! rdi 1032)
                  (set! r15 L.rp.22)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! tmp.75 rax))
            (begin
              (begin
                (return-point L.rp.23
                  (begin
                    (set! rsi 560)
                    (set! rdi 552)
                    (set! r15 L.rp.23)
                    (jump L.+.9 rbp r15 rdi rsi)))
                (set! tmp.76 rax))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.76)
                    (set! rdi tmp.75)
                    (set! r15 L.rp.24)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.74 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.25
                  (begin
                    (set! rsi 368)
                    (set! rdi 1424)
                    (set! r15 L.rp.25)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.78 rax))
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 296)
                      (set! rdi 864)
                      (set! r15 L.rp.26)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi tmp.79)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.27)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.28
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.74)
                  (set! r15 L.rp.28)
                  (jump L.-.11 rbp r15 rdi rsi)))
              (set! fixnum4.1 rax))))
        (begin (set! r15 tmp-ra.87) (jump L.fun/error21099.6 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.-.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.69 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.24 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.69 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.69 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.69 rbp rax))))))
    (define L.+.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.70 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.70 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.70 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.70 rbp rax))))))
    (define L.fun/void21257.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.71 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.71 rbp rax)))))
    (define L.fun/void21255.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.72 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.72 rbp rax)))))
    (define L.fun/void21256.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.73 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.73 rbp rax)))))
    (define L.fun/ascii-char21254.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.74 rbp rax)))))
    (begin
      (set! tmp-ra.75 r15)
      (begin
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/ascii-char21254.7 rbp r15)))
          (set! ascii-char0.5 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/void21255.5 rbp r15)))
          (set! void1.4 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.13
                (begin
                  (set! rsi 736)
                  (set! rdi 1728)
                  (set! r15 L.rp.13)
                  (jump L.+.9 rbp r15 rdi rsi)))
              (set! tmp.64 rax))
            (begin
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi 192)
                    (set! rdi 1624)
                    (set! r15 L.rp.14)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.65 rax))
              (begin
                (return-point L.rp.15
                  (begin
                    (set! rsi tmp.65)
                    (set! rdi tmp.64)
                    (set! r15 L.rp.15)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.63 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 888)
                    (set! rdi 160)
                    (set! r15 L.rp.16)
                    (jump L.-.10 rbp r15 rdi rsi)))
                (set! tmp.67 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1920)
                      (set! rdi 1968)
                      (set! r15 L.rp.17)
                      (jump L.-.10 rbp r15 rdi rsi)))
                  (set! tmp.68 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.68)
                      (set! rdi tmp.67)
                      (set! r15 L.rp.18)
                      (jump L.+.9 rbp r15 rdi rsi)))
                  (set! tmp.66 rax))))
            (begin
              (return-point L.rp.19
                (begin
                  (set! rsi tmp.66)
                  (set! rdi tmp.63)
                  (set! r15 L.rp.19)
                  (jump L.-.10 rbp r15 rdi rsi)))
              (set! fixnum2.3 rax))))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/void21256.6 rbp r15)))
          (set! void3.2 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void21257.4 rbp r15)))
          (set! void4.1 rax))
        (begin (set! rax void1.4) (jump tmp-ra.75 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.fun/error21502.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 12606) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/empty21506.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/empty21503.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/error21505.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 45630) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/empty21504.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.59 rbp rax)))))
    (begin
      (set! tmp-ra.60 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/error21502.4 rbp r15)))
          (set! error0.5 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/empty21503.6 rbp r15)))
          (set! empty1.4 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/empty21504.8 rbp r15)))
          (set! empty2.3 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/error21505.7 rbp r15)))
          (set! error3.2 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/empty21506.5 rbp r15)))
          (set! empty4.1 rax))
        (begin (set! rax error0.5) (jump tmp-ra.60 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () () () () () () ())))
    (define L.+.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.74 r15)
        (begin
          (set! tmp.21 rdi)
          (set! tmp.22 rsi)
          (if (begin
                (if (begin (set! tmp.56 (bitwise-and tmp.22 7)) (= tmp.56 0))
                  (set! tmp.55 14)
                  (set! tmp.55 6))
                (!= tmp.55 6))
            (if (begin
                  (if (begin (set! tmp.58 (bitwise-and tmp.21 7)) (= tmp.58 0))
                    (set! tmp.57 14)
                    (set! tmp.57 6))
                  (!= tmp.57 6))
              (begin (set! rax (+ tmp.21 tmp.22)) (jump tmp-ra.74 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.74 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.74 rbp rax))))))
    (define L.-.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.75 r15)
        (begin
          (set! tmp.23 rdi)
          (set! tmp.24 rsi)
          (if (begin
                (if (begin (set! tmp.60 (bitwise-and tmp.24 7)) (= tmp.60 0))
                  (set! tmp.59 14)
                  (set! tmp.59 6))
                (!= tmp.59 6))
            (if (begin
                  (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                    (set! tmp.61 14)
                    (set! tmp.61 6))
                  (!= tmp.61 6))
              (begin (set! rax (- tmp.23 tmp.24)) (jump tmp-ra.75 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.75 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.75 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.76 r15)
        (begin
          (set! tmp.19 rdi)
          (set! tmp.20 rsi)
          (if (begin
                (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                  (set! tmp.63 14)
                  (set! tmp.63 6))
                (!= tmp.63 6))
            (if (begin
                  (if (begin (set! tmp.66 (bitwise-and tmp.19 7)) (= tmp.66 0))
                    (set! tmp.65 14)
                    (set! tmp.65 6))
                  (!= tmp.65 6))
              (begin
                (set! tmp.67 (arithmetic-shift-right tmp.20 3))
                (begin (set! rax (* tmp.19 tmp.67)) (jump tmp-ra.76 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.76 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.76 rbp rax))))))
    (define L.fun/empty21523.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.77 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.77 rbp rax)))))
    (define L.fun/void21525.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.78 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.78 rbp rax)))))
    (define L.fun/void21524.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.79 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.79 rbp rax)))))
    (define L.fun/void21527.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.80 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.80 rbp rax)))))
    (define L.fun/ascii-char21526.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.81 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.81 rbp rax)))))
    (begin
      (set! tmp-ra.82 r15)
      (begin
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/empty21523.4 rbp r15)))
          (set! empty0.5 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/void21524.6 rbp r15)))
          (set! void1.4 rax))
        (begin
          (return-point L.rp.15
            (begin (set! r15 L.rp.15) (jump L.fun/void21525.5 rbp r15)))
          (set! void2.3 rax))
        (begin
          (return-point L.rp.16
            (begin (set! r15 L.rp.16) (jump L.fun/ascii-char21526.8 rbp r15)))
          (set! ascii-char3.2 rax))
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/void21527.7 rbp r15)))
          (set! void4.1 rax))
        (begin
          (begin
            (begin
              (return-point L.rp.18
                (begin
                  (set! rsi 1448)
                  (set! rdi 1072)
                  (set! r15 L.rp.18)
                  (jump L.*.10 rbp r15 rdi rsi)))
              (set! tmp.69 rax))
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 1416)
                    (set! rdi 496)
                    (set! r15 L.rp.19)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.70 rax))
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi tmp.70)
                    (set! rdi tmp.69)
                    (set! r15 L.rp.20)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.68 rax))))
          (begin
            (begin
              (begin
                (return-point L.rp.21
                  (begin
                    (set! rsi 1208)
                    (set! rdi 80)
                    (set! r15 L.rp.21)
                    (jump L.-.11 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 1032)
                      (set! rdi 888)
                      (set! r15 L.rp.22)
                      (jump L.-.11 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.23)
                      (jump L.+.12 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (set! rsi tmp.71)
              (set! rdi tmp.68)
              (set! r15 tmp-ra.82)
              (jump L.+.12 rbp r15 rdi rsi))))))))
(check-by-interp
 '(module
    ((new-frames (() () () () ())))
    (define L.fun/void22132.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.55 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.55 rbp rax)))))
    (define L.fun/void22129.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.56 rbp rax)))))
    (define L.fun/void22131.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.57 rbp rax)))))
    (define L.fun/empty22130.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.58 rbp rax)))))
    (define L.fun/ascii-char22133.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.59 rbp rax)))))
    (begin
      (set! tmp-ra.60 r15)
      (begin
        (begin
          (return-point L.rp.10
            (begin (set! r15 L.rp.10) (jump L.fun/void22129.5 rbp r15)))
          (set! void0.5 rax))
        (begin
          (return-point L.rp.11
            (begin (set! r15 L.rp.11) (jump L.fun/empty22130.7 rbp r15)))
          (set! empty1.4 rax))
        (begin
          (return-point L.rp.12
            (begin (set! r15 L.rp.12) (jump L.fun/void22131.6 rbp r15)))
          (set! void2.3 rax))
        (begin
          (return-point L.rp.13
            (begin (set! r15 L.rp.13) (jump L.fun/void22132.4 rbp r15)))
          (set! void3.2 rax))
        (begin
          (return-point L.rp.14
            (begin (set! r15 L.rp.14) (jump L.fun/ascii-char22133.8 rbp r15)))
          (set! ascii-char4.1 rax))
        (begin (set! rax ascii-char4.1) (jump tmp-ra.60 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.83 rbp rax))))))
    (define L.+.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.23 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.22 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.84 rbp rax))))))
    (define L.*.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.85 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/ascii-char35206.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin
          (begin
            (set! r15 tmp-ra.86)
            (jump L.fun/ascii-char35207.9 rbp r15)))))
    (define L.fun/ascii-char35208.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin
            (set! r15 tmp-ra.87)
            (jump L.fun/ascii-char35209.10 rbp r15)))))
    (define L.fun/empty35210.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin (set! r15 tmp-ra.88) (jump L.fun/empty35211.13 rbp r15)))))
    (define L.fun/error35213.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 36414) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/void35214.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin (set! r15 tmp-ra.90) (jump L.fun/void35215.11 rbp r15)))))
    (define L.fun/ascii-char35207.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/ascii-char35209.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/void35215.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/error35212.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/error35213.7 rbp r15)))))
    (define L.fun/empty35211.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.95 rbp rax)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/ascii-char35206.4 rbp r15)))
          (set! ascii-char0.6 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/ascii-char35208.5 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/empty35210.6 rbp r15)))
          (set! empty2.4 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/error35212.12 rbp r15)))
          (set! error3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 112)
                    (set! rdi 24)
                    (set! r15 L.rp.22)
                    (jump L.*.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 808)
                      (set! rdi 1176)
                      (set! r15 L.rp.23)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.24)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1560)
                      (set! rdi 1752)
                      (set! r15 L.rp.25)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1400)
                        (set! rdi 40)
                        (set! r15 L.rp.26)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.27)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.28
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.28)
                    (jump L.-.17 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi 1136)
                      (set! rdi 680)
                      (set! r15 L.rp.29)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi 1040)
                        (set! rdi 1016)
                        (set! r15 L.rp.30)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.31)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 288)
                        (set! rdi 256)
                        (set! r15 L.rp.32)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.33
                        (begin
                          (set! rsi 1544)
                          (set! rdi 280)
                          (set! r15 L.rp.33)
                          (jump L.+.16 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.34
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.34)
                          (jump L.+.16 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.35)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.36
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.36)
                  (jump L.+.16 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/void35214.8 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax ascii-char1.5) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () () () ())))
    (define L.error?.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (set! tmp.48 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.48 255)) (= tmp.56 62))
            (begin (set! rax 14) (jump tmp-ra.87 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.87 rbp rax))))))
    (define L.+.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.88 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.88 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.88 rbp rax))))))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.25 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.24 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.89 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.89 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.89 rbp rax))))))
    (define L.*.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.21 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.20 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin
                (set! tmp.69 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.69)) (jump tmp-ra.90 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.90 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.90 rbp rax))))))
    (define L.void?.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (set! tmp.46 rdi)
          (if (begin (set! tmp.70 (bitwise-and tmp.46 255)) (= tmp.70 30))
            (begin (set! rax 14) (jump tmp-ra.91 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.91 rbp rax))))))
    (define L.fun/void43341.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin (set! r15 tmp-ra.92) (jump L.fun/void43342.11 rbp r15)))))
    (define L.fun/empty43339.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/any43337.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin (begin (set! rax 13630) (jump tmp-ra.94 rbp rax)))))
    (define L.fun/any43340.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 480) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/empty43335.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin
          (begin (set! r15 tmp-ra.96) (jump L.fun/empty43336.13 rbp r15)))))
    (define L.fun/empty43333.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (begin (set! r15 tmp-ra.97) (jump L.fun/empty43334.12 rbp r15)))))
    (define L.fun/empty43338.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (begin (set! r15 tmp-ra.98) (jump L.fun/empty43339.5 rbp r15)))))
    (define L.fun/void43342.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.99 rbp rax)))))
    (define L.fun/empty43334.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.100 rbp rax)))))
    (define L.fun/empty43336.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.101 rbp rax)))))
    (begin
      (set! tmp-ra.102 r15)
      (begin
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/empty43333.9 rbp r15)))
          (set! empty0.6 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/empty43335.8 rbp r15)))
          (set! empty1.5 rax))
        (begin
          (begin
            (return-point L.rp.22
              (begin (set! r15 L.rp.22) (jump L.fun/any43337.6 rbp r15)))
            (set! tmp.71 rax))
          (begin
            (return-point L.rp.23
              (begin
                (set! rdi tmp.71)
                (set! r15 L.rp.23)
                (jump L.void?.15 rbp r15 rdi)))
            (set! boolean2.4 rax)))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi 704)
                    (set! rdi 832)
                    (set! r15 L.rp.24)
                    (jump L.*.16 rbp r15 rdi rsi)))
                (set! tmp.74 rax))
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1832)
                      (set! rdi 1040)
                      (set! r15 L.rp.25)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi tmp.75)
                      (set! rdi tmp.74)
                      (set! r15 L.rp.26)
                      (jump L.*.16 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi 568)
                      (set! rdi 952)
                      (set! r15 L.rp.27)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 1880)
                        (set! rdi 1392)
                        (set! r15 L.rp.28)
                        (jump L.+.18 rbp r15 rdi rsi)))
                    (set! tmp.78 rax))
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi tmp.78)
                        (set! rdi tmp.77)
                        (set! r15 L.rp.29)
                        (jump L.+.18 rbp r15 rdi rsi)))
                    (set! tmp.76 rax))))
              (begin
                (return-point L.rp.30
                  (begin
                    (set! rsi tmp.76)
                    (set! rdi tmp.73)
                    (set! r15 L.rp.30)
                    (jump L.+.18 rbp r15 rdi rsi)))
                (set! tmp.72 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi 456)
                      (set! rdi 616)
                      (set! r15 L.rp.31)
                      (jump L.*.16 rbp r15 rdi rsi)))
                  (set! tmp.81 rax))
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 432)
                        (set! rdi 1928)
                        (set! r15 L.rp.32)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.82 rax))
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi tmp.82)
                        (set! rdi tmp.81)
                        (set! r15 L.rp.33)
                        (jump L.+.18 rbp r15 rdi rsi)))
                    (set! tmp.80 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi 1376)
                        (set! rdi 400)
                        (set! r15 L.rp.34)
                        (jump L.+.18 rbp r15 rdi rsi)))
                    (set! tmp.84 rax))
                  (begin
                    (begin
                      (return-point L.rp.35
                        (begin
                          (set! rsi 1920)
                          (set! rdi 576)
                          (set! r15 L.rp.35)
                          (jump L.*.16 rbp r15 rdi rsi)))
                      (set! tmp.85 rax))
                    (begin
                      (return-point L.rp.36
                        (begin
                          (set! rsi tmp.85)
                          (set! rdi tmp.84)
                          (set! r15 L.rp.36)
                          (jump L.*.16 rbp r15 rdi rsi)))
                      (set! tmp.83 rax))))
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi tmp.83)
                      (set! rdi tmp.80)
                      (set! r15 L.rp.37)
                      (jump L.*.16 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))))
            (begin
              (return-point L.rp.38
                (begin
                  (set! rsi tmp.79)
                  (set! rdi tmp.72)
                  (set! r15 L.rp.38)
                  (jump L.+.18 rbp r15 rdi rsi)))
              (set! fixnum3.3 rax))))
        (begin
          (return-point L.rp.39
            (begin (set! r15 L.rp.39) (jump L.fun/empty43338.10 rbp r15)))
          (set! empty4.2 rax))
        (begin
          (begin
            (return-point L.rp.40
              (begin (set! r15 L.rp.40) (jump L.fun/any43340.7 rbp r15)))
            (set! tmp.86 rax))
          (begin
            (return-point L.rp.41
              (begin
                (set! rdi tmp.86)
                (set! r15 L.rp.41)
                (jump L.error?.19 rbp r15 rdi)))
            (set! boolean5.1 rax)))
        (begin (set! r15 tmp-ra.102) (jump L.fun/void43341.4 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.93 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.93 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.93 rbp rax))))))
    (define L.+.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.23 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.22 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.94 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.94 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.94 rbp rax))))))
    (define L.*.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.95 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.95 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.95 rbp rax))))))
    (define L.fun/ascii-char44115.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin
          (begin
            (set! r15 tmp-ra.96)
            (jump L.fun/ascii-char44116.11 rbp r15)))))
    (define L.fun/ascii-char44119.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (begin
            (set! r15 tmp-ra.97)
            (jump L.fun/ascii-char44120.10 rbp r15)))))
    (define L.fun/empty44111.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (begin (set! r15 tmp-ra.98) (jump L.fun/empty44112.13 rbp r15)))))
    (define L.fun/void44117.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin (begin (set! r15 tmp-ra.99) (jump L.fun/void44118.9 rbp r15)))))
    (define L.fun/ascii-char44114.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.100 rbp rax)))))
    (define L.fun/void44118.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.101 rbp rax)))))
    (define L.fun/ascii-char44120.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.102 rbp rax)))))
    (define L.fun/ascii-char44116.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.103 rbp rax)))))
    (define L.fun/ascii-char44113.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin
          (begin
            (set! r15 tmp-ra.104)
            (jump L.fun/ascii-char44114.8 rbp r15)))))
    (define L.fun/empty44112.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.105 rbp rax)))))
    (begin
      (set! tmp-ra.106 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/empty44111.6 rbp r15)))
          (set! empty0.6 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/ascii-char44113.12 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/ascii-char44115.4 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void44117.7 rbp r15)))
          (set! void3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi 1768)
                    (set! rdi 488)
                    (set! r15 L.rp.22)
                    (jump L.*.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 1968)
                      (set! rdi 992)
                      (set! r15 L.rp.23)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.24)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 120)
                      (set! rdi 1304)
                      (set! r15 L.rp.25)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 336)
                        (set! rdi 296)
                        (set! r15 L.rp.26)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.27)
                        (jump L.+.16 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.28
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.28)
                    (jump L.+.16 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi 136)
                      (set! rdi 200)
                      (set! r15 L.rp.29)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi 1016)
                        (set! rdi 544)
                        (set! r15 L.rp.30)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.31)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi 712)
                        (set! rdi 1440)
                        (set! r15 L.rp.32)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.33
                        (begin
                          (set! rsi 1408)
                          (set! rdi 1984)
                          (set! r15 L.rp.33)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.34
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.34)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.35)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.36
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.36)
                  (jump L.+.16 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/ascii-char44119.5 rbp r15)))
          (set! ascii-char5.1 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.38
                  (begin
                    (set! rsi 736)
                    (set! rdi 416)
                    (set! r15 L.rp.38)
                    (jump L.+.16 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.39
                    (begin
                      (set! rsi 136)
                      (set! rdi fixnum4.2)
                      (set! r15 L.rp.39)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.40
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.40)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.41
                    (begin
                      (set! rsi 192)
                      (set! rdi 520)
                      (set! r15 L.rp.41)
                      (jump L.+.16 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.42
                      (begin
                        (set! rsi fixnum4.2)
                        (set! rdi fixnum4.2)
                        (set! r15 L.rp.42)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.43
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.43)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.44
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.44)
                    (jump L.+.16 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.45
                    (begin
                      (set! rsi fixnum4.2)
                      (set! rdi 440)
                      (set! r15 L.rp.45)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (return-point L.rp.46
                    (begin
                      (set! rsi tmp.92)
                      (set! rdi fixnum4.2)
                      (set! r15 L.rp.46)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.91 rax)))
              (begin
                (return-point L.rp.47
                  (begin
                    (set! rsi tmp.91)
                    (set! rdi fixnum4.2)
                    (set! r15 L.rp.47)
                    (jump L.*.15 rbp r15 rdi rsi)))
                (set! tmp.90 rax)))
            (begin
              (set! rsi tmp.90)
              (set! rdi tmp.83)
              (set! r15 tmp-ra.106)
              (jump L.-.17 rbp r15 rdi rsi))))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.fixnum?.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.43 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.43 7)) (= tmp.56 0))
            (begin (set! rax 14) (jump tmp-ra.99 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.99 rbp rax))))))
    (define L.+.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.23 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.22 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.100 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.100 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.100 rbp rax))))))
    (define L.*.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin
                (set! tmp.65 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.65)) (jump tmp-ra.101 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.101 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.101 rbp rax))))))
    (define L.-.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.67 (bitwise-and tmp.25 7)) (= tmp.67 0))
                  (set! tmp.66 14)
                  (set! tmp.66 6))
                (!= tmp.66 6))
            (if (begin
                  (if (begin (set! tmp.69 (bitwise-and tmp.24 7)) (= tmp.69 0))
                    (set! tmp.68 14)
                    (set! tmp.68 6))
                  (!= tmp.68 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.102 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.102 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.102 rbp rax))))))
    (define L.fun/empty58877.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.103 rbp rax)))))
    (define L.fun/error58875.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin (begin (set! rax 38462) (jump tmp-ra.104 rbp rax)))))
    (define L.fun/ascii-char58879.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/any58880.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.106 rbp rax)))))
    (define L.fun/empty58876.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin
          (begin (set! r15 tmp-ra.107) (jump L.fun/empty58877.4 rbp r15)))))
    (define L.fun/ascii-char58878.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.108 r15)
        (begin
          (begin
            (set! r15 tmp-ra.108)
            (jump L.fun/ascii-char58879.6 rbp r15)))))
    (define L.fun/error58874.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.109 r15)
        (begin
          (begin (set! r15 tmp-ra.109) (jump L.fun/error58875.5 rbp r15)))))
    (begin
      (set! tmp-ra.110 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 1192)
                    (set! rdi 2024)
                    (set! r15 L.rp.16)
                    (jump L.-.12 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1848)
                      (set! rdi 472)
                      (set! r15 L.rp.17)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.18)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 200)
                      (set! rdi 1496)
                      (set! r15 L.rp.19)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (begin
                    (return-point L.rp.20
                      (begin
                        (set! rsi 400)
                        (set! rdi 936)
                        (set! r15 L.rp.20)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.76 rax))
                  (begin
                    (return-point L.rp.21
                      (begin
                        (set! rsi tmp.76)
                        (set! rdi tmp.75)
                        (set! r15 L.rp.21)
                        (jump L.-.12 rbp r15 rdi rsi)))
                    (set! tmp.74 rax))))
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi tmp.74)
                    (set! rdi tmp.71)
                    (set! r15 L.rp.22)
                    (jump L.-.12 rbp r15 rdi rsi)))
                (set! tmp.70 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 512)
                      (set! rdi 1400)
                      (set! r15 L.rp.23)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 568)
                        (set! rdi 304)
                        (set! r15 L.rp.24)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.80 rax))
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi tmp.80)
                        (set! rdi tmp.79)
                        (set! r15 L.rp.25)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.78 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1904)
                        (set! rdi 1000)
                        (set! r15 L.rp.26)
                        (jump L.-.12 rbp r15 rdi rsi)))
                    (set! tmp.82 rax))
                  (begin
                    (begin
                      (return-point L.rp.27
                        (begin
                          (set! rsi 448)
                          (set! rdi 528)
                          (set! r15 L.rp.27)
                          (jump L.*.13 rbp r15 rdi rsi)))
                      (set! tmp.83 rax))
                    (begin
                      (return-point L.rp.28
                        (begin
                          (set! rsi tmp.83)
                          (set! rdi tmp.82)
                          (set! r15 L.rp.28)
                          (jump L.-.12 rbp r15 rdi rsi)))
                      (set! tmp.81 rax))))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.81)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.29)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.30
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.70)
                  (set! r15 L.rp.30)
                  (jump L.+.14 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.31
                  (begin
                    (set! rsi 8)
                    (set! rdi 888)
                    (set! r15 L.rp.31)
                    (jump L.-.12 rbp r15 rdi rsi)))
                (set! tmp.86 rax))
              (begin
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi 1296)
                      (set! rdi 1576)
                      (set! r15 L.rp.32)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.87 rax))
                (begin
                  (return-point L.rp.33
                    (begin
                      (set! rsi tmp.87)
                      (set! rdi tmp.86)
                      (set! r15 L.rp.33)
                      (jump L.+.14 rbp r15 rdi rsi)))
                  (set! tmp.85 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi 2000)
                      (set! rdi 1288)
                      (set! r15 L.rp.34)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.89 rax))
                (begin
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi 1088)
                        (set! rdi 488)
                        (set! r15 L.rp.35)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.90 rax))
                  (begin
                    (return-point L.rp.36
                      (begin
                        (set! rsi tmp.90)
                        (set! rdi tmp.89)
                        (set! r15 L.rp.36)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.88 rax))))
              (begin
                (return-point L.rp.37
                  (begin
                    (set! rsi tmp.88)
                    (set! rdi tmp.85)
                    (set! r15 L.rp.37)
                    (jump L.+.14 rbp r15 rdi rsi)))
                (set! tmp.84 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.38
                    (begin
                      (set! rsi 1032)
                      (set! rdi 1392)
                      (set! r15 L.rp.38)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.93 rax))
                (begin
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi 536)
                        (set! rdi 1776)
                        (set! r15 L.rp.39)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.94 rax))
                  (begin
                    (return-point L.rp.40
                      (begin
                        (set! rsi tmp.94)
                        (set! rdi tmp.93)
                        (set! r15 L.rp.40)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.92 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.41
                      (begin
                        (set! rsi 2016)
                        (set! rdi 1016)
                        (set! r15 L.rp.41)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.96 rax))
                  (begin
                    (begin
                      (return-point L.rp.42
                        (begin
                          (set! rsi 1736)
                          (set! rdi 1680)
                          (set! r15 L.rp.42)
                          (jump L.-.12 rbp r15 rdi rsi)))
                      (set! tmp.97 rax))
                    (begin
                      (return-point L.rp.43
                        (begin
                          (set! rsi tmp.97)
                          (set! rdi tmp.96)
                          (set! r15 L.rp.43)
                          (jump L.-.12 rbp r15 rdi rsi)))
                      (set! tmp.95 rax))))
                (begin
                  (return-point L.rp.44
                    (begin
                      (set! rsi tmp.95)
                      (set! rdi tmp.92)
                      (set! r15 L.rp.44)
                      (jump L.+.14 rbp r15 rdi rsi)))
                  (set! tmp.91 rax))))
            (begin
              (return-point L.rp.45
                (begin
                  (set! rsi tmp.91)
                  (set! rdi tmp.84)
                  (set! r15 L.rp.45)
                  (jump L.*.13 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (return-point L.rp.46
            (begin (set! r15 L.rp.46) (jump L.fun/error58874.10 rbp r15)))
          (set! error2.4 rax))
        (begin
          (return-point L.rp.47
            (begin (set! r15 L.rp.47) (jump L.fun/empty58876.8 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (return-point L.rp.48
            (begin (set! r15 L.rp.48) (jump L.fun/ascii-char58878.9 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (begin
            (return-point L.rp.49
              (begin (set! r15 L.rp.49) (jump L.fun/any58880.7 rbp r15)))
            (set! tmp.98 rax))
          (begin
            (return-point L.rp.50
              (begin
                (set! rdi tmp.98)
                (set! r15 L.rp.50)
                (jump L.fixnum?.15 rbp r15 rdi)))
            (set! boolean5.1 rax)))
        (begin (set! rax ascii-char4.2) (jump tmp-ra.110 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.void?.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.113 r15)
        (begin
          (set! tmp.46 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.46 255)) (= tmp.56 30))
            (begin (set! rax 14) (jump tmp-ra.113 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.113 rbp rax))))))
    (define L.-.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.114 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.25 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.24 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.114 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.114 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.114 rbp rax))))))
    (define L.+.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.115 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.23 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.22 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.115 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.115 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.115 rbp rax))))))
    (define L.*.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.116 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.21 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.20 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin
                (set! tmp.69 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.69)) (jump tmp-ra.116 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.116 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.116 rbp rax))))))
    (define L.fun/ascii-char63953.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.117 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.117 rbp rax)))))
    (define L.fun/error63956.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.118 r15)
        (begin (begin (set! rax 20030) (jump tmp-ra.118 rbp rax)))))
    (define L.fun/ascii-char63952.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.119 r15)
        (begin
          (begin
            (set! r15 tmp-ra.119)
            (jump L.fun/ascii-char63953.4 rbp r15)))))
    (define L.fun/error63955.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.120 r15)
        (begin
          (begin (set! r15 tmp-ra.120) (jump L.fun/error63956.5 rbp r15)))))
    (define L.fun/any63954.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.121 r15)
        (begin (begin (set! rax 1784) (jump tmp-ra.121 rbp rax)))))
    (begin
      (set! tmp-ra.122 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.14
                  (begin
                    (set! rsi 2024)
                    (set! rdi 224)
                    (set! r15 L.rp.14)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.15
                    (begin
                      (set! rsi 1720)
                      (set! rdi 1520)
                      (set! r15 L.rp.15)
                      (jump L.*.10 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.16
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.16)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 680)
                      (set! rdi 1768)
                      (set! r15 L.rp.17)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (begin
                    (return-point L.rp.18
                      (begin
                        (set! rsi 840)
                        (set! rdi 1488)
                        (set! r15 L.rp.18)
                        (jump L.-.12 rbp r15 rdi rsi)))
                    (set! tmp.76 rax))
                  (begin
                    (return-point L.rp.19
                      (begin
                        (set! rsi tmp.76)
                        (set! rdi tmp.75)
                        (set! r15 L.rp.19)
                        (jump L.*.10 rbp r15 rdi rsi)))
                    (set! tmp.74 rax))))
              (begin
                (return-point L.rp.20
                  (begin
                    (set! rsi tmp.74)
                    (set! rdi tmp.71)
                    (set! r15 L.rp.20)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.70 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 1392)
                      (set! rdi 1920)
                      (set! r15 L.rp.21)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 1024)
                        (set! rdi 1632)
                        (set! r15 L.rp.22)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.80 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.80)
                        (set! rdi tmp.79)
                        (set! r15 L.rp.23)
                        (jump L.-.12 rbp r15 rdi rsi)))
                    (set! tmp.78 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 1008)
                        (set! rdi 1096)
                        (set! r15 L.rp.24)
                        (jump L.*.10 rbp r15 rdi rsi)))
                    (set! tmp.82 rax))
                  (begin
                    (begin
                      (return-point L.rp.25
                        (begin
                          (set! rsi 416)
                          (set! rdi 416)
                          (set! r15 L.rp.25)
                          (jump L.+.11 rbp r15 rdi rsi)))
                      (set! tmp.83 rax))
                    (begin
                      (return-point L.rp.26
                        (begin
                          (set! rsi tmp.83)
                          (set! rdi tmp.82)
                          (set! r15 L.rp.26)
                          (jump L.*.10 rbp r15 rdi rsi)))
                      (set! tmp.81 rax))))
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi tmp.81)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.27)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.28
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.70)
                  (set! r15 L.rp.28)
                  (jump L.+.11 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (return-point L.rp.29
            (begin (set! r15 L.rp.29) (jump L.fun/ascii-char63952.6 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.30
                  (begin
                    (set! rsi 120)
                    (set! rdi 424)
                    (set! r15 L.rp.30)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.86 rax))
              (begin
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi 64)
                      (set! rdi 792)
                      (set! r15 L.rp.31)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.87 rax))
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi tmp.87)
                      (set! rdi tmp.86)
                      (set! r15 L.rp.32)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.85 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.33
                    (begin
                      (set! rsi 240)
                      (set! rdi 936)
                      (set! r15 L.rp.33)
                      (jump L.*.10 rbp r15 rdi rsi)))
                  (set! tmp.89 rax))
                (begin
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi 376)
                        (set! rdi 1192)
                        (set! r15 L.rp.34)
                        (jump L.*.10 rbp r15 rdi rsi)))
                    (set! tmp.90 rax))
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi tmp.90)
                        (set! rdi tmp.89)
                        (set! r15 L.rp.35)
                        (jump L.*.10 rbp r15 rdi rsi)))
                    (set! tmp.88 rax))))
              (begin
                (return-point L.rp.36
                  (begin
                    (set! rsi tmp.88)
                    (set! rdi tmp.85)
                    (set! r15 L.rp.36)
                    (jump L.-.12 rbp r15 rdi rsi)))
                (set! tmp.84 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi 1872)
                      (set! rdi 1392)
                      (set! r15 L.rp.37)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.93 rax))
                (begin
                  (begin
                    (return-point L.rp.38
                      (begin
                        (set! rsi 304)
                        (set! rdi 1024)
                        (set! r15 L.rp.38)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.94 rax))
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi tmp.94)
                        (set! rdi tmp.93)
                        (set! r15 L.rp.39)
                        (jump L.-.12 rbp r15 rdi rsi)))
                    (set! tmp.92 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.40
                      (begin
                        (set! rsi 688)
                        (set! rdi 1776)
                        (set! r15 L.rp.40)
                        (jump L.-.12 rbp r15 rdi rsi)))
                    (set! tmp.96 rax))
                  (begin
                    (begin
                      (return-point L.rp.41
                        (begin
                          (set! rsi 1240)
                          (set! rdi 552)
                          (set! r15 L.rp.41)
                          (jump L.+.11 rbp r15 rdi rsi)))
                      (set! tmp.97 rax))
                    (begin
                      (return-point L.rp.42
                        (begin
                          (set! rsi tmp.97)
                          (set! rdi tmp.96)
                          (set! r15 L.rp.42)
                          (jump L.+.11 rbp r15 rdi rsi)))
                      (set! tmp.95 rax))))
                (begin
                  (return-point L.rp.43
                    (begin
                      (set! rsi tmp.95)
                      (set! rdi tmp.92)
                      (set! r15 L.rp.43)
                      (jump L.*.10 rbp r15 rdi rsi)))
                  (set! tmp.91 rax))))
            (begin
              (return-point L.rp.44
                (begin
                  (set! rsi tmp.91)
                  (set! rdi tmp.84)
                  (set! r15 L.rp.44)
                  (jump L.-.12 rbp r15 rdi rsi)))
              (set! fixnum2.4 rax))))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.45
                  (begin
                    (set! rsi 1848)
                    (set! rdi 872)
                    (set! r15 L.rp.45)
                    (jump L.+.11 rbp r15 rdi rsi)))
                (set! tmp.100 rax))
              (begin
                (begin
                  (return-point L.rp.46
                    (begin
                      (set! rsi 1936)
                      (set! rdi 128)
                      (set! r15 L.rp.46)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.101 rax))
                (begin
                  (return-point L.rp.47
                    (begin
                      (set! rsi tmp.101)
                      (set! rdi tmp.100)
                      (set! r15 L.rp.47)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.99 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.48
                    (begin
                      (set! rsi 256)
                      (set! rdi 1336)
                      (set! r15 L.rp.48)
                      (jump L.-.12 rbp r15 rdi rsi)))
                  (set! tmp.103 rax))
                (begin
                  (begin
                    (return-point L.rp.49
                      (begin
                        (set! rsi 1576)
                        (set! rdi 2000)
                        (set! r15 L.rp.49)
                        (jump L.*.10 rbp r15 rdi rsi)))
                    (set! tmp.104 rax))
                  (begin
                    (return-point L.rp.50
                      (begin
                        (set! rsi tmp.104)
                        (set! rdi tmp.103)
                        (set! r15 L.rp.50)
                        (jump L.*.10 rbp r15 rdi rsi)))
                    (set! tmp.102 rax))))
              (begin
                (return-point L.rp.51
                  (begin
                    (set! rsi tmp.102)
                    (set! rdi tmp.99)
                    (set! r15 L.rp.51)
                    (jump L.*.10 rbp r15 rdi rsi)))
                (set! tmp.98 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.52
                    (begin
                      (set! rsi 1152)
                      (set! rdi 1528)
                      (set! r15 L.rp.52)
                      (jump L.+.11 rbp r15 rdi rsi)))
                  (set! tmp.107 rax))
                (begin
                  (begin
                    (return-point L.rp.53
                      (begin
                        (set! rsi 816)
                        (set! rdi 1792)
                        (set! r15 L.rp.53)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.108 rax))
                  (begin
                    (return-point L.rp.54
                      (begin
                        (set! rsi tmp.108)
                        (set! rdi tmp.107)
                        (set! r15 L.rp.54)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.106 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.55
                      (begin
                        (set! rsi 2032)
                        (set! rdi 512)
                        (set! r15 L.rp.55)
                        (jump L.+.11 rbp r15 rdi rsi)))
                    (set! tmp.110 rax))
                  (begin
                    (begin
                      (return-point L.rp.56
                        (begin
                          (set! rsi 360)
                          (set! rdi 1032)
                          (set! r15 L.rp.56)
                          (jump L.*.10 rbp r15 rdi rsi)))
                      (set! tmp.111 rax))
                    (begin
                      (return-point L.rp.57
                        (begin
                          (set! rsi tmp.111)
                          (set! rdi tmp.110)
                          (set! r15 L.rp.57)
                          (jump L.*.10 rbp r15 rdi rsi)))
                      (set! tmp.109 rax))))
                (begin
                  (return-point L.rp.58
                    (begin
                      (set! rsi tmp.109)
                      (set! rdi tmp.106)
                      (set! r15 L.rp.58)
                      (jump L.*.10 rbp r15 rdi rsi)))
                  (set! tmp.105 rax))))
            (begin
              (return-point L.rp.59
                (begin
                  (set! rsi tmp.105)
                  (set! rdi tmp.98)
                  (set! r15 L.rp.59)
                  (jump L.-.12 rbp r15 rdi rsi)))
              (set! fixnum3.3 rax))))
        (begin
          (begin
            (return-point L.rp.60
              (begin (set! r15 L.rp.60) (jump L.fun/any63954.8 rbp r15)))
            (set! tmp.112 rax))
          (begin
            (return-point L.rp.61
              (begin
                (set! rdi tmp.112)
                (set! r15 L.rp.61)
                (jump L.void?.13 rbp r15 rdi)))
            (set! boolean4.2 rax)))
        (begin
          (return-point L.rp.62
            (begin (set! r15 L.rp.62) (jump L.fun/error63955.7 rbp r15)))
          (set! error5.1 rax))
        (begin (set! rax ascii-char1.5) (jump tmp-ra.122 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.83 rbp rax))))))
    (define L.*.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.84 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.84 rbp rax))))))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.23 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.22 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/ascii-char71274.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/void71278.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.87 rbp rax)))))
    (define L.fun/ascii-char71271.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin
            (set! r15 tmp-ra.88)
            (jump L.fun/ascii-char71272.9 rbp r15)))))
    (define L.fun/ascii-char71273.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (begin
            (set! r15 tmp-ra.89)
            (jump L.fun/ascii-char71274.4 rbp r15)))))
    (define L.fun/empty71270.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/ascii-char71272.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/ascii-char71275.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin
            (set! r15 tmp-ra.92)
            (jump L.fun/ascii-char71276.13 rbp r15)))))
    (define L.fun/void71277.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! r15 tmp-ra.93) (jump L.fun/void71278.5 rbp r15)))))
    (define L.fun/empty71269.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/empty71270.8 rbp r15)))))
    (define L.fun/ascii-char71276.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.95 rbp rax)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 336)
                    (set! rdi 104)
                    (set! r15 L.rp.18)
                    (jump L.+.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 1800)
                      (set! rdi 888)
                      (set! r15 L.rp.19)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.20)
                      (jump L.*.16 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 1344)
                      (set! rdi 872)
                      (set! r15 L.rp.21)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 680)
                        (set! rdi 1360)
                        (set! r15 L.rp.22)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.23)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.24)
                    (jump L.+.15 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1248)
                      (set! rdi 72)
                      (set! r15 L.rp.25)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1360)
                        (set! rdi 896)
                        (set! r15 L.rp.26)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.27)
                        (jump L.-.17 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 152)
                        (set! rdi 680)
                        (set! r15 L.rp.28)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.29
                        (begin
                          (set! rsi 1392)
                          (set! rdi 1104)
                          (set! r15 L.rp.29)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.30)
                          (jump L.+.15 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.31)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.32
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.32)
                  (jump L.*.16 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (return-point L.rp.33
            (begin (set! r15 L.rp.33) (jump L.fun/empty71269.12 rbp r15)))
          (set! empty1.5 rax))
        (begin
          (return-point L.rp.34
            (begin (set! r15 L.rp.34) (jump L.fun/ascii-char71271.6 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (return-point L.rp.35
            (begin (set! r15 L.rp.35) (jump L.fun/ascii-char71273.7 rbp r15)))
          (set! ascii-char3.3 rax))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/ascii-char71275.10 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/void71277.11 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax empty1.5) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.97 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.97 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.97 rbp rax))))))
    (define L.-.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.25 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.24 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.98 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.98 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.98 rbp rax))))))
    (define L.*.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.99 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.99 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.99 rbp rax))))))
    (define L.fun/empty73456.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (begin (set! r15 tmp-ra.100) (jump L.fun/empty73457.8 rbp r15)))))
    (define L.fun/ascii-char73458.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin
          (begin
            (set! r15 tmp-ra.101)
            (jump L.fun/ascii-char73459.10 rbp r15)))))
    (define L.fun/empty73463.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.102 rbp rax)))))
    (define L.fun/error73460.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin
          (begin (set! r15 tmp-ra.103) (jump L.fun/error73461.9 rbp r15)))))
    (define L.fun/empty73457.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.104 rbp rax)))))
    (define L.fun/error73461.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 20542) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/ascii-char73459.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.106 rbp rax)))))
    (define L.fun/empty73462.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin
          (begin (set! r15 tmp-ra.107) (jump L.fun/empty73463.6 rbp r15)))))
    (begin
      (set! tmp-ra.108 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 2024)
                    (set! rdi 1888)
                    (set! r15 L.rp.16)
                    (jump L.*.13 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1464)
                      (set! rdi 696)
                      (set! r15 L.rp.17)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.18)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 416)
                      (set! rdi 1344)
                      (set! r15 L.rp.19)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.20
                      (begin
                        (set! rsi 88)
                        (set! rdi 464)
                        (set! r15 L.rp.20)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.21
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.21)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.22)
                    (jump L.-.14 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 1760)
                      (set! rdi 40)
                      (set! r15 L.rp.23)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 1688)
                        (set! rdi 104)
                        (set! r15 L.rp.24)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.25)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 952)
                        (set! rdi 1952)
                        (set! r15 L.rp.26)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.27
                        (begin
                          (set! rsi 584)
                          (set! rdi 1264)
                          (set! r15 L.rp.27)
                          (jump L.*.13 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.28
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.28)
                          (jump L.-.14 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.29)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.30
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.30)
                  (jump L.+.15 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (return-point L.rp.31
            (begin (set! r15 L.rp.31) (jump L.fun/empty73456.4 rbp r15)))
          (set! empty1.5 rax))
        (begin
          (return-point L.rp.32
            (begin (set! r15 L.rp.32) (jump L.fun/ascii-char73458.5 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (return-point L.rp.33
            (begin (set! r15 L.rp.33) (jump L.fun/error73460.7 rbp r15)))
          (set! error3.3 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.34
                  (begin
                    (set! rsi 1504)
                    (set! rdi 2008)
                    (set! r15 L.rp.34)
                    (jump L.-.14 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.35
                    (begin
                      (set! rsi 1856)
                      (set! rdi 1768)
                      (set! r15 L.rp.35)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.36
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.36)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.37
                    (begin
                      (set! rsi 1728)
                      (set! rdi 880)
                      (set! r15 L.rp.37)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.38
                      (begin
                        (set! rsi 152)
                        (set! rdi 1992)
                        (set! r15 L.rp.38)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.39)
                        (jump L.-.14 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.40
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.40)
                    (jump L.*.13 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.41
                    (begin
                      (set! rsi 672)
                      (set! rdi 840)
                      (set! r15 L.rp.41)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.42
                      (begin
                        (set! rsi 368)
                        (set! rdi 1704)
                        (set! r15 L.rp.42)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.43
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.43)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.44
                      (begin
                        (set! rsi 1992)
                        (set! rdi 624)
                        (set! r15 L.rp.44)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.45
                        (begin
                          (set! rsi 920)
                          (set! rdi 1976)
                          (set! r15 L.rp.45)
                          (jump L.-.14 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.46
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.46)
                          (jump L.*.13 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.47
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.47)
                      (jump L.-.14 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.48
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.48)
                  (jump L.-.14 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.49
            (begin (set! r15 L.rp.49) (jump L.fun/empty73462.11 rbp r15)))
          (set! empty5.1 rax))
        (begin (set! rax error3.3) (jump tmp-ra.108 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.+.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.83 rbp rax))))))
    (define L.-.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.25 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.24 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.84 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.84 rbp rax))))))
    (define L.*.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.85 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/void77842.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! r15 tmp-ra.86) (jump L.fun/void77843.9 rbp r15)))))
    (define L.fun/void77838.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin (set! r15 tmp-ra.87) (jump L.fun/void77839.10 rbp r15)))))
    (define L.fun/error77846.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (begin (set! r15 tmp-ra.88) (jump L.fun/error77847.13 rbp r15)))))
    (define L.fun/error77845.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 62014) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/empty77840.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin (set! r15 tmp-ra.90) (jump L.fun/empty77841.11 rbp r15)))))
    (define L.fun/void77843.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/void77839.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/empty77841.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/error77844.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/error77845.7 rbp r15)))))
    (define L.fun/error77847.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 48702) (jump tmp-ra.95 rbp rax)))))
    (begin
      (set! tmp-ra.96 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/void77838.5 rbp r15)))
          (set! void0.6 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 800)
                    (set! rdi 1560)
                    (set! r15 L.rp.19)
                    (jump L.*.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi 1936)
                      (set! rdi 1880)
                      (set! r15 L.rp.20)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.21)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 752)
                      (set! rdi 216)
                      (set! r15 L.rp.22)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi 800)
                        (set! rdi 1208)
                        (set! r15 L.rp.23)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.24)
                        (jump L.*.15 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.25
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.25)
                    (jump L.+.17 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 752)
                      (set! rdi 8)
                      (set! r15 L.rp.26)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi 1208)
                        (set! rdi 1448)
                        (set! r15 L.rp.27)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.28)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi 1784)
                        (set! rdi 1232)
                        (set! r15 L.rp.29)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi 1256)
                          (set! rdi 1288)
                          (set! r15 L.rp.30)
                          (jump L.-.16 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.31
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.31)
                          (jump L.*.15 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.32)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.33
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.33)
                  (jump L.+.17 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (return-point L.rp.34
            (begin (set! r15 L.rp.34) (jump L.fun/empty77840.8 rbp r15)))
          (set! empty2.4 rax))
        (begin
          (return-point L.rp.35
            (begin (set! r15 L.rp.35) (jump L.fun/void77842.4 rbp r15)))
          (set! void3.3 rax))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/error77844.12 rbp r15)))
          (set! error4.2 rax))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/error77846.6 rbp r15)))
          (set! error5.1 rax))
        (begin (set! rax empty2.4) (jump tmp-ra.96 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.ascii-char?.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin
          (set! tmp.47 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.47 255)) (= tmp.56 46))
            (begin (set! rax 14) (jump tmp-ra.101 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.101 rbp rax))))))
    (define L.boolean?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.57 (bitwise-and tmp.44 247)) (= tmp.57 6))
            (begin (set! rax 14) (jump tmp-ra.102 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.102 rbp rax))))))
    (define L.+.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.59 (bitwise-and tmp.23 7)) (= tmp.59 0))
                  (set! tmp.58 14)
                  (set! tmp.58 6))
                (!= tmp.58 6))
            (if (begin
                  (if (begin (set! tmp.61 (bitwise-and tmp.22 7)) (= tmp.61 0))
                    (set! tmp.60 14)
                    (set! tmp.60 6))
                  (!= tmp.60 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.103 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.103 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.103 rbp rax))))))
    (define L.*.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.63 (bitwise-and tmp.21 7)) (= tmp.63 0))
                  (set! tmp.62 14)
                  (set! tmp.62 6))
                (!= tmp.62 6))
            (if (begin
                  (if (begin (set! tmp.65 (bitwise-and tmp.20 7)) (= tmp.65 0))
                    (set! tmp.64 14)
                    (set! tmp.64 6))
                  (!= tmp.64 6))
              (begin
                (set! tmp.66 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.66)) (jump tmp-ra.104 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.104 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.104 rbp rax))))))
    (define L.-.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.68 (bitwise-and tmp.25 7)) (= tmp.68 0))
                  (set! tmp.67 14)
                  (set! tmp.67 6))
                (!= tmp.67 6))
            (if (begin
                  (if (begin (set! tmp.70 (bitwise-and tmp.24 7)) (= tmp.70 0))
                    (set! tmp.69 14)
                    (set! tmp.69 6))
                  (!= tmp.69 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.105 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.105 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.105 rbp rax))))))
    (define L.fun/any78095.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.106 rbp rax)))))
    (define L.fun/void78099.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.107 rbp rax)))))
    (define L.fun/error78092.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.108 r15)
        (begin
          (begin (set! r15 tmp-ra.108) (jump L.fun/error78093.9 rbp r15)))))
    (define L.fun/any78094.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.109 r15)
        (begin (begin (set! rax 6) (jump tmp-ra.109 rbp rax)))))
    (define L.fun/error78097.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.110 r15)
        (begin (begin (set! rax 36670) (jump tmp-ra.110 rbp rax)))))
    (define L.fun/error78093.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.111 r15)
        (begin (begin (set! rax 24126) (jump tmp-ra.111 rbp rax)))))
    (define L.fun/error78096.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.112 r15)
        (begin
          (begin (set! r15 tmp-ra.112) (jump L.fun/error78097.8 rbp r15)))))
    (define L.fun/void78098.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.113 r15)
        (begin
          (begin (set! r15 tmp-ra.113) (jump L.fun/void78099.5 rbp r15)))))
    (begin
      (set! tmp-ra.114 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/error78092.6 rbp r15)))
          (set! error0.6 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.19
                  (begin
                    (set! rsi 480)
                    (set! rdi 1192)
                    (set! r15 L.rp.19)
                    (jump L.-.13 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi 56)
                      (set! rdi 1584)
                      (set! r15 L.rp.20)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.21)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 128)
                      (set! rdi 1488)
                      (set! r15 L.rp.22)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))
                (begin
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi 1720)
                        (set! rdi 488)
                        (set! r15 L.rp.23)
                        (jump L.+.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi tmp.77)
                        (set! rdi tmp.76)
                        (set! r15 L.rp.24)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))))
              (begin
                (return-point L.rp.25
                  (begin
                    (set! rsi tmp.75)
                    (set! rdi tmp.72)
                    (set! r15 L.rp.25)
                    (jump L.+.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 1640)
                      (set! rdi 1312)
                      (set! r15 L.rp.26)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi 864)
                        (set! rdi 1184)
                        (set! r15 L.rp.27)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi tmp.81)
                        (set! rdi tmp.80)
                        (set! r15 L.rp.28)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi 296)
                        (set! rdi 208)
                        (set! r15 L.rp.29)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.83 rax))
                  (begin
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi 32)
                          (set! rdi 800)
                          (set! r15 L.rp.30)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.84 rax))
                    (begin
                      (return-point L.rp.31
                        (begin
                          (set! rsi tmp.84)
                          (set! rdi tmp.83)
                          (set! r15 L.rp.31)
                          (jump L.*.14 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))))
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi tmp.82)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.32)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.33
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.71)
                  (set! r15 L.rp.33)
                  (jump L.-.13 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (begin
            (return-point L.rp.34
              (begin (set! r15 L.rp.34) (jump L.fun/any78094.7 rbp r15)))
            (set! tmp.85 rax))
          (begin
            (return-point L.rp.35
              (begin
                (set! rdi tmp.85)
                (set! r15 L.rp.35)
                (jump L.boolean?.16 rbp r15 rdi)))
            (set! boolean2.4 rax)))
        (begin
          (begin
            (return-point L.rp.36
              (begin (set! r15 L.rp.36) (jump L.fun/any78095.4 rbp r15)))
            (set! tmp.86 rax))
          (begin
            (return-point L.rp.37
              (begin
                (set! rdi tmp.86)
                (set! r15 L.rp.37)
                (jump L.ascii-char?.17 rbp r15 rdi)))
            (set! boolean3.3 rax)))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.38
                  (begin
                    (set! rsi 1384)
                    (set! rdi 136)
                    (set! r15 L.rp.38)
                    (jump L.*.14 rbp r15 rdi rsi)))
                (set! tmp.89 rax))
              (begin
                (begin
                  (return-point L.rp.39
                    (begin
                      (set! rsi 592)
                      (set! rdi 1784)
                      (set! r15 L.rp.39)
                      (jump L.-.13 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))
                (begin
                  (return-point L.rp.40
                    (begin
                      (set! rsi tmp.90)
                      (set! rdi tmp.89)
                      (set! r15 L.rp.40)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.41
                    (begin
                      (set! rsi 584)
                      (set! rdi 1536)
                      (set! r15 L.rp.41)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.42
                      (begin
                        (set! rsi 1504)
                        (set! rdi 1528)
                        (set! r15 L.rp.42)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.43
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.43)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (return-point L.rp.44
                  (begin
                    (set! rsi tmp.91)
                    (set! rdi tmp.88)
                    (set! r15 L.rp.44)
                    (jump L.+.15 rbp r15 rdi rsi)))
                (set! tmp.87 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.45
                    (begin
                      (set! rsi 1824)
                      (set! rdi 1536)
                      (set! r15 L.rp.45)
                      (jump L.*.14 rbp r15 rdi rsi)))
                  (set! tmp.96 rax))
                (begin
                  (begin
                    (return-point L.rp.46
                      (begin
                        (set! rsi 840)
                        (set! rdi 1760)
                        (set! r15 L.rp.46)
                        (jump L.*.14 rbp r15 rdi rsi)))
                    (set! tmp.97 rax))
                  (begin
                    (return-point L.rp.47
                      (begin
                        (set! rsi tmp.97)
                        (set! rdi tmp.96)
                        (set! r15 L.rp.47)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.48
                      (begin
                        (set! rsi 1032)
                        (set! rdi 1864)
                        (set! r15 L.rp.48)
                        (jump L.-.13 rbp r15 rdi rsi)))
                    (set! tmp.99 rax))
                  (begin
                    (begin
                      (return-point L.rp.49
                        (begin
                          (set! rsi 688)
                          (set! rdi 88)
                          (set! r15 L.rp.49)
                          (jump L.-.13 rbp r15 rdi rsi)))
                      (set! tmp.100 rax))
                    (begin
                      (return-point L.rp.50
                        (begin
                          (set! rsi tmp.100)
                          (set! rdi tmp.99)
                          (set! r15 L.rp.50)
                          (jump L.-.13 rbp r15 rdi rsi)))
                      (set! tmp.98 rax))))
                (begin
                  (return-point L.rp.51
                    (begin
                      (set! rsi tmp.98)
                      (set! rdi tmp.95)
                      (set! r15 L.rp.51)
                      (jump L.+.15 rbp r15 rdi rsi)))
                  (set! tmp.94 rax))))
            (begin
              (return-point L.rp.52
                (begin
                  (set! rsi tmp.94)
                  (set! rdi tmp.87)
                  (set! r15 L.rp.52)
                  (jump L.-.13 rbp r15 rdi rsi)))
              (set! fixnum4.2 rax))))
        (begin
          (return-point L.rp.53
            (begin (set! r15 L.rp.53) (jump L.fun/error78096.10 rbp r15)))
          (set! error5.1 rax))
        (begin (set! r15 tmp-ra.114) (jump L.fun/void78098.11 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () () ())))
    (define L.boolean?.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.56 (bitwise-and tmp.44 247)) (= tmp.56 6))
            (begin (set! rax 14) (jump tmp-ra.85 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.85 rbp rax))))))
    (define L.-.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.58 (bitwise-and tmp.25 7)) (= tmp.58 0))
                  (set! tmp.57 14)
                  (set! tmp.57 6))
                (!= tmp.57 6))
            (if (begin
                  (if (begin (set! tmp.60 (bitwise-and tmp.24 7)) (= tmp.60 0))
                    (set! tmp.59 14)
                    (set! tmp.59 6))
                  (!= tmp.59 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.86 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.86 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.86 rbp rax))))))
    (define L.*.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.62 (bitwise-and tmp.21 7)) (= tmp.62 0))
                  (set! tmp.61 14)
                  (set! tmp.61 6))
                (!= tmp.61 6))
            (if (begin
                  (if (begin (set! tmp.64 (bitwise-and tmp.20 7)) (= tmp.64 0))
                    (set! tmp.63 14)
                    (set! tmp.63 6))
                  (!= tmp.63 6))
              (begin
                (set! tmp.65 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.65)) (jump tmp-ra.87 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.87 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.87 rbp rax))))))
    (define L.+.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.67 (bitwise-and tmp.23 7)) (= tmp.67 0))
                  (set! tmp.66 14)
                  (set! tmp.66 6))
                (!= tmp.66 6))
            (if (begin
                  (if (begin (set! tmp.69 (bitwise-and tmp.22 7)) (= tmp.69 0))
                    (set! tmp.68 14)
                    (set! tmp.68 6))
                  (!= tmp.68 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.88 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.88 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.88 rbp rax))))))
    (define L.fun/void101999.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.89 rbp rax)))))
    (define L.fun/empty101997.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.90 rbp rax)))))
    (define L.fun/ascii-char101994.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin
            (set! r15 tmp-ra.91)
            (jump L.fun/ascii-char101995.11 rbp r15)))))
    (define L.fun/void101992.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (begin (set! r15 tmp-ra.92) (jump L.fun/void101993.12 rbp r15)))))
    (define L.fun/void101998.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin (set! r15 tmp-ra.93) (jump L.fun/void101999.4 rbp r15)))))
    (define L.fun/empty101996.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/empty101997.5 rbp r15)))))
    (define L.fun/any101991.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/ascii-char101995.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/void101993.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.97 rbp rax)))))
    (begin
      (set! tmp-ra.98 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.18
                  (begin
                    (set! rsi 248)
                    (set! rdi 1216)
                    (set! r15 L.rp.18)
                    (jump L.+.14 rbp r15 rdi rsi)))
                (set! tmp.72 rax))
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 528)
                      (set! rdi 1128)
                      (set! r15 L.rp.19)
                      (jump L.+.14 rbp r15 rdi rsi)))
                  (set! tmp.73 rax))
                (begin
                  (return-point L.rp.20
                    (begin
                      (set! rsi tmp.73)
                      (set! rdi tmp.72)
                      (set! r15 L.rp.20)
                      (jump L.*.15 rbp r15 rdi rsi)))
                  (set! tmp.71 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.21
                    (begin
                      (set! rsi 1032)
                      (set! rdi 176)
                      (set! r15 L.rp.21)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.75 rax))
                (begin
                  (begin
                    (return-point L.rp.22
                      (begin
                        (set! rsi 1320)
                        (set! rdi 968)
                        (set! r15 L.rp.22)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.76 rax))
                  (begin
                    (return-point L.rp.23
                      (begin
                        (set! rsi tmp.76)
                        (set! rdi tmp.75)
                        (set! r15 L.rp.23)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.74 rax))))
              (begin
                (return-point L.rp.24
                  (begin
                    (set! rsi tmp.74)
                    (set! rdi tmp.71)
                    (set! r15 L.rp.24)
                    (jump L.*.15 rbp r15 rdi rsi)))
                (set! tmp.70 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi 1872)
                      (set! rdi 424)
                      (set! r15 L.rp.25)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.79 rax))
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 1288)
                        (set! rdi 480)
                        (set! r15 L.rp.26)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.80 rax))
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi tmp.80)
                        (set! rdi tmp.79)
                        (set! r15 L.rp.27)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.78 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi 1400)
                        (set! rdi 1368)
                        (set! r15 L.rp.28)
                        (jump L.-.16 rbp r15 rdi rsi)))
                    (set! tmp.82 rax))
                  (begin
                    (begin
                      (return-point L.rp.29
                        (begin
                          (set! rsi 544)
                          (set! rdi 704)
                          (set! r15 L.rp.29)
                          (jump L.+.14 rbp r15 rdi rsi)))
                      (set! tmp.83 rax))
                    (begin
                      (return-point L.rp.30
                        (begin
                          (set! rsi tmp.83)
                          (set! rdi tmp.82)
                          (set! r15 L.rp.30)
                          (jump L.-.16 rbp r15 rdi rsi)))
                      (set! tmp.81 rax))))
                (begin
                  (return-point L.rp.31
                    (begin
                      (set! rsi tmp.81)
                      (set! rdi tmp.78)
                      (set! r15 L.rp.31)
                      (jump L.-.16 rbp r15 rdi rsi)))
                  (set! tmp.77 rax))))
            (begin
              (return-point L.rp.32
                (begin
                  (set! rsi tmp.77)
                  (set! rdi tmp.70)
                  (set! r15 L.rp.32)
                  (jump L.+.14 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (begin
            (return-point L.rp.33
              (begin (set! r15 L.rp.33) (jump L.fun/any101991.10 rbp r15)))
            (set! tmp.84 rax))
          (begin
            (return-point L.rp.34
              (begin
                (set! rdi tmp.84)
                (set! r15 L.rp.34)
                (jump L.boolean?.17 rbp r15 rdi)))
            (set! boolean1.5 rax)))
        (begin
          (return-point L.rp.35
            (begin (set! r15 L.rp.35) (jump L.fun/void101992.7 rbp r15)))
          (set! void2.4 rax))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/ascii-char101994.6 rbp r15)))
          (set! ascii-char3.3 rax))
        (begin
          (return-point L.rp.37
            (begin (set! r15 L.rp.37) (jump L.fun/empty101996.9 rbp r15)))
          (set! empty4.2 rax))
        (begin
          (return-point L.rp.38
            (begin (set! r15 L.rp.38) (jump L.fun/void101998.8 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax empty4.2) (jump tmp-ra.98 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames (() () () () () ())))
    (define L.fun/void102362.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.56 r15)
        (begin
          (begin (set! r15 tmp-ra.56) (jump L.fun/void102363.9 rbp r15)))))
    (define L.fun/empty102356.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.57 r15)
        (begin
          (begin (set! r15 tmp-ra.57) (jump L.fun/empty102357.12 rbp r15)))))
    (define L.fun/error102354.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.58 r15)
        (begin
          (begin (set! r15 tmp-ra.58) (jump L.fun/error102355.10 rbp r15)))))
    (define L.fun/void102358.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.59 r15)
        (begin
          (begin (set! r15 tmp-ra.59) (jump L.fun/void102359.13 rbp r15)))))
    (define L.fun/void102353.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.60 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.60 rbp rax)))))
    (define L.fun/void102363.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.61 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.61 rbp rax)))))
    (define L.fun/error102355.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.62 r15)
        (begin (begin (set! rax 37694) (jump tmp-ra.62 rbp rax)))))
    (define L.fun/void102352.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.63 r15)
        (begin
          (begin (set! r15 tmp-ra.63) (jump L.fun/void102353.8 rbp r15)))))
    (define L.fun/empty102357.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.64 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.64 rbp rax)))))
    (define L.fun/void102359.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.65 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.65 rbp rax)))))
    (define L.fun/void102360.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.66 r15)
        (begin
          (begin (set! r15 tmp-ra.66) (jump L.fun/void102361.15 rbp r15)))))
    (define L.fun/void102361.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.67 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.67 rbp rax)))))
    (begin
      (set! tmp-ra.68 r15)
      (begin
        (begin
          (return-point L.rp.17
            (begin (set! r15 L.rp.17) (jump L.fun/void102352.11 rbp r15)))
          (set! void0.6 rax))
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/error102354.6 rbp r15)))
          (set! error1.5 rax))
        (begin
          (return-point L.rp.19
            (begin (set! r15 L.rp.19) (jump L.fun/empty102356.5 rbp r15)))
          (set! empty2.4 rax))
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/void102358.7 rbp r15)))
          (set! void3.3 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/void102360.14 rbp r15)))
          (set! void4.2 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/void102362.4 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax error1.5) (jump tmp-ra.68 rbp rax))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () () ())))
    (define L.+.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.85 rbp rax))))))
    (define L.*.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.86 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.86 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.86 rbp rax))))))
    (define L.-.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.87 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.87 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.87 rbp rax))))))
    (define L.boolean?.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin
          (set! tmp.44 rdi)
          (if (begin (set! tmp.69 (bitwise-and tmp.44 247)) (= tmp.69 6))
            (begin (set! rax 14) (jump tmp-ra.88 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.88 rbp rax))))))
    (define L.fun/void122163.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (begin (set! r15 tmp-ra.89) (jump L.fun/void122164.13 rbp r15)))))
    (define L.fun/ascii-char122159.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin
            (set! r15 tmp-ra.90)
            (jump L.fun/ascii-char122160.6 rbp r15)))))
    (define L.fun/ascii-char122160.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.91 rbp rax)))))
    (define L.fun/ascii-char122162.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/empty122158.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/error122165.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/error122166.10 rbp r15)))))
    (define L.fun/error122166.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 46910) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/any122156.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/empty122157.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (begin (set! r15 tmp-ra.97) (jump L.fun/empty122158.8 rbp r15)))))
    (define L.fun/void122164.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.98 rbp rax)))))
    (define L.fun/ascii-char122161.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (begin
            (set! r15 tmp-ra.99)
            (jump L.fun/ascii-char122162.7 rbp r15)))))
    (begin
      (set! tmp-ra.100 r15)
      (begin
        (begin
          (begin
            (return-point L.rp.20
              (begin (set! r15 L.rp.20) (jump L.fun/any122156.11 rbp r15)))
            (set! tmp.70 rax))
          (begin
            (return-point L.rp.21
              (begin
                (set! rdi tmp.70)
                (set! r15 L.rp.21)
                (jump L.boolean?.16 rbp r15 rdi)))
            (set! boolean0.6 rax)))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/empty122157.12 rbp r15)))
          (set! empty1.5 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.23
                  (begin
                    (set! rsi 96)
                    (set! rdi 1368)
                    (set! r15 L.rp.23)
                    (jump L.-.17 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi 448)
                      (set! rdi 1560)
                      (set! r15 L.rp.24)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.25
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.25)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 224)
                      (set! rdi 952)
                      (set! r15 L.rp.26)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))
                (begin
                  (begin
                    (return-point L.rp.27
                      (begin
                        (set! rsi 1680)
                        (set! rdi 1856)
                        (set! r15 L.rp.27)
                        (jump L.*.18 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))
                  (begin
                    (return-point L.rp.28
                      (begin
                        (set! rsi tmp.77)
                        (set! rdi tmp.76)
                        (set! r15 L.rp.28)
                        (jump L.+.19 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))))
              (begin
                (return-point L.rp.29
                  (begin
                    (set! rsi tmp.75)
                    (set! rdi tmp.72)
                    (set! r15 L.rp.29)
                    (jump L.-.17 rbp r15 rdi rsi)))
                (set! tmp.71 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.30
                    (begin
                      (set! rsi 304)
                      (set! rdi 104)
                      (set! r15 L.rp.30)
                      (jump L.-.17 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi 1296)
                        (set! rdi 1432)
                        (set! r15 L.rp.31)
                        (jump L.*.18 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (return-point L.rp.32
                      (begin
                        (set! rsi tmp.81)
                        (set! rdi tmp.80)
                        (set! r15 L.rp.32)
                        (jump L.+.19 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi 1160)
                        (set! rdi 728)
                        (set! r15 L.rp.33)
                        (jump L.+.19 rbp r15 rdi rsi)))
                    (set! tmp.83 rax))
                  (begin
                    (begin
                      (return-point L.rp.34
                        (begin
                          (set! rsi 464)
                          (set! rdi 416)
                          (set! r15 L.rp.34)
                          (jump L.-.17 rbp r15 rdi rsi)))
                      (set! tmp.84 rax))
                    (begin
                      (return-point L.rp.35
                        (begin
                          (set! rsi tmp.84)
                          (set! rdi tmp.83)
                          (set! r15 L.rp.35)
                          (jump L.*.18 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))))
                (begin
                  (return-point L.rp.36
                    (begin
                      (set! rsi tmp.82)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.36)
                      (jump L.+.19 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.37
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.71)
                  (set! r15 L.rp.37)
                  (jump L.-.17 rbp r15 rdi rsi)))
              (set! fixnum2.4 rax))))
        (begin
          (return-point L.rp.38
            (begin (set! r15 L.rp.38) (jump L.fun/ascii-char122159.5 rbp r15)))
          (set! ascii-char3.3 rax))
        (begin
          (return-point L.rp.39
            (begin
              (set! r15 L.rp.39)
              (jump L.fun/ascii-char122161.14 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (return-point L.rp.40
            (begin (set! r15 L.rp.40) (jump L.fun/void122163.4 rbp r15)))
          (set! void5.1 rax))
        (begin (set! r15 tmp-ra.100) (jump L.fun/error122165.9 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (() () () () () () () () () () () () () () () () () () () ())))
    (define L.-.19
      ((new-frames ()))
      (begin
        (set! tmp-ra.83 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.83 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.83 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.83 rbp rax))))))
    (define L.*.18
      ((new-frames ()))
      (begin
        (set! tmp-ra.84 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.84 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.84 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.84 rbp rax))))))
    (define L.+.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.85 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.23 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.22 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.85 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.85 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.85 rbp rax))))))
    (define L.fun/void124216.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.86 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.86 rbp rax)))))
    (define L.fun/ascii-char124211.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.87 r15)
        (begin
          (begin
            (set! r15 tmp-ra.87)
            (jump L.fun/ascii-char124212.10 rbp r15)))))
    (define L.fun/error124206.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.88 r15)
        (begin (begin (set! rax 8766) (jump tmp-ra.88 rbp rax)))))
    (define L.fun/error124213.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (begin (set! r15 tmp-ra.89) (jump L.fun/error124214.11 rbp r15)))))
    (define L.fun/ascii-char124207.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (begin
            (set! r15 tmp-ra.90)
            (jump L.fun/ascii-char124208.14 rbp r15)))))
    (define L.fun/void124215.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (begin (set! r15 tmp-ra.91) (jump L.fun/void124216.4 rbp r15)))))
    (define L.fun/ascii-char124212.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.92 rbp rax)))))
    (define L.fun/error124214.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin (begin (set! rax 1598) (jump tmp-ra.93 rbp rax)))))
    (define L.fun/error124209.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/error124210.13 rbp r15)))))
    (define L.fun/error124210.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 830) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/ascii-char124208.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/error124205.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (begin (set! r15 tmp-ra.97) (jump L.fun/error124206.6 rbp r15)))))
    (begin
      (set! tmp-ra.98 r15)
      (begin
        (begin
          (return-point L.rp.20
            (begin (set! r15 L.rp.20) (jump L.fun/error124205.15 rbp r15)))
          (set! error0.6 rax))
        (begin
          (return-point L.rp.21
            (begin (set! r15 L.rp.21) (jump L.fun/ascii-char124207.8 rbp r15)))
          (set! ascii-char1.5 rax))
        (begin
          (return-point L.rp.22
            (begin (set! r15 L.rp.22) (jump L.fun/error124209.12 rbp r15)))
          (set! error2.4 rax))
        (begin
          (return-point L.rp.23
            (begin (set! r15 L.rp.23) (jump L.fun/ascii-char124211.5 rbp r15)))
          (set! ascii-char3.3 rax))
        (begin
          (return-point L.rp.24
            (begin (set! r15 L.rp.24) (jump L.fun/error124213.7 rbp r15)))
          (set! error4.2 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.25
                  (begin
                    (set! rsi 16)
                    (set! rdi 1640)
                    (set! r15 L.rp.25)
                    (jump L.+.17 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.26
                    (begin
                      (set! rsi 120)
                      (set! rdi 1384)
                      (set! r15 L.rp.26)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.27
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.27)
                      (jump L.*.18 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.28
                    (begin
                      (set! rsi 1400)
                      (set! rdi 712)
                      (set! r15 L.rp.28)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi 912)
                        (set! rdi 936)
                        (set! r15 L.rp.29)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.30)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.31
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.31)
                    (jump L.+.17 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi 952)
                      (set! rdi 1344)
                      (set! r15 L.rp.32)
                      (jump L.*.18 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.33
                      (begin
                        (set! rsi 264)
                        (set! rdi 648)
                        (set! r15 L.rp.33)
                        (jump L.-.19 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.34
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.34)
                        (jump L.*.18 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi 1552)
                        (set! rdi 560)
                        (set! r15 L.rp.35)
                        (jump L.-.19 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.36
                        (begin
                          (set! rsi 952)
                          (set! rdi 592)
                          (set! r15 L.rp.36)
                          (jump L.+.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.37
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.37)
                          (jump L.+.17 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.38
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.38)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.39
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.39)
                  (jump L.*.18 rbp r15 rdi rsi)))
              (set! fixnum5.1 rax))))
        (begin (set! r15 tmp-ra.98) (jump L.fun/void124215.9 rbp r15))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.+.17
      ((new-frames ()))
      (begin
        (set! tmp-ra.89 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.23 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.22 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.89 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.89 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.89 rbp rax))))))
    (define L.*.16
      ((new-frames ()))
      (begin
        (set! tmp-ra.90 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.21 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.20 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin
                (set! tmp.64 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.64)) (jump tmp-ra.90 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.90 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.90 rbp rax))))))
    (define L.-.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.91 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.66 (bitwise-and tmp.25 7)) (= tmp.66 0))
                  (set! tmp.65 14)
                  (set! tmp.65 6))
                (!= tmp.65 6))
            (if (begin
                  (if (begin (set! tmp.68 (bitwise-and tmp.24 7)) (= tmp.68 0))
                    (set! tmp.67 14)
                    (set! tmp.67 6))
                  (!= tmp.67 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.91 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.91 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.91 rbp rax))))))
    (define L.pair?.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.92 r15)
        (begin
          (set! tmp.49 rdi)
          (if (begin (set! tmp.69 (bitwise-and tmp.49 7)) (= tmp.69 1))
            (begin (set! rax 14) (jump tmp-ra.92 rbp rax))
            (begin (set! rax 6) (jump tmp-ra.92 rbp rax))))))
    (define L.fun/ascii-char156382.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.93 r15)
        (begin
          (begin
            (set! r15 tmp-ra.93)
            (jump L.fun/ascii-char156383.11 rbp r15)))))
    (define L.fun/void156379.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.94 r15)
        (begin
          (begin (set! r15 tmp-ra.94) (jump L.fun/void156380.6 rbp r15)))))
    (define L.fun/void156380.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.95 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.95 rbp rax)))))
    (define L.fun/void156387.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.96 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.96 rbp rax)))))
    (define L.fun/any156381.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin (begin (set! rax 11326) (jump tmp-ra.97 rbp rax)))))
    (define L.fun/void156386.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (begin (set! r15 tmp-ra.98) (jump L.fun/void156387.7 rbp r15)))))
    (define L.fun/ascii-char156384.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (begin
            (set! r15 tmp-ra.99)
            (jump L.fun/ascii-char156385.12 rbp r15)))))
    (define L.fun/ascii-char156383.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.100 rbp rax)))))
    (define L.fun/ascii-char156385.12
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.101 rbp rax)))))
    (begin
      (set! tmp-ra.102 r15)
      (begin
        (begin
          (return-point L.rp.18
            (begin (set! r15 L.rp.18) (jump L.fun/void156379.5 rbp r15)))
          (set! void0.6 rax))
        (begin
          (begin
            (return-point L.rp.19
              (begin (set! r15 L.rp.19) (jump L.fun/any156381.8 rbp r15)))
            (set! tmp.70 rax))
          (begin
            (return-point L.rp.20
              (begin
                (set! rdi tmp.70)
                (set! r15 L.rp.20)
                (jump L.pair?.14 rbp r15 rdi)))
            (set! boolean1.5 rax)))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.21
                  (begin
                    (set! rsi 2016)
                    (set! rdi 232)
                    (set! r15 L.rp.21)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.73 rax))
              (begin
                (begin
                  (return-point L.rp.22
                    (begin
                      (set! rsi 304)
                      (set! rdi 504)
                      (set! r15 L.rp.22)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi tmp.74)
                      (set! rdi tmp.73)
                      (set! r15 L.rp.23)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.24
                    (begin
                      (set! rsi 1152)
                      (set! rdi 368)
                      (set! r15 L.rp.24)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))
                (begin
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi 424)
                        (set! rdi 1752)
                        (set! r15 L.rp.25)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi tmp.77)
                        (set! rdi tmp.76)
                        (set! r15 L.rp.26)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))))
              (begin
                (return-point L.rp.27
                  (begin
                    (set! rsi tmp.75)
                    (set! rdi tmp.72)
                    (set! r15 L.rp.27)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.71 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.28
                    (begin
                      (set! rsi 1616)
                      (set! rdi 1736)
                      (set! r15 L.rp.28)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.80 rax))
                (begin
                  (begin
                    (return-point L.rp.29
                      (begin
                        (set! rsi 568)
                        (set! rdi 320)
                        (set! r15 L.rp.29)
                        (jump L.*.16 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (return-point L.rp.30
                      (begin
                        (set! rsi tmp.81)
                        (set! rdi tmp.80)
                        (set! r15 L.rp.30)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.31
                      (begin
                        (set! rsi 1224)
                        (set! rdi 192)
                        (set! r15 L.rp.31)
                        (jump L.+.17 rbp r15 rdi rsi)))
                    (set! tmp.83 rax))
                  (begin
                    (begin
                      (return-point L.rp.32
                        (begin
                          (set! rsi 2016)
                          (set! rdi 1976)
                          (set! r15 L.rp.32)
                          (jump L.-.15 rbp r15 rdi rsi)))
                      (set! tmp.84 rax))
                    (begin
                      (return-point L.rp.33
                        (begin
                          (set! rsi tmp.84)
                          (set! rdi tmp.83)
                          (set! r15 L.rp.33)
                          (jump L.+.17 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))))
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi tmp.82)
                      (set! rdi tmp.79)
                      (set! r15 L.rp.34)
                      (jump L.*.16 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))))
            (begin
              (return-point L.rp.35
                (begin
                  (set! rsi tmp.78)
                  (set! rdi tmp.71)
                  (set! r15 L.rp.35)
                  (jump L.+.17 rbp r15 rdi rsi)))
              (set! fixnum2.4 rax))))
        (begin
          (return-point L.rp.36
            (begin (set! r15 L.rp.36) (jump L.fun/ascii-char156382.4 rbp r15)))
          (set! ascii-char3.3 rax))
        (begin
          (return-point L.rp.37
            (begin
              (set! r15 L.rp.37)
              (jump L.fun/ascii-char156384.10 rbp r15)))
          (set! ascii-char4.2 rax))
        (begin
          (return-point L.rp.38
            (begin (set! r15 L.rp.38) (jump L.fun/void156386.9 rbp r15)))
          (set! void5.1 rax))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.39
                  (begin
                    (set! rsi 1944)
                    (set! rdi 912)
                    (set! r15 L.rp.39)
                    (jump L.+.17 rbp r15 rdi rsi)))
                (set! tmp.87 rax))
              (begin
                (begin
                  (return-point L.rp.40
                    (begin
                      (set! rsi fixnum2.4)
                      (set! rdi 1896)
                      (set! r15 L.rp.40)
                      (jump L.+.17 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (return-point L.rp.41
                    (begin
                      (set! rsi tmp.88)
                      (set! rdi tmp.87)
                      (set! r15 L.rp.41)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))))
            (begin
              (return-point L.rp.42
                (begin
                  (set! rsi tmp.86)
                  (set! rdi fixnum2.4)
                  (set! r15 L.rp.42)
                  (jump L.*.16 rbp r15 rdi rsi)))
              (set! tmp.85 rax)))
          (begin
            (set! rsi tmp.85)
            (set! rdi fixnum2.4)
            (set! r15 tmp-ra.102)
            (jump L.-.15 rbp r15 rdi rsi)))))))
(check-by-interp
 '(module
    ((new-frames
      (()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ()
       ())))
    (define L.-.15
      ((new-frames ()))
      (begin
        (set! tmp-ra.97 r15)
        (begin
          (set! tmp.24 rdi)
          (set! tmp.25 rsi)
          (if (begin
                (if (begin (set! tmp.57 (bitwise-and tmp.25 7)) (= tmp.57 0))
                  (set! tmp.56 14)
                  (set! tmp.56 6))
                (!= tmp.56 6))
            (if (begin
                  (if (begin (set! tmp.59 (bitwise-and tmp.24 7)) (= tmp.59 0))
                    (set! tmp.58 14)
                    (set! tmp.58 6))
                  (!= tmp.58 6))
              (begin (set! rax (- tmp.24 tmp.25)) (jump tmp-ra.97 rbp rax))
              (begin (set! rax 830) (jump tmp-ra.97 rbp rax)))
            (begin (set! rax 830) (jump tmp-ra.97 rbp rax))))))
    (define L.+.14
      ((new-frames ()))
      (begin
        (set! tmp-ra.98 r15)
        (begin
          (set! tmp.22 rdi)
          (set! tmp.23 rsi)
          (if (begin
                (if (begin (set! tmp.61 (bitwise-and tmp.23 7)) (= tmp.61 0))
                  (set! tmp.60 14)
                  (set! tmp.60 6))
                (!= tmp.60 6))
            (if (begin
                  (if (begin (set! tmp.63 (bitwise-and tmp.22 7)) (= tmp.63 0))
                    (set! tmp.62 14)
                    (set! tmp.62 6))
                  (!= tmp.62 6))
              (begin (set! rax (+ tmp.22 tmp.23)) (jump tmp-ra.98 rbp rax))
              (begin (set! rax 574) (jump tmp-ra.98 rbp rax)))
            (begin (set! rax 574) (jump tmp-ra.98 rbp rax))))))
    (define L.*.13
      ((new-frames ()))
      (begin
        (set! tmp-ra.99 r15)
        (begin
          (set! tmp.20 rdi)
          (set! tmp.21 rsi)
          (if (begin
                (if (begin (set! tmp.65 (bitwise-and tmp.21 7)) (= tmp.65 0))
                  (set! tmp.64 14)
                  (set! tmp.64 6))
                (!= tmp.64 6))
            (if (begin
                  (if (begin (set! tmp.67 (bitwise-and tmp.20 7)) (= tmp.67 0))
                    (set! tmp.66 14)
                    (set! tmp.66 6))
                  (!= tmp.66 6))
              (begin
                (set! tmp.68 (arithmetic-shift-right tmp.21 3))
                (begin (set! rax (* tmp.20 tmp.68)) (jump tmp-ra.99 rbp rax)))
              (begin (set! rax 318) (jump tmp-ra.99 rbp rax)))
            (begin (set! rax 318) (jump tmp-ra.99 rbp rax))))))
    (define L.fun/empty171686.4
      ((new-frames ()))
      (begin
        (set! tmp-ra.100 r15)
        (begin
          (begin (set! r15 tmp-ra.100) (jump L.fun/empty171687.10 rbp r15)))))
    (define L.fun/void171689.5
      ((new-frames ()))
      (begin
        (set! tmp-ra.101 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.101 rbp rax)))))
    (define L.fun/ascii-char171684.6
      ((new-frames ()))
      (begin
        (set! tmp-ra.102 r15)
        (begin
          (begin
            (set! r15 tmp-ra.102)
            (jump L.fun/ascii-char171685.9 rbp r15)))))
    (define L.fun/void171688.7
      ((new-frames ()))
      (begin
        (set! tmp-ra.103 r15)
        (begin
          (begin (set! r15 tmp-ra.103) (jump L.fun/void171689.5 rbp r15)))))
    (define L.fun/void171691.8
      ((new-frames ()))
      (begin
        (set! tmp-ra.104 r15)
        (begin (begin (set! rax 30) (jump tmp-ra.104 rbp rax)))))
    (define L.fun/ascii-char171685.9
      ((new-frames ()))
      (begin
        (set! tmp-ra.105 r15)
        (begin (begin (set! rax 25390) (jump tmp-ra.105 rbp rax)))))
    (define L.fun/empty171687.10
      ((new-frames ()))
      (begin
        (set! tmp-ra.106 r15)
        (begin (begin (set! rax 22) (jump tmp-ra.106 rbp rax)))))
    (define L.fun/void171690.11
      ((new-frames ()))
      (begin
        (set! tmp-ra.107 r15)
        (begin
          (begin (set! r15 tmp-ra.107) (jump L.fun/void171691.8 rbp r15)))))
    (begin
      (set! tmp-ra.108 r15)
      (begin
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.16
                  (begin
                    (set! rsi 1576)
                    (set! rdi 2008)
                    (set! r15 L.rp.16)
                    (jump L.*.13 rbp r15 rdi rsi)))
                (set! tmp.71 rax))
              (begin
                (begin
                  (return-point L.rp.17
                    (begin
                      (set! rsi 1128)
                      (set! rdi 1088)
                      (set! r15 L.rp.17)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.72 rax))
                (begin
                  (return-point L.rp.18
                    (begin
                      (set! rsi tmp.72)
                      (set! rdi tmp.71)
                      (set! r15 L.rp.18)
                      (jump L.+.14 rbp r15 rdi rsi)))
                  (set! tmp.70 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.19
                    (begin
                      (set! rsi 48)
                      (set! rdi 1968)
                      (set! r15 L.rp.19)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.74 rax))
                (begin
                  (begin
                    (return-point L.rp.20
                      (begin
                        (set! rsi 624)
                        (set! rdi 592)
                        (set! r15 L.rp.20)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.75 rax))
                  (begin
                    (return-point L.rp.21
                      (begin
                        (set! rsi tmp.75)
                        (set! rdi tmp.74)
                        (set! r15 L.rp.21)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.73 rax))))
              (begin
                (return-point L.rp.22
                  (begin
                    (set! rsi tmp.73)
                    (set! rdi tmp.70)
                    (set! r15 L.rp.22)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.69 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.23
                    (begin
                      (set! rsi 672)
                      (set! rdi 1184)
                      (set! r15 L.rp.23)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.78 rax))
                (begin
                  (begin
                    (return-point L.rp.24
                      (begin
                        (set! rsi 1584)
                        (set! rdi 1320)
                        (set! r15 L.rp.24)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.79 rax))
                  (begin
                    (return-point L.rp.25
                      (begin
                        (set! rsi tmp.79)
                        (set! rdi tmp.78)
                        (set! r15 L.rp.25)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.77 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.26
                      (begin
                        (set! rsi 904)
                        (set! rdi 1768)
                        (set! r15 L.rp.26)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.81 rax))
                  (begin
                    (begin
                      (return-point L.rp.27
                        (begin
                          (set! rsi 104)
                          (set! rdi 928)
                          (set! r15 L.rp.27)
                          (jump L.*.13 rbp r15 rdi rsi)))
                      (set! tmp.82 rax))
                    (begin
                      (return-point L.rp.28
                        (begin
                          (set! rsi tmp.82)
                          (set! rdi tmp.81)
                          (set! r15 L.rp.28)
                          (jump L.-.15 rbp r15 rdi rsi)))
                      (set! tmp.80 rax))))
                (begin
                  (return-point L.rp.29
                    (begin
                      (set! rsi tmp.80)
                      (set! rdi tmp.77)
                      (set! r15 L.rp.29)
                      (jump L.+.14 rbp r15 rdi rsi)))
                  (set! tmp.76 rax))))
            (begin
              (return-point L.rp.30
                (begin
                  (set! rsi tmp.76)
                  (set! rdi tmp.69)
                  (set! r15 L.rp.30)
                  (jump L.-.15 rbp r15 rdi rsi)))
              (set! fixnum0.6 rax))))
        (begin
          (begin
            (begin
              (begin
                (return-point L.rp.31
                  (begin
                    (set! rsi 240)
                    (set! rdi 1160)
                    (set! r15 L.rp.31)
                    (jump L.*.13 rbp r15 rdi rsi)))
                (set! tmp.85 rax))
              (begin
                (begin
                  (return-point L.rp.32
                    (begin
                      (set! rsi 256)
                      (set! rdi 1520)
                      (set! r15 L.rp.32)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.86 rax))
                (begin
                  (return-point L.rp.33
                    (begin
                      (set! rsi tmp.86)
                      (set! rdi tmp.85)
                      (set! r15 L.rp.33)
                      (jump L.*.13 rbp r15 rdi rsi)))
                  (set! tmp.84 rax))))
            (begin
              (begin
                (begin
                  (return-point L.rp.34
                    (begin
                      (set! rsi 240)
                      (set! rdi 960)
                      (set! r15 L.rp.34)
                      (jump L.+.14 rbp r15 rdi rsi)))
                  (set! tmp.88 rax))
                (begin
                  (begin
                    (return-point L.rp.35
                      (begin
                        (set! rsi 728)
                        (set! rdi 624)
                        (set! r15 L.rp.35)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.89 rax))
                  (begin
                    (return-point L.rp.36
                      (begin
                        (set! rsi tmp.89)
                        (set! rdi tmp.88)
                        (set! r15 L.rp.36)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.87 rax))))
              (begin
                (return-point L.rp.37
                  (begin
                    (set! rsi tmp.87)
                    (set! rdi tmp.84)
                    (set! r15 L.rp.37)
                    (jump L.-.15 rbp r15 rdi rsi)))
                (set! tmp.83 rax))))
          (begin
            (begin
              (begin
                (begin
                  (return-point L.rp.38
                    (begin
                      (set! rsi 1080)
                      (set! rdi 1880)
                      (set! r15 L.rp.38)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.92 rax))
                (begin
                  (begin
                    (return-point L.rp.39
                      (begin
                        (set! rsi 1000)
                        (set! rdi 440)
                        (set! r15 L.rp.39)
                        (jump L.*.13 rbp r15 rdi rsi)))
                    (set! tmp.93 rax))
                  (begin
                    (return-point L.rp.40
                      (begin
                        (set! rsi tmp.93)
                        (set! rdi tmp.92)
                        (set! r15 L.rp.40)
                        (jump L.+.14 rbp r15 rdi rsi)))
                    (set! tmp.91 rax))))
              (begin
                (begin
                  (begin
                    (return-point L.rp.41
                      (begin
                        (set! rsi 608)
                        (set! rdi 936)
                        (set! r15 L.rp.41)
                        (jump L.-.15 rbp r15 rdi rsi)))
                    (set! tmp.95 rax))
                  (begin
                    (begin
                      (return-point L.rp.42
                        (begin
                          (set! rsi 984)
                          (set! rdi 568)
                          (set! r15 L.rp.42)
                          (jump L.-.15 rbp r15 rdi rsi)))
                      (set! tmp.96 rax))
                    (begin
                      (return-point L.rp.43
                        (begin
                          (set! rsi tmp.96)
                          (set! rdi tmp.95)
                          (set! r15 L.rp.43)
                          (jump L.-.15 rbp r15 rdi rsi)))
                      (set! tmp.94 rax))))
                (begin
                  (return-point L.rp.44
                    (begin
                      (set! rsi tmp.94)
                      (set! rdi tmp.91)
                      (set! r15 L.rp.44)
                      (jump L.-.15 rbp r15 rdi rsi)))
                  (set! tmp.90 rax))))
            (begin
              (return-point L.rp.45
                (begin
                  (set! rsi tmp.90)
                  (set! rdi tmp.83)
                  (set! r15 L.rp.45)
                  (jump L.-.15 rbp r15 rdi rsi)))
              (set! fixnum1.5 rax))))
        (begin
          (return-point L.rp.46
            (begin (set! r15 L.rp.46) (jump L.fun/ascii-char171684.6 rbp r15)))
          (set! ascii-char2.4 rax))
        (begin
          (return-point L.rp.47
            (begin (set! r15 L.rp.47) (jump L.fun/empty171686.4 rbp r15)))
          (set! empty3.3 rax))
        (begin
          (return-point L.rp.48
            (begin (set! r15 L.rp.48) (jump L.fun/void171688.7 rbp r15)))
          (set! void4.2 rax))
        (begin
          (return-point L.rp.49
            (begin (set! r15 L.rp.49) (jump L.fun/void171690.11 rbp r15)))
          (set! void5.1 rax))
        (begin (set! rax fixnum1.5) (jump tmp-ra.108 rbp rax))))))
