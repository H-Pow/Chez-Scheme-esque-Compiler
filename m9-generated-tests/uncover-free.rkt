 ...
 '(module
    (letrec ((vector-init-loop.8
      ...
      (let ((vector0.5 (unsafe-procedure-call make-vector.58 8))
            (procedure1.4 (letrec ((lam.59 (lambda () 72))) lam.59)))
        empty))))
(check-by-interp
 '(module
 ...
 '(module
    (letrec ((fun/ascii-char8689.4 (lambda () #\I))
      ...
      (let ((ascii-char0.12 (unsafe-procedure-call fun/ascii-char8688.7))
            ...
            (procedure3.9
             (letrec ((lam.65
                       (lambda () (unsafe-procedure-call fun/error8691.5))))
               lam.65)))
       ...
       (unsafe-procedure-call
         ...
         *.109
         (let ((error0.44 (error 211))
               ...
               (procedure3.41 (letrec ((lam.110 (lambda () #\X))) lam.110)))
           226)
    ...
    (letrec ((<=.78
      ...
      (if (unsafe-procedure-call vector?.77 (if #f empty (void)))
        ...
        (let ((procedure0.23
               (letrec ((lam.80
                         (lambda ()
                           (unsafe-procedure-call fun/ascii-char9132.7))))
                 lam.80))
    ...
    (letrec ((fixnum?.73 (lambda (tmp.57) (fixnum? tmp.57)))
      ...
      (let ((empty0.19 (unsafe-procedure-call fun/empty10708.9))
            (procedure1.18
             (letrec ((lam.74
                       (lambda ()
                         (unsafe-procedure-call fun/ascii-char10710.8))))
               lam.74))
 ...
 '(module
    (letrec ((unsafe-vector-ref.3
      ...
      (let ((vector0.16 (unsafe-procedure-call fun/vector10789.8))
            (procedure1.15
             (letrec ((lam.73
                       (lambda () (unsafe-procedure-call fun/void10791.4))))
               lam.73))
            (ascii-char2.14 (unsafe-procedure-call fun/ascii-char10793.11))
            (procedure3.13
             (letrec ((lam.74
                       (lambda ()
                         (unsafe-procedure-call fun/ascii-char10795.7))))
               lam.74))
            (procedure4.12
             (letrec ((lam.75
                       (lambda ()
...
(check-by-interp
 '(module
    (letrec ((cons.74 (lambda (tmp.69 tmp.70) (cons tmp.69 tmp.70)))
      ...
      (let ((procedure0.21
             (letrec ((lam.75
                       (lambda () (unsafe-procedure-call fun/pair13985.9))))
               lam.75))
