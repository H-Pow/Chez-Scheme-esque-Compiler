...
(define (sequentialize-let p)
  ...
  (define (seq-let-effect fx)
    (match fx
      [`(let ([,aloc* ,val*] ...) ,fx) (make-begin (map make-fx+ aloc* val*)
                                                   (seq-let-effect fx))]
      [`(begin ,fx* ... ,fx) (make-begin-effect (map seq-let-effect `(,@fx* ,fx)))]
      [_ fx]))
  ...
  (define (seq-let-pred pred)
    (match pred
      ...
      [`(begin ,fx* ... ,pred) (make-begin (map seq-let-effect fx*)
                                           (seq-let-pred pred))]
      [_ pred]))
  (define (seq-let-value val)
    (match val
      ...
      [`(begin ,fx* ... ,value)
       (make-begin (map seq-let-effect fx*)
                   (seq-let-value value))]
      [_ val]))
    ...
    (match tail
      ...
      [`(if ,pred ,tail1 ,tail2)
       `(if ,(seq-let-pred pred)
            ,(seq-let-tail tail1)
            ,(seq-let-tail tail2))]
      [`(begin ,fx* ... ,tail) (make-begin (map seq-let-effect fx*)
                                           (seq-let-tail tail))]
      ;; (call triv opand ...) exists for both langs despite showing on the diff
      [_ tail]))
  (define (seq-let-p p)
