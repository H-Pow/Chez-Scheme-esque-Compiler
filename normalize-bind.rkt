...
(module+ test
  ...
  (define-syntax-rule (check-by-interp-v6 v6)
    (check-equal? (interp-imp-mf-lang-v6 v6) (interp-proc-imp-cmf-lang-v6 (normalize-bind v6))))
      ...
      (lambda (bat.0.6 bar.8.5 foobar.2.4 ball.5.3 bat.4.2 ball.7.1)
        (if (false)
          (begin
            ...
            (begin (set! foobar.2.8 9223372036854775807) ball.5.3))
          (call
           L.proc.0.1
           9223372036854775807
           bat.4.2
...
(check-by-interp-v6 '(module
    ...
    (begin
      (set! foobar.3.10
        (call L.func.0.1 0 9223372036854775807 9223372036854775807))
      (if (not
           (if (= -9223372036854775808 649827574)
        ...
        (if (< -9223372036854775808 -9223372036854775808)
          ...
          (begin (set! bat.8.11 foobar.3.10) bat.8.11))
        (begin
        ...
        (begin
          (set! foo.3.8
            (if (begin (set! ball.4.9 foo.7.7) (< bar.9.6 0))
              (begin (set! foo.3.10 ball.4.4) -224488027)
...
(check-by-interp-v6 '(module
    (define L.x.0.1
      (lambda (foobar.5.3 foo.8.2 foobar.6.1)
        (call L.x.0.1 0 foobar.5.3 foobar.5.3)))
    (if (begin
      ...
      (begin
        (set! foobar.6.5
          (call L.x.0.1 9223372036854775807 9223372036854775807 -1530356459))
...
(check-by-interp-v6 '(module
    ...
    (begin
      (set! foo.2.6
        (if (if (!= 1 0) (<= -9223372036854775808 9223372036854775807) (= 0 0))
...
(check-by-interp-v6 '(module
    (define L.tmp.0.1
      (lambda (bat.8.2 bar.7.1)
        (call L.fn.1.2 bat.8.2 bat.8.2 bat.8.2 9223372036854775807)))
    (define L.fn.1.2
      (lambda (bar.3.6 foobar.6.5 bar.9.4 bat.8.3)
        (call L.tmp.0.1 bat.8.3 bar.9.4)))
    ...
    (define L.func.0.1
      (lambda (foo.4.5 ball.5.4 bar.2.3 bat.3.2 bar.1.1)
        (if (begin
              ...
              (set! foo.4.6
                (call L.func.0.1 1 bar.2.3 foo.4.5 foo.4.5 bar.1.1))
              (false))
    ...
    (define L.tmp.1.2
      (lambda (bar.3.15
        ...
        (begin
          ...
          (set! bar.3.16
            (call L.tmp.0.1 foo.1.14 foo.1.14 0 bat.2.11 bat.2.11))
          (call
           L.tmp.1.2
           foobar.7.10
           foo.1.14
