...
(define (generate-let recur-depth iter-depth env k expected-type)
  ...
  (define env/updated (for/foldr ([env/updated env])
                        ([name var-name*]
                         [type var-type*])
                        (when (eq? type 'pair?)
                          (set! env/updated (dict-set env/updated `(call car ,name) (cons '() 'fixnum?)))
                          (set! env/updated (dict-set env/updated `(call cdr ,name) (cons '() 'fixnum?))))
                        (when (eq? type 'vector?)
                        ...
                        (dict-set env/updated name (cons '() type))
                        ))
